#!/usr/bin/env bash
#
# Runs benchmarkoor-replay against a Rsil datadir mounted by schelk.
#
# Usage:
...
#
# The prepare mode starts Rsil on the imported snapshot and replays benchmarkoor's
...
# scratch volume mounted at the post-prerun head and later uses Rsil unwind to
# reset each test.
#
# The run mode resets to the post-prerun baseline for each test, starts Rsil,
...
RSIL_SCOPE="${RSIL_SCOPE:-rsil-benchmarkoor.scope}"
...
cleanup() {
  ...
  sudo systemctl stop "$RSIL_SCOPE" 2>/dev/null || true
  sudo systemctl reset-failed "$RSIL_SCOPE" 2>/dev/null || true
}
...
wait_for_engine_port() {
  ...
  echo "::error::rsil (${label}/${phase}) engine port ${ENGINE_HOST}:${ENGINE_PORT} did not open within 30s"
  cat "$CURRENT_LOG"
  exit 1
}
...
wait_for_snapshot_pipeline() {
  ...
  if [ -n "$expected_block" ]; then
    echo "Waiting for Rsil snapshot pipeline to finish at block ${expected_block}"
  else
    echo "Waiting for Rsil snapshot pipeline to finish"
  fi
    ...
    if jq -e '.result == false' <<< "$sync_response" >/dev/null 2>&1; then
      ...
      if [[ "$block_hex" =~ ^0x[0-9a-fA-F]+$ ]]; then
        ...
        if [ -z "$expected_block" ] || [ "$block_dec" -ge "$expected_block" ]; then
          echo "Rsil snapshot pipeline finished after $(elapsed_secs "$start_ns")s at block ${block_dec}"
          return 0
        fi
      elif [ -z "$expected_block" ]; then
        echo "Rsil snapshot pipeline finished after $(elapsed_secs "$start_ns")s"
        return 0
      fi
    fi
    sleep 0.05
  ...
  echo "::error::rsil snapshot pipeline did not finish within ${timeout}s before benchmarkoor prerun"
  cat "$CURRENT_LOG"
  exit 1
}
...
finish_stage_matches() {
  ...
  finish="$(rsil_db_capture "$binary" stage-checkpoints get --stage finish || true)"
...
persisted_prerun_head_present() {
  ...
  header="$(rsil_db_capture "$binary" get static-file headers "$expected_number" || true)"
  ...
  canonical="$(rsil_db_capture "$binary" get mdbx CanonicalHeaders "$expected_number" || true)"
  if output_has_expected_hash "$canonical" "$expected_hash"; then
    header="$(rsil_db_capture "$binary" get mdbx Headers "$expected_number" || true)"
...
print_prerun_persistence_diagnostics() {
  ...
  rsil_db_capture "$binary" get static-file headers "$expected_number" || true
  echo "Finish stage checkpoint:"
  rsil_db_capture "$binary" stage-checkpoints get --stage finish || true
  echo "CanonicalHeaders:"
  rsil_db_capture "$binary" get mdbx CanonicalHeaders "$expected_number" || true
  echo "MDBX header:"
  rsil_db_capture "$binary" get mdbx Headers "$expected_number" || true
}
...
unwind_to_prerun_head() {
  ...
  start="$(now_ns)"
  echo "Unwinding Rsil datadir to post-prerun head ${expected_number} (${expected_hash})"
  rsil_stage_unwind "$binary" "$expected_number"
...
stop_node_required() {
  ...
  if sudo systemctl list-units --all --full --no-legend "$RSIL_SCOPE" | grep -q "$RSIL_SCOPE"; then
    if ! sudo systemctl stop "$RSIL_SCOPE"; then
      echo "::error::Failed to stop rsil systemd scope ${RSIL_SCOPE}"
      sudo systemctl status "$RSIL_SCOPE" --no-pager || true
      exit 1
    fi
  fi
  sudo systemctl reset-failed "$RSIL_SCOPE" 2>/dev/null || true
...
reset_to_prerun_state() {
  ...
  case "$strategy" in
    ...
    unwind)
      ...
      time_reset_step "rsil.unwind" unwind_to_prerun_head "$binary"
      drop_caches
      ;;
  esac
}
...
start_node() {
  ...
  local online max_rsil rsil_cpus
  online="$(nproc --all)"
  max_rsil=$(( online - 1 ))
  if [ "${BENCH_CORES:-0}" -gt 0 ] && [ "$BENCH_CORES" -lt "$max_rsil" ]; then
    max_rsil="$BENCH_CORES"
  fi
  if [ "$max_rsil" -lt 1 ]; then
    rsil_cpus="0"
  else
    rsil_cpus="1-${max_rsil}"
  fi

  local -a rsil_args=(
    node
    --datadir "$DATADIR"
    --log.file.directory "$output_dir/rsil-logs"
  ...
  if [ -f "$DATADIR/genesis.json" ]; then
    rsil_args+=(--chain "$DATADIR/genesis.json")
  fi
  ...
  if grep -qF -- '--debug.startup-sync-state-idle' <<< "$node_help"; then
    rsil_args+=(--debug.startup-sync-state-idle)
  fi
  ...
  if [[ "$phase" == "prerun" || "$phase" == *-setup ]] &&
    ...
    rsil_args+=(--engine.persistence-threshold 0 --engine.persistence-backpressure-threshold 1)
  fi
  ...
  if [ -n "$extra_node_args" ]; then
    ...
    rsil_args+=($extra_node_args)
  fi

  if [ -n "${BENCH_METRICS_ADDR:-}" ]; then
    rsil_args+=(--metrics "$BENCH_METRICS_ADDR")
  fi
  ...
  echo "Starting rsil (${label}/${phase}) with AllowedCPUs=${rsil_cpus}, MemoryMax=${mem_limit}"

  sudo systemd-run --quiet --scope --collect --unit="$RSIL_SCOPE" \
    -p MemoryMax="$mem_limit" -p AllowedCPUs="$rsil_cpus" \
    nice -n -20 "$binary" "${rsil_args[@]}" \
    >> "$CURRENT_LOG" 2>&1 &

  if [ "$follow_log" = "true" ]; then
    stdbuf -oL tail -f "$CURRENT_LOG" | sed -u "s/^/[rsil] /" &
    TAIL_PID=$!
  fi
