...
# snapshot is replaced via `rsil download` and promoted as the new schelk
# baseline.
#
# Usage: bench-rsil-snapshot.sh [--check]
#   --check   Exit 0 if the local snapshot is current, 10 if it needs refresh.
#
# Required env:
#   SCHELK_MOUNT       - schelk mount point (e.g. /rsil-bench)
#   BENCH_RSIL_BINARY  - path to the rsil-compatible binary (required to refresh)
#
# Optional env:
...
resolve_big_blocks_manifest() {
  ...
  local root="${BENCH_BIG_BLOCKS_SNAPSHOT_ROOT:-http://10.10.0.50:9000/rsil-snapshots}"
  local prefix="${BENCH_BIG_BLOCKS_SNAPSHOT_PREFIX:-rsil-1-minimal-stable-weekly}"
...
REMOTE_MANIFEST="$(mktemp "${TMPDIR:-/tmp}/rsil-bench-remote-manifest.XXXXXX")"
REMOTE_CANONICAL="$(mktemp "${TMPDIR:-/tmp}/rsil-bench-remote-canonical.XXXXXX")"
LOCAL_CANONICAL="$(mktemp "${TMPDIR:-/tmp}/rsil-bench-local-canonical.XXXXXX")"
DOWNLOAD_MANIFEST="$(mktemp "${TMPDIR:-/tmp}/rsil-bench-download-manifest.XXXXXX")"
...
RSIL="${BENCH_RSIL_BINARY:?BENCH_RSIL_BINARY must be set when refreshing the snapshot}"
if [ ! -x "$RSIL" ]; then
  echo "::error::rsil binary not found or not executable at ${RSIL}"
  exit 1
fi
...
# rsil download runs as the current user and needs write access.
