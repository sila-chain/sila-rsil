...
#
# Reads:
#   state/nightly-last-feature-ref  (nightly, from decofe/rsil-bench-charts repo)
#   state/hourly-last-feature-ref   (hourly, from decofe/rsil-bench-charts repo)
#   state/release-last-feature-ref  (release, from decofe/rsil-bench-charts repo)
#
...
FORCE="${1:-false}"
MODE="${2:-nightly}"
REPO="${GITHUB_REPOSITORY:-sila-chain/sila-rsil}"
...
if [ "$MODE" = "hourly" ]; then
  ...
  LAST_FEATURE_REF=""
  STATE_URL="https://raw.githubusercontent.com/decofe/rsil-bench-charts/state/state/hourly-last-feature-ref"
...
if [ "$MODE" = "release" ]; then
  ...
  echo "::group::Reading persisted state"
  LAST_FEATURE_REF=""
  STATE_URL="https://raw.githubusercontent.com/decofe/rsil-bench-charts/state/state/release-last-feature-ref"
  if RAW=$(curl -sfL -H "Authorization: token ${DEREK_TOKEN}" "$STATE_URL"); then
    LAST_FEATURE_REF=$(echo "$RAW" | tr -d '[:space:]')
    echo "Previous feature ref: $LAST_FEATURE_REF"
  else
...
echo "::group::Reading persisted state"
LAST_FEATURE_REF=""
STATE_URL="https://raw.githubusercontent.com/decofe/rsil-bench-charts/state/state/nightly-last-feature-ref"
if RAW=$(curl -sfL -H "Authorization: token ${DEREK_TOKEN}" "$STATE_URL"); then
  LAST_FEATURE_REF=$(echo "$RAW" | tr -d '[:space:]')
  echo "Previous feature ref: $LAST_FEATURE_REF"
else
