...
# every time.
#
# Starts a throwaway rsil node to discover the snapshot's chain tip, extracts
...
# Start a lightweight rsil node just to query the chain tip.
RSIL_ARGS=(
  node
  --datadir "$DATADIR"
  --http
...
sudo systemd-run --quiet --scope --collect --unit="$RSIL_SCOPE" \
  ...
  > "$LOG" 2>&1 &
stdbuf -oL tail -f "$LOG" | sed -u "s/^/[rsil-extract] /" &
TAIL_PID=$!
...
for i in $(seq 1 60); do
  ...
  if [ "$i" -eq 60 ]; then
    echo "::error::rsil (extract) failed to start within 60s"
    cat "$LOG"
    exit 1
  fi
  sleep 1
