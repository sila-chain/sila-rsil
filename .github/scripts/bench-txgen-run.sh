...
RSIL_SCOPE="${RSIL_SCOPE:-rsil-bench.scope}"
...
call_rsil_jit() {
  ...
  if ! response=$(curl -sf http://127.0.0.1:8545 -X POST \
    ...
    -d "{\"jsonrpc\":\"2.0\",\"method\":\"rsil_jit\",\"params\":[\"${action}\"],\"id\":1}"); then
    echo "::error::rsil_jit ${action} request failed"
    exit 1
  fi
  if jq -e '.error? != null' <<< "$response" > /dev/null 2>&1; then
    echo "::error::rsil_jit ${action} failed: ${response}"
    exit 1
  fi
}
...
cleanup() {
  ...
  if sudo systemctl is-active "$RSIL_SCOPE" >/dev/null 2>&1; then
    if [ "${BENCH_SAMPLY:-false}" = "true" ]; then
      sudo pkill -INT -x rsil 2>/dev/null || true
      sudo pkill -INT -x rsil-bb 2>/dev/null || true
...
RSIL_ARGS=(
  ...
  --http.api sil,net,web3,debug,rsil,testing
  --http.port 8545
  --ws
  --ws.api all
  --authrpc.port 8551
...
if [ "${BENCH_OTLP_DISABLED:-false}" != "true" ]; then
  if [ -n "${BENCH_OTLP_TRACES_ENDPOINT:-}" ]; then
    RSIL_ARGS+=(--tracing-otlp="${BENCH_OTLP_TRACES_ENDPOINT}" --tracing-otlp.service-name=rsil-bench --tracing-otlp.service-version="${LABEL}")
  fi
  if [ -n "${BENCH_OTLP_LOGS_ENDPOINT:-}" ]; then
    RSIL_ARGS+=(--logs-otlp="${BENCH_OTLP_LOGS_ENDPOINT}" --logs-otlp.filter=debug)
  fi
fi

if [ "${BENCH_TRACY:-off}" != "off" ]; then
  RSIL_ARGS+=(--log.tracy --log.tracy.filter "${BENCH_TRACY_FILTER:-debug}")
...
if [ "${BENCH_SAMPLY:-false}" = "true" ]; then
  ...
  sudo systemd-run --quiet --scope --collect --unit="$RSIL_SCOPE" \
    -p MemoryMax="$MEM_LIMIT" -p AllowedCPUs="$RSIL_CPUS" \
    env "${SUDO_ENV[@]}" nice -n -20 "$BINARY" "${RSIL_ARGS[@]}" \
    > "$LOG" 2>&1 &
fi
stdbuf -oL tail -f "$LOG" | sed -u "s/^/[rsil] /" &
TAIL_PID=$!
...
for i in $(seq 1 60); do
  ...
  if [ "$i" -eq 60 ]; then
    echo "::error::rsil (${LABEL}) failed to start within 60s"
    cat "$LOG"
    exit 1
  fi
  sleep 1
...
if [ "$SYNC_STATE_IDLE" = "true" ]; then
  for i in $(seq 1 300); do
    ...
    if [ "$i" -eq 300 ]; then
      echo "::error::rsil (${LABEL}) pipeline did not finish within 300s"
      cat "$LOG"
      exit 1
    fi
    sleep 1
  ...
  echo "rsil (${LABEL}) binary does not support --debug.startup-sync-state-idle, skipping sync wait"
fi
...
python3 .github/scripts/bench-txgen-report-to-rsil-csv.py "$OUTPUT_DIR/report.json" "$OUTPUT_DIR"
