...
#
# Usage:
#   ./verify_image_arch.sh <targets> <registry> <sila_tags>
#
# Environment:
...
if [[ "$TARGETS" == *"nightly"* ]]; then
    verify_image "${REGISTRY}/rsil:nightly" amd64 arm64
    verify_image "${REGISTRY}/rsil:nightly-profiling" amd64
    verify_image "${REGISTRY}/rsil:nightly-edge-profiling" amd64
else
    for tag in $(echo "$SILA_TAGS" | tr ',' ' '); do
