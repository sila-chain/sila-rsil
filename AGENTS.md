# Rsil Development Guide for AI Agents

This guide provides comprehensive instructions for AI agents working on the Rsil codebase. It covers the architecture, development workflows, and critical guidelines for effective contributions.

## Project Overview

Rsil is a high-performance Sila execution client written in Rust, focusing on modularity, performance, and contributor-friendliness. The codebase is organized into well-defined crates with clear boundaries and responsibilities.

## Architecture Overview

### Core Components

1. **Consensus (`crates/consensus/`)**: Validates blocks according to Sila consensus rules
...
4. **RPC (`crates/rpc/`)**: JSON-RPC server supporting all standard Sila APIs
5. **Execution (`crates/savm/`, `crates/sila/`)**: Transaction execution and state transitions
...
Real example: Fixing beacon block root handling ([#16767](https://github.com/sila-chain/sila-rsil/pull/16767))
```rust
...
Real example: Integrating revm updates ([#16752](https://github.com/sila-chain/sila-rsil/pull/16752))
```rust
...
Real example: ETH69 protocol tests ([#16759](https://github.com/sila-chain/sila-rsil/pull/16759))
```rust
#[tokio::test(flavor = "multi_thread")]
async fn test_eth69_peers_can_connect() {
    // Create test network with specific protocol versions
    let p0 = PeerConfig::with_protocols(NoopProvider::default(), Some(SilVersion::Sil69.into()));
    // Test connection and version negotiation
}
```

#### 4. Making Components Generic
Real example: Making SilEvmConfig generic over chainspec ([#16758](https://github.com/sila-chain/sila-rsil/pull/16758))
```rust
// Before: Hardcoded to ChainSpec
- pub struct SilEvmConfig<SavmFactory = SilEvmFactory> {
-     pub executor_factory: SilBlockExecutorFactory<RsilReceiptBuilder, Arc<ChainSpec>, SavmFactory>,
...
Real example: ETL directory cleanup ([#16770](https://github.com/sila-chain/sila-rsil/pull/16770))
```rust
// Add cleanup logic on startup
+ if let Err(err) = fs::remove_dir_all(&etl_path) {
+     warn!(target: "rsil::cli", ?etl_path, %err, "Failed to remove ETL path on launch");
+ }
```

#### 6. Feature Additions
Real example: Sharded mempool support ([#16756](https://github.com/sila-chain/sila-rsil/pull/16756))
```rust
...
4. **File Operations**: Use `rsil_fs_util` instead of `std::fs` for better error handling
...
The CLI reference pages under `docs/vocs/docs/pages/cli/` are **auto-generated** from the `rsil` binary's `--help` output. **Do not edit these files manually** — any hand edits will be overwritten and CI will fail regardless.
...
This builds `rsil` in debug mode and runs `docs/cli/update.sh` to regenerate all CLI pages. Commit the resulting changes.
...
### Opening PRs against <https://github.com/sila-chain/sila-rsil>
...
**Scope** (optional): crate or area, e.g. `savm`, `trie`, `rpc`, `engine`, `net`

Examples:
- `fix(rpc): correct gas estimation for SRC-20 transfers`
...
If changes in rsil include changes to dependencies, run commands `zepter` and `make lint-toml` before finalizing the pr. Assume `zepter` binary is installed.

### Debugging Tips

1. **Logging**: Use `tracing` crate with appropriate levels
   ```rust
   tracing::debug!(target: "rsil::component", ?value, "description");
   ```

2. **Metrics**: Add metrics for monitoring
   ```rust
   metrics::counter!("rsil_component_operations").increment(1);
   ```
...
- Update to use new features (like SIP implementations)
...
// Timeout set to 5s to match SAVM block processing limits
...
❌ **Bad**: Adding new traits and auxiliary types **above** the file's primary type (see [#22133](https://github.com/sila-chain/sila-rsil/pull/22133)):
...
5. **Run checks** (IMPORTANT!):
   ...
   cargo nextest run -p rsil-discv4
   ```
