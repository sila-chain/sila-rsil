## Contributing to Rsil

Thanks for your interest in improving Rsil!
...
The Rsil project adheres to the [Rust Code of Conduct][rust-coc]. This code of conduct describes the _minimum_ behavior
...
1. **By opening an issue:** For example, if you believe that you have uncovered a bug
   in Rsil, creating a new issue in the issue tracker is the way to report it.
...
As Rsil is still in heavy development, the documentation can be a bit scattered. The [Rsil Docs][rsil-docs] is our
...
- The Rsil version you are on (and that it is up to date)
...
Pull requests are the way concrete changes are made to the code, documentation, and dependencies of Rsil.
...
If the change being proposed alters code, it is either adding new functionality to Rsil, or fixing existing, broken
functionality.
In both of these cases, the pull request should include one or more tests to ensure that Rsil does not regress in the
future.
...
e.g. `cargo test mytest`) navigate to the directory of that file (e.g. `rsil/crates/rpc/rpc/`) or use
the `-p <package-name>` option to run specific tests of a crate from anywhere in the
workspace (`cargo test -p rsil-rpc mytest`).
...
**Any Rsil community member is welcome to review any pull request**.
...
1. Does this change make sense for Rsil?
2. Does this change make Rsil better, even if only incrementally?
...
can typically be fixed by the Rsil maintainers merging the pull request, but they can also be an opportunity for the
...
of the pull request. Yes, we may merge a particular change that makes Rsil better, but the individual might just not
want to have anything to do with Rsil ever again. The goal is not just having good code.
...
[dev-tg]: https://t.me/paradigm_rsil

[rsil-docs]: https://github.com/sila-chain/sila-rsil/tree/main/docs
