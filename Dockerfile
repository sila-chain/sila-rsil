...
LABEL org.opencontainers.image.source=https://github.com/sila-chain/sila-rsil
...
RUN if [ -n "$RUSTFLAGS" ]; then \
    ...
    cargo build --profile $BUILD_PROFILE --features "$FEATURES" --locked --bin rsil

# ARG is not resolved in COPY so we have to hack around it by copying the
# binary to a temporary location
RUN cp /app/target/$BUILD_PROFILE/rsil /app/rsil

# Use Ubuntu as the release image
FROM ubuntu:24.04 AS runtime
WORKDIR /app

# Copy rsil over from the build stage
COPY --from=builder /app/rsil /usr/local/bin
