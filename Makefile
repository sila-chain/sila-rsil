...
# The release tag of https://github.com/sila-chain/tests to use for EF tests
EF_TESTS_TAG := v17.0
EF_TESTS_URL := https://github.com/sila-chain/tests/archive/refs/tags/$(EF_TESTS_TAG).tar.gz
EF_TESTS_DIR := ./testing/ef-tests/sila-tests

# The release tag of https://github.com/sila-chain/execution-spec-tests to use for EEST tests
EEST_TESTS_TAG := v4.5.0
EEST_TESTS_URL := https://github.com/sila-chain/execution-spec-tests/releases/download/$(EEST_TESTS_TAG)/fixtures_stable.tar.gz
...
.PHONY: install
install: ## Build and install the rsil binary under `$(CARGO_HOME)/bin`.
	cargo install --path bin/rsil --bin rsil --force --locked \
		--features "$(FEATURES)" \
		--profile "$(PROFILE)" \
		$(CARGO_INSTALL_EXTRA_FLAGS)

.PHONY: build
build: ## Build the rsil binary into `target` directory.
	cargo build --bin rsil --features "$(FEATURES)" --profile "$(PROFILE)"
...
# `reproducible` only supports rsil on x86_64-unknown-linux-gnu
build-%-reproducible:
	@if [ "$*" != "rsil" ]; then \
		echo "Error: Reproducible builds are only supported for rsil, not $*"; \
		exit 1; \
	fi
	...
	cargo build --bin rsil --features "$(FEATURES)" --profile "reproducible" --locked --target x86_64-unknown-linux-gnu

.PHONY: build-debug
build-debug: ## Build the rsil binary into `target/debug` directory.
	cargo build --bin rsil --features "$(FEATURES)"
# Builds the rsil binary natively.
build-native-%:
	$(if $(EXTRA_RUSTFLAGS),RUSTFLAGS="$(EXTRA_RUSTFLAGS)") cargo build --bin rsil --target $* --features "$(FEATURES)" --profile "$(PROFILE)"
...
# pages. See: https://github.com/sila-chain/sila-rsil/issues/6742
...
build-%:
	RUSTFLAGS="-C link-arg=-lgcc -Clink-arg=-static-libgcc $(EXTRA_RUSTFLAGS)" \
		cross build --bin rsil --target $* --features "$(FEATURES)" --profile "$(PROFILE)"
...
build-deb-%:
	...
	cargo deb --profile $(PROFILE) --no-build --no-dbgsym --no-strip \
		...
		$(if $(VERSION),--output "target/$*/$(PROFILE)/rsil-$(VERSION)-$*-$(PROFILE).deb")
...
define tarball_release_binary
	...
	cd $(BIN_DIR) && \
		tar -czf rsil-$(GIT_TAG)-$(1)$(3).tar.gz $(2) LICENSES README.md && \
...
build-release-tarballs: ## Create a series of `.tar.gz` files in the BIN_DIR directory, each containing a `rsil` binary for a different target.
	[ -d $(BIN_DIR) ] || mkdir -p $(BIN_DIR)
	$(MAKE) build-x86_64-unknown-linux-gnu
	$(call tarball_release_binary,"x86_64-unknown-linux-gnu","rsil","")
	$(MAKE) build-aarch64-unknown-linux-gnu
	$(call tarball_release_binary,"aarch64-unknown-linux-gnu","rsil","")
...
# Downloads and unpacks Sila Foundation tests in the `$(EF_TESTS_DIR)` directory.
#
# Requires `wget` and `tar`
$(EF_TESTS_DIR):
	mkdir $(EF_TESTS_DIR)
	wget $(EF_TESTS_URL) -O sila-tests.tar.gz
	tar -xzf sila-tests.tar.gz --strip-components=1 -C $(EF_TESTS_DIR)
	rm sila-tests.tar.gz
...
update-book-cli: build-debug ## Update book cli documentation.
	...
	@./docs/cli/update.sh $(CARGO_TARGET_DIR)/debug/rsil

.PHONY: profiling
profiling: ## Builds `rsil` with optimisations, but also symbols.
	RUSTFLAGS="-C target-cpu=native" cargo build --profile profiling

.PHONY: maxperf
maxperf: ## Builds `rsil` with the most aggressive optimisations.
	RUSTFLAGS="-C target-cpu=native" cargo build --profile maxperf

.PHONY: maxperf-no-asm
maxperf-no-asm: ## Builds `rsil` with the most aggressive optimisations, minus the "asm-keccak" feature.
	RUSTFLAGS="-C target-cpu=native" cargo build --profile maxperf --no-default-features --features jemalloc,min-trace-logs,otlp,otlp-logs,rsil-revm/portable,js-tracer,keccak-cache-global,gmp,rocksdb
