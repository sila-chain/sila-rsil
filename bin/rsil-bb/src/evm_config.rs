//! Big-block SAVM configuration.
//!
//! Wraps [`SilEvmConfig`] to create executors that handle multi-segment
//! big-block execution internally. At transaction boundaries defined by
//! [`BigBlockData`], the executor swaps the SAVM environment (block env,
//! cfg env) and applies pre/post execution changes for each segment.

use alloy_primitives::Bytes;
pub(crate) use rsil_engine_primitives::BigBlockData;
use rsil_engine_primitives::ExecutionPayload as _;
use rsil_storage_errors::any::AnyError;
...
pub(crate) struct BigBlockSegment<'a> {
    ...
    pub start_tx: usize,
    /// The SAVM environment for this segment.
    pub evm_env: SavmEnv,
    /// The execution context for this segment.
    pub ctx: SilBlockExecutionCtx<'a>,
}
...
/// SAVM configuration for big-block execution.
///
/// Wraps [`SilEvmConfig`]. When a big-block payload is received, the plan is
...
/// segment boundary via a [`BlockHashSeeder`](crate::savm::BlockHashSeeder)
/// callback injected in [`ConfigureEvm::create_executor`].
#[derive(Debug, Clone)]
pub struct BbEvmConfig<C = ChainSpec> {
    /// The inner Sila SAVM configuration (used for env computation).
    pub inner: SilEvmConfig<C>,
...
impl<C> BbEvmConfig<C> {
    /// Creates a new big-block SAVM configuration.
    pub fn new(inner: SilEvmConfig<C>) -> Self
    where
        C: Clone,
    {
...
///
/// This is used as a [`BlockHashSeeder`](crate::savm::BlockHashSeeder) callback,
/// injected into [`BbBlockExecutor`](crate::savm::BbBlockExecutor) from
...
/// generic [`BbBlockExecutor`](crate::savm::BbBlockExecutor) can pick its
...
///
/// Used as a [`BalIndexBumper`](crate::savm::BalIndexBumper) callback so the
/// generic [`BbBlockExecutor`](crate::savm::BbBlockExecutor) can advance
...
///
/// Used as a [`BalIndexSetter`](crate::savm::BalIndexSetter) callback so
/// [`BbBlockExecutor::initialize`](crate::savm::BbBlockExecutor) can renumber
...
where
    C: SilExecutorSpec + SilChainSpec<Header = Header> + Hardforks + 'static,
{
    ...
    fn context_for_block<'a>(
        &self,
        _block: &'a SealedBlock<rsil_sila_primitives::Block>,
    ) -> Result<BbEvmPlan<'a>, Self::Error> {
        unreachable!("big blocks SAVM should only be used from within the engine pipeline")
    }
...
impl<Spec: SilExecutorSpec + 'static> BlockAssembler<BbBlockExecutorFactory<Spec>>
    for BbBlockAssembler
{
    ...
    fn assemble_block(
        ...
        _input: rsil_evm::execute::BlockAssemblerInput<
            ...
            <Self::Block as rsil_node_api::Block>::Header,
        >,
    ) -> Result<Self::Block, rsil_errors::BlockExecutionError> {
        unreachable!("block building is not supported for big blocks SAVM")
    }
}
