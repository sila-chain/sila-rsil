...
//!
//! [`BbBlockExecutor`] wraps [`SilBlockExecutor`] and intercepts
...
/// Block executor that wraps [`SilBlockExecutor`] and handles segment-boundary
...
/// its start-of-block logic: SIP-2935/SIP-4788 system calls).
///
    ...
    DB: Database,
{
    ...
    inner: Option<SilBlockExecutor<'a, SilEvm<DB, I, P>, Spec, RsilReceiptBuilder>>,
    plan: BbEvmPlan<'a>,
    ...
    TxEnv: FromRecoveredTx<TransactionSigned> + FromTxWithEncoded<TransactionSigned>,
{
    ...
    pub(crate) fn new(
        ...
        plan: BbEvmPlan<'a>,
        spec: Spec,
        receipt_builder: RsilReceiptBuilder,
        block_hash_seeder: Option<BlockHashSeeder<DB>>,
        bal_index_reader: Option<BalIndexReader<DB>>,
        bal_index_bumper: Option<BalIndexBumper<DB>>,
        bal_index_setter: Option<BalIndexSetter<DB>>,
    ...
    const fn inner(&self) -> &SilBlockExecutor<'a, SilEvm<DB, I, P>, Spec, RsilReceiptBuilder> {
        self.inner.as_ref().expect("inner executor must exist")
    }

    const fn inner_mut(
        &mut self,
    ) -> &mut SilBlockExecutor<'a, SilEvm<DB, I, P>, Spec, RsilReceiptBuilder> {
    ...
    fn apply_segment_boundary(&mut self) -> Result<(), BlockExecutionError> {
        ...
        // post-execution system calls (SIP-7002/7251) and withdrawal balance
        // increments via SilBlockExecutor::finish() at the current bal_index
        ...
        let (mut savm, result) = inner.finish()?;
        ...
        // Swap SAVM env to the next segment's values (using real gas_limit).
        let ctx = savm.ctx_mut();
        ...
        // gas_limit correctly.
        let mut new_inner =
            SilBlockExecutor::new(savm, new_segment.ctx.clone(), spec, receipt_builder);
        ...
        // Apply pre-execution changes for the new segment (SIP-2935, SIP-4788)
        // at bal_index K+1.
        self.inner_mut().apply_pre_execution_changes()?;

        trace!(target: "engine::bb::savm", "Started segment {seg_idx}");
    ...
    TxEnv: FromRecoveredTx<TransactionSigned> + FromTxWithEncoded<TransactionSigned>,
{
    ...
    fn apply_pre_execution_changes(&mut self) -> Result<(), BlockExecutionError> {
        // Init swaps the SAVM's block_env and executor ctx to the starting
        // segment's values so the SIP-2935/SIP-4788 system calls use the
        ...
        // wrong SIP-2935 slots and corrupting state.
        self.initialize()?;
        self.inner_mut().apply_pre_execution_changes()
    }
    ...
    fn finish(
        ...
        let last_ctx = SilBlockExecutionCtx {
        ...
        let (savm, mut result) = inner.finish()?;
...
impl<Spec> BbBlockExecutorFactory<Spec> {
    ...
    pub const fn evm_factory(&self) -> &SilEvmFactory {
        &self.evm_factory
    }
    ...
    pub const fn receipt_builder(&self) -> &RsilReceiptBuilder {
    ...
    TxEnv: FromRecoveredTx<TransactionSigned> + FromTxWithEncoded<TransactionSigned>,
{
    ...
    type Transaction = TransactionSigned;
    type Receipt = Receipt;
    type TxExecutionResult = SilTxResult<
        <SilEvmFactory as SavmFactory>::HaltReason,
    ...
    where
        ...
        I: Inspector<SilEvmContext<DB>>,
    {
        BbBlockExecutor::new(savm, ctx, &self.spec, self.receipt_builder, None, None, None, None)
    }
}
