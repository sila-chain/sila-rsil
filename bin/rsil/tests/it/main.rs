...
const RSIL: &str = env!("CARGO_BIN_EXE_rsil");

// ── Helpers ──────────────────────────────────────────────────────────────────

/// Runs `rsil <args>` and returns stdout, asserting exit code 0.
///
...
#[track_caller]
fn rsil_ok(args: &[&str]) -> String {
    let output = Command::new(RSIL).env("RUST_LOG", "off").args(args).output().unwrap();
...
/// Spawns an isolated dev-mode rsil node.
///
...
/// concurrent test runs never collide on the default `rsil/dev/` path.
fn spawn_dev() -> (alloy_node_bindings::RsilInstance, tempfile::TempDir) {
    use alloy_node_bindings::Rsil;

    let datadir = tempfile::tempdir().expect("failed to create temp dir");

    let instance = Rsil::at(RSIL)
        .dev()
        .disable_discovery()
...
#[test]
fn version() {
    let stdout = rsil_ok(&["--version"]);
    assert!(stdout.to_lowercase().contains("rsil"), "stdout: {stdout}");
}

#[test]
fn node_help() {
    let stdout = rsil_ok(&["node", "--help"]);
...
fn unknown_subcommand() {
    let output = Command::new(RSIL).arg("definitely-not-a-cmd").output().unwrap();
    assert!(!output.status.success());
}

#[test]
fn unknown_flag() {
    let output = Command::new(RSIL).args(["node", "--no-such-flag"]).output().unwrap();
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(!output.status.success());
...
async fn dev_node_eth_syncing() {
    ...
    let provider = ProviderBuilder::new().connect_http(rsil.endpoint().parse().unwrap());
...
#[test]
fn init_help() {
    let stdout = rsil_ok(&["init", "--help"]);
    assert!(stdout.contains("--chain"), "stdout: {stdout}");
}

#[test]
fn init_state_help() {
    let stdout = rsil_ok(&["init-state", "--help"]);
    assert!(stdout.contains("--chain"), "stdout: {stdout}");
}

#[test]
fn import_help() {
    let stdout = rsil_ok(&["import", "--help"]);
    assert!(stdout.contains("--chain"), "stdout: {stdout}");
}

#[test]
fn import_era_help() {
    let stdout = rsil_ok(&["import-era", "--help"]);
    assert!(stdout.contains("--chain"), "stdout: {stdout}");
}

#[test]
fn export_era_help() {
    let stdout = rsil_ok(&["export-era", "--help"]);
    assert!(stdout.contains("--chain"), "stdout: {stdout}");
}

#[test]
fn dump_genesis_help() {
    let stdout = rsil_ok(&["dump-genesis", "--help"]);
    assert!(stdout.contains("--chain"), "stdout: {stdout}");
}

#[test]
fn db_help() {
    let stdout = rsil_ok(&["db", "--help"]);
    assert!(stdout.contains("stats"), "stdout: {stdout}");
}

#[test]
fn stage_help() {
    let stdout = rsil_ok(&["stage", "--help"]);
    assert!(stdout.contains("run"), "stdout: {stdout}");
}

#[test]
fn p2p_help() {
    let stdout = rsil_ok(&["p2p", "--help"]);
    assert!(stdout.contains("header"), "stdout: {stdout}");
}

#[test]
fn config_help() {
    let stdout = rsil_ok(&["config", "--help"]);
    assert!(stdout.contains("--default"), "stdout: {stdout}");
}

#[test]
fn prune_help() {
    let stdout = rsil_ok(&["prune", "--help"]);
    assert!(stdout.contains("--chain"), "stdout: {stdout}");
}

#[test]
fn download_help() {
    let stdout = rsil_ok(&["download", "--help"]);
    assert!(stdout.contains("--chain"), "stdout: {stdout}");
...
fn download_force_removes_only_snapshot_paths() {
    ...
    fs::write(datadir.join("rsil.toml"), b"old config").expect("failed to write old config");
    ...
    rsil_ok(&["download", "--datadir", datadir_arg, "--url", &archive_url, "--force"]);
    ...
    assert!(!datadir.join("rsil.toml").exists(), "old config file should be removed");
    assert_eq!(
...
#[test]
fn re_execute_help() {
    let stdout = rsil_ok(&["re-execute", "--help"]);
    assert!(stdout.contains("--chain"), "stdout: {stdout}");
}
...
fn config_default_valid_toml() {
    let stdout = rsil_ok(&["config", "--default"]);
...
fn dump_genesis_mainnet_valid_json() {
    let stdout = rsil_ok(&["dump-genesis"]);
...
fn dump_genesis_sepolia_valid_json() {
    let stdout = rsil_ok(&["dump-genesis", "--chain", "sepolia"]);
