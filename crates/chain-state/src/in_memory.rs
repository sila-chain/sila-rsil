...
pub(crate) struct InMemoryState<N: NodePrimitives = SilPrimitives> {
...
type PendingBlockAndReceipts<N> =
    (RecoveredBlock<<N as NodePrimitives>::Block>, Vec<rsil_primitives_traits::ReceiptTy<N>>);
...
/// is in memory.
#[derive(Debug, Clone)]
pub struct CanonicalInMemoryState<N: NodePrimitives = SilPrimitives> {
...
#[derive(Debug, Clone)]
pub struct BlockState<N: NodePrimitives = SilPrimitives> {
...
#[derive(Clone, Debug)]
pub struct ExecutedBlock<N: NodePrimitives = SilPrimitives> {
    /// Recovered Block
...
#[derive(Debug)]
pub enum NewCanonicalChain<N: NodePrimitives = SilPrimitives> {
...
mod tests {
    ...
    fn create_mock_state(
        test_block_builder: &mut TestBlockBuilder<SilPrimitives>,
        block_number: u64,
        parent_hash: B256,
    ) -> BlockState {
        BlockState::new(
    ...
    fn test_canonical_in_memory_state_canonical_chain_single_block() {
        let block = TestBlockBuilder::sil().get_executed_block_with_number(1, B256::random());
        let hash = block.recovered_block().hash();
        let mut blocks = B256Map::default();
    ...
    fn test_canonical_in_memory_state_canonical_chain_with_pending_block() {
        let mut parent_hash = B256::random();
        let mut block_builder = TestBlockBuilder::<SilPrimitives>::sil();
        let state: CanonicalInMemoryState = CanonicalInMemoryState::empty();
