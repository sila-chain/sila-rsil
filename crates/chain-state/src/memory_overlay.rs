...
pub struct MemoryOverlayStateProviderRef<
    'a,
    N: NodePrimitives = rsil_sila_primitives::SilPrimitives,
> {
...
#[expect(missing_debug_implementations)]
pub struct MemoryOverlayStateProvider<N: NodePrimitives = rsil_sila_primitives::SilPrimitives> {
    /// Historical state provider for state lookups that are not found in memory blocks.
...
rsil_storage_api::macros::delegate_provider_impls!(MemoryOverlayStateProvider<N> where [N: NodePrimitives]);
