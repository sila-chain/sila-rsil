...
#[cfg(feature = "rayon")]
use rsil_tasks::WorkerPool;
use rsil_trie::{updates::TrieUpdatesSorted, HashedPostStateSorted, TrieInputSorted};
use std::{
    fmt,
    sync::{Arc, OnceLock},
    time::Instant,
...
#[derive(Clone)]
pub struct StateTrieOverlayManager<N: NodePrimitives = SilPrimitives> {
...
mod tests {
    ...
    fn test_blocks() -> Vec<ExecutedBlock<SilPrimitives>> {
        TestBlockBuilder::sil()
    ...
    fn errors_for_unknown_parent() {
        let manager = StateTrieOverlayManager::<SilPrimitives>::default();
    ...
    fn precompute_skips_in_progress_overlay() {
        let manager = StateTrieOverlayManager::<SilPrimitives>::new(Arc::new(WorkerPool::new(
            1,
            "test-ovly",
        )));
        let key = OverlayCacheKey {
