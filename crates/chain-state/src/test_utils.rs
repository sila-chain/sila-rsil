...
/// their execution.
#[derive(Debug)]
pub struct TestBlockBuilder<N: NodePrimitives = SilPrimitives> {
...
impl<N: NodePrimitives> TestBlockBuilder<N> {
    ...
    fn get_executed_block(
        ...
        let hashed_state = rsil_trie::HashedPostState::from_bundle_state::<
            rsil_trie::KeccakKeyHasher,
        >(bundle.state.iter())
        .into_sorted();
...
impl TestBlockBuilder {
    /// Creates a `TestBlockBuilder` configured for Sila primitives.
    pub fn sil() -> Self {
        Self::default()
    }
}
