//! Cli abstraction for rsil based nodes.
...
pub trait RsilCli: Sized {
    /// The associated `ChainSpecParser` type
    type ChainSpecParser: ChainSpecParser;

    /// The name of the implementation, eg. `rsil`.
    fn name(&self) -> Cow<'static, str>;

    /// The version of the node, such as `rsil/v1.0.0`
    ...
    where
        Self: Parser,
    {
        <Self as RsilCli>::try_parse_from(std::env::args_os())
    }
