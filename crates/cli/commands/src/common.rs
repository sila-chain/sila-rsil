//! Contains common `rsil` arguments

pub use rsil_primitives_traits::header::HeaderMut;

use alloy_primitives::B256;
use clap::Parser;
use rsil_chainspec::SilChainSpec;
use rsil_cli::chainspec::ChainSpecParser;
use rsil_config::{config::EtlConfig, Config};
...
use rsil_stages::{sets::DefaultStages, Pipeline, PipelineTarget};
...
impl<C: ChainSpecParser> EnvironmentArgs<C> {
        ...
        C: ChainSpecParser<ChainSpec = N::ChainSpec>,
    {
        ...
        if access.is_read_write() {
            rsil_fs_util::create_dir_all(&db_path)?;
            rsil_fs_util::create_dir_all(&sf_path)?;
            rsil_fs_util::create_dir_all(&rocksdb_path)?;
        }
        ...
        let mut config = Config::from_path(config_path)
            .inspect_err(
                |err| warn!(target: "rsil::cli", %err, "Failed to load config file, using default"),
            )
            .unwrap_or_default();
        ...
        info!(target: "rsil::cli", ?db_path, ?sf_path, "Opening storage");
        ...
        let rocksdb_provider = if !access.is_read_write() && !RocksDBProvider::exists(&rocksdb_path)
        {
            ...
            debug!(target: "rsil::cli", ?rocksdb_path, "RocksDB not found, initializing empty database");
            rsil_fs_util::create_dir_all(&rocksdb_path)?;
        ...
        if access.is_read_write() {
            debug!(target: "rsil::cli", chain=%self.chain.chain(), genesis=?self.chain.genesis_hash(), "Initializing genesis");
        ...
        C: ChainSpecParser<ChainSpec = N::ChainSpec>,
    {
                ...
                factory.static_file_provider().check_consistency(&factory.provider()?)?
        {
            ...
            info!(target: "rsil::cli", unwind_target = %unwind_target, "Executing an unwind after a failed storage consistency check.");
...
{
    type Savm = <<N::ComponentsBuilder as NodeComponentsBuilder<FullTypesAdapter<Self>>>::Components as NodeComponents<FullTypesAdapter<Self>>>::Savm;
...
type SavmFor<N> = <<<N as Node<FullTypesAdapter<N>>>::ComponentsBuilder as NodeComponentsBuilder<
    FullTypesAdapter<N>,
>>::Components as NodeComponents<FullTypesAdapter<N>>>::Savm;
...
pub trait CliNodeComponents<N: CliNodeTypes>: Send + Sync + 'static {
    /// Returns the configured SAVM.
    fn evm_config(&self) -> &SavmFor<N>;
    /// Returns the consensus implementation.
    fn consensus(&self) -> &ConsensusFor<N>;
}

impl<N: CliNodeTypes> CliNodeComponents<N> for (SavmFor<N>, ConsensusFor<N>) {
    fn evm_config(&self) -> &SavmFor<N> {
        &self.0
    }
