...
use rsil_db::{static_file::iter_static_files, DatabaseEnv};
use rsil_db_api::{
    cursor::DbCursorRO, table::Table, transaction::DbTx, RawKey, RawTable, RawValue, TableViewer,
    Tables,
};
use rsil_db_common::DbTool;
use rsil_node_builder::{NodeTypesWithDB, NodeTypesWithDBAdapter};
use rsil_provider::{providers::ProviderNodeTypes, DBProvider, StaticFileProviderFactory};
use rsil_static_file_types::{ChangesetOffset, StaticFileSegment};
use std::{
...
#[derive(Parser, Debug)]
/// The arguments for the `rsil db checksum` command
pub struct Command {
    #[command(subcommand)]
...
impl Command {
    ...
    pub fn execute<N: CliNodeTypes<ChainSpec: SilaHardforks>>(
        self,
...
fn checksum_hasher() -> impl Hasher {
    FixedState::with_seed(u64::from_be_bytes(*b"RSILRSIL")).build_hasher()
}

fn checksum_static_file<N: CliNodeTypes<ChainSpec: SilaHardforks>>(
    tool: &DbTool<NodeTypesWithDBAdapter<N, DatabaseEnv>>,
    segment: StaticFileSegment,
    start_block: Option<u64>,
    end_block: Option<u64>,
...
fn checksum_change_based_segment<H: Hasher>(
    ...
    cursor: &mut rsil_db::static_file::StaticFileCursor<'_>,
) -> eyre::Result<bool> {
