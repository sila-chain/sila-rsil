use clap::Parser;
use rsil_db::{open_db_read_only, tables_to_generic, DatabaseEnv};
use rsil_db_api::{
...
#[derive(Parser, Debug)]
/// The arguments for the `rsil db diff` command
pub struct Command {
    /// The path to the data dir for all rsil files and subdirectories.
...
impl Command {
    ...
    pub fn execute<T: NodeTypes>(
        ...
        warn!("Make sure the node is not running when running `rsil db diff`!");
        // open second db
