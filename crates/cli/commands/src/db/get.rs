...
use rsil_node_api::{HeaderTy, ReceiptTy, TxTy};
use rsil_node_builder::NodeTypesWithDB;
use rsil_primitives_traits::ValueWithSubKey;
use rsil_provider::{
    providers::ProviderNodeTypes, ChangeSetReader, RocksDBProviderFactory,
    StaticFileProviderFactory,
};
use rsil_static_file_types::StaticFileSegment;
use rsil_storage_api::StorageChangeSetReader;
use tracing::error;

/// The arguments for the `rsil db get` command
#[derive(Parser, Debug)]
pub struct Command {
    #[command(subcommand)]
...
enum Subcommand {
    ...
    ///
    /// Examples:
    ///   rsil db get rocksdb accounts-history 0xdBBE3D8c2d2b22A2611c5A94A9a12C2fCD49Eb29
    ///   rsil db get rocksdb accounts-history 0xdBBE...Eb29 --block 1000000
    ///   rsil db get rocksdb accounts-history 0xdBBE...Eb29 --all-shards
    ///   rsil db get rocksdb storages-history 0xdBBE...Eb29 --storage-key 0x0000...0003
    Rocksdb {
        /// The RocksDB table
        #[arg(value_enum)]
        table: RocksDbTable,
                ...
                if let StaticFileSegment::StorageChangeSets = segment {
                    ...
                    if let Some(storage_key) = storage_key {
                        ...
                        if let Some(entry) = entry {
                            let se: rsil_primitives_traits::StorageEntry = entry;
                            println!("{}", serde_json::to_string_pretty(&se)?);
                        } else {
                            error!(target: "rsil::cli", "No content for the given table key.");
                        }
                        return Ok(());
                    }
...
fn get_rocksdb_account_history(
    rocksdb: &rsil_provider::providers::RocksDBProvider,
    key: &str,
    block: Option<u64>,
    all_shards: bool,
    raw: bool,
...
fn get_rocksdb_table<T: Table>(
    ...
    if raw {
        ...
        match content {
            ...
            None => error!(target: "rsil::cli", "No content for the given table key."),
        }
    } else {
...
impl<N: ProviderNodeTypes> TableViewer<()> for GetValueViewer<'_, N> {
    ...
    where
        T::Value: rsil_primitives_traits::ValueWithSubKey<SubKey = T::SubKey>,
    {
        ...
        if let Some(ref end_key_str) = self.end_key {
            ...
            match content {
                ...
                None => {
                    error!(target: "rsil::cli", "No content for the given table subkey.");
                }
            };
        }
        Ok(())
