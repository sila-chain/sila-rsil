...
use rsil_db::{transaction::DbTx, DatabaseEnv};
use rsil_db_api::{database::Database, table::Table, RawValue, TableViewer, Tables};
use rsil_db_common::{DbTool, ListFilter};
use rsil_node_builder::{NodeTypes, NodeTypesWithDBAdapter};
use std::cell::RefCell;
use tracing::error;

#[derive(Parser, Debug)]
/// The arguments for the `rsil db list` command
pub struct Command {
    /// The table name
    table: Tables,
...
impl Command {
    ...
    pub fn execute<N: NodeTypes<ChainSpec: SilaHardforks>>(
        self,
