//! `rsil db migrate-v2` command for migrating v1 storage layout to v2.
...
use rsil_node_builder::NodeTypesWithDBAdapter;
use rsil_provider::{
...
use rsil_storage_api::StageCheckpointReader;
use tracing::{info, warn};

/// `rsil db migrate-v2` command
#[derive(Debug, Parser)]
pub struct Command;
...
impl Command {
    ...
    {
        // === Phase 0: Preflight ===
        info!(target: "rsil::cli", "Starting v1 → v2 storage migration");

        let provider = provider_factory.provider()?;
        let current_settings = provider.storage_settings()?;

        if current_settings.is_some_and(|s| s.is_v2()) {
            info!(target: "rsil::cli", "Storage is already v2, nothing to do");
            return Ok(());
        }
        ...
        info!(target: "rsil::cli", tip, "Chain tip block number");
        ...
        info!(target: "rsil::cli", "Writing StorageSettings v2 metadata");
        {
            let provider_rw = provider_factory.database_provider_rw()?;
            provider_rw.write_storage_settings(StorageSettings::v2())?;
            provider_rw.commit()?;
        }
        info!(target: "rsil::cli", "Storage settings updated to v2");
        ...
        info!(target: "rsil::cli", "Migration complete. You should now restart the node and let it run the pipeline to rebuild the remaining data.");
        Ok(())
    }
    ...
    fn migrate_account_changesets<N: ProviderNodeTypes>(
        ...
        info!(target: "rsil::cli", "Migrating AccountChangeSets → static files");
        ...
        info!(target: "rsil::cli", count, "AccountChangeSets migrated");
        Ok(())
    }
    ...
    fn migrate_storage_changesets<N: ProviderNodeTypes>(
        ...
        info!(target: "rsil::cli", "Migrating StorageChangeSets → static files");
        let provider = factory.provider()?.disable_long_read_transaction_safety();
        let sf_provider = factory.static_file_provider();
        ...
        info!(target: "rsil::cli", count, "StorageChangeSets migrated");
        Ok(())
    }
        ...
        >,
    {
        ...
        if !provider.prune_modes_ref().receipts_log_filter.is_empty() {
            info!(target: "rsil::cli", "Receipt log filter pruning is enabled, keeping receipts in MDBX");
            return Ok(());
        }
        drop(provider);
        ...
        if existing.is_some_and(|b| b >= tip) {
            info!(target: "rsil::cli", "Receipts already in static files, skipping");
            return Ok(());
        }

        info!(target: "rsil::cli", "Migrating Receipts → static files");
        ...
        let segment = rsil_static_file::segments::Receipts;
        rsil_static_file::segments::Segment::copy_to_static_files(&segment, provider, block_range)?;
        ...
        info!(target: "rsil::cli", count, "Receipts migrated");
        Ok(())
    }
    ...
    fn migrate_to_rocksdb<N: ProviderNodeTypes, T: Table>(
        ...
        info!(target: "rsil::cli", table = T::NAME, "Migrating MDBX table → RocksDB");
        ...
        info!(target: "rsil::cli", table = T::NAME, count, "MDBX table migrated to RocksDB");
        Ok(())
    }
    ...
    fn clear_migrated_and_recomputable_tables<N: ProviderNodeTypes>(
        ...
        info!(target: "rsil::cli", "Clearing migrated and recomputable MDBX tables");
        ...
        macro_rules! clear_table {
            ($table:ty) => {{
                ...
                tx.clear::<$table>()?;
                tx.commit()?;
                info!(target: "rsil::cli", table = <$table as Table>::NAME, "Cleared");
            }};
        }
        ...
        info!(target: "rsil::cli", "Resetting stage checkpoints");
        ...
        for stage in [StageId::SenderRecovery, StageId::MerkleExecute, StageId::MerkleUnwind] {
            ...
            info!(target: "rsil::cli", %stage, "Checkpoint reset to 0");
        }
        ...
        info!(target: "rsil::cli", "Recomputable tables cleared");
        Ok(())
    }
    ...
    fn compact_mdbx(db: &mdbx::DatabaseEnv) -> eyre::Result<()> {
        ...
        rsil_fs_util::create_dir_all(&compact_path)?;

        info!(target: "rsil::cli", ?db_path, ?compact_path, "Compacting MDBX database");
        ...
        info!(target: "rsil::cli", "MDBX compaction complete");
        Ok(())
    }
    ...
    fn swap_compacted_db(
        ...
        info!(target: "rsil::cli", ?db_path, ?compact_path, "Swapping compacted database");
        ...
        info!(target: "rsil::cli", "Database compaction swap complete");
        Ok(())
    }
}
