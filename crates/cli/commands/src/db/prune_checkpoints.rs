//! `rsil db prune-checkpoints` command for viewing and setting prune checkpoint values.

use clap::{Args, Parser, Subcommand, ValueEnum};
use rsil_db_common::DbTool;
use rsil_provider::{providers::ProviderNodeTypes, DBProvider, DatabaseProviderFactory};
use rsil_prune_types::{PruneCheckpoint, PruneMode, PruneSegment};
use rsil_storage_api::{PruneCheckpointReader, PruneCheckpointWriter};

use crate::common::AccessRights;

/// `rsil db prune-checkpoints` subcommand
#[derive(Debug, Parser)]
pub struct Command {
    #[command(subcommand)]
    command: Subcommands,
