...
use rsil_cli_util::parse_socket_address;
use rsil_db_api::{
...
/// The arguments for the `rsil db repair-trie` command
#[derive(Parser, Debug)]
pub struct Command {
...
fn verify_only<N: ProviderNodeTypes>(tool: &DbTool<N>) -> eyre::Result<()> {
    ...
    rsil_trie_db::with_adapter!(tool.provider_factory, |A| do_verify_only::<_, A>(&tx))
}
...
fn verify_checkpoints(provider: impl StageCheckpointReader) -> eyre::Result<()> {
    ...
    if account_hashing_checkpoint.block_number != merkle_checkpoint.block_number {
        return Err(eyre::eyre!(
            "MerkleExecute stage checkpoint ({}) != AccountHashing stage checkpoint ({}), you must first complete the pipeline sync by running `rsil node`",
    ...
    if storage_hashing_checkpoint.block_number != merkle_checkpoint.block_number {
        return Err(eyre::eyre!(
            "MerkleExecute stage checkpoint ({}) != StorageHashing stage checkpoint ({}), you must first complete the pipeline sync by running `rsil node`",
            merkle_checkpoint.block_number,
    ...
    if merkle_checkpoint_progress.is_some_and(|progress| !progress.is_empty()) {
        return Err(eyre::eyre!(
            "MerkleExecute sync stage in-progress, you must first complete the pipeline sync by running `rsil node`",
        ))
    }
...
fn verify_and_repair<N: ProviderNodeTypes>(tool: &DbTool<N>) -> eyre::Result<()> {
    ...
    let inconsistent_nodes = rsil_trie_db::with_adapter!(tool.provider_factory, |A| {
...
fn do_verify_and_repair<N: ProviderNodeTypes, A: TrieTableAdapter>(
    provider_rw: &mut rsil_provider::DatabaseProviderRW<N::DB, N>,
    block_number: u64,
) -> eyre::Result<usize>
where
    <N::DB as rsil_db_api::database::Database>::TXMut: DbTxMut + DbTx,
{
...
fn verify_repaired_state_root<N: ProviderNodeTypes, A: TrieTableAdapter>(
    provider_rw: &rsil_provider::DatabaseProviderRW<N::DB, N>,
    block_number: u64,
) -> eyre::Result<()>
where
    <N::DB as rsil_db_api::database::Database>::TXMut: DbTxMut + DbTx,
{
    type DbStateRoot<'a, TX, A> = rsil_trie::StateRoot<
