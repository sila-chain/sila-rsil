...
use rsil_provider::{providers::ProviderNodeTypes, StaticFileProviderFactory};
use rsil_storage_api::{BlockNumReader, StateProvider, StorageSettingsCache};
use rsil_tasks::spawn_scoped_os_thread;
use std::{
...
/// The arguments for the `rsil db state` command
#[derive(Parser, Debug)]
pub struct Command {
...
impl Command {
                    ...
                    spawn_scoped_os_thread(s, "db-state-worker", move || {
                        loop {
                            ...
                            tool.provider_factory.db_ref().view(|tx| {
                                ...
                                let start_key =
                                    rsil_db_api::models::BlockNumberAddress((chunk_start, address));
                                let end_key =
                                    rsil_db_api::models::BlockNumberAddress((chunk_end, address));
    ...
    fn print_results(
        ...
        address: Address,
        block: Option<BlockNumber>,
        account: Option<rsil_primitives_traits::Account>,
