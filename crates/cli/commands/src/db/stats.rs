...
use rsil_db::{mdbx, static_file::iter_static_files, DatabaseEnv};
use rsil_db_api::{database::Database, TableViewer, Tables};
use rsil_db_common::DbTool;
use rsil_fs_util as fs;
use rsil_node_builder::{NodePrimitives, NodeTypesWithDB, NodeTypesWithDBAdapter};
use rsil_node_core::dirs::{ChainPath, DataDirPath};
use rsil_provider::{
    providers::{ProviderNodeTypes, StaticFileProvider},
    RocksDBProviderFactory,
};
use rsil_static_file_types::SegmentRangeInclusive;
use std::time::Duration;

#[derive(Parser, Debug)]
/// The arguments for the `rsil db stats` command
pub struct Command {
    ...
    /// entire database.
    ///
    /// For individual table checksums, use the `rsil db checksum` command.
    #[arg(long, default_value_t = false)]
    checksum: bool,
}
