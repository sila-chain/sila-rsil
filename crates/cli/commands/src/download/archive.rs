...
/// Schedules modular archive work for one run of `rsil download`.
...
impl ArchiveProcessor {
    ...
    fn run(self) -> Result<()> {
        ...
        if self.try_reuse_outputs()? {
            info!(target: "rsil::cli", file = %archive.file_name, component = %self.archive.component, "Skipping already verified plain files");
            return Ok(());
        }
            ...
            match state {
                ...
                ArchiveAttemptState::VerifyOutputs => {
                    if self.verify_outputs_with_progress()? {
                        ...
                        warn!(target: "rsil::cli", file = %archive.file_name, component = %self.archive.component, attempt, "Archive extracted, but output verification failed, retrying");
                        state = ArchiveAttemptState::RetryAttempt;
                    }
                }
    ...
    fn run_cached_attempt(&self, format: CompressionFormat) -> Result<()> {
        ...
        if self.archive.ty == super::manifest::SnapshotComponentType::State {
            debug!(target: "rsil::cli", url = %self.archive().url, "Downloading state snapshot archive");
        }
