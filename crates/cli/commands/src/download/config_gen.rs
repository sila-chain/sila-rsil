...
use rsil_chainspec::{SilaHardfork, SilaHardforks};
use rsil_config::config::{BlocksPerFileConfig, Config, PruneConfig, StaticFilesConfig};
use rsil_db::tables;
...
/// Writes a [`Config`] as TOML to `<data_dir>/rsil.toml`.
///
...
pub fn write_config(config: &Config, data_dir: &Path) -> eyre::Result<bool> {
    let config_path = data_dir.join("rsil.toml");

    if config_path.exists() {
        info!(target: "rsil::cli",
            path = ?config_path,
            "rsil.toml already exists, skipping config generation"
        );
        return Ok(false);
    }

    let toml_str = toml::to_string_pretty(config)?;
    rsil_fs_util::write(&config_path, toml_str)?;

    info!(target: "rsil::cli",
        path = ?config_path,
        "Generated rsil.toml based on downloaded components"
    );
    ...
    Tx: DbTx + DbTxMut,
{
    for stage_id in INDEX_STAGE_IDS {
        ...
        info!(target: "rsil::cli", stage = stage_id, "Reset stage checkpoint to block 0");
    }
...
pub(crate) fn config_for_selections(
    ...
    chain_spec: Option<&impl SilaHardforks>,
) -> Config {
...
/// instead of producing `PruneMode::Full` which rsil would reject.
...
mod tests {
    ...
    fn write_prune_checkpoints_sets_all_segments() {
        ...
        let db = rsil_db::init_db(dir.path(), rsil_db::mdbx::DatabaseArguments::default()).unwrap();
    ...
    fn full_preset_matches_default_full_prune_config() {
        ...
        let chain_spec = rsil_chainspec::SILA_MAINNET.clone();
        let config = config_for_selections(
            &selections,
            &empty_manifest(),
        ...
        let paris_block = chain_spec
            .sila_fork_activation(SilaHardfork::SilaParis)
            .block_number()
            .expect("sila-mainnet SilaParis block should be known");
