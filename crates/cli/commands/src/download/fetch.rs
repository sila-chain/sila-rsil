...
impl ArchiveFetcher {
    ...
    fn download_sequential(
        ...
        if !quiet {
            info!(target: "rsil::cli", file = %self.paths.file_name(), "Connecting to download server");
        }
        ...
        for attempt in 1..=max_download_retries {
            ...
            if existing_size > 0 {
                ...
                if !quiet && attempt == 1 {
                    info!(target: "rsil::cli", file = %self.paths.file_name(), "Resuming from {} bytes", existing_size);
                }
            }
    ...
    fn finalize_download(&self, size: u64) -> Result<DownloadedArchive> {
        self.paths.finalize()?;
        if !self.quiet() {
            info!(target: "rsil::cli", file = %self.paths.file_name(), "Download complete");
        }
...
impl SegmentedDownload {
    ...
    fn run(self) -> Result<DownloadedArchive> {
        ...
        paths.finalize()?;
        info!(target: "rsil::cli", file = %paths.file_name(), "Download complete");
