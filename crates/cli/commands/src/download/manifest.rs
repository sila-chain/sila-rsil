...
/// define the block range per archive, matching rsil's static file segment boundaries.
///
...
pub struct SnapshotManifest {
    ...
    /// Rsil version that produced this snapshot.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub rsil_version: Option<String>,
...
pub struct ChunkedArchive {
    /// Number of blocks per archive file. Matches rsil's `blocks_per_file` config.
...
pub fn generate_manifest(
    ...
    Ok(SnapshotManifest {
        ...
        rsil_version: Some(rsil_node_core::version::version_metadata().short_version.to_string()),
        components,
    })
}
