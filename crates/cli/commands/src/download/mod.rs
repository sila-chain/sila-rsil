...
//!
//! `rsil download` prepares a data directory from published snapshot archives. [`DownloadCommand`]
...
use rsil_chainspec::{SilChainSpec, SilaHardfork, SilaHardforks, SILA_MAINNET};
use rsil_cli::chainspec::ChainSpecParser;
use rsil_cli_util::cancellation::CancellationToken;
use rsil_db::{init_db, Database};
use rsil_db_api::transaction::DbTx;
use rsil_fs_util as fs;
use rsil_node_core::args::DefaultPruningValues;
...
const RSIL_SNAPSHOTS_BASE_URL: &str = "https://snapshots-r2.rsil.rs";
const RSIL_SNAPSHOTS_API_URL: &str = "https://snapshots.rsil.rs/api/snapshots";
const RSIL_SNAPSHOTS_SOURCE: &str = "https://snapshots.rsil.rs (default)";
const SNAPSHOT_API_PATH: &str = "/api/snapshots";
const FORCE_REMOVED_DATADIR_PATHS: &[&str] = &["db", "rocksdb", "static_files", "rsil.toml"];
...
pub struct DownloadDefaults {
    ...
    ///
    /// Defaults to `https://snapshots.rsil.rs/api/snapshots`.
...
impl DownloadDefaults {
    ...
    /// Default download configuration with defaults from snapshots.rsil.rs and publicnode
    pub fn default_download_defaults() -> Self {
        Self {
            available_snapshots: vec![
                Cow::Borrowed(RSIL_SNAPSHOTS_SOURCE),
                Cow::Borrowed("https://publicnode.com/snapshots (full nodes & testnets)"),
            ],
            default_base_url: Cow::Borrowed(RSIL_SNAPSHOTS_BASE_URL),
            default_chain_aware_base_url: None,
            snapshot_api_url: Cow::Borrowed(RSIL_SNAPSHOTS_API_URL),
            long_help: None,
        }
    }
    ...
    pub fn long_help(&self) -> String {
        ...
        let implicit_download_help = if self.mainnet_only_discovery() {
            "\nIf no URL is provided, the latest archive snapshot will only be proposed\nfor Sila sila-mainnet. For other chains, provide --manifest-url, --manifest-path,\nor -u explicitly."
        } else {
    ...
    fn mainnet_only_discovery(&self) -> bool {
        self.snapshot_api_url.trim_end_matches('/') == RSIL_SNAPSHOTS_API_URL
    }
    ...
    pub fn with_snapshot_api_url(mut self, url: impl Into<Cow<'static, str>>) -> Self {
        ...
        for source in &mut self.available_snapshots {
            if source.as_ref() == RSIL_SNAPSHOTS_SOURCE {
...
/// CLI command that downloads snapshot archives and configures a rsil node from them.
#[derive(Debug, Parser)]
pub struct DownloadCommand<C: ChainSpecParser> {
    ...
    /// Overwrite existing snapshot data by removing db, rocksdb, static_files, and rsil.toml.
...
impl<C: ChainSpecParser<ChainSpec: SilChainSpec + SilaHardforks>> DownloadCommand<C> {
    ...
    pub async fn execute<N>(self) -> Result<()> {
        ...
        if let Some(ref url) = self.url {
            ...
            .await?;
            info!(target: "rsil::cli", "Snapshot downloaded and extracted successfully");
    ...
    async fn load_manifest(&self, chain_id: u64) -> Result<SnapshotManifest> {
        ...
        info!(target: "rsil::cli", source = %manifest_source, "Fetching snapshot manifest");
    ...
    fn finalize_modular_download(
        ...
        if write_config(&config, target_dir)? {
            ...
            info!(target: "rsil::cli", "{}", desc.join(", "));
        }
        ...
        info!(target: "rsil::cli", "Snapshot download complete. Run `{}` to start syncing.", start_command);
    ...
    async fn resolve_manifest_source(&self, chain_id: u64) -> Result<String> {
        ...
        match &self.manifest_url {
            ...
            None => {
                ...
                if defaults.mainnet_only_discovery() && chain_id != SILA_MAINNET.chain.id() {
                    eyre::bail!(
                        "Snapshots are only auto-discovered for Sila sila-mainnet.\n\n\
...
/// Removes existing snapshot data that is managed by `rsil download`.
fn clear_existing_datadir(target_dir: &Path) -> Result<()> {
    if !target_dir.try_exists()? {
        return Ok(());
    }

    info!(target: "rsil::cli", dir = ?target_dir, "Clearing existing snapshot data");
...
mod tests {
    ...
    use rsil_chainspec::{HOLESKY, SILA_MAINNET};
    ...
    fn test_long_help_generation() {
        ...
        assert!(help.contains("Sila sila-mainnet"));
        assert!(help.contains("snapshots.rsil.rs"));
    ...
    fn test_download_resumable_defaults_to_true() {
        let args = CommandParser::<DownloadCommand<SilaChainSpecParser>>::parse_from(["rsil"]).args;
    ...
    fn test_download_resumable_implicit_true() {
        let args = CommandParser::<DownloadCommand<SilaChainSpecParser>>::parse_from([
            "rsil",
            "--resumable",
        ])
        .args;
    ...
    fn resolve_manifest_source_requires_explicit_source_for_non_mainnet_defaults() {
        ...
        assert!(message.contains("only auto-discovered for Sila sila-mainnet"));
    ...
    fn startup_node_command_omits_default_chain_arg() {
        let command =
            startup_node_command_for_binary::<SilaChainSpecParser>("rsil", SILA_MAINNET.as_ref());

        assert_eq!(command, "rsil node");
    }

    #[test]
    fn startup_node_command_includes_non_default_chain_arg() {
        let command =
            startup_node_command_for_binary::<SilaChainSpecParser>("rsil", HOLESKY.as_ref());

        assert_eq!(command, "rsil node --chain holesky");
    }

    #[test]
    fn startup_node_command_uses_running_binary_name() {
        let command =
            startup_node_command_for_binary::<SilaChainSpecParser>("tempo", HOLESKY.as_ref());
