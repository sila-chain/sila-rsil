...
impl<C: ChainSpecParser<ChainSpec: SilChainSpec + SilaHardforks>> ImportCommand<C> {
        ...
        Comp: CliNodeComponents<N>,
    {
        ...
        info!(target: "rsil::cli", "Starting import of {} file(s)", self.paths.len());
        ...
        for (index, path) in self.paths.iter().enumerate() {
            info!(target: "rsil::cli", "Importing file {} of {}: {}", index + 1, self.paths.len(), path.display());
...
mod tests {
    ...
    fn parse_common_import_command_chain_args() {
        for chain in SUPPORTED_CHAINS {
            let args: ImportCommand<SilaChainSpecParser> =
                ImportCommand::parse_from(["rsil", "--chain", chain, "."]);
            assert_eq!(
                Ok(args.env.chain.chain),
                chain.parse::<rsil_chainspec::Chain>(),
    ...
    fn parse_import_command_with_multiple_paths() {
        let args: ImportCommand<SilaChainSpecParser> =
            ImportCommand::parse_from(["rsil", "file1.rlp", "file2.rlp", "file3.rlp"]);
    ...
    fn parse_import_command_with_fail_on_invalid_block() {
        let args: ImportCommand<SilaChainSpecParser> =
            ImportCommand::parse_from(["rsil", "--fail-on-invalid-block", "chain.rlp"]);
    ...
    fn parse_import_command_default_stops_on_invalid_block() {
        let args: ImportCommand<SilaChainSpecParser> =
            ImportCommand::parse_from(["rsil", "chain.rlp"]);
        assert!(!args.fail_on_invalid_block);
    }
}
