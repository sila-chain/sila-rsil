...
use rsil_db_api::{tables, transaction::DbTx};
...
use rsil_node_api::BlockTy;
use rsil_node_events::node::NodeEvent;
use rsil_provider::{
...
use rsil_stages::{prelude::*, ControlFlow, Pipeline, StageId, StageSet};
    ...
    N: ProviderNodeTypes,
{
    if import_config.no_state {
        info!(target: "rsil::import", "Disabled stages requiring state");
    }

    debug!(target: "rsil::import",
        chunk_byte_len=import_config.chunk_len.unwrap_or(DEFAULT_BYTE_LEN_CHUNK_CHAIN_FILE),
        "Chunking chain import"
    );

    info!(target: "rsil::import", "Consensus engine initialized");
        ...
        .await?
    {
        ...
        info!(target: "rsil::import",
            "Importing chain file chunk"
        );

        if file_client.headers_len() == 0 {
            debug!(target: "rsil::import", "Skipping chain file chunk without valid blocks");
            continue;
        }

        let tip = file_client.tip().ok_or(eyre::eyre!("file client has no tip"))?;
        info!(target: "rsil::import", "Chain file chunk read");
        ...
        debug!(target: "rsil::import", ?tip, "Tip manually set");

        let latest_block_number =
            provider_factory.get_stage_checkpoint(StageId::Finish)?.map(|ch| ch.block_number);
        tokio::spawn(rsil_node_events::node::handle_events(None, latest_block_number, events));

        // Run pipeline
        info!(target: "rsil::import", "Starting sync pipeline");
        if import_config.fail_on_invalid_block {
            // Original behavior: fail on unwind
            tokio::select! {
                res = pipeline.run() => res?,
                _ = tokio::signal::ctrl_c() => {
                    info!(target: "rsil::import", "Import interrupted by user");
                    break;
                },
            }
        } else {
            ...
            match result {
                ...
                Ok(ControlFlow::Continue { block_number }) => {
                    debug!(target: "rsil::import", block_number, "Pipeline chunk completed");
                }
                Ok(ControlFlow::NoProgress { block_number }) => {
                    debug!(target: "rsil::import", ?block_number, "Pipeline made no progress");
                }
                Err(e) => {
