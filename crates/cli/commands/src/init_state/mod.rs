...
use rsil_db_common::init::init_from_state_dump;
use rsil_node_api::NodePrimitives;
use rsil_primitives_traits::{header::HeaderMut, SealedHeader};
use rsil_provider::{
...
pub struct InitStateCommand<C: ChainSpecParser> {
    ...
    /// Specifies whether to initialize the state without relying on SAVM historical data.
    ///
    /// When enabled, and before inserting the state, it creates a dummy chain up to the last SAVM
...
impl<C: ChainSpecParser<ChainSpec: SilChainSpec + SilaHardforks>> InitStateCommand<C> {
    ...
    where
        ...
        >,
    {
        info!(target: "rsil::cli", "Rsil init-state starting");
        ...
        if self.without_evm {
            ...
            if last_block_number == 0 {
                ...
                return Err(eyre::eyre!(
                    "Data directory should be empty when calling init-state with --without-savm."
                ));
            }
        ...
        info!(target: "rsil::cli", "Initiating state dump");

        let reader = BufReader::new(rsil_fs_util::open(self.state)?);

        let hash = init_from_state_dump(reader, &provider_factory, config.stages.etl)?;

        info!(target: "rsil::cli", hash = ?hash, "Genesis block written");
        Ok(())
    }
}
...
mod tests {
    ...
    fn parse_init_state_command_with_without_evm() {
        let cmd: InitStateCommand<SilaChainSpecParser> = InitStateCommand::parse_from([
            "rsil",
            "--chain",
            "sepolia",
            "--without-savm",
