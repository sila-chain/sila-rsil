...
use rsil_stages::{StageCheckpoint, StageId};
...
/// Creates a dummy chain (with no transactions) up to the last SAVM block and appends the
/// first valid block.
...
where
    ...
    F: Fn(BlockNumber) -> <Provider::Primitives as NodePrimitives>::BlockHeader
        ...
        + 'static,
{
    info!(target: "rsil::cli", new_tip = ?header.num_hash(), "Setting up dummy SAVM chain before importing state.");

    let static_file_provider = provider_rw.static_file_provider();
    // Write SAVM dummy data up to `header - 1` block. Skip when the supplied
    ...
    info!(target: "rsil::cli", "Appending first valid block.");
    ...
    info!(target: "rsil::cli", "Set up finished.");
    ...
    F: Fn(BlockNumber) -> N::BlockHeader + Send + Sync + 'static,
{
    ...
    for segment in [
        ...
        rsil_tasks::spawn_os_thread(thread_name, move || {
    ...
    let provider = sf_provider.clone();
    rsil_tasks::spawn_os_thread("init-state-headers", move || {
    ...
    while let Ok(append_result) = rx.recv() {
        if let Err(err) = append_result {
            tracing::error!(target: "rsil::cli", "Error appending dummy chain: {err}");
            return Err(err)
        }
    }
...
mod tests {
    ...
    use rsil_provider::{test_utils::create_test_provider_factory, DatabaseProviderFactory};
    use std::{
        io::Write,
        sync::{
