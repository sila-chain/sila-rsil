...
use clap::Parser;
use rsil_cli_util::{get_secret_key, load_secret_key::rng_secret_key};
    ...
    pub async fn execute(self) -> eyre::Result<()> {
        ...
        let (_discv4, mut discv4_service, _discv5, mut discv5_forwarder) = if self.v5 {
            ...
            if self.addr.is_ipv4() {
                ...
                if let Some(mut addr) = rsil_discv5::config::ipv6(&discv5_cfg.listen_config) {
                    addr.set_port(shared_port);
                    ipv6 = Some(bind_socket(SocketAddr::V6(addr)).await?);
                }
            } else {
                ipv6 = Some(shared_socket);
                if let Some(mut addr) = rsil_discv5::config::ipv4(&discv5_cfg.listen_config) {
                    addr.set_port(shared_port);
...
mod tests {
    ...
    fn repeated_nat_requires_extip_values() {
        let command = Command::parse_from(["rsil", "--nat", "any", "--nat", "extip:2001:db8::1"]);
    ...
    fn repeated_nat_requires_distinct_ip_families() {
        let command =
            Command::parse_from(["rsil", "--nat", "extip:1.2.3.4", "--nat", "extip:5.6.7.8"]);
    ...
    fn single_extip_is_advertised() {
        let command =
            Command::parse_from(["rsil", "--addr", "0.0.0.0:30301", "--nat", "extip:1.2.3.4"]);
