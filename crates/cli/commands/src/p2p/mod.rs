...
use rsil_network::{BlockDownloaderProvider, NetworkConfigBuilder};
use rsil_network_p2p::bodies::client::BodiesClient;
use rsil_node_core::{
...
impl<C: ChainSpecParser<ChainSpec: SilChainSpec + Hardforks + SilaHardforks>> Command<C> {
    /// Execute `p2p` command
        ...
        match self.command {
            Subcommands::Header { args, id } => {
                ...
                let header = (move || get_single_header(fetch_client.clone(), id))
                    .retry(backoff)
                    .notify(|err, _| tracing::warn!(target: "rsil::cli", error = %err, "Error requesting header. Retrying..."))
                    .await?;
                tracing::info!(target: "rsil::cli", ?header, "Successfully downloaded header");
            }
            ...
            Subcommands::Body { args, id } => {
                ...
                let hash = match id {
                    ...
                    BlockHashOrNumber::Number(number) => {
                        tracing::info!(target: "rsil::cli", "Block number provided. Downloading header first...");
                ...
                .retry(backoff)
                .notify(|err, _| tracing::warn!(target: "rsil::cli", error = %err, "Error requesting block. Retrying..."))
                .await?
                .split();
                if result.len() != 1 {
                    eyre::bail!(
                ...
                tracing::info!(target: "rsil::cli", ?body, "Successfully downloaded body")
            }
...
impl<C: ChainSpecParser> DownloadArgs<C> {
    ...
    pub async fn launch_network<N>(
        &self,
    ) -> eyre::Result<rsil_network::NetworkHandle<N::NetworkPrimitives>>
    where
        C::ChainSpec: SilChainSpec + Hardforks + SilaHardforks + Send + Sync + 'static,
...
mod tests {
    ...
    #[test]
    fn parse_header_cmd() {
        let _args: Command<SilaChainSpecParser> =
            Command::parse_from(["rsil", "header", "--chain", "sila-mainnet", "1000"]);
    }

    #[test]
    fn parse_body_cmd() {
        let _args: Command<SilaChainSpecParser> =
            Command::parse_from(["rsil", "body", "--chain", "sila-mainnet", "1000"]);
    }

    #[test]
    fn parse_enode_cmd() {
        let _args: Command<SilaChainSpecParser> =
            Command::parse_from(["rsil", "enode", "/tmp/secret"]);
    }

    #[test]
    fn parse_enode_cmd_with_ip() {
        let _args: Command<SilaChainSpecParser> =
            Command::parse_from(["rsil", "enode", "/tmp/secret", "--ip", "192.168.1.1"]);
    }
}
