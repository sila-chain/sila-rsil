...
use rsil_node_builder::common::metrics_hooks;
use rsil_node_core::{args::MetricArgs, version::version_metadata};
...
impl<C: ChainSpecParser<ChainSpec: SilChainSpec + SilaHardforks>> PruneCommand<C> {
    ...
    pub async fn execute<N: CliNodeTypes<ChainSpec = C::ChainSpec>>(
        ...
        info!(target: "rsil::cli", "Copying data from database to static files...");
        ...
        info!(target: "rsil::cli", ?lowest_static_file_height, "Copied data from database to static files");

        // Delete data which has been copied to static files.
        if let Some(prune_tip) = lowest_static_file_height {
            info!(target: "rsil::cli", ?prune_tip, ?config, "Pruning data from database...");
            ...
            loop {
                if cancellation.is_cancelled() {
                    info!(target: "rsil::cli", total_pruned, "Pruning interrupted by user");
                    break;
                }
                ...
                if output.progress.is_finished() {
                    info!(target: "rsil::cli", total_pruned, "Pruned data from database");
                    break;
                }
        ...
        {
            info!(target: "rsil::cli", "Flushing and compacting RocksDB...");
            provider_factory.rocksdb_provider().flush_and_compact()?;
            info!(target: "rsil::cli", "RocksDB compaction complete");
        }
