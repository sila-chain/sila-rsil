...
use rsil_db_api::{database::Database, models::BlockNumberAddress, table::TableImporter, tables};
...
fn unwind_and_copy<N: ProviderNodeTypes>(
    ...
    let execute_input =
        rsil_stages::ExecInput { target: Some(to), checkpoint: Some(StageCheckpoint::new(from)) };
...
where
    N: ProviderNodeTypes,
{
    info!(target: "rsil::cli", "Executing stage.");
    ...
    loop {
        let input = rsil_stages::ExecInput {
            target: Some(to),
