...
use rsil_provider::{providers::ProviderNodeTypes, BlockNumReader, ProviderFactory};
use rsil_stages::{
...
impl<C: ChainSpecParser<ChainSpec: SilChainSpec + SilaHardforks>> Command<C> {
        ...
        F: FnOnce(Arc<C::ChainSpec>) -> Comp,
    {
        ...
        if self.offline {
            info!(target: "rsil::cli", "Performing an unwind for offline-only data!");
        }

        let highest_static_file_block = provider_factory.provider()?.last_block_number()?;
        info!(target: "rsil::cli", ?target, ?highest_static_file_block, prune_config=?config.prune,  "Executing a pipeline unwind.");
        ...
        info!(target: "rsil::cli", ?target, "Unwound blocks");
    ...
    fn build_pipeline<N: ProviderNodeTypes<ChainSpec = C::ChainSpec>>(
        ...
        let builder = if self.offline {
            Pipeline::<N>::builder().add_stages(
                ...
                .builder()
                .disable(rsil_stages::StageId::SenderRecovery),
            )
        } else {
...
mod tests {
    use super::*;
    use rsil_chainspec::SEPOLIA;
    use rsil_sila_cli::chainspec::SilaChainSpecParser;

    #[test]
    fn parse_unwind() {
        let cmd = Command::<SilaChainSpecParser>::parse_from([
            "rsil",
            "--datadir",
            "dir",
            "to-block",
    ...
    fn parse_unwind_chain() {
        let cmd = Command::<SilaChainSpecParser>::parse_from([
            "rsil", "--chain", "sepolia", "to-block", "100",
        ]);
        assert_eq!(cmd.command, Subcommands::ToBlock { target: BlockHashOrNumber::Number(100) });
