...
use rsil_trie::{hash_builder::HashBuilderValue, TrieMask};
use rsil_trie_common::{hash_builder::HashBuilderState, StoredNibbles, StoredNibblesSubKey};
...
pub fn read_vectors_with(read: &[fn() -> eyre::Result<()>]) -> Result<()> {
    ...
    if let Some(err_list) = errors {
        ...
        return Err(eyre::eyre!(
            "If there are missing types, make sure to run `rsil test-vectors compact --write` first.\n
...
pub fn type_name<T>() -> String {
    // With alloy type transition <https://github.com/sila-chain/sila-rsil/pull/15768> the types are renamed, we map them here to the original name so that test vector files remain consistent
    let name = std::any::type_name::<T>();
    match name {
        "alloy_consensus::transaction::envelope::SilaTypedTransaction<alloy_consensus::transaction::sip4844::TxEip4844>" => "Transaction".to_string(),
        "alloy_consensus::transaction::envelope::SilaTxEnvelope<alloy_consensus::transaction::sip4844::TxEip4844>" => "TransactionSigned".to_string(),
        name => {
