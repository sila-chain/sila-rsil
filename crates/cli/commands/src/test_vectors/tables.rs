                ...
                match table.as_str() {
                    $(
                        stringify!($table_type) => {
                            tracing::info!(target: "rsil::cli", "Generating test vectors for {} <{}>.", stringify!($table_or_dup), tables::$table_type$(::<$($generic),+>)?::NAME);

                            generate_vector!($table_type$(<$($generic),+>)?, $per_table, $table_or_dup);
                        },
                    )*
                    _ => {
                        error!(target: "rsil::cli", "Unknown table: {}", table);
                    }
                }
            }
        }
