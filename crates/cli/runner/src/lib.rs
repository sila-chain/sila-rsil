...
use rsil_tasks::{PanickedTaskError, TaskExecutor};
...
impl CliRunner {
    ...
    pub fn try_default_runtime() -> Result<Self, rsil_tasks::RuntimeBuildError> {
        Self::try_with_runtime_config(rsil_tasks::RuntimeConfig::default())
    }

    /// Creates a new [`CliRunner`] with the given [`RuntimeConfig`](rsil_tasks::RuntimeConfig).
    pub fn try_with_runtime_config(
        config: rsil_tasks::RuntimeConfig,
    ) -> Result<Self, rsil_tasks::RuntimeBuildError> {
        let runtime = rsil_tasks::RuntimeBuilder::new(config).build()?;
    ...
    /// Returns a clone of the underlying [`Runtime`](rsil_tasks::Runtime).
    pub fn runtime(&self) -> rsil_tasks::Runtime {
        self.runtime.clone()
    }
    ...
    where
        ...
        E: Send
            ...
            + std::fmt::Display
            + From<std::io::Error>
            + From<rsil_tasks::PanickedTaskError>
            + 'static,
    {
        ...
        if let Err(err) = &command_res {
            error!(target: "rsil::cli", %err, "shutting down due to error");
        } else {
            debug!(target: "rsil::cli", "shutting down gracefully");
...
where
    F: Future<Output = Result<(), E>>,
    E: Send + Sync + From<rsil_tasks::PanickedTaskError> + 'static,
{
    let fut = pin!(fut);
    tokio::select! {
    ...
    E: Send + Sync + 'static + From<std::io::Error>,
{
    ...
    #[cfg(unix)]
    {
        ...
        tokio::select! {
            _ = ctrl_c => {
                info!(target: "rsil::cli", "Received ctrl-c");
            },
            _ = sigterm => {
                info!(target: "rsil::cli", "Received SIGTERM");
            },
            res = fut => res?,
        }
    }
...
/// Shut down the given [`Runtime`](rsil_tasks::Runtime), and wait for it if `wait` is set.
///
...
fn runtime_shutdown(rt: rsil_tasks::Runtime, wait: bool) {
    ...
    if wait {
        let _ = rx.recv_timeout(DEFAULT_RUNTIME_SHUTDOWN_TIMEOUT).inspect_err(|err| {
            tracing::warn!(target: "rsil::cli", %err, "runtime shutdown timed out");
        });
    }
}
