...
/// Wrapper around [`rsil_fs_util::read_json_file`] which can be used as a clap value parser.
pub fn read_json_from_file<T: serde::de::DeserializeOwned>(path: &str) -> Result<T, FsPathError> {
    rsil_fs_util::read_json_file(Path::new(path))
}

/// Parses an sila value from a string.
///
/// The amount in sil like "1.05" will be interpreted in wei (1.05 * 1e18).
/// Supports both decimal and integer inputs.
///
/// # Examples
/// - "1.05" -> 1.05 SIL = 1.05 * 10^18 wei
/// - "2" -> 2 SIL = 2 * 10^18 wei
pub fn parse_ether_value(value: &str) -> eyre::Result<u128> {
    let sil = value.parse::<f64>()?;
    if sil.is_sign_negative() {
        return Err(eyre::eyre!("Sila value cannot be negative"))
    }
    let wei = sil * 1e18;
    Ok(wei as u128)
}
