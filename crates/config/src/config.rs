...
use rsil_prune_types::{PruneModes, MINIMUM_UNWIND_SAFE_DISTANCE};
use rsil_stages_types::ExecutionStageThresholds;
use rsil_static_file_types::{StaticFileMap, StaticFileSegment};
use std::{
    path::{Path, PathBuf},
    time::Duration,
};
...
/// Configuration for the rsil node.
    ...
    pub fn save(&self, path: &Path) -> Result<(), std::io::Error> {
        if path.extension() != Some(std::ffi::OsStr::new(EXTENSION)) {
            return Err(std::io::Error::new(
                ...
                format!("rsil config file extension must be '{EXTENSION}'"),
            ));
        }
...
mod tests {
    ...
    use rsil_network_peers::TrustedPeer;
...
#";
    ...
    "#;
        ...
        let conf: Config = toml::from_str(rsil_toml).unwrap();
