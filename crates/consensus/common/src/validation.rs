...
/// The maximum RLP length of a block, defined in [SIP-7934](https://sips.sila.org/SIPS/sip-7934).
///
...
/// Ensure the SIP-1559 base fee is set if the London hardfork is active.
#[inline]
pub fn validate_header_base_fee<H: BlockHeader, ChainSpec: SilaHardforks>(
    header: &H,
...
/// Validate that withdrawals are present in SilaShanghai
///
/// See [SIP-4895]: Beacon chain push withdrawals as operations
///
...
/// Validate that blob gas is present in the block if SilaCancun is active.
///
/// See [SIP-4844]: Shard Blob Transactions
///
...
/// * SIP-4895 withdrawals validation, if shanghai is active based on the given chainspec. See more
///   information about the specific checks in [`validate_shanghai_withdrawals`].
/// * SIP-4844 blob gas validation, if cancun is active based on the given chainspec. See more
///   information about the specific checks in [`validate_cancun_gas`].
/// * SIP-7934 block size limit validation, if osaka is active based on the given chainspec.
...
/// Validates that the SIP-4844 header fields exist and conform to the spec. This ensures that:
///
...
/// Validates the base fee against the parent and SIP-1559 rules.
#[inline]
pub fn validate_against_parent_eip1559_base_fee<ChainSpec: SilChainSpec + SilaHardforks>(
...
/// Validates that the SIP-4844 header fields are correct with respect to the parent block. This
...
pub fn validate_against_parent_4844<H: BlockHeader>(
    ...
    // From [SIP-4844](https://sips.sila.org/SIPS/sip-4844#header-extension):
    //
...
mod tests {
    ...
    fn mock_blob_tx(nonce: u64, num_blobs: usize) -> TransactionSigned {
        ...
        let request = Transaction::Sip4844(TxEip4844 {
            chain_id: 1u64,
            nonce,
