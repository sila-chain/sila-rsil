...
pub enum ConsensusError {
    ...
    /// SIP-7825: Transaction gas limit exceeds maximum allowed
    #[error(transparent)]
    TransactionGasLimitTooHigh(Box<TxGasLimitTooHighErr>),
    /// SIP-7928: Error when an unexpected block access list cost is encountered.
    #[error(transparent)]
    BlockAccessListCostMoreThanGasLimit(Box<BlockAccessListGasError>),
    /// SIP-7928: Error when the block access list hash doesn't match the expected value.
    #[error("block access list hash mismatch: {0}")]
    BlockAccessListHashMismatch(GotExpectedBoxed<B256>),
    /// SIP-7928: Error when the block access list cannot be decoded or converted.
