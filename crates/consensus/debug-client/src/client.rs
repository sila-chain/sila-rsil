...
/// provider like SilaScan or an RPC endpoint.
#[derive(Debug)]
    ...
    pub async fn run(self) {
        ...
        while let Some(payload) = payload_stream.recv().await {
            ...
            let (safe_block_hash, finalized_block_hash) = match (
                ...
                (safe_block_hash, finalized_block_hash) => {
                    warn!(target: "consensus::debug-client", ?safe_block_hash, ?finalized_block_hash, "failed to fetch safe or finalized hash from silascan");
                    continue;
                }
            };
