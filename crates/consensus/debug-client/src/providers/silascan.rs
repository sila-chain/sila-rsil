...
use reqwest::Client;
use rsil_node_api::ExecutionPayload;
use rsil_tracing::tracing::{debug, warn};
use serde::{de::DeserializeOwned, Serialize};
use std::{sync::Arc, time::Duration};
use tokio::{sync::mpsc, time::interval};

/// Block provider that fetches new blocks from SilaScan API.
#[derive(derive_more::Debug, Clone)]
pub struct SilaScanBlockProvider<RpcBlock, ExecutionData> {
    http_client: Client,
    base_url: String,
    api_key: String,
    chain_id: u64,
...
impl<RpcBlock, ExecutionData> SilaScanBlockProvider<RpcBlock, ExecutionData>
where
    RpcBlock: Serialize + DeserializeOwned,
{
    /// Create a new SilaScan block provider with the given base URL and API key.
    pub fn new(
        base_url: String,
        api_key: String,
        chain_id: u64,
    ...
    /// Load block using SilaScan API. Note: only `BlockNumberOrTag::Latest`,
    ...
    pub async fn load_payload(
        ...
        debug!(target: "silascan", %resp, "fetched block from silascan");

        let resp: Response<RpcBlock> = serde_json::from_str(&resp).inspect_err(|err| {
            warn!(target: "silascan", "Failed to parse block response from silascan: {}", err);
        })?;
...
impl<RpcBlock, ExecutionData> PayloadProvider for SilaScanBlockProvider<RpcBlock, ExecutionData>
where
