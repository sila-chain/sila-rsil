...
use rsil_cli_commands::import_core::{import_blocks_from_file, ImportConfig};
use rsil_config::Config;
use rsil_db::DatabaseEnv;
use rsil_node_api::{NodeTypesWithDBAdapter, TreeConfig};
use rsil_node_builder::{EngineNodeLauncher, Node, NodeBuilder, NodeConfig, NodeHandle};
use rsil_node_core::args::{DiscoveryArgs, NetworkArgs, RpcServerArgs};
...
pub struct ChainImportResult {
    ...
    pub nodes: Vec<NodeHelperType<SilaNode>>,
...
/// Creates a test setup with Sila nodes that have pre-imported chain data from RLP files.
///
/// This function:
...
///
/// Note: This function is currently specific to `SilaNode` because the import process
/// uses Sila-specific consensus and block format. It can be made generic in the future
...
/// which allows importing test chains that may not strictly conform to sila-mainnet consensus rules. The
...
pub async fn setup_engine_with_chain_import(
    ...
    let mut nodes: Vec<NodeHelperType<SilaNode>> = Vec::with_capacity(num_nodes);
    ...
    for idx in 0..num_nodes {
        ...
        node_config.datadir.datadir =
            rsil_node_core::dirs::MaybePlatformPath::from(datadir.clone());
        ...
        let db_args = rsil_node_core::args::DatabaseArgs::default().database_args();
        let db = rsil_db::init_db(&db_path, db_args)?;
        ...
        let provider_factory =
            ProviderFactory::<NodeTypesWithDBAdapter<SilaNode, DatabaseEnv>>::new(
                db.clone(),
                chain_spec.clone(),
                rsil_provider::providers::StaticFileProvider::read_write(
                    static_files_path.clone(),
                )?,
                rsil_provider::providers::RocksDBProvider::builder(rocksdb_dir_path)
                    .with_default_tables()
                    .build()
                    .unwrap(),
        ...
        rsil_db_common::init::init_genesis(&provider_factory)?;
        ...
        // Create SAVM and consensus for Sila
        let evm_config = rsil_node_sila::SilEvmConfig::new(chain_spec.clone());
        // Use NoopConsensus to skip gas limit validation for test imports
        let consensus = rsil_consensus::noop::NoopConsensus::arc();
...
mod tests {
    ...
    async fn test_stage_checkpoints_persistence() {
        ...
        {
            let db_args = rsil_node_core::args::DatabaseArgs::default().database_args();
            let db = rsil_db::init_db(&db_path, db_args).unwrap();

            let provider_factory: ProviderFactory<
                NodeTypesWithDBAdapter<rsil_node_sila::SilaNode, DatabaseEnv>,
            > = ProviderFactory::new(
                db.clone(),
                chain_spec.clone(),
                rsil_provider::providers::StaticFileProvider::read_write(static_files_path.clone())
                    .unwrap(),
                rsil_provider::providers::RocksDBProvider::builder(rocksdb_dir_path.clone())
                    .with_default_tables()
                    .build()
                    .unwrap(),
                rsil_tasks::Runtime::test(),
            ...
            // Initialize genesis
            rsil_db_common::init::init_genesis(&provider_factory).unwrap();
        ...
        {
            let db = rsil_db::init_db(&db_path, DatabaseArguments::default()).unwrap();
            ...
            > = ProviderFactory::new(
                db,
                chain_spec.clone(),
                rsil_provider::providers::StaticFileProvider::read_only(static_files_path).unwrap(),
                rsil_provider::providers::RocksDBProvider::builder(rocksdb_dir_path)
                    .with_default_tables()
                    .build()
                    .unwrap(),
    ...
    async fn test_import_blocks_only() {
        ...
        let db_path = datadir.join("db");
        let db_args = rsil_node_core::args::DatabaseArgs::default().database_args();
        let db_env = rsil_db::init_db(&db_path, db_args).unwrap();
        let db = Arc::new(rsil_db::test_utils::TempDatabase::new(db_env, db_path));
        ...
        let provider_factory: ProviderFactory<MockNodeTypesWithDB> = ProviderFactory::new(
            db.clone(),
            chain_spec.clone(),
            rsil_provider::providers::StaticFileProvider::read_write(static_files_path).unwrap(),
            rsil_provider::providers::RocksDBProvider::builder(rocksdb_dir_path)
                .with_default_tables()
                .build()
                .unwrap(),
