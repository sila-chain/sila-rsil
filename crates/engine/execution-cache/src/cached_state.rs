...
/// observed sila-mainnet average (~7 KiB). Using `MAX_CODE_SIZE` (48 KiB) overestimates by ~7x,
...
/// readers. Canonical execution usually leaves this disabled because the SAVM database `State`
...
impl<S: HashedPostStateProvider> HashedPostStateProvider for CachedStateProvider<S> {
    fn hashed_post_state(&self, bundle_state: &rsil_revm::db::BundleState) -> HashedPostState {
...
///
/// Since SIP-6780, SELFDESTRUCT only works within the same transaction where the
