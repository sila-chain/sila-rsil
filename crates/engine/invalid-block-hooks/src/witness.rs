...
type CollectionResult =
    (BTreeMap<B256, Bytes>, BTreeMap<B256, Bytes>, rsil_trie::HashedPostState, BundleState);
...
fn generate(
    ...
    let state = state_provider.witness(
        Default::default(),
        hashed_state,
        rsil_trie::ExecutionWitnessMode::Legacy,
    )?;
    Ok(ExecutionWitness {
        state,
...
pub struct InvalidBlockWitnessHook<P, E> {
    /// The provider to read the historical state and do the SAVM execution.
    provider: P,
    /// The SAVM configuration to use for the execution.
    evm_config: E,
...
mod tests {
    ...
    fn test_proof_generator_generate() {
        ...
        let hashed_state = rsil_trie::HashedPostState::default();
    ...
    fn create_test_trie_updates() -> TrieUpdates {
        ...
        use rsil_trie::{updates::TrieUpdates, BranchNodeCompact, Nibbles};
    ...
    fn test_on_invalid_block_calls_all_validation_methods() {
        ...
        let output = BlockExecutionOutput {
            state: bundle_state,
            result: rsil_provider::BlockExecutionResult {
                receipts: vec![],
