...
/// Timing breakdown for `rsil_newPayload` responses.
...
///
/// This is used by the `rsil_newPayload` endpoint to pass environment switches
...
pub struct BigBlockData<ExecutionData> {
    ...
    /// `ExecutionData`, which must be used to derive the initial SAVM environment.
...
pub enum BeaconEngineMessage<Payload: PayloadTypes> {
    ...
    /// Message with new payload used by `rsil_newPayload` endpoint.
    ///
    ...
    ForkchoiceUpdated {
        ...
        tx: oneshot::Sender<RsilResult<OnForkChoiceUpdated>>,
    },
}
...
impl<Payload: PayloadTypes> Display for BeaconEngineMessage<Payload> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::RsilNewPayload { payload, .. } => {
                write!(
                    f,
                    "RsilNewPayload(parent: {}, number: {}, hash: {})",
                    payload.parent_hash(),
                    payload.block_number(),
                    payload.block_hash()
                )
    ...
    Payload: PayloadTypes,
{
    ...
    ///
    /// See also <https://github.com/sila-chain/execution-apis/blob/3d627c95a4d3510a8187dd02e0250ecb4331d27e/src/engine/shanghai.md#engine_newpayloadv2>
    ...
    /// Sends a new payload message used by `rsil_newPayload` endpoint.
    ///
    ...
    pub async fn rsil_new_payload(
        ...
        let (tx, rx) = oneshot::channel();
        let _ = self.to_engine.send(BeaconEngineMessage::RsilNewPayload {
            payload,
    ...
    fn send_fork_choice_updated(
        ...
        state: ForkchoiceState,
        payload_attrs: Option<Payload::PayloadAttributes>,
    ) -> oneshot::Receiver<RsilResult<OnForkChoiceUpdated>> {
        let (tx, rx) = oneshot::channel();
