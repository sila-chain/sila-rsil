...
/// Writes parts of rsil's in memory tree state to the database and static files.
///
...
#[derive(Debug)]
pub enum PersistenceAction<N: NodePrimitives = SilPrimitives> {
...
#[derive(Debug, Clone)]
pub struct PersistenceHandle<N: NodePrimitives = SilPrimitives> {
...
mod tests {
    ...
    use rsil_chain_state::test_utils::TestBlockBuilder;
    use rsil_exex_types::FinishedExExHeight;
    use rsil_provider::{
    ...
    use rsil_prune::Pruner;
    use rsil_prune_types::PruneMode;
    use tokio::sync::mpsc::unbounded_channel;

    fn default_persistence_handle() -> PersistenceHandle<SilPrimitives> {
        ...
        PersistenceHandle::<SilPrimitives>::spawn_service(provider, pruner, sync_metrics_tx)
    }
    ...
    fn test_save_blocks_then_prune_preserves_new_history() {
        use rsil_db::{models::ShardedKey, tables, BlockNumberList};
        use rsil_provider::RocksDBProviderFactory;
    ...
    fn test_read_only_consistency_across_reorg() {
        ...
        rsil_db::test_utils::enable_legacy_multiopen();

        let provider_factory = create_test_provider_factory();
        provider_factory.set_storage_settings_cache(rsil_provider::StorageSettings::v2());
        ...
        secondary.set_storage_settings_cache(rsil_provider::StorageSettings::v2());

        // --- Phase 1: Write blocks 0..3 via the primary ---
        let mut test_block_builder = TestBlockBuilder::sil().with_state();
        ...
        let single_cost = TestBlockBuilder::<SilPrimitives>::single_tx_cost();
