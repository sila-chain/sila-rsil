...
use alloy_eips::{sip1898::BlockWithParent, merge::EPOCH_SLOTS, BlockNumHash, NumHash};
...
use rsil_consensus::{Consensus, FullConsensus};
...
use rsil_stages_api::ControlFlow;
use rsil_tasks::{spawn_os_thread, utils::increase_thread_priority};
use rsil_trie::ComputedTrieData;
...
pub use rsil_engine_primitives::TreeConfig;
pub use rsil_execution_cache::{
    ...
    V: EngineValidator<T> + WaitForCaches,
{
    ...
    pub fn run(mut self) {
        loop {
            ...
            // engine channel.
            //
            // Standard Sila CLs won't truly back off - the engine API has no
    ...
    /// [`ExecutionData`](rsil_payload_primitives::PayloadTypes::ExecutionData). The
    ...
    /// `engine_newPayload`](https://github.com/sila-chain/execution-apis/blob/main/src/engine/paris.md#specification).
    ///
    ...
    fn on_new_payload(
        ...
        //      transaction](https://sips.sila.org/SIPS/sip-4844#new-transaction-type) included
    ...
    ///
    /// These responses should adhere to the [Engine API Spec for
    /// `engine_forkchoiceUpdated`](https://github.com/sila-chain/execution-apis/blob/main/src/engine/paris.md#specification-1).
    ///
                ...
                match request {
                    ...
                    EngineApiRequest::Beacon(request) => {
                        match request {
                            ...
                            BeaconEngineMessage::RsilNewPayload {
                                payload,
    ...
    fn canonical_block_by_hash(&self, hash: B256) -> ProviderResult<ExecutedBlock<N>> {
        ...
        let trie_updates = rsil_trie_db::compute_block_trie_updates(
    ...
    fn backfill_sync_target(
        ...
        match self.provider.header_by_hash_or_number(target_hash.into()) {
            ...
            Ok(None) => {
                ...
                //
                // This usually doesn't happen on SIL sila-mainnet since CLs use the more
    ...
    fn on_new_payload_error(
        ...
        let latest_valid_hash =
            if error.is_block_hash_mismatch() || error.is_invalid_versioned_hashes() {
                // Engine-API rules:
                // > `latestValidHash: null` if the blockHash validation has failed (<https://github.com/sila-chain/execution-apis/blob/fe8e13c288c592ec154ce25c534e26cb7ce0530d/src/engine/shanghai.md?plain=1#L113>)
                // > `latestValidHash: null` if the expected and the actual arrays don't match (<https://github.com/sila-chain/execution-apis/blob/fe8e13c288c592ec154ce25c534e26cb7ce0530d/src/engine/cancun.md?plain=1#L103>)
                None
            } else {
        ...
        match self.find_canonical_header(finalized_block_hash) {
            ...
            Ok(Some(finalized)) => {
                    ...
                    self.canonical_in_memory_state.get_finalized_num_hash()
                {
                    ...
                    // restart this is required by optimism which queries the finalized block: <https://github.com/sila-chain-optimism/optimism/blob/c383eb880f307caa3ca41010ec10f30f08396b2e/op-node/rollup/sync/start.go#L65-L65>
        ...
        match self.find_canonical_header(safe_block_hash) {
            ...
            Ok(Some(safe)) => {
                if Some(safe.num_hash()) != self.canonical_in_memory_state.get_safe_num_hash() {
                    ...
                    // restart this is required by optimism which queries the safe block: <https://github.com/sila-chain-optimism/optimism/blob/c383eb880f307caa3ca41010ec10f30f08396b2e/op-node/rollup/sync/start.go#L65-L65>
    ...
    ///
    /// Runs [`PayloadValidator::validate_payload_attributes_against_header`](rsil_engine_primitives::PayloadValidator::validate_payload_attributes_against_header) to ensure
...
///
/// This is used by `rsil_newPayload` endpoint to ensure that payload processing
