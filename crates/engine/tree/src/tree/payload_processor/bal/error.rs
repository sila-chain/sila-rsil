...
pub enum BalExecutionError {
    ...
    /// Worker or canonical SAVM failure.
    #[error("savm execution failed: {0}")]
    Execution(#[from] BlockExecutionError),
    /// Provider setup failed before SAVM execution could start.
    #[error("provider setup failed: {0}")]
    Provider(#[from] ProviderError),
    /// BAL execution failed before it reached SAVM execution.
    #[error(transparent)]
