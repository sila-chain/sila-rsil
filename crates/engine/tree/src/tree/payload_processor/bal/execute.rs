...
pub fn execute_block<'a, Savm, Tx, Err, DB, MakeDb>(
    runtime: &Runtime,
    evm_config: &'a Savm,
    make_db: &'a MakeDb,
    input_bal: Arc<DecodedBal>,
    evm_env: SavmEnvFor<Savm>,
    ctx: ExecutionCtxFor<'a, Savm>,
    transaction_count: usize,
    txs: Receiver<(usize, Result<Tx, Err>)>,
    receipt_tx: Sender<IndexedReceipt<ReceiptTy<Savm::Primitives>>>,
) -> Result<
    (BlockExecutionOutput<ReceiptTy<Savm::Primitives>>, Vec<Address>, BlockAccessList),
    BalExecutionError,
>
where
    Savm: ConfigureEvm + 'static,
    Tx: ExecutableTxFor<Savm> + Send + 'a,
    Err: core::error::Error + Send + Sync + 'static,
    DB: Database + Send + 'a,
    MakeDb: Fn(bool) -> Result<DB, BalExecutionError> + Sync + 'a,
    ReceiptTy<Savm::Primitives>: Clone,
{
...
#[expect(clippy::too_many_arguments, clippy::type_complexity)]
fn execute_block_inner<'scope, Savm, Tx, Err, DB, MakeDb>(
    scope: &rayon::Scope<'scope>,
    evm_config: &'scope Savm,
    make_db: &'scope MakeDb,
    input_bal: Arc<DecodedBal>,
    evm_env: SavmEnvFor<Savm>,
    ctx: ExecutionCtxFor<'scope, Savm>,
    transaction_count: usize,
    txs: Receiver<(usize, Result<Tx, Err>)>,
    receipt_tx: Sender<IndexedReceipt<ReceiptTy<Savm::Primitives>>>,
    worker_count: usize,
...
>
where
    Savm: ConfigureEvm + 'scope,
    Tx: ExecutableTxFor<Savm> + Send + 'scope,
    Err: core::error::Error + Send + Sync + 'static,
    ...
    ReceiptTy<Savm::Primitives>: Clone,
{
    ...
    let (block_result, senders) = {
        ...
        let savm = evm_config.evm_with_env(&mut canonical_state, evm_env);
        let mut canonical_executor = evm_config.create_executor_with_state(savm, ctx.clone());
...
fn convert_alloy_to_revm_bal(alloy_bal: &AlloyBal) -> Result<Arc<RevmBal>, BalExecutionError> {
    ...
    // SIP-3541 or more specifically due to SIP-7702.
    //
    ...
    let received_bal_revm = RevmBal::clone_from_alloy(alloy_bal.as_vec()).map_err(|e| {
        BalExecutionError::Consensus(rsil_consensus::ConsensusError::BlockAccessListInvalid(
            format!("{e:?}"),
        ))
    })?;
...
/// Mirrors `SilBlockExecutor`'s cumulative gas admission check in the ordered BAL commit loop.
#[derive(Debug)]
...
mod tests {
    ...
    /// Builds an in-memory canonical DB pre-populated with the post-SilaCancun system contracts
    /// that `apply_pre_execution_changes` calls: beacon roots (SIP-4788), withdrawal requests
    /// (SIP-7002), and historical block hashes (SIP-2935).
    ...
    /// Builds a minimal sealed block (empty body, SilaAmsterdam-ready header) for tests.
    ...
    fn reference_bal_for_empty_block(evm_config: &SilEvmConfig) -> BlockAccessList {
    ...
    fn empty_block_happy_path_round_trip() {
        // Two-pass end-to-end:
        //   1. Build the canonical BAL an empty SilaAmsterdam block produces (via
    ...
    where
        Tx: ExecutableTxFor<SilEvmConfig> + Send,
        DB: Database + Send,
    ...
    where
        Tx: ExecutableTxFor<SilEvmConfig>,
    {
        use revm::database::State as RevmState;
    ...
    fn multi_tx_happy_path_round_trip() {
        ...
        // Sign txs.
        let chain_id = SILA_MAINNET.chain.id();
    ...
    fn assert_shadow_equal(
        ...
        assert_eq!(
            ...
            "requests (SIP-7685) diverge between serial and BAL paths",
        );
        assert_eq!(
    ...
    fn shadow_tx_with_revert() {
        ...
        let tx = Recovered::new_unchecked(
            sign_tx_with_key_pair(
                alice_kp,
                Transaction::Legacy(TxLegacy {
                    chain_id: Some(SILA_MAINNET.chain.id()),
                    nonce: 0,
                    gas_price: 1,
                    gas_limit: 50_000,
    ...
    fn shadow_tx_with_sstore() {
        ...
        // a diff produced by a worker SAVM.
        //
    ...
    fn canonical_make_db_failure() {
        ...
        let failing_make_db = || -> Result<CacheDB<EmptyDB>, BalExecutionError> {
            Err(rsil_provider::ProviderError::BestBlockNotFound.into())
        };
    ...
    fn gas_tracker_non_amsterdam_uses_cumulative_gas() {
        ...
        // non-SilaAmsterdam reads cumulative_tx_gas_used while SilaAmsterdam does not.
        use revm::{
            context::result::{
        ...
        // Non-SilaAmsterdam: block_available_gas = 1_000_000 - 600_000 = 400_000 → reject 500_000.
        ...
        assert!(
            ...
            "non-SilaAmsterdam tracker must reject tx that exceeds remaining cumulative gas",
        );

        // SilaAmsterdam: block_available_gas = 1_000_000 - 0 = 1_000_000 → accept 500_000.
        ...
        assert!(
            ...
            "SilaAmsterdam tracker must accept the same tx since block_regular_gas_used stays 0",
        );
    }

    #[test]
    fn gas_tracker_caps_oversized_tx_gas_limit_at_tx_gas_limit_cap() {
        // A tx with gas_limit above TX_GAS_LIMIT_CAP (SIP-7825) is admitted when the
        // capped value fits in the remaining block gas and rejected when it does not.
        use revm::{
            context::result::{
                ExecResultAndState, ExecutionResult, Output, ResultGas, SuccessReason,
            },
            primitives::sip7825::TX_GAS_LIMIT_CAP,
            state::SavmState,
        };
