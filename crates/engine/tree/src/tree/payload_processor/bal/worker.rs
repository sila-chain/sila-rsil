...
pub(super) enum BalWorkerError {
    ...
    /// Transaction recovery or conversion failed before SAVM execution.
    #[error("BAL worker transaction conversion failed: {0}")]
    Transaction(Box<dyn core::error::Error + Send + Sync + 'static>),
    /// SAVM transaction execution failed.
    #[error("BAL worker SAVM execution failed: {0}")]
...
pub(super) fn spawn_worker<'scope, Savm, Tx, Err, DB, MakeDb>(
    scope: &rayon::Scope<'scope>,
    tx_rx: Receiver<(usize, Result<Tx, Err>)>,
    abort_rx: Receiver<()>,
    result_tx: WorkerResultSender<Savm>,
    ...
    scope.spawn(move |_| {
        let worker_result = (|| -> Result<(), BalWorkerError> {
            ...
            let mut worker_state = State::builder()
                ...
                .with_bundle_update()
                .build();
            let savm = evm_config.evm_with_env(&mut worker_state, evm_env);
            let mut executor = evm_config.create_executor_with_state(savm, ctx.clone());
