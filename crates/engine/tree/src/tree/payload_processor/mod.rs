...
use rsil_provider::{BlockExecutionOutput, BlockReader, StateProviderFactory, StateReader};
...
type IteratorPayloadHandle<Savm, I> = PayloadHandle<
    ...
    <<Savm as ConfigureEvm>::Primitives as NodePrimitives>::Receipt,
>;

type IteratorPrewarmTxReceiver<Savm, I> =
    PrewarmTxReceiver<TxEnvFor<Savm>, <I as ExecutableTxIterator<Savm>>::Recovered>;

type IteratorExecuteTxReceiver<Savm, I> = ExecuteTxReceiver<
    TxEnvFor<Savm>,
    <I as ExecutableTxIterator<Savm>>::Recovered,
    <I as ExecutableTxTuple>::Error,
>;
...
#[derive(Debug)]
pub struct PayloadProcessor<Savm>
where
    Savm: ConfigureEvm,
{
    ...
    /// Determines how to configure the savm for execution.
    evm_config: Savm,
...
impl<Savm> PayloadProcessor<Savm>
where
    Savm: ConfigureEvm,
{
    ...
    Savm: ConfigureEvm + 'static,
{
    ...
    pub fn spawn_with_state_root_streams<P, I: ExecutableTxIterator<Savm>>(
        &self,
        env: ExecutionEnv<Savm>,
        transactions: I,
        provider_builder: StateProviderBuilder<Savm::Primitives, P>,
        hint_stream: Option<StateRootHintStream>,
        hashed_update_stream: Option<StateRootUpdateStream>,
        parallel_bal_execution: bool,
    ) -> IteratorPayloadHandle<Savm, I>
    where
    ...
    #[instrument(level = "debug", target = "engine::tree::payload_processor", skip_all)]
    fn spawn_tx_iterator<I: ExecutableTxIterator<Savm>>(
        &self,
        transactions: I,
        transaction_count: usize,
        parallel_bal_execution: bool,
    ) -> (IteratorPrewarmTxReceiver<Savm, I>, IteratorExecuteTxReceiver<Savm, I>) {
    ...
    fn spawn_caching_with<P>(
        &self,
        env: ExecutionEnv<Savm>,
        transactions: mpsc::Receiver<(usize, impl ExecutableTxFor<Savm> + Clone + Send + 'static)>,
        provider_builder: StateProviderBuilder<Savm::Primitives, P>,
        hint_stream: Option<StateRootHintStream>,
        hashed_update_stream: Option<StateRootUpdateStream>,
        parallel_bal_execution: bool,
    ) -> CacheTaskHandle<<Savm::Primitives as NodePrimitives>::Receipt>
    where
        P: BlockReader + StateProviderFactory + StateReader + Clone + 'static,
    {
...
mod tests {
    ...
    fn on_inserted_executed_block_populates_cache() {
        let payload_processor = PayloadProcessor::new(
            ...
            SilEvmConfig::new(Arc::new(ChainSpec::default())),
            &TreeConfig::default(),
