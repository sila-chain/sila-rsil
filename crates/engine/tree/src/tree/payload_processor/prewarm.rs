...
use rsil_tasks::{pool::WorkerPool, Runtime};
use rsil_trie_common::MultiProofTargetsV2;
use std::sync::{
...
#[derive(Debug)]
pub struct PrewarmCacheTask<N, P, Savm>
where
    N: NodePrimitives,
    Savm: ConfigureEvm<Primitives = N>,
{
...
impl<N, P, Savm> PrewarmCacheTask<N, P, Savm>
where
    N: NodePrimitives,
    ...
    Savm: ConfigureEvm<Primitives = N> + 'static,
{
    ...
    ///
    /// Kicks off SAVM init on every pool thread, then uses `in_place_scope` to dispatch
    ...
    /// their SAVM state on first access via [`get_or_init`](rsil_tasks::pool::Worker::get_or_init).
    fn spawn_txs_prewarm<Tx>(
        ...
        Tx: ExecutableTxFor<Savm> + Send + 'static,
    {
        ...
        self.executor.spawn_blocking_named("prewarm-txs", move || {
            ...
            pool.in_place_scope(|s| {
                s.spawn(|_| {
                    pool.init::<PrewarmEvmState<Savm>>(|_| ctx.evm_for_ctx());
                });
            ...
            // All tasks are done — clear per-thread SAVM state for the next block.
            pool.clear();
    ...
    /// Executes a single prewarm transaction on the current pool thread's SAVM.
    ///
    /// Lazily initialises per-thread [`PrewarmEvmState`] via
    /// [`get_or_init`](rsil_tasks::pool::Worker::get_or_init) on first access.
    fn transact_worker<Tx>(
        ...
        Tx: ExecutableTxFor<Savm>,
    {
        WorkerPool::with_worker_mut(|worker| {
            let Some(savm) =
                worker.get_or_init::<PrewarmEvmState<Savm>>(|| ctx.evm_for_ctx()).as_mut()
            else {
                return;
            };
            ...
            let res = match savm.transact(tx_env) {
                Ok(res) => res,
                Err(err) => {
                    trace!(
                        target: "engine::tree::payload_processor::prewarm",
...
#[derive(Debug, Clone)]
pub struct PrewarmContext<N, P, Savm>
where
    N: NodePrimitives,
    Savm: ConfigureEvm<Primitives = N>,
{
    /// The execution environment.
    pub env: ExecutionEnv<Savm>,
    ...
    pub precompile_cache_map: PrecompileCacheMap<SpecFor<Savm>>,
...
/// Per-thread SAVM state initialised by [`PrewarmContext::evm_for_ctx`] and stored in
/// [`WorkerPool`] workers via [`Worker::get_or_init`](rsil_tasks::pool::Worker::get_or_init).
type PrewarmEvmState<Savm> =
    Option<SavmFor<Savm, StateProviderDatabase<rsil_provider::StateProviderBox>>>;

impl<N, P, Savm> PrewarmContext<N, P, Savm>
where
    N: NodePrimitives,
    P: BlockReader + StateProviderFactory + StateReader + Clone + 'static,
    Savm: ConfigureEvm<Primitives = N> + 'static,
{
    /// Creates a per-thread SAVM for prewarming.
    #[instrument(level = "debug", target = "engine::tree::payload_processor::prewarm", skip_all)]
    fn evm_for_ctx(&self) -> PrewarmEvmState<Savm> {
        ...
        let mut savm = self.evm_config.evm_with_env(state_provider, evm_env);

        if !self.precompile_cache_disabled {
            // Only cache pure precompiles to avoid issues with stateful precompiles
            savm.precompiles_mut().map_cacheable_precompiles(|address, precompile| {
    ...
    fn send_bal_hashed_state(
        ...
        if !account_changes.storage_changes.is_empty() {
            ...
            let mut storage_map = rsil_trie::HashedStorage::new(false);
            ...
            let mut hashed_state = rsil_trie::HashedPostState::default();
...
pub struct PrewarmMetrics {
    ...
    /// A histogram of SAVM execution duration per transaction prewarming
