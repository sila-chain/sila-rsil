...
//! 2. Build the parent state provider, SAVM environment, transaction iterator, lazy ancestor
//!    overlay, and optional decoded SIP-7928 block access list (BAL).
...
//!     participant Exec as SAVM execution
...
//!     Main->>Main: parent provider, SAVM env, optional BAL decode
...
use rsil_consensus::{ConsensusError, FullConsensus, ReceiptRootBloom};
use rsil_engine_primitives::{
...
use rsil_revm::db::{states::bundle_state::BundleRetention, BundleAccount, State};
use rsil_trie::{
...
struct JitPauseGuard<Savm: ConfigureEvm>(Savm);

impl<Savm: ConfigureEvm> JitPauseGuard<Savm> {
    fn new(evm_config: &Savm) -> Self {
...
impl<Savm: ConfigureEvm> Drop for JitPauseGuard<Savm> {
    fn drop(&mut self) {
...
/// used by network-specific payload validators (e.g., Sila, Optimism). It is not meant to be
/// used as a standalone component, but rather as a building block for concrete implementations.
#[derive(derive_more::Debug)]
pub struct BasicEngineValidator<P, Savm, V>
where
    Savm: ConfigureEvm,
{
    ...
    consensus: Arc<dyn FullConsensus<Savm::Primitives>>,
    /// SAVM configuration.
    evm_config: Savm,
    /// Configuration for the tree.
    config: TreeConfig,
    /// Payload processor for transaction conversion, prewarming, and execution caching.
    payload_processor: PayloadProcessor<Savm>,
    /// Precompile cache map.
    precompile_cache_map: PrecompileCacheMap<SpecFor<Savm>>,
    ...
    #[debug(skip)]
    invalid_block_hook: Box<dyn InvalidBlockHook<Savm::Primitives>>,
    ...
    state_trie_overlays: StateTrieOverlayManager<Savm::Primitives>,
    /// State-root strategy used to prepare per-block commitment tasks.
    #[debug(skip)]
    state_root_strategy: Arc<dyn StateRootStrategy<Savm::Primitives, P, Savm>>,
}

impl<N, P, Savm, V> BasicEngineValidator<P, Savm, V>
where
    N: NodePrimitives,
    ...
    OverlayStateProviderFactory<P, N>: DatabaseProviderROFactory<Provider: TrieCursorFactory + HashedCursorFactory>
        ...
        + Sync
        + 'static,
    Savm: ConfigureEvm<Primitives = N> + 'static,
{
    ...
    pub fn with_state_root_strategy(
        mut self,
        state_root_strategy: Arc<dyn StateRootStrategy<N, P, Savm>>,
    ) -> Self {
    ...
    /// Returns SAVM environment for the given payload or block.
    pub fn evm_env_for<T: PayloadTypes<BuiltPayload: BuiltPayload<Primitives = N>>>(
        &self,
        input: &BlockOrPayload<T>,
    ) -> Result<SavmEnvFor<Savm>, Savm::Error>
    where
        V: PayloadValidator<T, Block = N::Block>,
        Savm: ConfigureEngineEvm<T::ExecutionData, Primitives = N>,
    {
        match input {
    ...
    ) -> Result<impl ExecutableTxIterator<Savm>, NewPayloadError>
    where
        V: PayloadValidator<T, Block = N::Block>,
        Savm: ConfigureEngineEvm<T::ExecutionData, Primitives = N>,
    {
    ...
    pub fn execution_ctx_for<'a, T: PayloadTypes<BuiltPayload: BuiltPayload<Primitives = N>>>(
        &self,
        input: &'a BlockOrPayload<T>,
    ) -> Result<ExecutionCtxFor<'a, Savm>, Savm::Error>
    where
        V: PayloadValidator<T, Block = N::Block>,
        Savm: ConfigureEngineEvm<T::ExecutionData, Primitives = N>,
    {
    ...
    /// 1. Sets up the SAVM with state database and precompile caching
    ...
    fn execute_block<S, Err, T>(
        &mut self,
        state_provider: S,
        env: ExecutionEnv<Savm>,
        input: &BlockOrPayload<T>,
        handle: &mut PayloadHandle<impl ExecutableTxFor<Savm>, Err, N::Receipt>,
        ...
        Savm: ConfigureEngineEvm<T::ExecutionData, Primitives = N>,
    {
        ...
        let (spec_id, mut executor) = {
            ...
            let savm = evm_config.evm_with_env(&mut db, env.evm_env);
            let ctx = self
                .execution_ctx_for(input)
                .map_err(|e| InsertBlockErrorKind::Other(Box::new(e)))?;
            let executor = self.evm_config.create_executor(savm, ctx);
            (spec_id, executor)
        };
        ...
        let (_evm, result) = debug_span!(target: "engine::tree", "BlockExecutor::finish")
            ...
            .map(|(savm, result)| (savm.into_db(), result))?;
    ...
    // TODO: extend with stronger gating before enabling on sila-mainnet:
    //   - Fork check: `SilaAmsterdam.active_at_timestamp(env.evm_env.timestamp)`. Today a BAL only
    //     exists post-SilaAmsterdam, so the BAL-presence check is a sufficient proxy. It is a proxy,
    //     not a guarantee.
        ...
        Err: core::error::Error + Send + Sync + 'static,
    {
        ...
        // Bump BAL index after pre-execution changes (SIP-7928: index 0 is pre-execution)
        if has_bal {
        ...
        loop {
            ...
            // Bump BAL index after each transaction (SIP-7928)
            if has_bal {
                executor.evm_mut().db_mut().bump_bal_index();
            }
        }
    ...
    fn spawn_payload_processor<T: ExecutableTxIterator<Savm>>(
        &self,
        env: ExecutionEnv<Savm>,
        txs: T,
        ...
        PayloadHandle<
            impl ExecutableTxFor<Savm> + use<N, P, Savm, V, T>,
            impl core::error::Error + Send + Sync + 'static + use<N, P, Savm, V, T>,
            N::Receipt,
        >,
        InsertBlockErrorKind,
    > {
    ...
    fn calculate_timing_stats(
        ...
        let sip7702_delegations_set =
            output.state.contracts.values().filter(|bytecode| bytecode.is_eip7702()).count();
        // Delegations cleared: accounts where bytecode changed FROM SIP-7702 TO empty
        // This detects when an SIP-7702 delegation is removed by setting code to empty
        // Note: Clearing a delegation does NOT destroy the account - it just empties the
        // bytecode
        let sip7702_delegations_cleared = output
            .state
            .state
            .values()
            .filter(|acc| {
                // Check if original bytecode was SIP-7702
        ...
        Box::new(ExecutionTimingStats {
            ...
            bytecodes_changed,
            code_bytes_written,
            sip7702_delegations_set,
            sip7702_delegations_cleared,
            account_cache_hits,
...
>: Send + Sync + 'static
{
    ...
    ///
    /// See also: <https://github.com/sila-chain/execution-apis/blob/main/src/engine/common.md#specification-1>
...
impl<N, Types, P, Savm, V> EngineValidator<Types> for BasicEngineValidator<P, Savm, V>
where
    P: DatabaseProviderFactory<
            Provider: BlockReader
                          + StageCheckpointReader
    ...
    Savm: ConfigureEngineEvm<Types::ExecutionData, Primitives = N> + 'static,
...
impl<P, Savm, V> WaitForCaches for BasicEngineValidator<P, Savm, V>
where
    Savm: ConfigureEvm,
{
