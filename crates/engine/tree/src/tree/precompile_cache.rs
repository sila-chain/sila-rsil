    ...
    S: Eq + Hash + std::fmt::Debug + Send + Sync + Clone + 'static,
{
    ...
    /// Spec id associated to the SAVM from which this cached precompile was created.
    spec_id: S,
}
...
mod tests {
    ...
    fn test_precompile_cache_map_separate_addresses() {
        let mut savm = SilEvmFactory::default().create_evm(EmptyDB::default(), SavmEnv::default());
