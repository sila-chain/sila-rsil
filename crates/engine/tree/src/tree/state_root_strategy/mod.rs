...
//! - On the serial execution path, per-transaction `SavmState` updates arrive through the execution
...
use rsil_tasks::utils::increase_thread_priority;
use rsil_trie::{
...
use rsil_trie_parallel::proof_task::{ProofTaskCtx, ProofWorkerHandle};
pub use rsil_trie_parallel::{
...
use rsil_trie_sparse::debug_recorder::TrieDebugRecorder;
use rsil_trie_sparse::{ArenaParallelSparseTrie, RevealableSparseTrie, SparseStateTrie};
use std::{
    fmt,
    sync::{
...
pub type LazyHashedPostState = rsil_tasks::LazyHandle<Arc<HashedPostState>>;

/// Strategy used by engine-tree validation to prepare per-block state-root work.
pub trait StateRootStrategy<N, P, Savm>: Send + Sync
where
    N: NodePrimitives,
    Savm: ConfigureEvm<Primitives = N>,
{
    ...
    fn prepare(
        &self,
        ctx: StateRootJobContext<'_, N, P, Savm>,
    ...
    N: NodePrimitives,
{
    ...
    pub const fn executor(&self) -> &rsil_tasks::Runtime {
        self.executor
    }
...
pub struct StateRootJobContext<'a, N, P, Savm>
where
    N: NodePrimitives,
    Savm: ConfigureEvm<Primitives = N>,
{
...
impl<N, P, Savm> fmt::Debug for StateRootJobContext<'_, N, P, Savm>
where
    N: NodePrimitives,
    Savm: ConfigureEvm<Primitives = N>,
{
...
impl<'a, N, P, Savm> StateRootJobContext<'a, N, P, Savm>
where
    N: NodePrimitives,
    Savm: ConfigureEvm<Primitives = N>,
{
    ...
    pub const fn env(&self) -> &ExecutionEnv<Savm> {
        self.env
    }
...
impl DefaultStateRootStrategy {
    ...
    fn spawn_sparse_trie_task<N: NodePrimitives>(
        ...
        executor.clone().spawn_blocking_named("sparse-trie", move || {
            rsil_tasks::once!(increase_thread_priority);
...
impl<N, P, Savm> StateRootStrategy<N, P, Savm> for DefaultStateRootStrategy
where
    N: NodePrimitives,
...
mod tests {
    ...
    fn create_mock_state_updates(
        ...
        for _ in 0..updates_per_account {
            ...
            let mut state_update = SavmState::default();
    ...
    fn state_root_task_matches_serial_root() {
        ...
        let env: ExecutionEnv<SilEvmConfig> = ExecutionEnv::test_default();
        let runtime = rsil_tasks::Runtime::test();
        let state_trie_overlays = StateTrieOverlayManager::<SilPrimitives>::default();
        let mut state_root_handle = DefaultStateRootStrategy::default().spawn_state_root(
            &runtime,
            &state_trie_overlays,
            OverlayStateProviderFactory::new(
                provider_factory,
                OverlayBuilder::<SilPrimitives>::new(genesis_hash, ChangesetCache::new()),
            ),
            StateRootTaskOptions {
                parent_state_root: env.parent_state_root,
                transaction_count: Some(env.transaction_count),
