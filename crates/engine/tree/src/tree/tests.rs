...
impl rsil_engine_primitives::PayloadValidator<SilEngineTypes> for MockEngineValidator {
    type Block = Block;
    ...
    fn convert_payload_to_block(
        ...
        rsil_primitives_traits::SealedBlock<Self::Block>,
        rsil_payload_primitives::NewPayloadError,
    > {
        let block = rsil_sila_primitives::Block::try_from(payload.payload).map_err(|e| {
            rsil_payload_primitives::NewPayloadError::Other(format!("{e:?}").into())
        })?;
        Ok(block.seal_slow())
    }
}

impl EngineApiValidator<SilEngineTypes> for MockEngineValidator {
    fn validate_version_specific_fields(
        &self,
        _version: rsil_payload_primitives::EngineApiMessageVersion,
        _payload_or_attrs: rsil_payload_primitives::PayloadOrAttributes<
            '_,
            alloy_rpc_types_engine::ExecutionData,
            alloy_rpc_types_engine::PayloadAttributes,
        >,
    ) -> Result<(), rsil_payload_primitives::EngineObjectValidationError> {
...
struct TestHarness {
    ...
    to_tree_tx: crossbeam_channel::Sender<
        FromEngine<EngineApiRequest<SilEngineTypes, SilPrimitives>, Block>,
    >,
    from_tree_rx: UnboundedReceiver<EngineApiEvent>,
    payload_command_rx: UnboundedReceiver<PayloadServiceCommand<SilEngineTypes>>,
...
impl TestHarness {
    ...
    fn with_persistence_channel_and_config(
        ...
        let runtime = rsil_tasks::Runtime::test();
    ...
    fn with_blocks(mut self, blocks: Vec<ExecutedBlock>) -> Self {
        ...
        self.tree.state.tree_state = TreeState {
            ...
            parent_to_child,
            engine_kind: EngineApiKind::Sila,
            state_trie_overlays,
        };
    ...
    fn persist_blocks(&self, blocks: Vec<RecoveredBlock<rsil_sila_primitives::Block>>) {
...
impl ValidatorTestHarness {
    fn new(chain_spec: Arc<ChainSpec>) -> Self {
        ...
        let consensus = Arc::new(SilBeaconConsensus::new(chain_spec));
    ...
    ) -> ValidationOutcome<SilPrimitives> {
        let ctx = TreeCtx::new(
...
impl TestBlockFactory {
    fn new(chain_spec: ChainSpec) -> Self {
        Self { builder: TestBlockBuilder::sil().with_chain_spec(chain_spec) }
    }
...
fn test_tree_persist_block_batch() {
    ...
    let mut test_block_builder = TestBlockBuilder::sil().with_chain_spec((*chain_spec).clone());
...
fn on_new_persisted_block_queues_sparse_trie_prune_request() {
    let blocks: Vec<_> = TestBlockBuilder::sil().get_executed_blocks(1..4).collect();
    let mut test_harness = TestHarness::new(SILA_MAINNET.clone()).with_blocks(blocks.clone());
    test_harness
        .tree
        .persistence_state
...
fn remove_blocks_clears_pending_sparse_trie_prune_request() {
    let mut test_harness = TestHarness::new(SILA_MAINNET.clone());
...
async fn test_in_memory_state_trait_impl() {
    let blocks: Vec<_> = TestBlockBuilder::sil().get_executed_blocks(0..10).collect();
    let test_harness = TestHarness::new(SILA_MAINNET.clone()).with_blocks(blocks.clone());
...
async fn test_engine_request_during_backfill() {
    let tree_config = TreeConfig::default();
    let blocks: Vec<_> = TestBlockBuilder::sil()
...
fn test_validated_payload_bal_is_inserted_into_store() {
    let mut block_builder = TestBlockBuilder::sil();
    ...
    let mut test_harness = TestHarness::new(SILA_MAINNET.clone()).with_blocks(vec![parent]);
...
async fn test_tree_state_on_new_head_reorg() {
    ...
    let mut test_block_builder = TestBlockBuilder::sil();
...
fn test_validate_block_synchronous_strategy_during_persistence() {
    ...
    let mut test_harness = ValidatorTestHarness::new(SILA_MAINNET.clone());
    ...
    // Create valid block
    let mut block_factory = TestBlockFactory::new(SILA_MAINNET.as_ref().clone());
    let genesis_hash = SILA_MAINNET.genesis_hash();
...
// via `on_disconnected_downloaded_block`. The OP Stack branch targets head; the Sila
...
fn test_backfill_target_hash_eth_returns_finalized() {
    let test_harness = TestHarness::new(SILA_MAINNET.clone());
...
/// On Sila, a disconnected downloaded block whose missing parent is far ahead of the
    ...
    match event {
        Some(TreeEvent::BackfillAction(BackfillAction::Start(target))) => {
            assert_eq!(
                target.sync_target(),
                Some(finalized_hash),
                "Sila backfill should target finalized"
            );
        }
        other => panic!("Expected BackfillAction(Start), got: {other:?}"),
    }
...
/// On Sila, a zero finalized hash means optimistic sync. The helper still selects finalized,
    ...
    match event {
        Some(TreeEvent::BackfillAction(BackfillAction::Start(target))) => {
            assert_eq!(
                target.sync_target(),
                Some(head_hash),
                "Sila optimistic backfill should target head when finalized is zero"
            );
        }
        other => panic!("Expected BackfillAction(Start), got: {other:?}"),
    }
...
/// Stack, finalized on Sila — when that block is buffered far ahead of where the
...
async fn test_on_backfill_sync_finished_eth_retriggers_backfill_to_buffered_finalized() {
    assert_post_backfill_recheck_retriggers_to_buffered_target(EngineApiKind::Sila).await;
}
