...
type MaybeReorgResult<S, T, Provider, Savm, Validator, E> =
    Result<Either<EngineReorg<S, T, Provider, Savm, Validator>, S>, E>;
...
pub trait EngineMessageStreamExt<T: PayloadTypes>: Stream<Item = BeaconEngineMessage<T>> {
    ...
    fn reorg<Provider, Savm, Validator>(
        self,
        provider: Provider,
        evm_config: Savm,
        ...
        frequency: usize,
        depth: Option<usize>,
    ) -> EngineReorg<Self, T, Provider, Savm, Validator>
    where
        Self: Sized,
    {
        EngineReorg::new(
    ...
    fn maybe_reorg<Provider, Savm, Validator, E, F, Fut>(
        self,
        provider: Provider,
        evm_config: Savm,
        ...
        depth: Option<usize>,
    ) -> impl Future<Output = MaybeReorgResult<Self, T, Provider, Savm, Validator, E>> + Send
    where
        Self: Sized + Send,
        Provider: Send,
        Savm: Send,
