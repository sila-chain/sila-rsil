...
use rsil_engine_tree::tree::EngineValidator;
use rsil_errors::{BlockExecutionError, BlockValidationError, RsilError, RsilResult};
use rsil_evm::{
...
use rsil_storage_api::{errors::ProviderError, BlockReader, StateProviderFactory};
use std::{
...
type EngineReorgResponse = Result<
    Either<Result<PayloadStatus, BeaconOnNewPayloadError>, RsilResult<OnForkChoiceUpdated>>,
...
pub struct EngineReorg<S, T: PayloadTypes, Provider, Savm, Validator> {
    /// Underlying stream
    #[pin]
    stream: S,
    /// Database provider.
...
impl<S, T: PayloadTypes, Provider, Savm, Validator> EngineReorg<S, T, Provider, Savm, Validator> {
...
impl<S, T, Provider, Savm, Validator> Stream for EngineReorg<S, T, Provider, Savm, Validator>
where
    S: Stream<Item = BeaconEngineMessage<T>>,
    T: PayloadTypes<BuiltPayload: BuiltPayload<Primitives = Savm::Primitives>>,
    Provider: BlockReader<Header = HeaderTy<Savm::Primitives>, Block = BlockTy<Savm::Primitives>>
        + StateProviderFactory
        + ChainSpecProvider,
    Savm: ConfigureEvm,
    Validator: EngineValidator<T, Savm::Primitives>,
{
    type Item = S::Item;
...
fn create_reorg_head<Provider, Savm, T, Validator>(
    provider: &Provider,
    evm_config: &Savm,
    payload_validator: &Validator,
    mut depth: usize,
    next_payload: T::ExecutionData,
) -> RsilResult<(SealedBlock<BlockTy<Savm::Primitives>>, Option<Bytes>)>
where
    Provider: BlockReader<Header = HeaderTy<Savm::Primitives>, Block = BlockTy<Savm::Primitives>>
        + StateProviderFactory
        + ChainSpecProvider<ChainSpec: SilChainSpec>,
    Savm: ConfigureEvm,
    T: PayloadTypes<BuiltPayload: BuiltPayload<Primitives = Savm::Primitives>>,
    Validator: EngineValidator<T, Savm::Primitives>,
{
    ...
    let next_block =
        payload_validator.convert_payload_to_block(next_payload).map_err(RsilError::msg)?;
    ...
    let ctx = evm_config.context_for_block(&reorg_target).map_err(RsilError::other)?;
    let savm = evm_config.evm_for_block(&mut state, &reorg_target).map_err(RsilError::other)?;
    let mut builder = evm_config.create_block_builder(savm, &reorg_target_parent, ctx);
    ...
    for tx in candidate_transactions {
        ...
        let gas_used = match builder.execute_transaction(tx_recovered) {
            ...
            Err(error) => return Err(RsilError::Execution(error)),
        };
