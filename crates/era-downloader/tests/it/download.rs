...
async fn test_getting_era1_file_url_after_fetching_file_list(url: &str) {
    ...
    let expected_url = Some(base_url.join("sila-mainnet-00000-5ec1ffb8.era1").unwrap());
...
async fn test_getting_ere_file_url_after_fetching_file_list(url: &str) {
    ...
    let expected_url = Some(base_url.join("sila-mainnet-00000-a6860fef.erae").unwrap());
    let actual_url = client.url(0).await.unwrap();
...
#[test_case("https://sila-mainnet.era.nimbus.team/"; "nimbus")]
#[tokio::test]
async fn test_getting_era_file_url_after_fetching_file_list(url: &str) {
    ...
    let expected_url = Some(base_url.join("sila-mainnet-00000-4b363db9.era").unwrap());
    let actual_url = client.url(0).await.unwrap();
