...
use reqwest::Url;
use rsil_era_downloader::{EraClient, EraStream, EraStreamConfig};
use std::str::FromStr;
...
async fn test_streaming_files_after_fetching_file_list(url: &str) {
    ...
    let expected_file = folder.join("sila-mainnet-00000-5ec1ffb8.era1").into_boxed_path();
    let actual_file = stream.next().await.unwrap().unwrap();

    assert_eq!(actual_file.as_ref(), expected_file.as_ref());

    let expected_file = folder.join("sila-mainnet-00001-a5364e9a.era1").into_boxed_path();
    let actual_file = stream.next().await.unwrap().unwrap();
...
async fn test_streaming_ere_files_after_fetching_file_list(url: &str) {
    ...
    let expected_file = folder.join("sila-mainnet-00000-a6860fef.erae").into_boxed_path();
    let actual_file = stream.next().await.unwrap().unwrap();
    assert_eq!(actual_file.as_ref(), expected_file.as_ref());

    let expected_file = folder.join("sila-mainnet-00001-05c64fc4.erae").into_boxed_path();
    let actual_file = stream.next().await.unwrap().unwrap();
    assert_eq!(actual_file.as_ref(), expected_file.as_ref());
}
...
async fn test_streaming_era_files_after_fetching_file_list_into_missing_folder_fails() {
    let base_url = Url::from_str("https://sila-mainnet.era.nimbus.team").unwrap(); //TODO: change once ithaca host era files
    let folder = tempdir().unwrap().path().to_owned();
    let client = EraClient::new(StubClient, base_url, folder);
