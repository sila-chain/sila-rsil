...
fn entry_size(entry: Option<rsil_era::e2s::types::Entry>) -> i64 {
...
mod tests {
    ...
    use rsil_era::{common::file_ops::StreamReader, ere::file::EreReader};
    use rsil_sila_primitives::{BlockBody, Receipt as SilReceipt};
    use tempfile::tempdir;

    /// One empty block with the given number and difficulty (zero difficulty marks a post-merge
    /// block).
    fn export_block(number: u64, difficulty: U256) -> ExportBlock<Header, BlockBody, SilReceipt> {
        ExportBlock {
    ...
    fn export_blocks(
        count: u64,
        difficulty: U256,
    ) -> Vec<ExportBlock<Header, BlockBody, SilReceipt>> {
        (0..count).map(|number| export_block(number, difficulty)).collect()
    }

    fn write_and_read(blocks: &[ExportBlock<Header, BlockBody, SilReceipt>]) -> EreFile {
        let dir = tempdir().unwrap();
        let path =
            Ere::write_file("sila-mainnet", MAX_BLOCKS_PER_ERE as u64, blocks, dir.path()).unwrap();
        EreReader::new(std::fs::File::open(path).unwrap()).read("sila-mainnet".to_string()).unwrap()
    }
