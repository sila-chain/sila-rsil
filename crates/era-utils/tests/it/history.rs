...
use rsil_era::era1::types::execution::MAX_BLOCKS_PER_ERA1;
use rsil_era_downloader::{EraClient, EraStream, EraStreamConfig};
use rsil_era_utils::{export, import, Era1, Ere, ExportConfig};
use rsil_etl::Collector;
use rsil_fs_util as fs;
use rsil_provider::{test_utils::create_test_provider_factory, BlockNumReader, BlockReader};
use std::str::FromStr;
...
async fn test_roundtrip_export_after_import() {
    ...
    let export_config = ExportConfig {
        ...
        network: "sila-mainnet".to_string(),
    };
    ...
    for (i, file_path) in exported_files.iter().enumerate() {
        ...
        // Verify format: sila-mainnet-{era_number:05}-{era_count:05}-{8hexchars}.era1
        ...
        let expected_prefix = format!("sila-mainnet-{:05}-{:05}-", era_number, 1);
...
async fn test_ere_roundtrip_export_after_import() {
    // Populate a database by importing one sila-mainnet era1 file (blocks 0..=8191).
    let url = Url::from_str(ITHACA_ERA_INDEX_URL).unwrap();
    let download_folder = tempdir().unwrap();
    let download_folder = download_folder.path().to_owned().into_boxed_path();
    let client = EraClient::new(ClientWithFakeIndex(Client::new()), url, download_folder);
