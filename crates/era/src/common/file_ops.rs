...
impl EraFileType {
    ...
    /// See also <https://github.com/sil-clients/e2store-format-specs/blob/main/formats/era.md#file-name>
    ///
...
mod tests {
    ...
    fn test_from_url_detection() {
        ...
        assert_eq!(
            EraFileType::from_url("https://host/sila-mainnet-00000-abcd1234.ere"),
            EraFileType::Ere
        );
        assert_eq!(
            EraFileType::from_url("https://host/sila-mainnet-00000-abcd1234.era1"),
            EraFileType::Era1
        );
        assert_eq!(
            EraFileType::from_url("https://host/sila-mainnet-00000-abcd1234.era"),
            EraFileType::Era
        );

        // An ERE file under a path/mirror containing `era1` still resolves by its `.ere` extension.
        assert_eq!(
            EraFileType::from_url("https://host/era1/sila-mainnet-00000-abcd1234.ere"),
            EraFileType::Ere
        );
    ...
    fn test_from_filename_detection() {
        assert_eq!(
            EraFileType::from_filename("sila-mainnet-00000-abcd1234.era"),
            Some(EraFileType::Era)
        );
        assert_eq!(
            EraFileType::from_filename("sila-mainnet-00000-abcd1234.era1"),
            Some(EraFileType::Era1)
        );
        assert_eq!(
            EraFileType::from_filename("sila-mainnet-00000-abcd1234.ere"),
            Some(EraFileType::Ere)
        );
        // The alternate `.erae` extension also resolves to `Ere`.
        assert_eq!(
            EraFileType::from_filename("sila-mainnet-00000-abcd1234.erae"),
            Some(EraFileType::Ere)
        );
        // Profile postfixes don't change extension detection.
        assert_eq!(
            EraFileType::from_filename("sila-mainnet-00000-abcd1234-noproofs.ere"),
            Some(EraFileType::Ere)
        );
        assert_eq!(
            EraFileType::from_filename("sila-mainnet-00000-abcd1234-noproofs.erae"),
            Some(EraFileType::Ere)
        );
        assert_eq!(EraFileType::from_filename("sila-mainnet-00000-abcd1234.txt"), None);
    }

    #[test]
    fn from_dir_detects_single_format() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(dir.path().join("sila-mainnet-00000-aaaaaaaa.era1"), b"x").unwrap();
        std::fs::write(dir.path().join("sila-mainnet-00001-bbbbbbbb.era1"), b"x").unwrap();
    ...
    fn from_dir_errors_on_mixed_formats() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(dir.path().join("sila-mainnet-00000-aaaaaaaa.era1"), b"x").unwrap();
        std::fs::write(dir.path().join("sila-mainnet-00000-bbbbbbbb.era"), b"x").unwrap();
