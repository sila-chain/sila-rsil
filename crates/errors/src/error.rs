...
use rsil_storage_errors::{db::DatabaseError, provider::ProviderError};

/// Result alias for [`RsilError`].
pub type RsilResult<T> = Result<T, RsilError>;
...
impl RsilError {
    /// Create a new `RsilError` from a given error.
    ...
    /// Create a new `RsilError` from a given message.
...
mod size_asserts {
    ...
    static_assert_size!(RsilError, 64);
