...
type SingleBlockStreamItem<N = SilPrimitives> = (
    RecoveredBlock<<N as NodePrimitives>::Block>,
    BlockExecutionOutput<<N as NodePrimitives>::Receipt>,
);
type BatchBlockStreamItem<N = SilPrimitives> = Chain<N>;
...
mod tests {
    ...
    async fn test_single_blocks() -> eyre::Result<()> {
        ...
        let executor = SilEvmConfig::sila(chain_spec.clone());
    ...
    fn create_blocks(
        ...
        key_pair: Keypair,
        n: u64,
    ) -> Result<Vec<RecoveredBlock<rsil_sila_primitives::Block>>> {
    ...
    fn execute_and_commit_blocks<N>(
        ...
        chain_spec: &Arc<ChainSpec>,
        blocks: &[RecoveredBlock<rsil_sila_primitives::Block>],
    ) -> Result<()>
    where
        N: ProviderNodeTypes<
