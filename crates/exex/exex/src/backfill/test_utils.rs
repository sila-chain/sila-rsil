...
pub(crate) fn execute_block_and_commit_to_database<N>(
    ...
    block: &RecoveredBlock<rsil_sila_primitives::Block>,
) -> eyre::Result<BlockExecutionOutput<Receipt>>
where
    N: ProviderNodeTypes<
        Primitives: NodePrimitives<
            Block = rsil_sila_primitives::Block,
            BlockBody = rsil_sila_primitives::BlockBody,
    ...
    >,
{
    ...
    let mut block_execution_output = SilEvmConfig::sila(chain_spec)
...
fn blocks(
    ...
    RecoveredBlock<rsil_sila_primitives::Block>,
    RecoveredBlock<rsil_sila_primitives::Block>,
)> {
    // First block has a transaction that transfers some SIL to zero address
    let block1 = Block {
        header: Header {
...
pub(crate) fn blocks_and_execution_outcome<N>(
    ...
    chain_spec: Arc<ChainSpec>,
    key_pair: Keypair,
) -> eyre::Result<(Vec<RecoveredBlock<rsil_sila_primitives::Block>>, ExecutionOutcome)>
where
    N: ProviderNodeTypes,
