...
/// notification.
#[derive(Debug, Clone)]
pub struct Storage<N: NodePrimitives = SilPrimitives> {
    ...
    N: NodePrimitives,
{
    ...
    pub(super) fn new(path: impl AsRef<Path>) -> WalResult<Self> {
        rsil_fs_util::create_dir_all(&path)?;
    ...
    fn remove_notification(&self, file_id: u32) -> Option<u64> {
        ...
        match rsil_fs_util::remove_file(self.file_path(file_id)) {
            Ok(()) => {
    ...
    pub(super) fn files_range(&self) -> WalResult<Option<RangeInclusive<u32>>> {
        ...
        for entry in rsil_fs_util::read_dir(&self.path)? {
    ...
    pub(super) fn read_notification(
        ...
        let mut file = match File::open(&file_path) {
            ...
            Err(err) => return Err(rsil_fs_util::FsPathError::open(err, &file_path).into()),
        };
        ...
        let notification: rsil_exex_types::serde_bincode_compat::ExExNotification<'_, N> =
    ...
    pub(super) fn write_notification(
        ...
        let notification =
            rsil_exex_types::serde_bincode_compat::ExExNotification::<N>::from(notification);

        rsil_fs_util::atomic_write_file(&file_path, |file| {
...
mod tests {
    ...
    use rsil_provider::Chain;
    use rsil_testing_utils::generators::{self, random_block};
    ...
    /// ```sh
    /// cargo test -p rsil-exex generate_test_wal -- --ignored --nocapture
    /// ```
    #[test]
    #[ignore]
    fn generate_test_wal() -> eyre::Result<()> {
        ...
        let notification_compat =
            rsil_exex_types::serde_bincode_compat::ExExNotification::from(&notification);
    ...
    fn get_test_notification_data(
        ...
        let notification: ExExNotification<rsil_sila_primitives::SilPrimitives> =
