...
use rsil_node_builder::{
    ...
    BuilderContext, Node, NodeAdapter, RsilFullAdapter,
};
...
use rsil_payload_builder::noop::NoopPayloadBuilderService;
...
impl<Node, Savm: Send> PoolBuilder<Node, Savm> for TestPoolBuilder
where
...
{
    type SAVM = MockEvmConfig;

    async fn build_evm(self, _ctx: &BuilderContext<Node>) -> eyre::Result<Self::SAVM> {
...
pub type Adapter = NodeAdapter<RsilFullAdapter<TmpDB, TestNode>>;
...
pub struct TestExExHandle {
    ...
    pub genesis: RecoveredBlock<rsil_sila_primitives::Block>,
...
pub async fn test_exex_context_with_chain_spec(
    ...
    let (_, payload_builder_handle) = NoopPayloadBuilderService::<SilEngineTypes>::new();
    ...
    let ctx = ExExContext {
        ...
        rsil_config: rsil_config::Config::default(),
        events: events_tx,
        notifications,
        components,
    };
...
/// Creates a new [`ExExContext`] with (sila-mainnet)[`SILA_MAINNET`] chain spec.
///
...
pub async fn test_exex_context() -> eyre::Result<(ExExContext<Adapter>, TestExExHandle)> {
    test_exex_context_with_chain_spec(SILA_MAINNET.clone()).await
}
