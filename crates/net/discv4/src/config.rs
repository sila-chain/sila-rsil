...
use rsil_net_nat::{NatResolver, ResolveNatInterval};
...
pub struct Discv4Config {
    ...
    /// Whether to enforce SIP-868 extension.
    pub enable_eip868: bool,
    /// Whether to only respond with nodes that have a known SIP-868 ENR record (fork ID) in
    /// `Neighbours` responses.
    ///
    /// When enabled, the node will filter out entries that have not yet been confirmed to support
    /// SIP-868 (i.e., have no `fork_id`) when responding to `FindNode` requests. This helps
    ...
    /// Additional pairs to include in The [`Enr`](enr::Enr) if SIP-868 extension is enabled <https://sips.sila.org/SIPS/sip-868>
...
impl Discv4ConfigBuilder {
    ...
    /// Whether to enable SIP-868
    pub const fn enable_eip868(&mut self, enable_eip868: bool) -> &mut Self {
        self.config.enable_eip868 = enable_eip868;
        self
    }

    /// Whether to only include nodes with a known SIP-868 ENR record (fork ID) in `Neighbours`
    /// responses.
