//! Discovery v4 implementation: <https://github.com/sila-chain/devp2p/blob/master/discv4.md>
//!
...
use rsil_network_peers::{pk2id, PeerId};
...
pub use rsil_network_peers::NodeRecord;

#[cfg(any(test, feature = "test-utils"))]
pub mod test_utils;

use crate::table::PongTable;
use rsil_net_nat::ResolveNatInterval;
/// reexport to get public ip.
pub use rsil_net_nat::{external_ip, NatResolver};
...
/// Concurrency factor for `FindNode` requests to pick `ALPHA` closest nodes, <https://github.com/sila-chain/devp2p/blob/master/discv4.md#recursive-lookup>
const ALPHA: usize = 3;
...
impl Discv4 {
    ...
    ///
    /// ```
    /// use rsil_discv4::{Discv4, Discv4Config};
    ...
    /// Starts a `FindNode` recursive lookup that locates the closest nodes to the given node id. See also: <https://github.com/sila-chain/devp2p/blob/master/discv4.md#recursive-lookup>
    ///
    ...
    /// Sets the pair in the SIP-868 [`Enr`] of the node.
    ///
...
/// ## Lookups
///
/// See also [Recursive Lookups](https://github.com/sila-chain/devp2p/blob/master/discv4.md#recursive-lookup).
...
pub struct Discv4Service {
    ...
    local_address: SocketAddr,
    /// The local ENR for SIP-868 <https://sips.sila.org/SIPS/sip-868>
    ...
    ///
    /// Lookups behave like bursts of requests: Endpoint proof followed by `FindNode` request. [Recursive lookups](https://github.com/sila-chain/devp2p/blob/master/discv4.md#recursive-lookup) can trigger multiple followup Pings+FindNode requests.
...
impl Discv4Service {
    ...
    /// Starts the recursive lookup process for the given target, <https://github.com/sila-chain/devp2p/blob/master/discv4.md#recursive-lookup>.
    ///
    ...
    ///
    /// On re-ping we check for a changed `enr_seq` if sip868 is enabled and when it changed we sent
    ...
    fn update_on_reping(&mut self, record: NodeRecord, mut last_enr_seq: Option<u64>) {
        ...
        // If SIP868 extension is disabled then we want to ignore this
    ...
    fn on_ping(&mut self, ping: Ping, remote_addr: SocketAddr, remote_id: PeerId, hash: B256) {
        ...
        // See also <https://github.com/sila-chain/devp2p/blob/master/discv4.md#ping-packet-0x01>:
    ...
    fn respond_closest(&mut self, target: PeerId, to: SocketAddr) {
        ...
        // entries that have no SIP-868 fork ID
            ...
            while let Poll::Ready(Some(cmd)) = self.commands_rx.poll_recv(cx) {
                match cmd {
                    ...
                    Discv4Command::SetEIP868RLPPair { key, rlp } => {
                        debug!(target: "discv4", key=%String::from_utf8_lossy(&key), "Update SIP-868 extension pair");
...
pub enum DiscoveryUpdate {
    ...
    /// Received a [`ForkId`] via SIP-868 for the given [`NodeRecord`].
...
mod tests {
    ...
    use rand_08::Rng;
    use rsil_sila_forks::{EnrForkIdEntry, ForkHash};
    ...
    async fn test_configured_enr_forkid_entry() {
        ...
        disc_conf.add_eip868_pair("sil", EnrForkIdEntry::from(fork));
        let (_discv4, service) = create_discv4_with_config(disc_conf).await;
        let sil = service.local_eip_868_enr.get_raw_rlp(b"sil").unwrap();
        let fork_entry_id = EnrForkIdEntry::decode(&mut &sil[..]).unwrap();
    ...
    // Bootstraps with sila-mainnet boot nodes
