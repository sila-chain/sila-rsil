...
pub struct Discv5 {
    ...
    rlpx_ip_mode: IpMode,
    /// Key used in kv-pair to ID chain, e.g. 'opstack' or 'sil'.
...
impl Discv5 {
    ////////////////////////////////////////////////////////////////////////////////////////////////
    // Minimal interface with `rsil_network::discovery`
    ////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    /// Spawns [`discv5::Discv5`]. Returns [`discv5::Discv5`] handle in rsil compatible wrapper type
    /// [`Discv5`], a receiver of [`discv5::Event`]s from the underlying node, and the local
    /// [`Enr`](discv5::Enr) converted into the rsil compatible [`NodeRecord`] type.
    pub async fn start(
        sk: &SecretKey,
    ...
    pub fn on_discv5_update(&self, update: discv5::Event) -> Option<DiscoveredPeer> {
        ...
        match update {
            ...
            discv5::Event::NodeInserted { .. } => {
                ...
                // `replaced` partly covers `rsil_discv4::DiscoveryUpdate::Removed(_)`
            ...
            discv5::Event::SessionEstablished(enr, remote_socket) => {
                ...
                // `rsil_discv4::DiscoveryUpdate`: `DiscoveryUpdate::Added(_)` and
    ...
    pub fn get_fork_id<K: discv5::enr::EnrKey>(
        ...
        let fork_id = enr
            ...
            .or_else(|| {
                (key != NetworkStackId::SIL)
                    .then(|| {
                        // Fallback: trying to get fork id from Enr with 'sil' as network stack id
                        trace!(target: "net::discv5",
                            key = %String::from_utf8_lossy(key),
                            "Fork id not found for key, trying 'sil'..."
                        );
                        enr.get_decodable::<EnrForkIdEntry>(NetworkStackId::SIL)
                    })
                    .flatten()
            })
            .ok_or({
                trace!(target: "net::discv5", "Fork id not found for 'sil' network stack id");
...
mod test {
    ...
    async fn discv5() {
        ...
        let node_2_enr_rsil_compatible_ty: Enr<SecretKey> =
            EnrCombinedKeyWrapper(node_2_enr.clone()).into();
        node_1.add_node(node_2_enr_rsil_compatible_ty).unwrap();
    ...
    fn build_enr_from_config() {
        ...
        let fork_id = SILA_MAINNET.latest_fork_id();

        let config = Config::builder((Ipv4Addr::UNSPECIFIED, TCP_PORT).into())
            .fork(NetworkStackId::SIL, fork_id)
            .build();
        ...
        let decoded_fork_id = enr
            .get_decodable::<EnrForkIdEntry>(NetworkStackId::SIL)
            .unwrap()
            .map(Into::into)
            .unwrap();
    ...
    fn get_fork_id_with_different_network_stack_ids() {
        ...
        // Test 2: ENR with SIL fork ID, Discv5 configured for OPEL (fallback to SIL)
        ...
        // Test 3: ENR with neither OPEL nor SIL fork ID (should fail)
