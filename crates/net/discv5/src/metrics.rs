...
pub struct DiscoveredPeersMetrics {
    ...
    ///
    /// Note: the definition of 'discovered' is not exactly synonymous in `rsil_discv4::Discv4`.
...
pub struct AdvertisedChainMetrics {
    ...
    /// Frequency of node records with a kv-pair with [`SIL`](NetworkStackId::SIL) as key.
    sil: Counter,
...
impl AdvertisedChainMetrics {
    ...
    pub fn increment_once_by_network_type(&self, enr: &discv5::Enr) {
        ...
        if enr.get_raw_rlp(NetworkStackId::SIL).is_some() {
            self.sil.increment(1u64)
        }
