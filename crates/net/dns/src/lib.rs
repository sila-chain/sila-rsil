//! Implementation of [SIP-1459](https://sips.sila.org/SIPS/sip-1459) Node Discovery via DNS.
//!
//! ## Feature Flags
//!
...
impl<R: Resolver> DnsDiscoveryService<R> {
    ...
    ///
    /// ```
    /// use rsil_dns_discovery::{DnsDiscoveryService, DnsResolver};
    /// use std::sync::Arc;
    /// # fn t() {
...
fn convert_enr_node_record(enr: &Enr<SecretKey>) -> Option<DnsNodeRecordUpdate> {
    ...
    let fork_id =
        enr.get_decodable::<EnrForkIdEntry>(b"sil").transpose().ok().flatten().map(Into::into);
...
mod tests {
    ...
    fn test_convert_enr_node_record() {
        ...
        let enr = Enr::builder()
            ...
            .udp4(9000)
            .tcp4(30303)
            .add_value(b"sil", &EnrForkIdEntry::from(SILA_MAINNET.latest_fork_id()))
            .build(&secret_key)
            .unwrap();
        ...
        assert_eq!(node_record_update.fork_id, Some(SILA_MAINNET.latest_fork_id()));
    ...
    async fn test_get_node() {
        ...
        let fork_id = SILA_MAINNET.hardfork_fork_id(SilaHardfork::Frontier).unwrap();
        builder
            .ip4(Ipv4Addr::LOCALHOST)
            .udp4(30303)
            .tcp4(30303)
            .add_value(b"sil", &EnrForkIdEntry::from(fork_id));
