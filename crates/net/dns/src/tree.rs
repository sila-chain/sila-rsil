//! Support for the [SIP-1459 DNS Record Structure](https://sips.sila.org/SIPS/sip-1459#dns-record-structure)
//!
...
/// Represents all variants of DNS entries for Sila node lists.
#[derive(Debug, Clone)]
...
pub struct LinkEntry<K: EnrKeyUnambiguous = SecretKey> {
    ...
    pub domain: String,
    /// The public key corresponding to the Sila Node Record (ENR) used for the link entry.
...
/// Represents the actual Sila Node Record (ENR) entry in the DNS tree.
#[derive(Debug, Clone)]
pub struct NodeEntry<K: EnrKeyUnambiguous> {
    /// The Sila Node Record (ENR) associated with the node entry.
    pub enr: Enr<K>,
}
