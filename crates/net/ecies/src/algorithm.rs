...
use rsil_network_peers::{id2pk, pk2id};
use secp256k1::{
...
mod tests {
    ...
    fn sip8_test_server_key() -> SecretKey {
    ...
    fn sip8_test_client() -> ECIES {
        ...
        let server_id = pk2id(&PublicKey::from_secret_key(SECP256K1, &sip8_test_server_key()));

        ECIES::new_static_client(client_static_key, server_id, client_nonce, client_ephemeral_key)
            .unwrap()
    }

    fn sip8_test_server() -> ECIES {
        ...
        ECIES::new_static_server(sip8_test_server_key(), server_nonce, server_ephemeral_key)
            .unwrap()
    }

    #[test]
    /// Test vectors from <https://sips.sila.org/SIPS/sip-8>
    fn sip8_test() {
        // SIP-8 format with version 4 and no additional list elements
        let auth2 = hex!(
            "
        ...
        // SIP-8 format with version 56 and 3 additional list elements (sent from A to B)
        let auth3 = hex!(
            "
        ...
        // SIP-8 format with version 4 and no additional list elements (sent from B to A)
        let ack2 = hex!(
            "
        ...
        // SIP-8 format with version 57 and 3 additional list elements (sent from B to A)
        let ack3 = hex!(
            "
        ...
        sip8_test_server().read_auth(&mut auth2.to_vec()).unwrap();
        sip8_test_server().read_auth(&mut auth3.to_vec()).unwrap();

        let mut test_client = sip8_test_client();
        let mut test_server = sip8_test_server();
