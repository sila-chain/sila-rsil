...
pub struct SessionInfo {
    ...
    /// Negotiated sil version of the session.
...
pub enum DiscoveryEvent {
    ...
    ///
    /// See also <https://sips.sila.org/SIPS/sip-868>
...
#[derive(Debug)]
pub enum PeerRequest<N: NetworkPrimitives = SilNetworkPrimitives> {
    ...
    /// Requests receipts from the peer using sil/70 (supports `firstBlockReceiptIndex`).
    ///
    /// The response should be sent through the channel.
    GetReceipts70 {
        /// The request for receipts.
...
/// The wire message a [`PeerRequest`] resolves to before it's queued for sending: either an `sil`
/// message or a `snap/2` message.
#[derive(Debug)]
pub enum RequestMessage<N: NetworkPrimitives = SilNetworkPrimitives> {
    ...
    pub fn create_request_message(&self, request_id: u64) -> RequestMessage<N> {
        match self {
            Self::GetBlockHeaders { request, .. } => {
                RequestMessage::Sil(SilMessage::GetBlockHeaders(RequestPair {
                    request_id,
                    message: *request,
                }))
            }
            Self::GetBlockBodies { request, .. } => {
                RequestMessage::Sil(SilMessage::GetBlockBodies(RequestPair {
                    request_id,
                    message: request.clone(),
                }))
            }
            Self::GetPooledTransactions { request, .. } => {
                RequestMessage::Sil(SilMessage::GetPooledTransactions(RequestPair {
                    request_id,
                    message: request.clone(),
                }))
            }
            Self::GetNodeData { request, .. } => {
                RequestMessage::Sil(SilMessage::GetNodeData(RequestPair {
                    request_id,
                    message: request.clone(),
                }))
            }
            Self::GetReceipts { request, .. } | Self::GetReceipts69 { request, .. } => {
                RequestMessage::Sil(SilMessage::GetReceipts(RequestPair {
                    request_id,
                    message: request.clone(),
                }))
            }
            Self::GetReceipts70 { request, .. } => {
                RequestMessage::Sil(SilMessage::GetReceipts70(RequestPair {
                    request_id,
                    message: request.clone(),
                }))
            }
            Self::GetBlockAccessLists { request, .. } => {
                RequestMessage::Sil(SilMessage::GetBlockAccessLists(RequestPair {
                    request_id,
                    message: request.clone(),
                }))
            }
            Self::GetCells { request, .. } => {
                RequestMessage::Sil(SilMessage::GetCells(RequestPair {
                    request_id,
                    message: request.clone(),
                }))
            }
