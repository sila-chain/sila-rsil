...
use rsil_transaction_pool::{BlobStore, TransactionPool};
use tokio::sync::mpsc;
...
pub struct NetworkBuilder<Tx, Sil, N: NetworkPrimitives = SilNetworkPrimitives> {
    pub(crate) network: NetworkManager<N>,
    pub(crate) transactions: Tx,
    pub(crate) request_handler: Sil,
}

// === impl NetworkBuilder ===

impl<Tx, Sil, N: NetworkPrimitives> NetworkBuilder<Tx, Sil, N> {
    /// Consumes the type and returns all fields.
    pub fn split(self) -> (NetworkManager<N>, Tx, Sil) {
    ...
    pub fn split_with_handle(self) -> (NetworkHandle<N>, NetworkManager<N>, Tx, Sil) {
        let Self { network, transactions, request_handler } = self;
        let handle = network.handle().clone();
        (handle, network, transactions, request_handler)
    }

    /// Creates a new [`SilRequestHandler`] and wires it to the network.
    pub fn request_handler<Client>(
        self,
        client: Client,
    ) -> NetworkBuilder<Tx, SilRequestHandler<Client, N>, N>
    where
        Client: BalProvider,
    {
    ...
    /// Creates a new [`SilRequestHandler`] with access to a blob store and wires it to the network.
    ...
    ) -> NetworkBuilder<TransactionsManager<Pool, N>, Sil, N> {
