...
pub struct NetworkConfig<C, N: NetworkPrimitives = SilNetworkPrimitives> {
    ...
    /// Additional `RLPx` sub-protocols to announce and handle alongside `sil`.
    ///
...
#[derive(Debug)]
pub struct NetworkConfigBuilder<N: NetworkPrimitives = SilNetworkPrimitives> {
    /// The node's secret key, from which the node's identity is derived.
    ...
    /// How to set up discovery version 5.
    discovery_v5_builder: Option<rsil_discv5::ConfigBuilder>,
    /// All boot nodes to start network discovery with.
    ...
    /// Additional `RLPx` sub-protocols to announce and handle alongside `sil`. Does not cover
    ...
    /// Whether to advertise the `snap/2` satellite protocol (SIP-8189) in the handshake.
    snap_enabled: bool,
}

impl NetworkConfigBuilder<SilNetworkPrimitives> {
    /// Creates the `NetworkConfigBuilder` with [`SilNetworkPrimitives`] types.
    pub fn sil(secret_key: SecretKey, executor: Runtime) -> Self {
...
impl<N: NetworkPrimitives> NetworkConfigBuilder<N> {
    ...
    ///
    /// This effectively allows block propagation in the `sil` sub-protocol, which has been
    /// soft-deprecated with sila `PoS` after the merge. Even if block propagation is
    /// technically allowed, according to the sil protocol, it is not expected to be used in `PoS`
    ...
    /// [`DEFAULT_DISCOVERY_PORT`](rsil_discv4::DEFAULT_DISCOVERY_PORT)
    ...
    ///
    /// By default, this is [`DEFAULT_DISCOVERY_PORT`](rsil_discv4::DEFAULT_DISCOVERY_PORT)
    ...
    pub fn discovery_v5(mut self, builder: rsil_discv5::ConfigBuilder) -> Self {
    ...
    /// Convenience function for setting [`Self::boot_nodes`] to the sila-mainnet boot nodes.
    ...
    /// Toggles advertisement of the `snap/2` satellite protocol (SIP-8189).
    ///
    ...
    where
        ChainSpec: SilChainSpec + Hardforks + 'static,
    {
        self.build(NoopProvider::sil(chain_spec))
    }
    ...
    /// Sets the maximum allowed SIL message size for post-handshake SIL/Snap streams.
    ///
...
///
/// This affects block propagation in the `sil` sub-protocol [SIP-3675](https://sips.sila.org/SIPS/sip-3675#devp2p)
///
...
mod tests {
    ...
    fn test_snap_advertisement_when_enabled() {
        ...
        // sil is still advertised alongside snap.
        assert!(config.hello_message.protocols.iter().any(|p| p.cap.name == "sil"));
    }
    ...
    fn test_network_fork_filter_default() {
        let mut chain_spec = Arc::clone(&SILA_MAINNET);
    ...
    fn test_discv5_fork_id_default() {
        ...
        let config = builder()
            .discovery_v5(
                rsil_discv5::Config::builder((Ipv4Addr::LOCALHOST, 30303).into())
