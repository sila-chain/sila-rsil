...
use rsil_discv4::{DiscoveryUpdate, Discv4, Discv4Config};
use rsil_discv5::{DiscoveredPeer, Discv5};
...
impl Discovery {
    ...
    ///
    /// This will spawn the [`rsil_discv4::Discv4Service`] onto a new task and establish a listener
    ...
    pub async fn new(
        ...
        mut discv5_config: Option<rsil_discv5::Config>, // contains discv5 listen address
        ...
        let (discv5, discv5_updates) = if let Some(mut config) = discv5_config {
            ...
            if let Some(socket) = shared_socket {
                ...
                if discovery_v4_addr.is_ipv4() {
                    ipv4 = Some(socket);
                    if let Some(addr) = rsil_discv5::config::ipv6(&discv5_cfg.listen_config) {
                        ipv6 = Some(bind_socket(SocketAddr::V6(addr)).await?);
                    }
                } else {
                    ipv6 = Some(socket);
                    if let Some(addr) = rsil_discv5::config::ipv4(&discv5_cfg.listen_config) {
    ...
    /// Updates the `sil:ForkId` field in discv4/discv5.
    pub(crate) fn update_fork_id(&self, fork_id: ForkId) {
        if let Some(discv4) = &self.discv4 {
            // use forward-compatible forkid entry
            discv4.set_eip868_rlp(b"sil".to_vec(), EnrForkIdEntry::from(fork_id))
        }
        if let Some(discv5) = &self.discv5 {
            discv5
                .encode_and_set_eip868_in_local_enr(b"sil".to_vec(), EnrForkIdEntry::from(fork_id))
        }
    }
...
const fn set_bound_rlpx_port_if_unset(config: &mut rsil_discv5::Config, port: u16) {
...
mod tests {
    ...
    use rsil_discv5::{enr::EnrCombinedKeyWrapper, enr_to_discv4_id};
    use tracing::trace;
    ...
    async fn start_discovery_node(udp_port_discv4: u16, udp_port_discv5: u16) -> Discovery {
        ...
        let discv5_config = rsil_discv5::Config::builder(discv5_addr)
    ...
    fn discv5_enr_advertises_bound_rlpx_port() {
        ...
        let mut discv5_config = rsil_discv5::Config::builder((Ipv4Addr::LOCALHOST, 0).into())
        ...
        let (enr, _, _, _) = rsil_discv5::build_local_enr(&secret_key, &discv5_config);
    ...
    fn discv5_enr_preserves_configured_rlpx_port() {
        ...
        let discv5_addr: SocketAddr = "127.0.0.1:0".parse().unwrap();
        let mut discv5_config = rsil_discv5::Config::builder(advertised_addr)
            .discv5_config(discv5::ConfigBuilder::new(discv5_addr.into()).build())
            .build();
    ...
    async fn start_shared_port_node(port: u16) -> Discovery {
        ...
        let discv5_config = rsil_discv5::Config::builder(tcp_addr)
            .discv5_config(discv5::ConfigBuilder::new(discv5_listen_config).build())
            .build();
