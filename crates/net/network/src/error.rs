...
use rsil_network_types::BackoffKind;
...
pub enum NetworkError {
    ...
    Discv5Error(#[from] rsil_discv5::Error),
    /// Error when setting up the DNS resolver failed
    ///
    /// See also [`DnsResolver`](rsil_dns_discovery::DnsResolver::from_system_conf)
...
impl SessionError for SilStreamError {
    fn merits_discovery_ban(&self) -> bool {
        match self {
            ...
            Self::SilHandshakeError(err) => {
                ...
                match err {
                    ...
                    SilHandshakeError::InvalidFork(_) => {
    ...
    fn should_backoff(&self) -> Option<BackoffKind> {
        ...
        match self {
            // timeouts
            Self::SilHandshakeError(SilHandshakeError::NoResponse) |
            Self::P2PStreamError(
            ...
            Self::SilHandshakeError(SilHandshakeError::InvalidFork(_)) => {
...
impl SessionError for PendingSessionHandshakeError {
    fn merits_discovery_ban(&self) -> bool {
        match self {
            Self::Sil(sil) => sil.merits_discovery_ban(),
    ...
    fn is_fatal_protocol_error(&self) -> bool {
        match self {
            Self::Sil(sil) => sil.is_fatal_protocol_error(),
            Self::Ecies(err) => matches!(
                err.inner(),
                ECIESErrorImpl::TagCheckDecryptFailed |
                    ECIESErrorImpl::TagCheckHeaderFailed |
    ...
    fn should_backoff(&self) -> Option<BackoffKind> {
        match self {
            Self::Sil(sil) => sil.should_backoff(),
...
mod tests {
    ...
    fn test_should_backoff() {
        let err = SilStreamError::P2PStreamError(P2PStreamError::HandshakeError(
