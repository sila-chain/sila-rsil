...
use rsil_storage_api::{BalProvider, BlockReader, GetBlockAccessListLimit, HeaderProvider};
use rsil_transaction_pool::{blobstore::NoopBlobStore, BlobStore};
use std::{
    future::Future,
    pin::Pin,
    task::{Context, Poll},
...
// Limits: <https://github.com/sila-chain/go-sila/blob/b0d44338bbcefee044f1f635a84487cbbd8f0538/sil/protocols/sil/handler.go#L34-L56>
...
/// Manages sil related requests on top of the p2p network.
///
...
impl<C, N> SilRequestHandler<C, N>
where
    N: NetworkPrimitives,
    C: BlockReader,
{
    ...
    C: BalProvider,
{
    ...
    ///
    /// SIP-8159 defines the final `BlockAccessLists` response semantics:
    /// <https://sips.sila.org/SIPS/sip-8159>
    ...
    /// Handles `snap/2` (SIP-8189) requests.
    ///
...
impl<C, N> Future for SilRequestHandler<C, N>
where
    N: NetworkPrimitives,
    C: BalProvider
...
pub enum IncomingEthRequest<N: NetworkPrimitives = SilNetworkPrimitives> {
    ...
    /// Request Receipts from the peer using sil/70.
    ///
    /// The response should be sent through the channel.
    GetReceipts70 {
        /// The ID of the peer to request receipts from.
...
mod tests {
    ...
    use rsil_transaction_pool::blobstore::{BlobStoreCleanupStat, BlobStoreError};
    use std::sync::{
    ...
    async fn get_cells_request_limits_blob_store_lookups() {
        ...
        let handler = SilRequestHandler::<NoopProvider>::new(
