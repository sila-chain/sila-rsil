...
impl<N: NetworkPrimitives> StateFetcher<N> {
    ...
    /// [`SilResponseValidator::reputation_change_err`]
    ...
    ///
    /// All receipt variants (legacy with bloom, sil/69, sil/70) are expected to be normalized
...
impl<N: NetworkPrimitives> DownloadRequest<N> {
    ...
    fn best_peer_requirements(&self) -> BestPeerRequirements {
        match self {
            ...
            Self::GetBlockAccessLists { .. } => BestPeerRequirements::SilVersion(SilVersion::Sil71),
...
pub(crate) enum FetchAction {
    /// Dispatch an sil request to the given peer.
    BlockRequest {
...
enum BestPeerRequirements {
    ...
    FullBlock,
    /// Peer must support at least this sil protocol version.
    SilVersion(SilVersion),
...
mod tests {
    ...
    async fn test_poll_fetcher() {
        ...
        let mut fetcher =
            StateFetcher::<SilNetworkPrimitives>::new(manager.handle(), Default::default());
    ...
    async fn test_header_response_outcome() {
        ...
        assert!(SilResponseValidator::reputation_change_err(&Err::<Vec<Header>, _>(
    ...
    fn fetcher_with_peer() -> (StateFetcher<SilNetworkPrimitives>, PeerId) {
        let manager = PeersManager::new(PeersConfig::default());
        let mut fetcher =
            StateFetcher::<SilNetworkPrimitives>::new(manager.handle(), Default::default());
        let peer_id = B512::random();
    ...
    async fn test_followup_skips_request_peer_cannot_serve() {
        ...
        let caps_71 = Arc::new(Capabilities::from(vec![Capability::new("sil".into(), 71)]));
        fetcher.new_active_peer(NewPeerInfo {
            peer_id: peer_71,
            best_hash: B256::random(),
            best_number: 100,
    ...
    async fn test_next_best_peer_eth71_no_support() {
        ...
        assert_eq!(
            fetcher.next_best_peer(BestPeerRequirements::SilVersion(SilVersion::Sil71)),
            None
        );
    }
    ...
    async fn test_next_best_peer_eth71_supported() {
        ...
        // Build capability list that includes Sil71
        let capabilities = Arc::new(Capabilities::from(vec![Capability::new("sil".into(), 71)]));
    ...
    async fn test_wakes_when_eth71_peer_connects() {
        ...
        // Add peer WITHOUT Sil71 support
    ...
    async fn test_snap_request_rejected_without_snap_peer() {
        ...
        // Only an sil-only peer is connected.
        fetcher.new_active_peer(NewPeerInfo {
            peer_id: B512::random(),
            best_hash: B256::random(),
            best_number: 100,
