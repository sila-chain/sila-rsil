...
//! ## Capabilities
//!
//! The network manages peers depending on their announced capabilities via their `RLPx` sessions. Most importantly the [Sila Wire Protocol](https://github.com/sila-chain/devp2p/blob/master/caps/sil.md)(`sil`).
//!
//! ## Overview
//!
...
//! Responsible for peer discovery is sila's discovery protocol (discv4, discv5). If the address
//! (IP+port) of our node is published via discovery, remote peers can initiate inbound connections
//! to the local node. Once a (tcp) connection is established, both peers start to authenticate a [RLPx session](https://github.com/sila-chain/devp2p/blob/master/rlpx.md) via a handshake. If the handshake was successful, both peers announce their capabilities and are now ready to exchange sub-protocol messages via the `RLPx` session.
...
use rsil_tasks::shutdown::GracefulShutdown;
...
///  ethrequest <--> |SIL request handing| NetworkManager
...
pub struct NetworkManager<N: NetworkPrimitives = SilNetworkPrimitives> {
    ...
    /// them from the bounded channel to the sil-request channel, it is possible that this
...
impl NetworkManager {
    /// Creates the manager of a new network with [`SilNetworkPrimitives`] types.
    ///
    /// ```no_run
    /// # async fn f() {
    /// use rsil_chainspec::SILA_MAINNET;
    /// use rsil_network::{NetworkConfig, NetworkManager};
    /// use rsil_tasks::Runtime;
    /// let config = NetworkConfig::builder_with_rng_secret_key(Runtime::test())
    ///     .build_with_noop_provider(SILA_MAINNET.clone());
    /// let manager = NetworkManager::sil(config).await;
    /// # }
    /// ```
    pub async fn sil<C: BlockNumReader + 'static>(
        config: NetworkConfig<C, SilNetworkPrimitives>,
...
impl<N: NetworkPrimitives> NetworkManager<N> {
    ...
    pub async fn new<C: BlockNumReader + 'static>(
        ...
        if let Some(disc_config) = discovery_v4_config.as_mut() {
            ...
            // add the forkid entry for SIP-868, but wrap it in an `EnrForkIdEntry` for proper
            // encoding
            disc_config.add_eip868_pair("sil", EnrForkIdEntry::from(status.forkid));
        }
    ...
    pub fn write_peers_to_file(&self, persistent_peers_file: &Path) -> Result<(), FsPathError> {
        ...
        rsil_fs_util::write_json_file(persistent_peers_file, &peers)?;
        Ok(())
    }
    ...
    /// `snap/2` requests via its [`SnapClient`](rsil_network_p2p::snap::client::SnapClient) impl.
    ...
    /// Enforces [SIP-3675](https://sips.sila.org/SIPS/sip-3675#devp2p) consensus rules for the network protocol
    ///
    ...
    fn on_handle_message(&mut self, msg: NetworkHandleMessage<N>) {
        match msg {
            ...
            NetworkHandleMessage::SilRequest { peer_id, request } => {
                self.swarm.sessions_mut().send_message(&peer_id, PeerMessage::SilRequest(request))
            }
            ...
            NetworkHandleMessage::SilMessage { peer_id, message } => {
