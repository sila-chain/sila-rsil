...
//! Capabilities are exchanged via the `RLPx` `Hello` message as pairs of `(id, version)`, <https://github.com/sila-chain/devp2p/blob/master/rlpx.md#capability-messaging>
...
#[derive(Debug, Clone)]
pub struct NewBlockMessage<P = NewBlock<rsil_sila_primitives::Block>> {
    /// Hash of the block
    pub hash: B256,
...
/// All Bi-directional sil-message variants that can be sent to a session or received from a
/// session.
#[derive(Debug)]
pub enum PeerMessage<N: NetworkPrimitives = SilNetworkPrimitives> {
    ...
    /// All `sil` request variants.
    SilRequest(PeerRequest<N>),
    /// Announces when `BlockRange` is updated.
    BlockRangeUpdated(BlockRangeUpdate),
    /// Any other or manually crafted sil message.
    ///
...
pub enum BlockRequest {
    ...
    /// Requests a `snap/2` (SIP-8189) message from the peer.
    ///
    /// The response should be sent through the channel.
...
#[derive(Debug)]
pub enum PeerResponse<N: NetworkPrimitives = SilNetworkPrimitives> {
    ...
    /// Represents a response to a request for receipts.
    ///
    /// This is a variant of `Receipts` that was introduced in `sil/69`.
    ...
    /// Represents a response to a request for receipts using sil/70.
    Receipts70 {
        /// The receiver channel for the response to a receipts request.
    ...
    /// Represents a response to a `snap/2` (SIP-8189) request.
    Snap {
...
#[derive(Debug)]
pub enum PeerResponseResult<N: NetworkPrimitives = SilNetworkPrimitives> {
    ...
    /// Represents a result containing receipts or an error for sil/69.
    Receipts69(RequestResult<Vec<Vec<N::Receipt>>>),
    /// Represents a result containing receipts or an error for sil/70.
...
impl<N: NetworkPrimitives> PeerResponseResult<N> {
    ...
    /// [`SilMessage`] for every variant except [`Self::Snap`], which becomes a
    /// [`SnapProtocolMessage`].
            ...
            ($response:ident, $item:ident, $request_id:ident) => {
                match $response {
                    Ok(res) => {
                        ...
                        Ok(RequestMessage::Sil(SilMessage::$item(request)))
                    }
                    Err(err) => Err(err),
                }
            };
        ...
        match self {
            ...
            Self::Receipts70(resp) => match resp {
                Ok(res) => {
                    ...
                    Ok(RequestMessage::Sil(SilMessage::Receipts70(request)))
                }
                Err(err) => Err(err),
            },
            Self::BlockAccessLists(resp) => match resp {
                Ok(res) => {
                    let request = RequestPair { request_id: id, message: res };
                    Ok(RequestMessage::Sil(SilMessage::BlockAccessLists(request)))
                }
                Err(err) => Err(err),
            },
            Self::Cells(resp) => match resp {
                Ok(res) => {
                    let request = RequestPair { request_id: id, message: res };
                    Ok(RequestMessage::Sil(SilMessage::Cells(request)))
                }
                Err(err) => Err(err),
            },
