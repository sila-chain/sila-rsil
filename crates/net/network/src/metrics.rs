...
pub struct TransactionsManagerMetrics {
    ...
    /// [`TransactionPool`](rsil_transaction_pool::TransactionPool) is considered to be at
...
macro_rules! duration_metered_exec {
    ($code:expr, $acc:expr) => {{
        let start = rsil_primitives_traits::FastInstant::now();
...
/// Metrics for the `SilRequestHandler`
#[derive(Metrics)]
#[metrics(scope = "network")]
...
pub struct AnnouncedTxTypesMetrics {
    ...
    /// Histogram for tracking frequency of SIP-2930 transaction type
    pub(crate) sip2930: Histogram,

    /// Histogram for tracking frequency of SIP-1559 transaction type
    pub(crate) sip1559: Histogram,

    /// Histogram for tracking frequency of SIP-4844 transaction type
    pub(crate) sip4844: Histogram,

    /// Histogram for tracking frequency of SIP-7702 transaction type
    pub(crate) sip7702: Histogram,
...
/// as defined by various Sila Improvement Proposals (SIPs).
#[derive(Debug, Default)]
pub struct TxTypesCounter {
    /// Count of legacy transactions (pre-SIP-2718).
    pub(crate) legacy: usize,

    /// Count of transactions conforming to SIP-2930 (Optional access lists).
    pub(crate) sip2930: usize,

    /// Count of transactions conforming to SIP-1559 (Fee market change).
    pub(crate) sip1559: usize,

    /// Count of transactions conforming to SIP-4844 (Shard Blob Transactions).
    pub(crate) sip4844: usize,

    /// Count of transactions conforming to SIP-7702 (Restricted Storage Windows).
    pub(crate) sip7702: usize,

    /// Count of unknown/other transaction types not matching any known SIP.
...
impl AnnouncedTxTypesMetrics {
    ...
    pub(crate) fn update_eth68_announcement_metrics(&self, tx_types_counter: TxTypesCounter) {
        ...
        self.sip2930.record(tx_types_counter.sip2930 as f64);
        self.sip1559.record(tx_types_counter.sip1559 as f64);
        self.sip4844.record(tx_types_counter.sip4844 as f64);
        self.sip7702.record(tx_types_counter.sip7702 as f64);
