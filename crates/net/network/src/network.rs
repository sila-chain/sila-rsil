...
#[derive(Clone, Debug)]
pub struct NetworkHandle<N: NetworkPrimitives = SilNetworkPrimitives> {
...
impl<N: NetworkPrimitives> NetworkHandle<N> {
    ...
    pub fn send_request(&self, peer_id: PeerId, request: PeerRequest<N>) {
        self.send_message(NetworkHandleMessage::SilRequest { peer_id, request })
    }
...
#[derive(Debug)]
pub(crate) enum NetworkHandleMessage<N: NetworkPrimitives = SilNetworkPrimitives> {
    ...
    /// Sends an `sil` protocol request to the peer.
    SilRequest {
    ...
    /// Sends an `sil` protocol message to the peer.
    SilMessage {
        /// The peer to send the message to.
        peer_id: PeerId,
        /// The `sil` protocol message to send to the peer's session.
