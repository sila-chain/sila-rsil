...
use crate::{
    message::PeerMessage,
    session::{active::BroadcastItemCounter, conn::SilRlpxConnection, Direction, SessionId},
...
///
/// Within an active session that supports the `Sila Wire Protocol`, three high-level tasks can
...
impl<N: NetworkPrimitives> ActiveSessionHandle<N> {
    ...
    /// Returns the negotiated sil version for this session.
...
///
/// This represents the state changes a session can undergo until it is ready to send capability messages <https://github.com/sila-chain/devp2p/blob/6b0abc3d956a626c28dce1307ee9f546db17b6bd/rlpx.md>.
///
/// A session starts with a `Handshake`, followed by a `Hello` message which
#[derive(Debug)]
pub enum PendingSessionEvent<N: NetworkPrimitives> {
    /// Represents a successful `Hello` and `Status` exchange: <https://github.com/sila-chain/devp2p/blob/6b0abc3d956a626c28dce1307ee9f546db17b6bd/rlpx.md#hello-0x00>
    Established {
        ...
        /// The actual connection stream which can be used to send and receive `sil` protocol
        /// messages
        conn: SilRlpxConnection<N>,
