...
pub use conn::SilRlpxConnection;
use handle::SessionCommandSender;
pub use handle::{
    ActiveSessionHandle, ActiveSessionMessage, PendingSessionEvent, PendingSessionHandle,
    SessionCommand,
};
pub use rsil_network_api::{Direction, PeerInfo};
...
impl<N: NetworkPrimitives> SessionManager<N> {
    ...
    pub(crate) fn poll(&mut self, cx: &mut Context<'_>) -> Poll<SessionEvent<N>> {
        ...
        match event {
            PendingSessionEvent::Established {
                ...
                let range_update_interval = (conn.version() >= SilVersion::Sil69).then(|| {
    ...
    /// and the shared local range information that gets propagated to active sessions (Sil69).
...
pub enum PendingSessionHandshakeError {
    /// The pending session failed due to an error while establishing the `sil` stream
    #[error(transparent)]
    Sil(SilStreamError),
...
async fn authenticate_stream<N: NetworkPrimitives>(
    ...
    let (mut p2p_stream, their_hello) = match stream.handshake(hello).await {
        ...
        Err(err) => {
            return PendingSessionEvent::Disconnected {
                ...
                session_id,
                direction,
                error: Some(PendingSessionHandshakeError::Sil(err.into())),
            }
        }
    };
    ...
    let (conn, their_status) = if p2p_stream.shared_capabilities().len() == 1 {
        ...
        // perform the sil protocol handshake
        match handshake
            ...
            .await
        {
            ...
            Err(err) => {
                return PendingSessionEvent::Disconnected {
                    ...
                    session_id,
                    direction,
                    error: Some(PendingSessionHandshakeError::Sil(err)),
                }
            }
        }
