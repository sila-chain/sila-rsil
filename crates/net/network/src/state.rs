...
pub(crate) struct BlockNumReader(Box<dyn rsil_storage_api::BlockNumReader>);

impl BlockNumReader {
    /// Create a new instance with the given reader.
    pub fn new(reader: impl rsil_storage_api::BlockNumReader + 'static) -> Self {
        Self(Box::new(reader))
    }
}
...
impl Deref for BlockNumReader {
    type Target = Box<dyn rsil_storage_api::BlockNumReader>;
...
impl<N: NetworkPrimitives> NetworkState<N> {
    ...
    pub(crate) fn on_session_activated(&mut self, activation: SessionActivation<N>) {
        ...
        // Use the block number from the peer's status (sil/69+) if available,
    ...
    ///
    /// See also <https://github.com/sila-chain/devp2p/blob/master/caps/sil.md>
    ...
    pub(crate) fn add_trusted_peer_node(&mut self, trusted: rsil_network_peers::TrustedPeer) {
    ...
    fn on_eth_response(&mut self, peer: PeerId, resp: PeerResponseResult<N>) {
        let outcome = match resp {
            ...
            PeerResponseResult::Receipts(res) => {
                // Legacy sil/66-68: strip bloom filters and wrap in ReceiptsResponse
...
pub(crate) struct SessionActivation<N: NetworkPrimitives> {
    ...
    pub(crate) capabilities: Arc<Capabilities>,
    /// The `Status` message the peer sent during the `sil` handshake.
...
pub(crate) enum StateAction<N: NetworkPrimitives> {
    ...
    /// Retrieved a [`ForkId`] from the peer via ENR request, See <https://sips.sila.org/SIPS/sip-868>
    DiscoveredEnrForkId {
        peer_id: PeerId,
...
mod tests {
    ...
    fn state() -> NetworkState<SilNetworkPrimitives> {
    ...
    fn capabilities() -> Arc<Capabilities> {
        Arc::new(vec![Capability::from(SilVersion::Sil67)].into())
    }
