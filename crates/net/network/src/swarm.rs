...
use futures::Stream;
use rsil_eth_wire::{
    errors::SilStreamError, Capabilities, DisconnectReason, SilNetworkPrimitives, SilVersion,
...
/// tasks. After a successful `RLPx` authentication, the task is ready to accept SIL requests or
...
/// connections. For each active session, the [`NetworkState`] keeps a sender half of the SIL
    ...
    fn on_state_action(&mut self, event: StateAction<N>) -> Option<SwarmEvent<N>> {
        match event {
            ...
            StateAction::DiscoveredNode { peer_id, addr, fork_id } => {
                ...
                // fork ID (via SIP-868 ENR) are deferred — they'll only be added once a
...
/// network.
pub(crate) enum SwarmEvent<N: NetworkPrimitives = SilNetworkPrimitives> {
