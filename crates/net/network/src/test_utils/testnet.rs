    ...
    Pool: TransactionPool,
{
    ...
    pub fn handles(&self) -> impl Iterator<Item = NetworkHandle<SilNetworkPrimitives>> + '_ {
...
impl<C, Pool> Testnet<C, Pool>
where
    C: ChainSpecProvider<ChainSpec: SilaHardforks>
        + StateProviderFactory
        + BlockReaderIdExt
    ...
    Pool: TransactionPool,
{
    /// Installs an sil pool on each peer
    pub fn with_eth_pool(
        self,
    ) -> Testnet<C, SilTransactionPool<C, InMemoryBlobStore, SilEvmConfig>> {
        self.map_pool(|peer| {
            ...
            peer.map_transactions_manager(SilTransactionPool::eth_pool(
                pool,
                blob_store,
                Default::default(),
            ))
    ...
    /// Installs an sil pool on each peer with custom transaction manager config
    ...
    /// Installs an sil pool on each peer with custom transaction manager config and policy.
    pub fn with_eth_pool_config_and_policy(
        ...
        self.map_pool(|peer| {
            ...
            peer.map_transactions_manager_with(
                SilTransactionPool::eth_pool(pool, blob_store, Default::default()),
...
pub struct Peer<C, Pool = TestPool> {
    #[pin]
    network: NetworkManager<SilNetworkPrimitives>,
    #[pin]
    request_handler: Option<SilRequestHandler<C, SilNetworkPrimitives>>,
    #[pin]
    transactions_manager: Option<TransactionsManager<Pool, SilNetworkPrimitives>>,
    pool: Option<Pool>,
    client: C,
    ...
    Pool: TransactionPool,
{
    ...
    pub const fn network_mut(&mut self) -> &mut NetworkManager<SilNetworkPrimitives> {
        &mut self.network
    }

    /// Returns the [`NetworkHandle`] of this peer.
    pub fn handle(&self) -> NetworkHandle<SilNetworkPrimitives> {
...
/// A helper config for setting up the rsil networking stack.
#[derive(Debug)]
...
pub struct PeerHandle<Pool> {
    network: NetworkHandle<SilNetworkPrimitives>,
    transactions: Option<TransactionsHandle<SilNetworkPrimitives>>,
    pool: Option<Pool>,
}
...
impl<Pool> PeerHandle<Pool> {
    ...
    /// Returns the [`NetworkHandle`] of this peer.
    pub const fn network(&self) -> &NetworkHandle<SilNetworkPrimitives> {
        &self.network
    }
}
