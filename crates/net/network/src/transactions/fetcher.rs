...
//! then packed into a request with respect to the [`SilVersion`] of the announcement. Any hashes
...
#[derive(Debug)]
#[pin_project]
pub struct TransactionFetcher<N: NetworkPrimitives = SilNetworkPrimitives> {
...
impl<N: NetworkPrimitives> TransactionFetcher<N> {
    ...
    /// [`TransactionPool`](rsil_transaction_pool::TransactionPool) are operating close to full
    ...
    pub fn on_resolved_get_pooled_transactions_request_fut(
        ...
        match result {
            Ok(Ok(transactions)) => {
                ...
                // invalid response <https://github.com/sila-chain/sila-rsil/issues/6529>. requires
...
mod test {
    ...
    fn pack_eth68_request() {
        ...
        let tx_fetcher = &mut TransactionFetcher::<SilNetworkPrimitives>::default();
    ...
    async fn test_on_fetch_pending_hashes() {
        ...
        let (mut peer_1_data, mut peer_1_mock_session_rx) =
            new_mock_session(peer_1, SilVersion::Sil66);
        for hash in &seen_hashes {
            peer_1_data.seen_transactions.insert(*hash);
        }
        let (mut peer_2_data, _) = new_mock_session(peer_2, SilVersion::Sil66);
        for hash in &seen_hashes {
