...
use rsil_eth_wire::{
    ...
    NewPooledTransactionHashes72, PooledTransactions, RequestTxHashes, SilNetworkPrimitives,
    SilVersion, Transactions, ValidAnnouncementData,
};
use rsil_metrics::common::mpsc::MemoryBoundedReceiver;
...
use rsil_primitives_traits::{InMemorySize, SignedTransaction};
...
#[derive(Debug, Clone)]
pub struct TransactionsHandle<N: NetworkPrimitives = SilNetworkPrimitives> {
...
///    - handling incoming sil messages for transactions.
...
pub struct TransactionsManager<Pool, N: NetworkPrimitives = SilNetworkPrimitives> {
    ...
    ///    [`SilTransactionValidator`](rsil_transaction_pool::validate::SilTransactionValidator)
        ...
        PoolTransaction<Consensus = N::BroadcastedTransaction, Pooled = N::PooledTransaction>,
{
    ...
    ///
    /// Note: SIP-4844 are disallowed from being broadcast in full and are only ever sent as hashes, see also <https://sips.sila.org/SIPS/sip-4844#networking>.
    ...
    /// Handles dedicated transaction events related to the `sil` protocol.
...
//
// spawned in `NodeConfig::start_network`(rsil_node_core::NodeConfig) and
// `NetworkConfig::start_network`(rsil_network::NetworkConfig)
impl<
        Pool: TransactionPool + Unpin + 'static,
        N: NetworkPrimitives<
                BroadcastedTransaction: SignedTransaction,
...
impl PropagateTransaction {
    ...
    ///
    /// Direct transactions use their SIP-2718 encoded length so sil/68+ hash announcements carry
...
impl PropagateTransactionsBuilder {
    ...
    /// transactions.
    fn pooled(version: SilVersion, capacity: usize) -> Self {
    ...
    fn full(version: SilVersion, capacity: usize) -> Self {
...
/// and enforces other propagation rules for SIP-4844 and tracks those transactions that can't be
/// broadcasted in full.
#[derive(Debug, Clone)]
...
impl FullTransactionsBuilder {
    ...
    fn new(version: SilVersion) -> Self {
        Self {
            total_size: 0,
    ...
    fn with_capacity(version: SilVersion, capacity: usize) -> Self {
        Self {
            total_size: 0,
            pooled: PooledTransactionsHashesBuilder::new(version),
    ...
    fn push(&mut self, transaction: &PropagateTransaction) {
        ...
        //
        // From: <https://sips.sila.org/SIPS/sip-4844#networking>
...
enum PooledTransactionsHashesBuilder {
    Sil66(NewPooledTransactionHashes66),
    Sil68(NewPooledTransactionHashes68),
    Sil72(NewPooledTransactionHashes72),
}
...
impl PooledTransactionsHashesBuilder {
    ...
    fn push_pooled<T: PoolTransaction>(&mut self, pooled_tx: Arc<ValidPoolTransaction<T>>) {
        match self {
            Self::Sil66(msg) => msg.push(*pooled_tx.hash()),
            Self::Sil68(msg) => {
    ...
    fn is_empty(&self) -> bool {
        match self {
            Self::Sil66(hashes) => hashes.is_empty(),
            Self::Sil68(hashes) => hashes.is_empty(),
            Self::Sil72(hashes) => hashes.is_empty(),
        }
    }
    ...
    fn len(&self) -> usize {
        match self {
            Self::Sil66(hashes) => hashes.len(),
            Self::Sil68(hashes) => hashes.len(),
            Self::Sil72(hashes) => hashes.len(),
        }
    }
    ...
    fn push(&mut self, tx: &PropagateTransaction) {
        match self {
            Self::Sil66(msg) => msg.push(*tx.tx_hash()),
            Self::Sil68(msg) => {
    ...
    fn new(version: SilVersion) -> Self {
        match version {
            SilVersion::Sil66 | SilVersion::Sil67 => Self::Sil66(Default::default()),
            SilVersion::Sil68 | SilVersion::Sil69 | SilVersion::Sil70 | SilVersion::Sil71 => {
                Self::Sil68(Default::default())
            }
            SilVersion::Sil72 => Self::Sil72(Default::default()),
        }
    }
    ...
    fn with_capacity(version: SilVersion, capacity: usize) -> Self {
        match version {
            SilVersion::Sil66 | SilVersion::Sil67 => {
                Self::Sil66(NewPooledTransactionHashes66::with_capacity(capacity))
            }
            SilVersion::Sil68 | SilVersion::Sil69 | SilVersion::Sil70 | SilVersion::Sil71 => {
                Self::Sil68(NewPooledTransactionHashes68::with_capacity(capacity))
            }
            SilVersion::Sil72 => Self::Sil72(NewPooledTransactionHashes72::with_capacity(capacity)),
        }
    }
...
impl<N: NetworkPrimitives> PeerMetadata<N> {
    ...
    /// Returns the negotiated `SilVersion` of the session.
...
#[derive(Debug)]
enum TransactionsCommand<N: NetworkPrimitives = SilNetworkPrimitives> {
...
mod tests {
    ...
    use alloy_eips::{sip2718::Encodable2718, sip4844::BlobTransactionValidationError};
    use alloy_primitives::{hex, Signature, TxKind, B256, U256};
    use alloy_rlp::Decodable;
    use futures::FutureExt;
    use rsil_chainspec::MIN_TRANSACTION_GAS;
    use rsil_network_api::{NetworkInfo, PeerKind};
    use rsil_network_p2p::{
        error::{RequestError, RequestResult},
    ...
    use rsil_transaction_pool::{
        ...
        error::{InvalidPoolTransactionError, PoolError, Sip4844PoolTransactionError},
        identifier::SenderIdentifiers,
        test_utils::{
            testing_pool, MockTransaction, MockTransactionFactory, OkValidator, TestPool,
            TransactionGenerator,
        },
        CoinbaseTipOrdering, Pool, SilPooledTransaction, TransactionOrigin, ValidPoolTransaction,
    };
    ...
    type SilTestPool = Pool<
        OkValidator<SilPooledTransaction>,
        CoinbaseTipOrdering<SilPooledTransaction>,
        InMemoryBlobStore,
    >;
    ...
    fn valid_eth_pool_transaction(
        transaction: SilPooledTransaction,
    ) -> Arc<ValidPoolTransaction<SilPooledTransaction>> {
    ...
    fn gen_eip1559_pooled_with_nonce<R: rand::RngCore>(
        ...
        nonce: u64,
    ) -> SilPooledTransaction {
        SilPooledTransaction::try_from_consensus(
    ...
    async fn test_ignored_tx_broadcasts_while_initially_syncing() {
        ...
        let config = NetworkConfigBuilder::sil(secret_key, Runtime::test())
            .disable_discovery()
            .listener_port(0)
            .build(client);
        let transactions_manager_config = config.transactions_manager_config.clone();
        ...
        // random tx: <https://silascan.io/getRawTx?tx=0x9448608d36e721ef403c53b00546068a6474d6cbab6816c3926de449898e7bce>
        let input = hex!(
    ...
    async fn test_session_closed_cleans_transaction_peer_state() {
        ...
        let (peer, _) = new_mock_session(peer_id, SilVersion::Sil66);
    ...
    async fn test_bad_blob_sidecar_not_cached_as_bad_import() {
        ...
        let err = PoolError::new(
            hash,
            InvalidPoolTransactionError::Sip4844(Sip4844PoolTransactionError::InvalidEip4844Blob(
    ...
    async fn test_missing_blob_sidecar_not_cached_as_bad_import() {
        ...
        let err = PoolError::new(
            hash,
            InvalidPoolTransactionError::Sip4844(
    ...
    async fn test_non_blob_sidecar_error_still_cached_as_bad_import() {
        ...
        let err = PoolError::new(
            hash,
            InvalidPoolTransactionError::Sip4844(Sip4844PoolTransactionError::NoEip4844Blobs),
        );
    ...
    async fn test_on_get_pooled_transactions_network() {
        ...
        let tx = MockTransaction::sip1559();
        let _ = transactions
            .pool
            .add_transaction(rsil_transaction_pool::TransactionOrigin::External, tx.clone())
            .await;
    ...
    async fn test_max_retries_tx_request() {
        ...
        let msg =
            NewPooledTransactionHashes::Sil66(NewPooledTransactionHashes66(seen_hashes.to_vec()));
    ...
    fn test_transaction_builder_empty() {
        let mut builder = PropagateTransactionsBuilder::pooled(SilVersion::Sil68, 0);
    ...
    fn test_transaction_builder_large() {
        let mut builder = PropagateTransactionsBuilder::full(SilVersion::Sil68, 0);
        assert!(builder.is_empty());
    ...
    async fn test_propagate_full() {
        ...
        let mut tx_gen = TransactionGenerator::new(rand::rng());
        let sip1559_tx = valid_eth_pool_transaction(tx_gen.gen_eip1559_pooled());
        propagate.push(sip1559_tx.clone());
        let sip4844_tx = valid_eth_pool_transaction(tx_gen.gen_eip4844_pooled());
        propagate.push(sip4844_tx.clone());
        ...
        let prop_txs = propagated.get(sip1559_tx.transaction.hash()).unwrap();
        assert_eq!(prop_txs.len(), 1);
        assert!(prop_txs[0].is_full());

        let prop_txs = propagated.get(sip4844_tx.transaction.hash()).unwrap();
        assert_eq!(prop_txs.len(), 1);
        assert!(prop_txs[0].is_hash());

        let peer = tx_manager.peers.get(&peer_id).unwrap();
        assert!(peer.seen_transactions.contains(sip1559_tx.transaction.hash()));
        assert!(peer.seen_transactions.contains(sip1559_tx.transaction.hash()));
        peer.seen_transactions.contains(sip4844_tx.transaction.hash());
    ...
    async fn test_truncated_hash_announcement_not_marked_seen() {
        ...
        let peer_id = PeerId::random();
        let (peer, _rx) = new_mock_session(peer_id, SilVersion::Sil68);
        tx_manager.peers.insert(peer_id, peer);
    ...
    async fn test_relaxed_filter_ignores_unknown_tx_types() {
        ...
        let (to_tx_manager_tx, from_network_rx) =
            rsil_metrics::common::mpsc::memory_bounded_channel::<
                NetworkTransactionEvent<SilNetworkPrimitives>,
            >(
        ...
        let mut tx_manager = TransactionsManager::<TestPool, SilNetworkPrimitives>::with_policy(
        ...
        let announcement_msg = NewPooledTransactionHashes::Sil68(NewPooledTransactionHashes68 {
        ...
        assert!(
            ...
            "Should have requested the known SIP-1559 transaction. Requested: {requested_hashes_in_getpooled:?}"
        );
        assert!(
