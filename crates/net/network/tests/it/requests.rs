...
//! Tests for sil related requests
...
async fn test_eth68_get_receipts() {
    ...
    let p1 = PeerConfig::with_protocols(mock_provider.clone(), Some(SilVersion::Sil68.into()));
    ...
    for block_num in 1..=10 {
        ...
        // Sil68 receipts should have bloom filters - verify the structure
...
async fn test_eth69_get_headers() {
    ...
    let p1 = PeerConfig::with_protocols(mock_provider.clone(), Some(SilVersion::Sil69.into()));
    net.add_peer_with_config(p1).await.unwrap();
...
// Ensures the fetch client can request BALs through an sil/71 peer.
#[tokio::test(flavor = "multi_thread")]
...
async fn test_eth71_get_block_access_lists_returns_empty_on_store_error() {
    ...
    let (net, _) = spawn_bal_testnet_with_store(
        [SilVersion::Sil71, SilVersion::Sil71],
...
// Ensures default fetch client BAL requests are rejected when no sil/71 peer is available.
#[tokio::test(flavor = "multi_thread")]
async fn test_eth70_fetch_client_rejects_default_block_access_lists_request() {
    rsil_tracing::init_test_tracing();
    let (net, _) = spawn_bal_testnet([SilVersion::Sil70, SilVersion::Sil70]).await;
...
async fn spawn_eth71_bal_testnet() -> (BalTestnetHandle, BalStoreHandle) {
    spawn_bal_testnet([SilVersion::Sil71, SilVersion::Sil71]).await
}

// Spawns a BAL testnet with one peer per requested sil protocol version.
...
// Spawns a BAL testnet with one peer per requested sil protocol version and the given BAL store.
...
async fn request_block_access_lists(net: &BalTestnetHandle, hashes: Vec<B256>) -> BlockAccessLists {
    ...
    requester.network().send_request(
        *responder.peer_id(),
        rsil_network::PeerRequest::GetBlockAccessLists {
