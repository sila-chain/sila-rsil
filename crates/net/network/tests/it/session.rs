    ...
    while let Some(event) = events.next().await {
        match event {
            ...
            NetworkEvent::ActivePeerSession { info, .. } => {
                ...
                assert_eq!(handle1.peer_id(), &peer_id);
                assert_eq!(status.version, SilVersion::LATEST);
            }
            ev => {
...
async fn test_session_established_with_different_capability() {
    ...
    let p1 = PeerConfig::with_protocols(NoopProvider::default(), Some(SilVersion::Sil66.into()));
    ...
    while let Some(event) = events.next().await {
        match event {
            ...
            NetworkEvent::ActivePeerSession { info, .. } => {
                let SessionInfo { peer_id, status, .. } = info;
                assert_eq!(handle1.peer_id(), &peer_id);
                assert_eq!(status.version, SilVersion::Sil66);
            }
            ev => {
                panic!("unexpected event: {ev:?}")
            }
...
async fn test_capability_version_mismatch() {
    ...
    let p0 = PeerConfig::with_protocols(NoopProvider::default(), Some(SilVersion::Sil66.into()));
    net.add_peer_with_config(p0).await.unwrap();

    let p1 = PeerConfig::with_protocols(NoopProvider::default(), Some(SilVersion::Sil67.into()));
    net.add_peer_with_config(p1).await.unwrap();
...
async fn test_eth69_peers_can_connect() {
    ...
    let p1 = PeerConfig::with_protocols(NoopProvider::default(), Some(SilVersion::Sil69.into()));
    net.add_peer_with_config(p1).await.unwrap();
    ...
    while let Some(event) = events.next().await {
        match event {
            ...
            NetworkEvent::ActivePeerSession { info, .. } => {
                ...
                assert_eq!(status.version, SilVersion::Sil69);
            }
            ev => {
                panic!("unexpected event: {ev:?}")
            }
...
async fn test_peers_negotiate_highest_version_eth69() {
    ...
    // Create one peer with multiple SIL versions including ETH69
    let p0 = PeerConfig::with_protocols(
        NoopProvider::default(),
        vec![
            SilVersion::Sil69.into(),
            SilVersion::Sil68.into(),
            SilVersion::Sil67.into(),
            SilVersion::Sil66.into(),
        ],
    );
    net.add_peer_with_config(p0).await.unwrap();

    // Create another peer with multiple SIL versions including ETH69
    let p1 = PeerConfig::with_protocols(
        NoopProvider::default(),
        vec![SilVersion::Sil69.into(), SilVersion::Sil68.into(), SilVersion::Sil67.into()],
    );
    net.add_peer_with_config(p1).await.unwrap();
...
async fn test_eth69_vs_eth68_incompatible() {
    ...
    let p1 = PeerConfig::with_protocols(NoopProvider::default(), Some(SilVersion::Sil68.into()));
    net.add_peer_with_config(p1).await.unwrap();
    ...
    // Peers with no shared SIL version should fail to connect and be removed.
    let removed_peer_id = event_stream.peer_removed().await.unwrap();
    assert_eq!(removed_peer_id, *handle1.peer_id());
...
async fn test_eth69_mixed_version_negotiation() {
    ...
    let p0 = PeerConfig::with_protocols(
        NoopProvider::default(),
        vec![SilVersion::Sil69.into(), SilVersion::Sil68.into()],
    );
    net.add_peer_with_config(p0).await.unwrap();
    ...
    while let Some(event) = events.next().await {
        match event {
            ...
            NetworkEvent::ActivePeerSession { info, .. } => {
                ...
                assert_eq!(status.version, SilVersion::Sil68);
            }
            ev => {
                panic!("unexpected event: {ev:?}")
            }
...
async fn test_multiple_peers_different_eth_versions() {
    ...
    let p2 = PeerConfig::with_protocols(
        NoopProvider::default(),
        vec![SilVersion::Sil67.into(), SilVersion::Sil66.into()],
    );
...
async fn test_eth69_capability_negotiation_fallback() {
    ...
    let p0 = PeerConfig::with_protocols(
        NoopProvider::default(),
        vec![SilVersion::Sil69.into(), SilVersion::Sil67.into()],
    );
    net.add_peer_with_config(p0).await.unwrap();
    ...
    while let Some(event) = events.next().await {
        match event {
            ...
            NetworkEvent::ActivePeerSession { info, .. } => {
                ...
                assert_eq!(status.version, SilVersion::Sil67);
            }
            ev => {
                panic!("unexpected event: {ev:?}")
            }
...
async fn test_overlapping_version_sets_negotiation() {
    ...
    let p0 = PeerConfig::with_protocols(
        NoopProvider::default(),
        vec![SilVersion::Sil69.into(), SilVersion::Sil67.into(), SilVersion::Sil66.into()],
    );
    net.add_peer_with_config(p0).await.unwrap();
    ...
    let p1 = PeerConfig::with_protocols(
        NoopProvider::default(),
        vec![SilVersion::Sil68.into(), SilVersion::Sil67.into(), SilVersion::Sil66.into()],
    );
    net.add_peer_with_config(p1).await.unwrap();
