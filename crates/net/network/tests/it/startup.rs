...
use rsil_discv4::{Discv4Config, NatResolver, DEFAULT_DISCOVERY_ADDR};
use rsil_network::{
    error::{NetworkError, ServiceKind},
    Discovery, NetworkConfigBuilder, NetworkManager,
};
use rsil_network_api::{NetworkInfo, PeersInfo};
...
fn is_addr_in_use_kind(err: &NetworkError, kind: ServiceKind) -> bool {
    match err {
        ...
        NetworkError::Discv5Error(rsil_discv5::Error::Discv5Error(discv5::Error::Io(err))) => {
...
async fn test_discv5_and_discv4_same_socket_ok() {
    ...
    let config = NetworkConfigBuilder::sil(secret_key, Runtime::test())
        ...
        .discovery_v5(
            rsil_discv5::Config::builder((DEFAULT_DISCOVERY_ADDR, test_port).into()).discv5_config(
...
// <https://github.com/sila-chain/sila-rsil/issues/8851>
#[tokio::test(flavor = "multi_thread")]
