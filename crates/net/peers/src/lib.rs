...
//!
//! This crate manages and converts Sila network entities such as node records, peer IDs, and
//! Sila Node Records (ENRs)
//!
//! ## An overview of Node Record types
//!
//! Sila uses different types of "node records" to represent peers on the network.
//!
...
//!
//! The most comprehensive node record type is the Sila Node Record ([`Enr`]), which is a
...
//! or [`TrustedPeer`], we use the [`AnyNode`] type. [`AnyNode`] is used in rsil's
...
//! - [`Enr`]: An Sila Node Record, which is a signed, versioned record that includes additional
