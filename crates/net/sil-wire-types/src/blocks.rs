...
generate_tests!(#[rlp, 10] BlockHeaders<alloy_consensus::Header>, SilBlockHeadersTests);
...
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[cfg_attr(any(test, feature = "arbitrary"), derive(arbitrary::Arbitrary))]
pub struct BlockBodies<B = rsil_sila_primitives::BlockBody>(
    /// The requested block bodies, each of which should correspond to a hash in the request.
    pub Vec<B>,
);

generate_tests!(#[rlp, 16] BlockBodies<rsil_sila_primitives::BlockBody>, SilBlockBodiesTests);
