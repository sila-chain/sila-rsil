...
///
/// <https://github.com/sila-chain/devp2p/blob/master/caps/sil.md#newpooledtransactionhashes-0x08>
...
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[cfg_attr(any(test, feature = "arbitrary"), derive(arbitrary::Arbitrary))]
pub struct NewBlock<B = rsil_sila_primitives::Block> {
    /// A new block.
    pub block: B,
...
generate_tests!(#[rlp, 25] NewBlock<rsil_sila_primitives::Block>, SilNewBlockTests);
...
/// per-peer messages reuse the same SIP-2718 encoding.
...
pub enum NewPooledTransactionHashes {
    ...
    ///
    /// This extends the sil/68 announcement payload with the `cell_mask` field introduced by
    /// [SIP-8070](https://sips.sila.org/SIPS/sip-8070).
    ///
    /// Note: it is assumed that the payload is valid (all vectors have the same length)
...
impl NewPooledTransactionHashes {
    /// Returns the message [`SilVersion`].
    pub const fn version(&self) -> SilVersion {
        match self {
            Self::Sil66(_) => SilVersion::Sil66,
            Self::Sil68(_) => SilVersion::Sil68,
            Self::Sil72(_) => SilVersion::Sil72,
        }
    }

    /// Returns `true` if the payload is valid for the given version
    pub const fn is_valid_for_version(&self, version: SilVersion) -> bool {
        match self {
            Self::Sil66(_) => {
                matches!(version, SilVersion::Sil67 | SilVersion::Sil66)
            }
            Self::Sil68(_) => {
                matches!(
                    version,
            ...
            Self::Sil72(_) => {
                matches!(version, SilVersion::Sil72)
            }
        }
    }
    ...
    pub fn iter_hashes(&self) -> impl Iterator<Item = &B256> + '_ {
        match self {
            Self::Sil66(msg) => msg.iter(),
            Self::Sil68(msg) => msg.hashes.iter(),
            Self::Sil72(msg) => msg.hashes.iter(),
        }
    }
    ...
    pub const fn hashes(&self) -> &Vec<B256> {
        match self {
            Self::Sil66(msg) => &msg.0,
            Self::Sil68(msg) => &msg.hashes,
            Self::Sil72(msg) => &msg.hashes,
        }
    }
    ...
    pub const fn hashes_mut(&mut self) -> &mut Vec<B256> {
        match self {
            Self::Sil66(msg) => &mut msg.0,
            Self::Sil68(msg) => &mut msg.hashes,
            Self::Sil72(msg) => &mut msg.hashes,
        }
    }
    ...
    pub fn into_iter_hashes(self) -> impl Iterator<Item = B256> {
        match self {
            Self::Sil66(msg) => msg.into_iter(),
            Self::Sil68(msg) => msg.hashes.into_iter(),
            Self::Sil72(msg) => msg.hashes.into_iter(),
        }
    }
    ...
    pub fn truncate(&mut self, len: usize) {
        match self {
            Self::Sil66(msg) => msg.truncate(len),
            Self::Sil68(msg) => {
    ...
    pub const fn is_empty(&self) -> bool {
        match self {
            Self::Sil66(msg) => msg.0.is_empty(),
            Self::Sil68(msg) => msg.hashes.is_empty(),
            Self::Sil72(msg) => msg.hashes.is_empty(),
        }
    }
    ...
    pub const fn len(&self) -> usize {
        match self {
            Self::Sil66(msg) => msg.0.len(),
            Self::Sil68(msg) => msg.hashes.len(),
            Self::Sil72(msg) => msg.hashes.len(),
        }
    }
    ...
    pub const fn as_eth72(&self) -> Option<&NewPooledTransactionHashes72> {
        match self {
            Self::Sil66(_) | Self::Sil68(_) => None,
            Self::Sil72(msg) => Some(msg),
        }
    }
    ...
    pub const fn as_eth68(&self) -> Option<&NewPooledTransactionHashes68> {
        match self {
            Self::Sil66(_) | Self::Sil72(_) => None,
            Self::Sil68(msg) => Some(msg),
        }
    }
    ...
    pub const fn as_eth66_mut(&mut self) -> Option<&mut NewPooledTransactionHashes66> {
        match self {
            Self::Sil66(msg) => Some(msg),
            Self::Sil68(_) | Self::Sil72(_) => None,
        }
    }
    ...
    pub fn take_eth68(&mut self) -> Option<NewPooledTransactionHashes68> {
        match self {
            Self::Sil66(_) | Self::Sil72(_) => None,
            Self::Sil68(msg) => Some(mem::take(msg)),
        }
    }
    ...
    pub fn take_eth66(&mut self) -> Option<NewPooledTransactionHashes66> {
        match self {
            Self::Sil66(msg) => Some(mem::take(msg)),
            Self::Sil68(_) | Self::Sil72(_) => None,
        }
    }
}

impl<N: NetworkPrimitives> From<NewPooledTransactionHashes> for SilMessage<N> {
    fn from(value: NewPooledTransactionHashes) -> Self {
        match value {
            NewPooledTransactionHashes::Sil66(msg) => Self::NewPooledTransactionHashes66(msg),
            NewPooledTransactionHashes::Sil68(msg) => Self::NewPooledTransactionHashes68(msg),
            NewPooledTransactionHashes::Sil72(msg) => Self::NewPooledTransactionHashes72(msg),
        }
    }
}
...
/// the node sends the transaction types and their sizes (as defined in SIP-2718) as well.
...
pub struct NewPooledTransactionHashes68 {
    ...
    ///
    /// In the [sil/68 spec](https://sips.sila.org/SIPS/sip-5793#specification) this is defined
    /// the following way:
    ...
    /// type](https://github.com/sila-chain/go-sila/blob/82d934b1dd80cdd8190803ea9f73ed2c345e2576/sil/protocols/sil/protocol.go#L308-L313),
    /// which [ends up being encoded as a RLP
    /// string](https://github.com/sila-chain/go-sila/blob/82d934b1dd80cdd8190803ea9f73ed2c345e2576/rlp/encode_test.go#L171-L176),
    /// **not** a RLP list.
    ///
    ...
    fn arbitrary_with(_args: ()) -> Self::Strategy {
        ...
        vec_length
            .prop_flat_map(|len| {
                ...
                let types_vec = vec(
                    proptest_arbitrary_interop::arb::<rsil_sila_primitives::TxType>()
...
/// Same as [`NewPooledTransactionHashes68`] but adds the sil/72 `cell_mask` field from
/// [SIP-8070](https://sips.sila.org/SIPS/sip-8070).
#[derive(Clone, Debug, PartialEq, Eq, Default)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct NewPooledTransactionHashes72 {
    ...
    /// ## Note on RLP encoding and decoding
    ///
    /// In the [sil/72 spec](https://sips.sila.org/SIPS/sip-8070#specification) this is defined
    /// the following way:
    ...
    ///
    /// Per [SIP-8070](https://sips.sila.org/SIPS/sip-8070), this is a `B_16`
...
pub type Sil68TxMetadata = Option<(u8, usize)>;

impl DedupPayload for NewPooledTransactionHashes {
    type Value = Sil68TxMetadata;
    ...
    fn dedup(self) -> PartiallyValidData<Self::Value> {
        match self {
            Self::Sil66(msg) => msg.dedup(),
            Self::Sil68(msg) => msg.dedup(),
            Self::Sil72(msg) => msg.dedup(),
        }
    }
}
...
impl DedupPayload for NewPooledTransactionHashes66 {
    ...
    fn dedup(self) -> PartiallyValidData<Self::Value> {
        ...
        let noop_value: Sil68TxMetadata = None;
...
pub trait HandleVersionedMempoolData {
    /// Returns the announcement version, either [`Sil66`](SilVersion::Sil66) or
    /// [`Sil68`](SilVersion::Sil68).
    fn msg_version(&self) -> SilVersion;
}
...
impl<V> PartiallyValidData<V> {
    /// Wraps raw data.
    pub const fn from_raw_data(data: B256Map<V>, version: Option<SilVersion>) -> Self {
        Self { data, version }
    }

    /// Wraps raw data with version [`SilVersion::Sil72`].
    pub const fn from_raw_data_eth72(data: B256Map<V>) -> Self {
        Self::from_raw_data(data, Some(SilVersion::Sil72))
    }

    /// Wraps raw data with version [`SilVersion::Sil68`].
    pub const fn from_raw_data_eth68(data: B256Map<V>) -> Self {
        Self::from_raw_data(data, Some(SilVersion::Sil68))
    }

    /// Wraps raw data with version [`SilVersion::Sil66`].
    pub const fn from_raw_data_eth66(data: B256Map<V>) -> Self {
        Self::from_raw_data(data, Some(SilVersion::Sil66))
    }

    /// Returns a new [`PartiallyValidData`] with empty data from an [`Sil72`](SilVersion::Sil72)
    /// announcement.
    pub fn empty_eth72() -> Self {
        Self::from_raw_data_eth72(B256Map::default())
    }

    /// Returns a new [`PartiallyValidData`] with empty data from an [`Sil68`](SilVersion::Sil68)
    /// announcement.
    pub fn empty_eth68() -> Self {
        Self::from_raw_data_eth68(B256Map::default())
    }

    /// Returns a new [`PartiallyValidData`] with empty data from an [`Sil66`](SilVersion::Sil66)
    /// announcement.
    ...
    /// message exist.
    pub const fn msg_version(&self) -> Option<SilVersion> {
        self.version
    }
...
impl ValidAnnouncementData {
    ...
    /// this is [`Sil68`](SilVersion::Sil68) announcement data, this drops the metadata.
    pub fn into_request_hashes(self) -> (RequestTxHashes, SilVersion) {
    ...
    /// from an announcement, should have some [`SilVersion`]. Panics if [`PartiallyValidData`] has
    /// version set to `None`.
    pub fn from_partially_valid_data(data: PartiallyValidData<Sil68TxMetadata>) -> Self {
        let PartiallyValidData { data, version } = data;

        let version = version.expect("should have sil version for conversion");

        Self { data, version }
    }

    /// Destructs returning the validated data.
    pub fn into_data(self) -> B256Map<Sil68TxMetadata> {
        self.data
    }
}
...
impl FromIterator<(TxHash, Sil68TxMetadata)> for RequestTxHashes {
    fn from_iter<I: IntoIterator<Item = (TxHash, Sil68TxMetadata)>>(iter: I) -> Self {
...
/// See [BlockRangeUpdate](https://github.com/sila-chain/devp2p/blob/master/caps/sil.md#blockrangeupdate-0x11).
