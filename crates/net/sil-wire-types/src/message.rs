//! Implements Sila wire protocol for versions 66 through 71.
//! Defines structs/enums for messages, request-response pairs, and broadcasts.
//! Handles compatibility with [`SilVersion`].
//!
//! Examples include creating, encoding, and decoding protocol messages.
//!
//! Reference: [Sila Wire Protocol](https://github.com/sila-chain/devp2p/blob/master/caps/sil.md).
...
// https://github.com/sila-chain/go-sila/blob/30602163d5d8321fbc68afdcbbaf2362b2641bde/sil/protocols/sil/protocol.go#L50
...
/// An `sil` protocol message, containing a message ID and payload.
#[derive(Clone, Debug, PartialEq, Eq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct ProtocolMessage<N: NetworkPrimitives = SilNetworkPrimitives> {
    /// The unique identifier representing the type of the Sila message.
    pub message_type: SilMessageID,
    /// The content of the message, including specific data based on the message type.
    #[cfg_attr(
        feature = "serde",
        serde(bound = "SilMessage<N>: serde::Serialize + serde::de::DeserializeOwned")
    )]
    pub message: SilMessage<N>,
}

impl<N: NetworkPrimitives> ProtocolMessage<N> {
    /// Decode only a Status message from RLP bytes.
    ///
    /// This is used during the sil handshake where only a Status message is a valid response.
    ...
    ///
    /// This will enforce decoding according to the given [`SilVersion`] of the connection.
    pub fn decode_message(version: SilVersion, buf: &mut &[u8]) -> Result<Self, MessageError> {
    ...
    pub fn decode_message_with_tx_memory_budget(
        ...
        // For SIP-7642 (https://github.com/sila-chain/SIPs/blob/master/SIPS/sip-7642.md):
        // pre-merge (legacy) status messages include total difficulty, whereas sil/69 omits it.
        let message = match message_type {
            ...
            SilMessageID::NewBlock => {
                SilMessage::NewBlock(Box::new(N::NewBlockPayload::decode(buf)?))
            }
            SilMessageID::Transactions => SilMessage::Transactions(
                Transactions::decode_with_memory_budget(buf, tx_memory_budget)?,
            ),
            SilMessageID::NewPooledTransactionHashes => {
                if version >= SilVersion::Sil72 {
                    SilMessage::NewPooledTransactionHashes72(NewPooledTransactionHashes72::decode(
                        buf,
                    )?)
                } else if version >= SilVersion::Sil68 {
                    SilMessage::NewPooledTransactionHashes68(NewPooledTransactionHashes68::decode(
                        buf,
                    )?)
                } else {
                    SilMessage::NewPooledTransactionHashes66(NewPooledTransactionHashes66::decode(
                        buf,
                    )?)
                }
            }
            SilMessageID::GetBlockHeaders => SilMessage::GetBlockHeaders(RequestPair::decode(buf)?),
            SilMessageID::BlockHeaders => SilMessage::BlockHeaders(RequestPair::decode(buf)?),
            SilMessageID::GetBlockBodies => SilMessage::GetBlockBodies(RequestPair::decode(buf)?),
            SilMessageID::BlockBodies => SilMessage::BlockBodies(RequestPair::decode(buf)?),
            SilMessageID::GetPooledTransactions => {
                SilMessage::GetPooledTransactions(RequestPair::decode(buf)?)
            }
            SilMessageID::PooledTransactions => {
                SilMessage::PooledTransactions(RequestPair::decode_with(buf, |buf| {
            ...
            SilMessageID::GetReceipts => {
                if version >= SilVersion::Sil70 {
                    SilMessage::GetReceipts70(RequestPair::decode(buf)?)
                } else {
                    SilMessage::GetReceipts(RequestPair::decode(buf)?)
                }
            }
            SilMessageID::Receipts => {
                match version {
                    v if v >= SilVersion::Sil70 => {
                        // sil/70 continues to omit bloom filters and adds the
                    ...
                    _ => {
                        ...
                        SilMessage::Receipts(RequestPair::decode(buf)?)
                    }
                }
            }
...
impl<N: NetworkPrimitives> From<SilMessage<N>> for ProtocolMessage<N> {
    fn from(message: SilMessage<N>) -> Self {
...
#[derive(Clone, Debug)]
pub struct ProtocolBroadcastMessage<N: NetworkPrimitives = SilNetworkPrimitives> {
    /// The unique identifier representing the type of the Sila message.
    pub message_type: SilMessageID,
    /// The content of the message to be broadcasted, including specific data based on the message
    /// type.
    pub message: SilBroadcastMessage<N>,
}
...
impl<N: NetworkPrimitives> From<SilBroadcastMessage<N>> for ProtocolBroadcastMessage<N> {
    fn from(message: SilBroadcastMessage<N>) -> Self {
        Self { message_type: message.message_id(), message }
    }
}

/// Represents a message in the sil wire protocol, versions 66, 67, 68 and 69.
///
/// The sila wire protocol is a set of messages that are broadcast to the network in two
/// styles:
...
///
/// The newer `sil/66` is an efficiency upgrade on top of `sil/65`, introducing a request id to
/// correlate request-response message pairs. This allows for request multiplexing.
///
/// The `sil/67` is based on `sil/66` but only removes two messages, [`GetNodeData`] and
/// [`NodeData`].
///
/// The `sil/68` changes only `NewPooledTransactionHashes` to include `types` and `sized`. For
/// it, `NewPooledTransactionHashes` is renamed as [`NewPooledTransactionHashes66`] and
/// [`NewPooledTransactionHashes68`] is defined.
///
/// The `sil/69` announces the historical block range served by the node. Removes total difficulty
/// information. And removes the Bloom field from receipts transferred over the protocol.
///
/// The `sil/70` (SIP-7975) keeps the sil/69 status format and introduces partial receipts.
/// requests/responses.
///
/// The `sil/71` draft extends sil/70 with block access list request/response messages.
#[derive(Clone, Debug, PartialEq, Eq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub enum SilMessage<N: NetworkPrimitives = SilNetworkPrimitives> {
    ...
    /// Represents a `NewPooledTransactionHashes` message for sil/66 version.
    NewPooledTransactionHashes66(NewPooledTransactionHashes66),
    /// Represents a `NewPooledTransactionHashes` message for sil/68 version.
    NewPooledTransactionHashes68(NewPooledTransactionHashes68),
    /// Represents a `NewPooledTransactionHashes` message for sil/72 version.
    ...
    /// Represents a `GetReceipts` request for sil/70.
    ///
    /// Note: Unlike earlier protocol versions, the sil/70 encoding for
    /// `GetReceipts` in SIP-7975 inlines the request id. The type still wraps
    /// a [`RequestPair`], but with a custom inline encoding.
    GetReceipts70(RequestPair<GetReceipts70>),
    /// Represents a `GetBlockAccessLists` request-response pair for sil/71.
    ...
    /// Represents a Receipts request-response pair for sil/69.
    #[cfg_attr(
        feature = "serde",
        serde(bound = "N::Receipt: serde::Serialize + serde::de::DeserializeOwned")
    )]
    Receipts69(RequestPair<Receipts69<N::Receipt>>),
    /// Represents a Receipts request-response pair for sil/70.
    #[cfg_attr(
        feature = "serde",
        serde(bound = "N::Receipt: serde::Serialize + serde::de::DeserializeOwned")
    )]
    ///
    /// Note: The sil/70 encoding for `Receipts` in SIP-7975 inlines the
    /// request id. The type still wraps a [`RequestPair`], but with a custom
    /// inline encoding.
    Receipts70(RequestPair<Receipts70<N::Receipt>>),
    /// Represents a `BlockAccessLists` request-response pair for sil/71.
    BlockAccessLists(RequestPair<BlockAccessLists>),
    /// Represents a `Cells` request-response pair for sil/72.
    Cells(RequestPair<Cells>),
    /// Represents a `GetCells` request-response pair for sil/72.
...
impl<N: NetworkPrimitives> SilMessage<N> {
    /// Returns the message's ID.
    pub const fn message_id(&self) -> SilMessageID {
        match self {
            Self::Status(_) => SilMessageID::Status,
            ...
            Self::BlockBodies(_) => SilMessageID::BlockBodies,
            Self::GetPooledTransactions(_) => SilMessageID::GetPooledTransactions,
            Self::PooledTransactions(_) => SilMessageID::PooledTransactions,
            Self::GetNodeData(_) => SilMessageID::GetNodeData,
            Self::NodeData(_) => SilMessageID::NodeData,
            Self::GetReceipts(_) | Self::GetReceipts70(_) => SilMessageID::GetReceipts,
            Self::Receipts(_) | Self::Receipts69(_) | Self::Receipts70(_) => SilMessageID::Receipts,
            Self::BlockRangeUpdate(_) => SilMessageID::BlockRangeUpdate,
            Self::GetBlockAccessLists(_) => SilMessageID::GetBlockAccessLists,
            Self::BlockAccessLists(_) => SilMessageID::BlockAccessLists,
            Self::Cells(_) => SilMessageID::Cells,
            Self::GetCells(_) => SilMessageID::GetCells,
            Self::Other(msg) => SilMessageID::Other(msg.id as u8),
        }
    }
    ...
    /// types.
    pub fn map_versioned(self, version: SilVersion) -> Self {
        // For sil/70 peers we send `GetReceipts` using the new sil/70
...
impl<N: NetworkPrimitives> Encodable for SilMessage<N> {
    fn encode(&self, out: &mut dyn BufMut) {
        match self {
...
/// Represents broadcast messages of [`SilMessage`] with the same object that can be sent to
/// multiple peers.
///
...
#[derive(Clone, Debug)]
pub enum SilBroadcastMessage<N: NetworkPrimitives = SilNetworkPrimitives> {
...
// === impl SilBroadcastMessage ===

impl<N: NetworkPrimitives> SilBroadcastMessage<N> {
    /// Returns the message's ID.
    pub const fn message_id(&self) -> SilMessageID {
        match self {
            Self::NewBlock(_) => SilMessageID::NewBlock,
            Self::Transactions(_) | Self::BroadcastPoolTransactions(_) => {
                SilMessageID::Transactions
            }
        }
    }
...
impl<N: NetworkPrimitives> Encodable for SilBroadcastMessage<N> {
    fn encode(&self, out: &mut dyn BufMut) {
        match self {
            Self::NewBlock(new_block) => new_block.encode(out),
            Self::Transactions(transactions) => transactions.encode(out),
...
/// Represents message IDs for sil protocol messages.
#[repr(u8)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub enum SilMessageID {
    /// Status message.
    Status = 0x00,
...
impl SilMessageID {
    /// Returns the corresponding `u8` value for an `SilMessageID`.
    ...
    ///
    /// <https://github.com/sila-chain/go-sila/blob/85077be58edea572f29c3b1a6a055077f1a56a8b/sil/protocols/sil/protocol.go#L45-L47>
    pub const fn message_count(version: SilVersion) -> u8 {
        Self::max(version) + 1
    }
}

impl Encodable for SilMessageID {
    fn encode(&self, out: &mut dyn BufMut) {
...
impl Decodable for SilMessageID {
...
impl TryFrom<usize> for SilMessageID {
...
/// This is used for all request-response style `sil` protocol messages.
...
mod tests {
    ...
    fn test_removed_message_at_eth67() {
        let get_node_data = SilMessage::<SilNetworkPrimitives>::GetNodeData(RequestPair {
            request_id: 1337,
            message: GetNodeData(vec![]),
        });
        let buf = encode(ProtocolMessage {
            message_type: SilMessageID::GetNodeData,
            message: get_node_data,
        });
        let msg = ProtocolMessage::<SilNetworkPrimitives>::decode_message(
            crate::SilVersion::Sil67,
            &mut &buf[..],
        );
        assert!(matches!(msg, Err(MessageError::Invalid(..))));

        let node_data = SilMessage::<SilNetworkPrimitives>::NodeData(RequestPair {
            request_id: 1337,
            message: NodeData(vec![]),
        });
        let buf =
            encode(ProtocolMessage { message_type: SilMessageID::NodeData, message: node_data });
        let msg = ProtocolMessage::<SilNetworkPrimitives>::decode_message(
            crate::SilVersion::Sil67,
            &mut &buf[..],
        );
    ...
    fn test_bal_message_version_gating() {
        let get_block_access_lists =
            SilMessage::<SilNetworkPrimitives>::GetBlockAccessLists(RequestPair {
                request_id: 1337,
                message: GetBlockAccessLists(vec![]),
            });
        let buf = encode(ProtocolMessage {
            message_type: SilMessageID::GetBlockAccessLists,
        ...
        assert!(matches!(
            msg,
            Err(MessageError::Invalid(SilVersion::Sil70, SilMessageID::GetBlockAccessLists))
        ));

        let block_access_lists =
            SilMessage::<SilNetworkPrimitives>::BlockAccessLists(RequestPair {
                request_id: 1337,
                message: BlockAccessLists(vec![]),
            });
        let buf = encode(ProtocolMessage {
            message_type: SilMessageID::BlockAccessLists,
        ...
        assert!(matches!(
            msg,
            Err(MessageError::Invalid(SilVersion::Sil70, SilMessageID::BlockAccessLists))
        ));
    }

    #[test]
    fn test_bal_message_eth71_roundtrip() {
        let msg = ProtocolMessage::from(SilMessage::<SilNetworkPrimitives>::GetBlockAccessLists(
            RequestPair { request_id: 42, message: GetBlockAccessLists(vec![]) },
        ));
        let encoded = encode(msg.clone());
        let decoded = ProtocolMessage::<SilNetworkPrimitives>::decode_message(
            SilVersion::Sil71,
            &mut &encoded[..],
        )
        .unwrap();
    ...
    fn empty_block_bodies_protocol() {
        let empty_block_bodies =
            ProtocolMessage::from(SilMessage::<SilNetworkPrimitives>::BlockBodies(RequestPair {
                request_id: 0,
        ...
        let decoded =
            ProtocolMessage::decode_message(SilVersion::Sil68, &mut buf.as_slice()).unwrap();
    ...
    fn custom_message_roundtrip() {
        ...
        let protocol_message = ProtocolMessage::<SilNetworkPrimitives> {
            message_type: SilMessageID::Other(0x20),
            message: SilMessage::Other(custom_message),
        };
    ...
    fn custom_message_empty_payload_roundtrip() {
        ...
        let protocol_message = ProtocolMessage::<SilNetworkPrimitives> {
            message_type: SilMessageID::Other(0x30),
            message: SilMessage::Other(custom_message),
        };
    ...
    fn decode_status_success() {
        ...
        let protocol_message = ProtocolMessage::<SilNetworkPrimitives>::from(SilMessage::Status(
            StatusMessage::Legacy(status),
        ));
        let encoded = encode(protocol_message);

        let decoded = ProtocolMessage::<SilNetworkPrimitives>::decode_status(
            SilVersion::Sil68,
            &mut &encoded[..],
        )
        .unwrap();
    ...
    fn decode_status_rejects_non_status() {
        let msg = SilMessage::<SilNetworkPrimitives>::GetBlockBodies(RequestPair {
            request_id: 1,
            message: crate::GetBlockBodies::default(),
        });
        let protocol_message =
            ProtocolMessage { message_type: SilMessageID::GetBlockBodies, message: msg };
        let encoded = encode(protocol_message);

        let result = ProtocolMessage::<SilNetworkPrimitives>::decode_status(
            SilVersion::Sil68,
            &mut &encoded[..],
        );
        ...
        assert!(matches!(
            result,
            Err(MessageError::ExpectedStatusMessage(SilMessageID::GetBlockBodies))
        ));
    }
}
