...
///
/// This trait defines the types used in the Sila Wire Protocol (devp2p) for
...
///
/// See [`crate::SilMessage`] for more context.
pub trait NetworkPrimitives: Send + Sync + Unpin + Clone + Debug + 'static {
    ...
    ///
    /// This is different from `PooledTransactions` to account for the Sila case where
    /// SIP-4844 blob transactions are not announced over the network and can only be
    ...
    ///
    /// For SIP-4844 blob transactions, this includes the full blob sidecar with
...
/// Network primitive types used by Sila networks.
pub type SilNetworkPrimitives = BasicNetworkPrimitives<SilPrimitives, PooledTransactionVariant>;
