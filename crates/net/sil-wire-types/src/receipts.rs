...
/// Sil/70 `GetReceipts` request payload that supports partial receipt queries.
///
/// When used with sil/70, the request id is carried by the surrounding
/// [`crate::message::RequestPair`], and the on-wire shape is the flattened list
/// `firstBlockReceiptIndex, [blockhash₁, ...]`.
///
/// See also [sip-7975](https://sips.sila.org/SIPS/sip-7975)
...
/// Sil/69 receipt response type that removes bloom filters from the protocol.
///
...
impl<T: TxReceipt> Receipts69<T> {
    ...
    ///
    /// Sil/69 omits bloom filters on the wire, while some internal callers
    /// (and legacy APIs) still operate on [`Receipts`] with
    /// [`ReceiptWithBloom`]. This helper reconstructs the bloom locally from
    /// each receipt's logs so the older API can be used on top of sil/69 data.
    ///
...
/// Sil/70 `Receipts` response payload.
///
...
impl<T: TxReceipt> Receipts70<T> {
    /// Encodes all receipts with the bloom filter.
    ///
    /// Just like sil/69, sil/70 does not transmit bloom filters over the wire.
    /// When higher layers still expect the older bloom-bearing [`Receipts`]
    /// type, this helper converts the sil/70 payload into that shape by
    /// recomputing the bloom locally from the contained receipts.
    ///
    /// Note: This is an expensive operation that recalculates the bloom for
    /// every receipt.
    pub fn into_with_bloom(self) -> Receipts<T> {
        // Reuse the sil/69 helper, since both variants carry the same
        // receipt list shape (only sil/70 adds request metadata).
...
mod tests {
    ...
    #[test]
    // Test vector from: https://sips.sila.org/SIPS/sip-2481
