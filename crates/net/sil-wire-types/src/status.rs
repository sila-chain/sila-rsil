...
pub struct UnifiedStatus {
    /// The sil protocol version (e.g. sil/66 to sil/70).
    pub version: SilVersion,
    /// The chain ID identifying the peer’s network.
    pub chain: Chain,
    /// The genesis block hash of the peer’s chain.
    pub genesis: B256,
    /// The fork ID as defined by SIP-2124.
    pub forkid: ForkId,
    /// The latest block hash known to the peer.
    pub blockhash: B256,
    /// The total difficulty of the peer’s best chain (sil/66–68 only).
    pub total_difficulty: Option<U256>,
    /// The earliest block this node can serve (sil/69 only).
    pub earliest_block: Option<u64>,
    /// The latest block number this node has (sil/69 only).
...
impl UnifiedStatus {
    ...
    /// Returns the peer's advertised `BlockRangeUpdate` if this status came from an `sil/69+`
    /// handshake.
    pub fn block_range_update(&self) -> Option<BlockRangeUpdate> {
        (self.version >= SilVersion::Sil69)
            .then_some(())
...
impl StatusBuilder {
    ...
    /// Sets the sil protocol version (e.g., sil/66, sil/70).
    pub const fn version(mut self, version: SilVersion) -> Self {
...
/// The status message is used in the sil protocol handshake to ensure that peers are on the same
...
pub struct Status {
    /// The current protocol version. For example, peers running `sil/66` would have a version of
    /// 66.
    pub version: SilVersion,

    /// The chain id, as introduced in
    /// [SIP155](https://sips.sila.org/SIPS/sip-155#list-of-chain-ids).
    pub chain: Chain,
    ...
    /// [SIP-2124](https://github.com/sila-chain/SIPs/blob/master/SIPS/sip-2124.md).
    /// This was added in [`sil/64`](https://sips.sila.org/SIPS/sip-2364)
    pub forkid: ForkId,
}

// <https://silascan.io/block/0>
...
/// Similar to [`Status`], but for `sil/69` version, which does not contain
...
/// `StatusMessage` can store either the Legacy version (with TD), or the sil/69+/sil/70 version
...
impl StatusMessage {
    ...
    pub const fn genesis(&self) -> B256 {
        match self {
            ...
            Self::Sil69(status_69) => status_69.genesis,
        }
    }
    ...
    pub const fn version(&self) -> SilVersion {
        match self {
            ...
            Self::Sil69(status_69) => status_69.version,
        }
    }
    ...
    pub const fn chain(&self) -> &Chain {
        match self {
            ...
            Self::Sil69(status_69) => &status_69.chain,
        }
    }
    ...
    pub const fn forkid(&self) -> ForkId {
        match self {
            ...
            Self::Sil69(status_69) => status_69.forkid,
        }
    }
    ...
    pub const fn blockhash(&self) -> B256 {
        match self {
            ...
            Self::Sil69(status_69) => status_69.blockhash,
        }
    }
}
...
impl Display for StatusMessage {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::Sil69(s69) => Display::fmt(s69, f),
        }
    }
}
#[cfg(test)]
mod tests {
    ...
    use rand::Rng;
    use rsil_chainspec::{Chain, ChainSpec, ForkCondition, NamedChain};
    use std::str::FromStr;
    ...
    fn block_range_update_is_none_for_legacy_status() {
        let status = UnifiedStatus::builder().version(SilVersion::Sil68).build();
    ...
    fn decode_eth69_status_message() {
        ...
        let expected_status = if let StatusMessage::Sil69(status69) = expected_message {
            status69
        } else {
            panic!("expected StatusMessage::Sil69 variant");
        };
    ...
    fn init_custom_status_fields() {
        ...
        // add a few hardforks
        let hardforks = vec![
            (SilaHardfork::Tangerine, ForkCondition::Block(1)),
            (SilaHardfork::SpuriousDragon, ForkCondition::Block(2)),
