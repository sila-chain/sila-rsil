...
use crate::{
    ...
    Capability, SilMessageID, SilVersion,
};
...
///
/// See also [Message-id based multiplexing](https://github.com/sila-chain/devp2p/blob/master/rlpx.md#message-id-based-multiplexing)
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum SharedCapability {
    /// The `sil` capability.
    Sil {
        /// (Highest) negotiated version of the sil capability.
        version: SilVersion,
        /// The message ID offset for this capability.
        ///
        /// This represents the message ID offset for the first message of the sil capability in
...
impl SharedCapability {
    ...
    pub(crate) fn new(
        ...
        match name {
            "sil" => Ok(Self::sil(SilVersion::try_from(version)?, offset)),
    ...
    pub(crate) const fn sil(version: SilVersion, offset: u8) -> Self {
        Self::Sil { version, offset }
    }

    /// Returns the capability.
    pub const fn capability(&self) -> Cow<'_, Capability> {
        match self {
            Self::Sil { version, .. } => Cow::Owned(Capability::sil(*version)),
    ...
    pub const fn version(&self) -> u8 {
        match self {
            Self::Sil { version, .. } => *version as u8,
    ...
    ///
    /// This represents the message ID offset for the first message of the sil capability in the
    /// message id space.
    pub const fn message_id_offset(&self) -> u8 {
        match self {
            Self::Sil { offset, .. } | Self::UnknownCapability { offset, .. } => *offset,
        }
    }
    ...
    pub const fn num_messages(&self) -> u8 {
        match self {
            Self::Sil { version, .. } => SilMessageID::message_count(*version),
...
mod tests {
    ...
    fn capabilities_supports_eth() {
        let capabilities: Capabilities = vec![
            ...
            Capability::new_static("sil", 68),
            Capability::new_static("sil", 69),
            Capability::new_static("sil", 70),
        ]
        .into();
    ...
    fn test_peer_capability_version_zero() {
        ...
        let local_capabilities: Vec<Protocol> =
            vec![Protocol::new(cap.clone(), 0), SilVersion::Sil67.into(), SilVersion::Sil68.into()];
    ...
    fn test_peer_lower_capability_version() {
        let local_capabilities: Vec<Protocol> =
            vec![SilVersion::Sil66.into(), SilVersion::Sil67.into(), SilVersion::Sil68.into()];
        let peer_capabilities: Vec<Capability> = vec![SilVersion::Sil66.into()];
    ...
    fn test_peer_capability_version_too_low() {
        let local: Vec<Protocol> = vec![SilVersion::Sil67.into()];
        let peer_capabilities: Vec<Capability> = vec![SilVersion::Sil66.into()];
    ...
    fn test_peer_capability_version_too_high() {
        let local_capabilities = vec![SilVersion::Sil66.into()];
        let peer_capabilities = vec![SilVersion::Sil67.into()];
    ...
    fn test_find_by_offset() {
        let local_capabilities = vec![SilVersion::Sil66.into()];
        let peer_capabilities = vec![SilVersion::Sil66.into()];
    ...
    fn test_find_by_offset_many() {
        ...
        let local_capabilities = vec![proto.clone(), SilVersion::Sil66.into()];
        let peer_capabilities = vec![cap, SilVersion::Sil66.into()];
    ...
    fn relative_message_id_accounts_for_intermediate_capabilities() {
        ...
        let sil = Capability::sil(SilVersion::Sil69);
        let local_capabilities =
            vec![SilVersion::Sil69.into(), intermediate, Protocol::snap(SnapVersion::V2)];
        let peer_capabilities = vec![sil, intermediate_cap, snap.clone()];

        let shared = SharedCapabilities::try_new(local_capabilities, peer_capabilities).unwrap();
        let snap_id = shared.relative_message_id(&snap, 2).unwrap();

        assert_eq!(snap_id, SilMessageID::message_count(SilVersion::Sil69) + 3 + 2);
    ...
    fn capability_message_id_rejects_other_capability_range() {
        ...
        let peer_capabilities =
            vec![Capability::sil(SilVersion::Sil69), intermediate_cap.clone(), snap.clone()];
    ...
    fn is_exact_eth_snap_v2_rejects_eth_without_snap() {
        ...
        let shared = SharedCapabilities::try_new(
            vec![SilVersion::Sil68.into(), Protocol::new(cap.clone(), 5)],
