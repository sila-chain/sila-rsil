//! Error handling for (`SilStream`)[`crate::SilStream`]
...
pub enum SilStreamError {
    ...
    #[error(transparent)]
    /// Failed Sila handshake.
    SilHandshakeError(#[from] SilHandshakeError),
    ...
    StreamTimeout,
    /// Error triggered when an unknown or unsupported Sila message ID is received.
    #[error("Received unknown SIL message ID: 0x{message_id:X}")]
    UnsupportedMessage {
        /// The identifier of the unknown Sila message.
        message_id: u8,
    },
}
...
impl From<io::Error> for SilStreamError {
    fn from(err: io::Error) -> Self {
        P2PStreamError::from(err).into()
    }
}

/// Error  that can occur during the `sil` sub-protocol handshake.
#[derive(thiserror::Error, Debug)]
pub enum SilHandshakeError {
    ...
    MismatchedProtocolVersion(GotExpected<SilVersion>),
