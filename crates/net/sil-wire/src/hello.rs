...
impl HelloMessageWithProtocols {
    ...
    ///
    /// snap/2 is negotiated as an `RLPx` capability and rides alongside `sil` on the same
...
// TODO: determine if we should allow for the extra fields at the end like SIP-706 suggests
...
///
/// See also <https://github.com/sila-chain/devp2p/blob/master/rlpx.md#hello-0x00>
...
impl HelloMessageBuilder {
    ...
    /// - `client_version`: [`RSIL_CLIENT_VERSION`]
    /// - `capabilities`: All [`SilVersion`]
    pub fn build(self) -> HelloMessageWithProtocols {
        ...
        HelloMessageWithProtocols {
            ...
            client_version: client_version.unwrap_or_else(|| RSIL_CLIENT_VERSION.to_string()),
            protocols: protocols.unwrap_or_else(|| {
                SilVersion::ALL_VERSIONS.iter().copied().map(Into::into).collect()
            }),
...
mod tests {
    ...
    fn test_hello_encoding_round_trip() {
        ...
        let hello = P2PMessage::Hello(HelloMessage {
            protocol_version: ProtocolVersion::V5,
            client_version: "rsil/0.1.0".to_string(),
            capabilities: vec![Capability::new_static("sil", SilVersion::Sil67 as usize)],
            port: 30303,
            id,
        });
