...
//!
//! Most of other subprotocols are "dependent satellite" protocols of "sil" and not a fully standalone protocol, for example "snap", See also [snap protocol](https://github.com/sila-chain/devp2p/blob/298d7a77c3bf833641579ecbbb5b13f0311eeeea/caps/snap.md?plain=1#L71)
//! Hence it is expected that the primary protocol is "sil" and the additional protocols are
...
use crate::{
    ...
    CanDisconnect, Capability, DisconnectReason, P2PStream, SilStream, UnifiedStatus,
    HANDSHAKE_TIMEOUT,
};
...
/// A Stream and Sink type that acts as a wrapper around a primary `RLPx` subprotocol (e.g. "sil")
///
...
/// Adapter so the injected `SilRlpxHandshake` can run over a multiplexed `ProtocolProxy`
...
/// A Stream and Sink type that acts as a wrapper around a primary `RLPx` subprotocol (e.g. "sil")
/// [`SilStream`] and can also handle additional subprotocols.
#[derive(Debug)]
...
mod tests {
    ...
    async fn satellite_mux_stops_polling_protocols_when_out_buffer_is_full() {
        ...
        let mut st = RlpxProtocolMultiplexer::new(conn)
            .into_satellite_stream(sil.capability().as_ref(), |proxy| PendingPrimary {
                _proxy: proxy,
            })
            .unwrap();
    ...
    /// A test that install a satellite stream sil+test protocol and sends messages between them.
