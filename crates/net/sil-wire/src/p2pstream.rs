...
/// This is defined in [SIP-706](https://sips.sila.org/SIPS/sip-706).
...
/// [`ECIESStream`](rsil_ecies::stream::ECIESStream) which internally already buffers a few MB of
/// encoded data.
...
///
/// See also <https://github.com/sila-chain/devp2p/blob/master/rlpx.md#message-id-based-multiplexing>
///
        ...
        while let Poll::Ready(res) = this.inner.poll_next_unpin(cx) {
            ...
            match id {
                ...
                _ => {
                    ...
                    //
                    // For example, if the shared capabilities are `sil/67` (containing 10
                    // messages), and "qrs/65" (containing 8 messages):
                    //
                    //  * The special case of `p2p`: `p2p` is reserved message IDs 0x00 - 0x0f.
                    //  * `sil/67` is reserved message IDs 0x10 - 0x19.
...
mod tests {
    use super::*;
    use crate::{
        capability::SharedCapability, test_utils::eth_hello, Capability, SilVersion,
        ProtocolVersion,
    };
