...
use rsil_node_core::{
    cli::config::{PayloadBuilderConfig, RsilTransactionPoolConfig},
...
use rsil_transaction_pool::{PoolConfig, PoolTransaction, TransactionPool};
...
/// The adapter type for a rsil node with the builtin provider type
// Note: we need to hardcode this because custom components might depend on it in associated types.
pub type RsilFullAdapter<DB, Types> =
...
///
///  - The SAVM and Executor configuration: [`ExecutorBuilder`](crate::components::ExecutorBuilder)
...
/// the [`Node`] trait, see `rsil_node_sila::SilaNode`.
///
...
/// [`FullNodeComponents`](rsil_node_api::FullNodeComponents) trait was introduced. This
...
/// implements the [`FullNodeComponents`](rsil_node_api::FullNodeComponents) trait and has access to
...
/// ### Limitations
///
/// Currently the launch process is limited to sila nodes and requires all the components
/// specified above. It also expects beacon consensus with the sila engine API that is
...
impl<DB, ChainSpec: SilChainSpec> NodeBuilder<DB, ChainSpec> {
    ...
    pub fn testing_node(
        ...
        NodeBuilder<Arc<rsil_db::test_utils::TempDatabase<rsil_db::DatabaseEnv>>, ChainSpec>,
    > {
        let path = rsil_db::test_utils::tempdir_path();
    ...
    pub fn testing_node_with_datadir(
        ...
        NodeBuilder<Arc<rsil_db::test_utils::TempDatabase<rsil_db::DatabaseEnv>>, ChainSpec>,
    > {
        let path = rsil_node_core::dirs::MaybePlatformPath::<DataDirPath>::from(datadir.into());
        self.config = self.config.with_datadir_args(rsil_node_core::args::DatadirArgs {
            datadir: path.clone(),
            ..Default::default()
        });
        ...
        let db = rsil_db::test_utils::create_test_rw_db_with_datadir(data_dir.data_dir());
    ...
    ChainSpec: SilChainSpec + SilaHardforks,
{
    ...
    pub fn with_types<T>(self) -> NodeBuilderWithTypes<RsilFullAdapter<DB, T>>
    where
    ...
    pub fn node<N>(
        self,
        node: N,
    ) -> NodeBuilderWithComponents<RsilFullAdapter<DB, N>, N::ComponentsBuilder, N::AddOns>
    where
        N: Node<RsilFullAdapter<DB, N>, ChainSpec = ChainSpec> + NodeTypesForProvider,
    {
    ...
    ChainSpec: SilChainSpec + SilaHardforks,
{
    ...
    /// Configures the types of the node.
    pub fn with_types<T>(self) -> WithLaunchContext<NodeBuilderWithTypes<RsilFullAdapter<DB, T>>>
    where
        T: NodeTypesForProvider<ChainSpec = ChainSpec>,
    {
    ...
    where
        N: Node<RsilFullAdapter<DB, N>, ChainSpec = ChainSpec> + NodeTypesForProvider,
        N::AddOns: RsilRpcAddOns<
            NodeAdapter<
                RsilFullAdapter<DB, N>,
                <N::ComponentsBuilder as NodeComponentsBuilder<RsilFullAdapter<DB, N>>>::Components,
            >,
        >,
...
where
    T: FullNodeTypes,
    CB: NodeComponentsBuilder<T>,
    AO: RsilRpcAddOns<NodeAdapter<T, CB::Components>>,
{
    /// Returns a reference to the node builder's config.
    ...
    where
        F: FnOnce(
                RpcContext<'_, NodeAdapter<T, CB::Components>, AO::SilApi>,
                RsilRpcServerHandles,
            ) -> eyre::Result<()>
            + Send
            + 'static,
    {
    ...
    where
        F: FnOnce(RpcContext<'_, NodeAdapter<T, CB::Components>, AO::SilApi>) -> eyre::Result<()>
            + Send
            + 'static,
    {
...
impl<Node: FullNodeTypes> BuilderContext<Node> {
    ...
    pub const fn rsil_config(&self) -> &rsil_config::Config {
    ...
    {
        let (handle, network, txpool, sil) = builder
        ...
        self.executor.spawn_critical_blocking_task("p2p sil request handler", sil);
            ...
            |shutdown| {
                network.run_until_graceful_shutdown(shutdown, |network| {
                    if let Some(peers_file) = known_peers_file {
                        ...
                        trace!(target: "rsil::cli", peers_file=?peers_file, num_peers=%num_known_peers, "Saving current peers");
                        match network.write_peers_to_file(peers_file.as_path()) {
                            Ok(_) => {
                                info!(target: "rsil::cli", peers_file=?peers_file, "Wrote network peers to file");
                            }
                            Err(err) => {
                                warn!(target: "rsil::cli", %err, "Failed to write network peers to file");
                            }
                        }
                    }
                })
