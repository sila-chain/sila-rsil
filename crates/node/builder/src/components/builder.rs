...
where
    ...
    PoolB: PoolBuilder<Node, ExecB::SAVM>,
{
    ...
    where
        PB: PayloadServiceBuilder<Node, PoolB::Pool, ExecB::SAVM>,
    {
        let Self {
            pool_builder,
            payload_builder: _,
...
where
    Node: FullNodeTypes,
    PoolB: PoolBuilder<Node, ExecB::SAVM, Pool: TransactionPool>,
    NetworkB: NetworkBuilder<
        ...
        >,
    >,
    PayloadB: PayloadServiceBuilder<Node, PoolB::Pool, ExecB::SAVM>,
    ExecB: ExecutorBuilder<Node>,
    ConsB: ConsensusBuilder<Node>,
{
    type Components =
        Components<Node, NetworkB::Network, PoolB::Pool, ExecB::SAVM, ConsB::Consensus>;
...
impl<Node, Net, F, Fut, Pool, SAVM, Cons> NodeComponentsBuilder<Node> for F
where
    Net: FullNetwork<
        Primitives: NetPrimitivesFor<
            PrimitivesTy<Node::Types>,
    ...
    Fut: Future<Output = eyre::Result<Components<Node, Net, Pool, SAVM, Cons>>> + Send,
...
{
    type Components = Components<Node, Net, Pool, SAVM, Cons>;
...
#[derive(Debug, Clone)]
pub struct NoopTransactionPoolBuilder<Tx = SilPooledTransaction>(PhantomData<Tx>);

impl<N, Tx, Savm> PoolBuilder<N, Savm> for NoopTransactionPoolBuilder<Tx>
where
    N: FullNodeTypes,
    Tx: SilPoolTransaction<Consensus = TxTy<N::Types>> + Unpin,
    Savm: Send,
{
...
/// Builds [`NoopNetwork`].
#[derive(Debug, Clone)]
pub struct NoopNetworkBuilder<Net = SilNetworkPrimitives>(PhantomData<Net>);

impl NoopNetworkBuilder {
    /// Returns the instance with sila types.
    pub fn sil() -> Self {
        Self::default()
    }
}
...
impl<N, Pool, SAVM> PayloadServiceBuilder<N, Pool, SAVM> for NoopPayloadBuilder
where
    N: FullNodeTypes,
    Pool: TransactionPool,
    SAVM: ConfigureEvm<Primitives = PrimitivesTy<N::Types>> + 'static,
{
