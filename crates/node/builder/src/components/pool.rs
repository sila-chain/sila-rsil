...
use rsil_node_api::{BlockTy, NodeTypes, TxTy};
...
pub trait PoolBuilder<Node: FullNodeTypes, Savm>: Send {
...
impl<Node, F, Fut, Pool, Savm> PoolBuilder<Node, Savm> for F
where
    Node: FullNodeTypes,
    Pool: TransactionPool<Transaction: PoolTransaction<Consensus = TxTy<Node::Types>>>
        + Unpin
        + 'static,
    F: FnOnce(&BuilderContext<Node>, Savm) -> Fut + Send,
...
where
    Node: FullNodeTypes<Types: NodeTypes<ChainSpec: SilaHardforks>>,
    V: TransactionValidator<Block = BlockTy<Node::Types>> + 'static,
    V::Transaction:
        PoolTransaction<Consensus = TxTy<Node::Types>> + rsil_transaction_pool::SilPoolTransaction,
{
    /// Consume the type and build the [`rsil_transaction_pool::Pool`] with the given config and
    /// blob store.
    pub fn build<BS>(
        self,
        blob_store: BS,
        pool_config: PoolConfig,
    ) -> rsil_transaction_pool::Pool<
        ...
        BS: BlobStore,
    {
        ...
        rsil_transaction_pool::Pool::new(
            validator,
    ...
    pub fn build_with_ordering_and_spawn_maintenance_task<BS, O>(
        ...
        blob_store: BS,
        pool_config: PoolConfig,
    ) -> eyre::Result<rsil_transaction_pool::Pool<TransactionValidationTaskExecutor<V>, O, BS>>
    where
        BS: BlobStore + Clone,
        O: TransactionOrdering<Transaction = V::Transaction>,
    {
        ...
        let transaction_pool =
            rsil_transaction_pool::Pool::new(validator, ordering, blob_store, pool_config.clone());
...
pub fn create_blob_store_with_cache<Node: FullNodeTypes>(
    ...
    let config = if let Some(cache_size) = cache_size {
        rsil_transaction_pool::blobstore::DiskFileBlobStoreConfig::default()
            .with_max_cached_entries(cache_size)
    } else {
        Default::default()
    };

    Ok(rsil_transaction_pool::blobstore::DiskFileBlobStore::open(data_dir.blobstore(), config)?)
}
    ...
    Pool: TransactionPool + Clone + 'static,
{
    if !ctx.config().txpool.disable_transactions_backup {
        ...
        let transactions_backup_config =
            rsil_transaction_pool::maintain::LocalTransactionBackupConfig::with_local_txs_backup(
                transactions_path,
            );
        ...
        ctx.task_executor().spawn_critical_with_graceful_shutdown_signal(
            ...
            |shutdown| {
                rsil_transaction_pool::maintain::backup_local_transactions_task(
                    shutdown,
                    pool,
...
where
    Node: FullNodeTypes<Types: NodeTypes<ChainSpec: SilaHardforks>>,
    Pool: rsil_transaction_pool::TransactionPoolExt<Block = BlockTy<Node::Types>> + Clone + 'static,
    Pool::Transaction: PoolTransaction<Consensus = TxTy<Node::Types>>,
{
    ...
    ctx.task_executor().spawn_critical_task(
        ...
        rsil_transaction_pool::maintain::maintain_transaction_pool_future(
            client,
            pool,
            chain_events,
            ctx.task_executor().clone(),
            rsil_transaction_pool::maintain::MaintainPoolConfig {
...
mod tests {
    use super::*;
    use rsil_transaction_pool::PoolConfig;
