...
use alloy_eips::sip2124::Head;
use alloy_primitives::{BlockNumber, B256};
use eyre::Context;
use rayon::ThreadPoolBuilder;
use rsil_chainspec::{Chain, SilChainSpec, SilaHardforks};
use rsil_config::{config::EtlConfig, PruneConfig};
use rsil_consensus::noop::NoopConsensus;
use rsil_db_api::{database::Database, database_metrics::DatabaseMetrics};
use rsil_db_common::init::{
...
use rsil_node_api::{FullNodeTypes, NodeTypes, NodeTypesWithDB, NodeTypesWithDBAdapter};
use rsil_node_core::{
...
use rsil_prune::{PruneMode, PruneModes, PrunerBuilder};
...
use rsil_static_file::{blocks_per_file_for_prune_distance, StaticFileProducer, StaticFileSegment};
...
impl LaunchContext {
    ...
    /// Loads the rsil config with the configured `data_dir` and overrides settings according to the
    /// `config`.
    ///
    /// Attaches both the `NodeConfig` and the loaded `rsil.toml` config to the launch context.
    pub fn with_loaded_toml_config<ChainSpec>(
        self,
        config: NodeConfig<ChainSpec>,
    ) -> eyre::Result<LaunchContextWith<WithConfigs<ChainSpec>>>
    where
        ChainSpec: SilChainSpec + rsil_chainspec::SilaHardforks,
    {
    ...
    pub fn load_toml_config<ChainSpec>(
        &self,
        config: &NodeConfig<ChainSpec>,
    ) -> eyre::Result<rsil_config::Config>
    where
        ChainSpec: SilChainSpec + rsil_chainspec::SilaHardforks,
    {
        let config_path = config.config.clone().unwrap_or_else(|| self.data_dir.config());

        let mut toml_config = rsil_config::Config::from_path(&config_path)
            .wrap_err_with(|| format!("Could not load config file {config_path:?}"))?;

        Self::save_pruning_config(&mut toml_config, config, &config_path)?;

        info!(target: "rsil::cli", path = ?config_path, "Configuration loaded");
    ...
    fn save_pruning_config<ChainSpec>(
        rsil_config: &mut rsil_config::Config,
        config: &NodeConfig<ChainSpec>,
    ...
    where
        ChainSpec: SilChainSpec + rsil_chainspec::SilaHardforks,
    {
        let mut should_save = rsil_config.prune.segments.migrate();

        if let Some(prune_config) = config.prune_config() {
            if rsil_config.prune != prune_config {
                rsil_config.set_prune_config(prune_config);
                should_save = true;
            }
        } else if !rsil_config.prune.is_default() {
            info!(target: "rsil::cli", "Pruning configuration is present in the config file, but no CLI arguments are provided. Using config from file.");
        }

        if should_save {
            info!(target: "rsil::cli", "Saving prune config to toml file");
            rsil_config.save(config_path.as_ref())?;
        }
...
impl<ChainSpec> LaunchContextWith<WithConfigs<ChainSpec>> {
    ...
    pub fn with_resolved_peers(mut self) -> eyre::Result<Self> {
        if !self.attachment.config.network.trusted_peers.is_empty() {
            info!(target: "rsil::cli", "Adding trusted nodes");
...
impl<R, ChainSpec: SilChainSpec> LaunchContextWith<Attached<WithConfigs<ChainSpec>, R>> {
    ...
    /// Returns the attached toml config [`rsil_config::Config`].
    pub const fn toml_config(&self) -> &rsil_config::Config {
        &self.left().toml_config
    }

    /// Returns the attached toml config [`rsil_config::Config`].
    pub const fn toml_config_mut(&mut self) -> &mut rsil_config::Config {
    ...
    where
        ChainSpec: rsil_chainspec::SilaHardforks,
    {
    ...
    ChainSpec: SilChainSpec + SilaHardforks + 'static,
{
    ...
    /// check.**
    pub async fn create_provider_factory<N, Savm>(
        &self,
    ...
    where
        ...
        Savm: ConfigureEvm<Primitives = N::Primitives> + 'static,
    {
        ...
        if let Some(unwind_block) = unwind_target {
            ...
            info!(target: "rsil::cli", %unwind_target, %inconsistency_source, "Executing unwind after consistency check.");
            ...
            rx.await?.inspect_err(|err| {
                error!(target: "rsil::cli", %unwind_target, %inconsistency_source, %err, "failed to run unwind")
            })?;
        }
    ...
    pub async fn with_provider_factory<N, Savm>(
        self,
        changeset_cache: ChangesetCache,
        rocksdb_provider: Option<RocksDBProvider>,
        disabled_stages: &[StageId],
    ...
    {
        let factory = self
            .create_provider_factory::<N, Savm>(changeset_cache, rocksdb_provider, disabled_stages)
            .await?;
    ...
    T: ProviderNodeTypes,
{
    ...
    pub fn with_metrics_task(
        ...
        debug!(target: "rsil::cli", "Spawning stages metrics listener task");
        let sync_metrics_listener = rsil_stages::MetricsListener::new(metrics_receiver);
        self.task_executor()
    ...
    T: FullNodeTypes<Types: NodeTypesForProvider>,
{
        ...
        CB: NodeComponentsBuilder<T>,
    {
        ...
        debug!(target: "rsil::cli", "creating components");
        ...
        debug!(target: "rsil::cli", "calling on_component_initialized hook");
    ...
    CB: NodeComponentsBuilder<T>,
{
    ...
    fn ensure_chain_specific_db_checks(&self) -> ProviderResult<()> {
            ...
            self.chain_id() == Chain::optimism_mainnet()
        {
            ...
            if latest < 105235063 {
                error!(
                    "Op-sila-mainnet has been launched without importing the pre-Bedrock state. The chain can't progress without this. See also https://rsil.rs/run/sync-op-sila-mainnet.html?minimal-bootstrap-recommended"
                );
    ...
    ///  * stage data was dropped manually through `rsil stage drop ...`
    ///
    /// # Returns
    ///
    ...
    where
        T::Provider: rsil_provider::CanonChainTracker,
    {
...
pub struct WithConfigs<ChainSpec> {
    ...
    /// The loaded rsil.toml config.
    pub toml_config: rsil_config::Config,
}
...
mod tests {
    ...
    use rsil_node_core::args::PruningArgs;
    ...
    fn test_save_prune_config() {
        with_tempdir("prune-store-test", |config_path| {
            let mut rsil_config = Config::default();
            ...
            LaunchContext::save_pruning_config(&mut rsil_config, &node_config, config_path)
                .unwrap();

            let loaded_config = Config::from_path(config_path).unwrap();

            assert_eq!(rsil_config, loaded_config);
        })
    }
}
