...
use crate::{rpc::RsilRpcAddOns, EngineNodeLauncher, Node, NodeHandle};
...
///
/// - **SilaScan Integration**: Use SilaScan as a consensus client to follow the chain and submit
...
pub trait DebugNode<N: FullNodeComponents>: Node<N> {
    ...
    /// # Example
    ///
    /// For Sila nodes, this typically converts from `alloy_rpc_types_eth::Block`
...
///
/// ## SilaScan Consensus Client
///
/// When `--debug.silascan [URL]` is provided, the launcher will:
/// - Use SilaScan API as a consensus client
/// - Fetch recent blocks from SilaScan
/// - Submit them to the local engine
/// - Requires `ETHERSCAN_API_KEY` environment variable
/// - Falls back to default SilaScan URL for the chain if URL not provided
#[derive(Debug, Clone)]
...
/// Type alias for the default debug block provider. We use silascan provider to satisfy the
/// bounds.
pub type DefaultDebugBlockProvider<N> = SilaScanBlockProvider<
    ...
    B: PayloadProvider<ExecutionData = PayloadDataTy<N::Types>> + Clone,
{
    ...
    ///
    /// When set, this provider will be used instead of creating an `SilaScanBlockProvider`
    ...
    async fn launch_node(self) -> eyre::Result<NodeHandle<N, AddOns>> {
        ...
        if let Some(provider) = debug_block_provider {
            info!(target: "rsil::cli", "Using custom debug block provider");
            ...
            info!(target: "rsil::cli", "Using RPC consensus client: {}", url);
        ...
        } else if let Some(maybe_custom_silascan_url) = config.debug.silascan.clone() {
            info!(target: "rsil::cli", "Using silascan as consensus client");

            let chain = config.chain.chain();
            let silascan_url = maybe_custom_silascan_url.map(Ok).unwrap_or_else(|| {
                chain
                    .silascan_urls()
                    .map(|urls| urls.0.to_string())
                    .ok_or_else(|| eyre::eyre!("failed to get silascan url for chain: {chain}"))
            })?;

            let block_provider = SilaScanBlockProvider::new(
                silascan_url,
                chain.silascan_api_key().ok_or_else(|| {
                    eyre::eyre!(
                        "silascan api key not found for rpc consensus client for chain: {chain}"
                    )
                })?,
                chain.id(),
                |rpc_block| {
            ...
            handle
                .node
                .task_executor
                .spawn_critical_task("silascan consensus client", async move {
                    rpc_consensus_client.run().await
                });
        }

        if config.dev.dev {
            info!(target: "rsil::cli", "Using local payload attributes builder for dev mode");
