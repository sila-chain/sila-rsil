...
use rsil_db::{database_metrics::DatabaseMetrics, Database};
use rsil_engine_tree::{
    chain::{ChainEvent, FromOrchestrator},
    engine::{EngineApiKind, EngineApiRequest, EngineRequestHandler},
    launch::build_engine_orchestrator,
    tree::TreeConfig,
};
use rsil_engine_util::EngineMessageStreamExt;
use rsil_exex::ExExManagerHandle;
use rsil_network::{types::BlockRangeUpdate, NetworkSyncUpdater, SyncState};
use rsil_network_api::BlockDownloaderProvider;
use rsil_node_api::{
...
use rsil_tracing::tracing::{debug, error, info};
...
impl EngineNodeLauncher {
    /// Create a new instance of the sila node launcher.
    pub const fn new(
    ...
    where
        N: Node<RsilFullAdapter<DB, N>> + NodeTypesForProvider,
            ...
            + EngineValidatorAddOn<NodeAdapter<T, CB::Components>>,
    {
        ...
        let ctx = ctx
            ...
            .with_provider_factory::<_, <CB::Components as NodeComponents<T>>::Savm>(
            ...
            .await?
            .inspect(|_| {
                info!(target: "rsil::cli", "Database opened");
            })
            .with_prometheus_server().await?
            .inspect(|this| {
                debug!(target: "rsil::cli", chain=%this.chain_id(), genesis=?this.genesis_hash(), "Initializing genesis");
            })
            .with_genesis()?
            .inspect(|this: &LaunchContextWith<Attached<WithConfigs<<T::Types as NodeTypes>::ChainSpec>, _>>| {
                ...
                info!(target: "rsil::cli", ?settings, ?pruning_mode, "Loaded storage settings");
            })
            .with_metrics_task()
        ...
        info!(target: "rsil::cli", "StaticFileProducer initialized");
        ...
        info!(target: "rsil::cli", prune_config=?ctx.prune_config(), "Pruner initialized");
        ...
        info!(target: "rsil::cli", "Consensus engine initialized");
        ...
        info!(target: "rsil::cli", "Starting consensus engine");
        let consensus_engine = move |mut on_graceful_shutdown| async move {
            if let Some(initial_target) = initial_target {
                debug!(target: "rsil::cli", %initial_target,  "start backfill sync");
            ...
            loop {
                tokio::select! {
                    event = orchestrator.next() => {
                        ...
                        debug!(target: "rsil::cli", "Event: {event}");
                        match event {
                            ChainEvent::BackfillSyncFinished => {
                                if terminate_after_backfill {
                                    debug!(target: "rsil::cli", "Terminating after initial backfill");
                                    break
                                }
                            ...
                            ChainEvent::FatalError => {
                                error!(target: "rsil::cli", "Fatal error in consensus engine");
                    ...
                    payload = built_payloads.select_next_some(), if !built_payloads.is_terminated() => {
                        if let Some(executed_block) = payload.executed_block() {
                            debug!(target: "rsil::cli", block=?executed_block.recovered_block.num_hash(),  "inserting built payload");
                            orchestrator.handler_mut().handler_mut().on_event(EngineApiRequest::InsertExecutedBlock(executed_block).into());
                        }
                    }
                    shutdown_req = &mut shutdown_rx => {
                        if let Ok(req) = shutdown_req {
                            debug!(target: "rsil::cli", "received engine shutdown request");
                    ...
                    _guard = &mut on_graceful_shutdown => {
                        ...
                        debug!(target: "rsil::cli", "shutdown signal received, terminating engine");
