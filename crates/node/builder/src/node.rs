...
pub use rsil_node_api::{FullNodeTypes, NodeTypes};

use crate::{
    components::NodeComponentsBuilder, rpc::RsilRpcAddOns, NodeAdapter, NodeAddOns, NodeHandle,
    RsilFullAdapter,
};
use rsil_node_api::{EngineTypes, FullNodeComponents, PayloadTypes};
use rsil_node_core::{
    dirs::{ChainPath, DataDirPath},
    node_config::NodeConfig,
};
use rsil_payload_builder::PayloadBuilderHandle;
use rsil_provider::ChainSpecProvider;
use rsil_rpc_api::EngineApiClient;
use rsil_rpc_builder::{auth::AuthServerHandle, RpcServerHandle};
...
pub trait Node<N: FullNodeTypes>: NodeTypes + Clone {
    ...
    fn disabled_stages() -> &'static [rsil_stages::StageId] {
        &[]
    }
}
...
pub type FullNodeFor<N, DB = DatabaseEnv> =
    FullNode<NodeAdapter<RsilFullAdapter<DB, N>>, <N as Node<RsilFullAdapter<DB, N>>>::AddOns>;

/// Helper type alias to define [`NodeHandle`] for a given [`Node`].
pub type NodeHandleFor<N, DB = DatabaseEnv> =
    NodeHandle<NodeAdapter<RsilFullAdapter<DB, N>>, <N as Node<RsilFullAdapter<DB, N>>>::AddOns>;
