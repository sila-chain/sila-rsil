...
use rsil_engine_tree::tree::WaitForCaches;
pub use rsil_engine_tree::tree::{BasicEngineValidator, EngineValidator};
pub use rsil_rpc_builder::{
    middleware::{RsilAuthHttpMiddleware, RsilRpcMiddleware},
    Identity, Stack,
};
pub use rsil_trie_db::ChangesetCache;
...
use rsil_node_core::{
    cli::config::RsilTransactionPoolConfig,
...
#[derive(Debug, Clone)]
pub struct RsilRpcServerHandles {
...
pub struct RpcHooks<Node: FullNodeComponents, SilApi> {
    /// Hooks to run once RPC server is running.
    pub on_rpc_started: Box<dyn OnRpcStarted<Node, SilApi>>,
    /// Hooks to run to configure RPC server API.
    pub extend_rpc_modules: Box<dyn ExtendRpcModules<Node, SilApi>>,
}

impl<Node, SilApi> Default for RpcHooks<Node, SilApi>
where
...
impl<Node, SilApi> RpcHooks<Node, SilApi>
where
    Node: FullNodeComponents,
    SilApi: SilApiTypes,
{
    ...
    where
        F: OnRpcStarted<Node, SilApi> + 'static,
    {
    ...
    where
        F: ExtendRpcModules<Node, SilApi> + 'static,
    {
...
impl<Node, SilApi> fmt::Debug for RpcHooks<Node, SilApi>
where
    Node: FullNodeComponents,
    SilApi: SilApiTypes,
{
...
pub trait OnRpcStarted<Node: FullNodeComponents, SilApi: SilApiTypes>: Send {
    /// The hook that is called once the rpc server is started.
    fn on_rpc_started(
        self: Box<Self>,
        ctx: RpcContext<'_, Node, SilApi>,
        handles: RsilRpcServerHandles,
    ) -> eyre::Result<()>;
}

impl<Node, SilApi, F> OnRpcStarted<Node, SilApi> for F
where
    F: FnOnce(RpcContext<'_, Node, SilApi>, RsilRpcServerHandles) -> eyre::Result<()> + Send,
    Node: FullNodeComponents,
    SilApi: SilApiTypes,
{
    fn on_rpc_started(
...
impl<Node, SilApi> OnRpcStarted<Node, SilApi> for ()
where
    Node: FullNodeComponents,
    SilApi: SilApiTypes,
{
    fn on_rpc_started(
        self: Box<Self>,
        _: RpcContext<'_, Node, SilApi>,
        _: RsilRpcServerHandles,
    ) -> eyre::Result<()> {
        Ok(())
    }
}
...
pub trait ExtendRpcModules<Node: FullNodeComponents, SilApi: SilApiTypes>: Send {
    /// The hook that is called once the rpc server is started.
    fn extend_rpc_modules(self: Box<Self>, ctx: RpcContext<'_, Node, SilApi>) -> eyre::Result<()>;
}

impl<Node, SilApi, F> ExtendRpcModules<Node, SilApi> for F
where
    F: FnOnce(RpcContext<'_, Node, SilApi>) -> eyre::Result<()> + Send,
    Node: FullNodeComponents,
    SilApi: SilApiTypes,
{
    fn extend_rpc_modules(self: Box<Self>, ctx: RpcContext<'_, Node, SilApi>) -> eyre::Result<()> {
...
impl<Node, SilApi> ExtendRpcModules<Node, SilApi> for ()
where
    Node: FullNodeComponents,
    SilApi: SilApiTypes,
{
    fn extend_rpc_modules(self: Box<Self>, _: RpcContext<'_, Node, SilApi>) -> eyre::Result<()> {
        Ok(())
    }
}
...
pub struct RpcRegistry<Node: FullNodeComponents, SilApi: SilApiTypes> {
...
impl<Node, SilApi> Deref for RpcRegistry<Node, SilApi>
where
    Node: FullNodeComponents,
    SilApi: SilApiTypes,
{
...
impl<Node, SilApi> DerefMut for RpcRegistry<Node, SilApi>
where
    Node: FullNodeComponents,
    SilApi: SilApiTypes,
{
...
pub struct RpcModuleContainer<'a, Node: FullNodeComponents, SilApi: SilApiTypes> {
...
/// [`rsil_rpc::sil::SilApi`], and ultimately merge additional rpc handler into the configured
/// transport modules [`TransportRpcModules`] as well as configured authenticated methods
/// [`AuthRpcModule`].
#[expect(missing_debug_implementations)]
pub struct RpcContext<'a, Node: FullNodeComponents, SilApi: SilApiTypes> {
    /// The node components.
    pub(crate) node: Node,
...
impl<Node, SilApi> RpcContext<'_, Node, SilApi>
where
    Node: FullNodeComponents,
    SilApi: SilApiTypes,
{
...
pub struct RpcHandle<Node: FullNodeComponents, SilApi: SilApiTypes> {
    /// Handles to launched servers.
    pub rpc_server_handles: RsilRpcServerHandles,
    /// Configured RPC modules.
    pub rpc_registry: RpcRegistry<Node, SilApi>,
...
impl<Node: FullNodeComponents, SilApi: SilApiTypes> Clone for RpcHandle<Node, SilApi> {
    fn clone(&self) -> Self {
        Self {
...
impl<Node: FullNodeComponents, SilApi: SilApiTypes> Deref for RpcHandle<Node, SilApi> {
    type Target = RpcRegistry<Node, SilApi>;
...
impl<Node: FullNodeComponents, SilApi: SilApiTypes> Debug for RpcHandle<Node, SilApi>
where
    RpcRegistry<Node, SilApi>: Debug,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
impl<Node: FullNodeComponents, SilApi: SilApiTypes> RpcHandle<Node, SilApi> {
    /// Returns the RPC server handles.
    pub const fn rpc_server_handles(&self) -> &RsilRpcServerHandles {
    ...
    where
        <Node::Types as NodeTypes>::ChainSpec: SilaHardforks,
    {
...
/// RPC functionality.
#[derive(Debug, Clone)]
pub struct RpcServerOnlyHandle<Node: FullNodeComponents, SilApi: SilApiTypes> {
...
impl<Node: FullNodeComponents, SilApi: SilApiTypes> RpcServerOnlyHandle<Node, SilApi> {
...
#[derive(Debug, Clone)]
pub struct AuthServerOnlyHandle<Node: FullNodeComponents, SilApi: SilApiTypes> {
...
impl<Node: FullNodeComponents, SilApi: SilApiTypes> AuthServerOnlyHandle<Node, SilApi> {
    /// Returns the consensus engine handle.
    ///
    /// This handle can be used to interact with the engine service directly.
    pub const fn consensus_engine_handle(
...
struct RpcSetupContext<'a, Node: FullNodeComponents, SilApi: SilApiTypes> {
    node: Node,
    config: &'a NodeConfig<<Node::Types as NodeTypes>::ChainSpec>,
    modules: TransportRpcModules,
    auth_module: AuthRpcModule,
    auth_config: rsil_rpc_builder::auth::AuthServerConfig,
    registry: RpcRegistry<Node, SilApi>,
    on_rpc_started: Box<dyn OnRpcStarted<Node, SilApi>>,
...
/// Node add-ons containing RPC server configuration, with customizable sil API handler.
///
...
/// modified as part of the [`NodeAddOns`] see for example `OpRpcAddons`, `SilaAddOns`.
///
...
impl<Node, SilB, PVB, EB, EVB, RpcMiddleware, AuthHttpMiddleware> Debug
    for RpcAddOns<Node, SilB, PVB, EB, EVB, RpcMiddleware, AuthHttpMiddleware>
where
    Node: FullNodeComponents,
    SilB: SilApiBuilder<Node>,
    PVB: Debug,
...
impl<Node, SilB, PVB, EB, EVB, RpcMiddleware, AuthHttpMiddleware>
    RpcAddOns<Node, SilB, PVB, EB, EVB, RpcMiddleware, AuthHttpMiddleware>
where
    Node: FullNodeComponents,
    SilB: SilApiBuilder<Node>,
{
    ...
    ///
    /// ```ignore
    /// use rsil_rpc_builder::{RpcServiceBuilder, RpcRequestMetrics};
    /// use tower::Layer;
    ///
    ...
    where
        F: FnOnce(RpcContext<'_, Node, SilB::SilApi>, RsilRpcServerHandles) -> eyre::Result<()>
            + Send
            + 'static,
    {
    ...
    where
        F: FnOnce(RpcContext<'_, Node, SilB::SilApi>) -> eyre::Result<()> + Send + 'static,
    {
...
impl<Node, SilB, EV, EB, Engine> Default
    for RpcAddOns<Node, SilB, EV, EB, Engine, Identity, Identity>
where
    Node: FullNodeComponents,
    SilB: SilApiBuilder<Node>,
    EV: Default,
...
impl<N, SilB, PVB, EB, EVB, RpcMiddleware, AuthHttpMiddleware>
    RpcAddOns<N, SilB, PVB, EB, EVB, RpcMiddleware, AuthHttpMiddleware>
where
    N: FullNodeComponents,
    N::Provider: ChainSpecProvider<ChainSpec: SilaHardforks>,
    ...
    AuthHttpMiddleware: RsilAuthHttpMiddleware<Identity>,
{
    ...
    pub async fn launch_rpc_server<F>(
        ...
        ext: F,
    ) -> eyre::Result<RpcServerOnlyHandle<N, SilB::SilApi>>
    where
        F: FnOnce(RpcModuleContainer<'_, N, SilB::SilApi>) -> eyre::Result<()>,
    {
        ...
        let handles =
            RsilRpcServerHandles { rpc: rpc_server_handle.clone(), auth: AuthServerHandle::noop() };
    ...
    pub async fn launch_add_ons_with<F>(
        ...
        ctx: AddOnsContext<'_, N>,
        ext: F,
    ) -> eyre::Result<RpcHandle<N, SilB::SilApi>>
    where
        F: FnOnce(RpcModuleContainer<'_, N, SilB::SilApi>) -> eyre::Result<()>,
    {
        ...
        F: FnOnce(RpcModuleContainer<'_, N, SilB::SilApi>) -> eyre::Result<()>,
    {
        ...
        let handles = RsilRpcServerHandles { rpc, auth };
    ...
    async fn setup_rpc_components<'a, F>(
        ...
        ext: F,
    ) -> eyre::Result<RpcSetupContext<'a, N, SilB::SilApi>>
    where
        F: FnOnce(RpcModuleContainer<'_, N, SilB::SilApi>) -> eyre::Result<()>,
    {
        ...
        info!(target: "rsil::cli", "Engine API handler initialized");

        let cache = SilStateCache::spawn_with(
            node.provider().clone(),
        ...
        debug!(target: "rsil::cli", http=?module_config.http(), ws=?module_config.ws(), "Using RPC module config");
        ...
        M: RsilRpcMiddleware,
    {
        ...
        if let Some(path) = handle.ipc_endpoint() {
            info!(target: "rsil::cli", %path, "RPC IPC server started");
        }
        if let Some(addr) = handle.http_local_addr() {
            info!(target: "rsil::cli", url=%addr, "RPC HTTP server started");
        }
        if let Some(addr) = handle.ws_local_addr() {
            info!(target: "rsil::cli", url=%addr, "RPC WS server started");
        }
    ...
    async fn launch_auth_server_internal(
        start_fut: impl Future<Output = Result<AuthServerHandle, rsil_rpc_builder::error::RpcError>>,
        ...
        start_fut
            ...
            .inspect(|handle| {
                ...
                if let Some(ipc_endpoint) = handle.ipc_endpoint() {
                    info!(target: "rsil::cli", url=%addr, ipc_endpoint=%ipc_endpoint, "RPC auth server started");
                } else {
                    info!(target: "rsil::cli", url=%addr, "RPC auth server started");
                }
            })
    }
    ...
    fn finalize_rpc_setup(
        registry: &mut RpcRegistry<N, SilB::SilApi>,
        modules: &mut TransportRpcModules,
        auth_module: &mut AuthRpcModule,
        node: &N,
        config: &NodeConfig<<N::Types as NodeTypes>::ChainSpec>,
        on_rpc_started: Box<dyn OnRpcStarted<N, SilB::SilApi>>,
        handles: RsilRpcServerHandles,
    ) -> eyre::Result<()> {
...
impl<N, SilB, PVB, EB, EVB, RpcMiddleware, AuthHttpMiddleware> NodeAddOns<N>
    for RpcAddOns<N, SilB, PVB, EB, EVB, RpcMiddleware, AuthHttpMiddleware>
where
    N: FullNodeComponents,
    <N as FullNodeTypes>::Provider: ChainSpecProvider<ChainSpec: SilaHardforks>,
    SilB: SilApiBuilder<N>,
...
/// implementations.
pub trait RsilRpcAddOns<N: FullNodeComponents>:
    NodeAddOns<N, Handle = RpcHandle<N, Self::SilApi>>
{
    /// sil API implementation.
    type SilApi: SilApiTypes;

    /// Returns a mutable reference to RPC hooks.
    fn hooks_mut(&mut self) -> &mut RpcHooks<N, Self::SilApi>;
}

impl<N: FullNodeComponents, SilB, EV, EB, Engine, RpcMiddleware, AuthHttpMiddleware>
    RsilRpcAddOns<N> for RpcAddOns<N, SilB, EV, EB, Engine, RpcMiddleware, AuthHttpMiddleware>
where
    Self: NodeAddOns<N, Handle = RpcHandle<N, SilB::SilApi>>,
    SilB: SilApiBuilder<N>,
{
...
/// `SilApiCtx` struct
/// This struct is used to pass the necessary context to the `SilApiBuilder` to build the `SilApi`.
#[derive(Debug)]
pub struct SilApiCtx<'a, N: FullNodeTypes> {
    /// Reference to the node components
    pub components: &'a N,
    /// Sil API configuration
    pub config: SilConfig,
    /// Cache for sil state
    pub cache: SilStateCache<PrimitivesTy<N::Types>>,
    /// Handle to the beacon consensus engine
    pub engine_handle: ConsensusEngineHandle<<N::Types as NodeTypes>::Payload>,
}

impl<'a, N: FullNodeComponents<Types: NodeTypes<ChainSpec: Hardforks + SilaHardforks>>>
    SilApiCtx<'a, N>
{
...
where
    Node: FullNodeComponents<
        Savm: ConfigureEngineEvm<
            <<Node::Types as NodeTypes>::Payload as PayloadTypes>::ExecutionData,
        >,
    >,
    EV: PayloadValidatorBuilder<Node>,
    EV::Validator: rsil_engine_primitives::PayloadValidator<
            <Node::Types as NodeTypes>::Payload,
            Block = BlockTy<Node::Types>,
        > + Clone,
{
    type EngineValidator = BasicEngineValidator<Node::Provider, Node::Savm, EV::Validator>;
...
///
/// This provides a basic default implementation for opstack and sila engine API via
...
/// engine API module
///
/// This is intended to be used as a workaround for reusing all the existing sila node launch
...
/// satisfy trait bounds in the regular sila launch routine that mandate an engine API instance.
#[derive(Debug, Clone, Default)]
#[non_exhaustive]
