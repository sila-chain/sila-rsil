...
use rsil_storage_errors::db::LogLevel;
...
impl DatabaseArgs {
    ...
    pub fn database_args(&self) -> rsil_db::mdbx::DatabaseArguments {
    ...
    pub fn get_database_args(
    ...
    ) -> rsil_db::mdbx::DatabaseArguments {
        ...
        rsil_db::mdbx::DatabaseArguments::new(client_version)
...
mod tests {
    use super::*;
    use clap::Parser;
    use rsil_db::mdbx::{GIGABYTE, KILOBYTE, MEGABYTE, TERABYTE};
    ...
    fn test_default_database_args() {
        ...
        let args = CommandParser::<DatabaseArgs>::parse_from(["rsil"]).args;
    ...
    fn test_command_parser_disable_metrics() {
        ...
        let args = CommandParser::<DatabaseArgs>::parse_from(["rsil", "--db.disable-metrics"]).args;
    ...
    fn test_command_parser_with_invalid_max_size() {
        let result =
            CommandParser::<DatabaseArgs>::try_parse_from(["rsil", "--db.max-size", "invalid"]);
    ...
    fn test_command_parser_with_invalid_growth_step() {
        let result =
            CommandParser::<DatabaseArgs>::try_parse_from(["rsil", "--db.growth-step", "invalid"]);
        assert!(result.is_err());
    }
    ...
    fn test_command_parser_max_size_and_growth_step_from_str_invalid_unit() {
        let result =
            CommandParser::<DatabaseArgs>::try_parse_from(["rsil", "--db.growth-step", "1 PB"]);
        assert!(result.is_err());

        let result =
            CommandParser::<DatabaseArgs>::try_parse_from(["rsil", "--db.max-size", "2PB"]);
        assert!(result.is_err());
    }

    #[test]
    fn test_command_parser_with_valid_page_size_from_str() {
        let cmd = CommandParser::<DatabaseArgs>::try_parse_from(["rsil", "--db.page-size", "8KB"])
            .unwrap();
        assert_eq!(cmd.args.page_size, Some(KILOBYTE * 8));

        let cmd = CommandParser::<DatabaseArgs>::try_parse_from(["rsil", "--db.page-size", "1MB"])
            .unwrap();
        assert_eq!(cmd.args.page_size, Some(MEGABYTE));

        // Test with spaces
        let cmd =
            CommandParser::<DatabaseArgs>::try_parse_from(["rsil", "--db.page-size", "16 KB"])
                .unwrap();
        assert_eq!(cmd.args.page_size, Some(KILOBYTE * 16));

        // Test with just a number (bytes)
        let cmd = CommandParser::<DatabaseArgs>::try_parse_from(["rsil", "--db.page-size", "4096"])
            .unwrap();
    ...
    fn test_command_parser_with_invalid_page_size() {
        // Invalid text
        let result =
            CommandParser::<DatabaseArgs>::try_parse_from(["rsil", "--db.page-size", "invalid"]);
        assert!(result.is_err());

        // Invalid unit
        let result =
            CommandParser::<DatabaseArgs>::try_parse_from(["rsil", "--db.page-size", "7 ZB"]);
        assert!(result.is_err());
    }
    ...
    fn test_command_parser_with_valid_log_level() {
        let cmd =
            CommandParser::<DatabaseArgs>::try_parse_from(["rsil", "--db.log-level", "Debug"])
                .unwrap();
    ...
    fn test_command_parser_with_invalid_log_level() {
        let result =
            CommandParser::<DatabaseArgs>::try_parse_from(["rsil", "--db.log-level", "invalid"]);
        assert!(result.is_err());
    }

    #[test]
    fn test_command_parser_without_log_level() {
        let cmd = CommandParser::<DatabaseArgs>::try_parse_from(["rsil"]).unwrap();
    ...
    fn test_command_parser_with_valid_sync_mode_durable() {
        let cmd =
            CommandParser::<DatabaseArgs>::try_parse_from(["rsil", "--db.sync-mode", "durable"])
                .unwrap();
    ...
    fn test_command_parser_with_invalid_sync_mode() {
        let result =
            CommandParser::<DatabaseArgs>::try_parse_from(["rsil", "--db.sync-mode", "ultra-fast"]);
        assert!(result.is_err());
    }
