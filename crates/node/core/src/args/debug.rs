...
pub struct DebugArgs {
    ...
    /// on SilaScan. If specified, requires an `ETHERSCAN_API_KEY` environment variable.
    #[arg(
        long = "debug.silascan",
        help_heading = "Debug",
        conflicts_with = "tip",
    ...
    pub silascan: Option<Option<String>>,
    ...
    /// importers, state-actor) and want rsil to trust the DB-resident
...
///
/// ```
/// use rsil_node_core::args::{InvalidBlockHookType, InvalidBlockSelection};
...
mod tests {
    ...
    fn test_parse_default_debug_args() {
        ...
        let args = CommandParser::<DebugArgs>::parse_from(["rsil"]).args;
    ...
    fn test_parse_skip_state_root() {
        ...
        let args = CommandParser::<DebugArgs>::parse_from(["rsil", "--debug.skip-state-root"]).args;
    ...
    fn test_parse_invalid_block_args_none() {
        ...
        let args =
            CommandParser::<DebugArgs>::parse_from(["rsil", "--debug.invalid-block-hook", ""]).args;
        assert_eq!(args, expected_args);
    }
