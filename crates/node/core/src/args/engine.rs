...
pub struct EngineArgs {
    ...
    /// Configure the number of reserved CPU cores for non-rsil processes
    ...
    /// Disable BAL (Block Access List, SIP-7928) based parallel execution.
...
impl EngineArgs {
    ...
    pub fn tree_config(&self) -> TreeConfig {
        ...
        if self.legacy_state_root_task_enabled {
            tracing::warn!(target: "rsil::cli", "--engine.legacy-state-root has no effect anymore, use --engine.state-root-fallback to force synchronous state root computation");
        }
...
mod tests {
    ...
    fn test_parse_engine_args() {
        ...
        let args = CommandParser::<EngineArgs>::parse_from(["rsil"]).args;
    ...
    fn test_parse_slow_block_threshold() {
        ...
        let args =
            CommandParser::<EngineArgs>::parse_from(["rsil", "--engine.slow-block-threshold", "0"])
                .args;
