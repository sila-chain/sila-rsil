...
use rsil_cli_util::{get_secret_key, load_secret_key::SecretKeyError};
use rsil_config::Config;
use rsil_discv4::{NodeRecord, DEFAULT_DISCOVERY_ADDR, DEFAULT_DISCOVERY_PORT};
use rsil_discv5::{
    discv5::ListenConfig, DEFAULT_COUNT_BOOTSTRAP_LOOKUPS, DEFAULT_DISCOVERY_V5_PORT,
    DEFAULT_SECONDS_BOOTSTRAP_LOOKUP_INTERVAL, DEFAULT_SECONDS_LOOKUP_INTERVAL,
};
use rsil_net_banlist::IpFilter;
use rsil_net_nat::{NatResolver, DEFAULT_NET_IF_NAME};
use rsil_network::{
    transactions::{
...
use rsil_network_peers::{mainnet_nodes, TrustedPeer};
...
pub struct NetworkArgs {
    ...
    /// Enforce SIP-868 ENR fork ID validation for discovered peers.
    ///
    /// When enabled, peers discovered without a confirmed fork ID are not added to the peer set
    /// until their fork ID is verified via SIP-868 ENR request. This filters out peers from other
...
impl NetworkArgs {
    ...
    pub fn resolved_addr(&self) -> IpAddr {
        if let Some(ref if_name) = self.net_if {
            ...
            return match rsil_net_nat::net_if::resolve_net_if_ip(if_name) {
                Ok(addr) => addr,
                Err(err) => {
                    error!(target: "rsil::cli",
                        if_name,
                        %err,
    ...
    /// Build a [`NetworkConfigBuilder`] from a [`Config`] and a [`SilChainSpec`], in addition to
    ...
    /// 3. default to sila-mainnet nodes
    pub fn network_config<N: NetworkPrimitives>(
        &self,
        config: &Config,
        chain_spec: impl SilChainSpec,
...
impl DiscoveryArgs {
        ...
        N: NetworkPrimitives,
    {
        ...
        if self.disable_nat {
            // we only check for `disable-nat` here and not for disable discovery because nat:extip can be used without discovery: <https://github.com/sila-chain/sila-rsil/issues/14878>
    ...
    /// Creates a [`rsil_discv5::ConfigBuilder`] filling it with the values from this struct.
    pub fn discovery_v5_builder(
        &self,
        rlpx_tcp_socket: SocketAddr,
        boot_nodes: impl IntoIterator<Item = NodeRecord>,
    ) -> rsil_discv5::ConfigBuilder {
        let Self {
            discv5_addr,
            discv5_addr_ipv6,
            discv5_port,
        ...
        let mut discv5_config_builder =
            rsil_discv5::discv5::ConfigBuilder::new(ListenConfig::from_two_sockets(
        ...
        rsil_discv5::Config::builder(rlpx_tcp_socket)
...
mod tests {
    ...
    #[test]
    fn parse_nat_args() {
        let args = CommandParser::<NetworkArgs>::parse_from(["rsil", "--nat", "none"]).args;
        assert_eq!(args.nat, NatResolver::None);

        let args =
            CommandParser::<NetworkArgs>::parse_from(["rsil", "--nat", "extip:0.0.0.0"]).args;
    ...
    fn parse_peer_args() {
        let args =
            CommandParser::<NetworkArgs>::parse_from(["rsil", "--max-outbound-peers", "50"]).args;
    ...
    fn parse_disable_tx_gossip_args() {
        let args = CommandParser::<NetworkArgs>::parse_from(["rsil", "--disable-tx-gossip"]).args;
        assert!(args.disable_tx_gossip);
    }

    #[test]
    fn parse_max_peers_flag() {
        let args = CommandParser::<NetworkArgs>::parse_from(["rsil", "--max-peers", "90"]).args;
    ...
    fn max_peers_small_values() {
        let args1 = CommandParser::<NetworkArgs>::parse_from(["rsil", "--max-peers", "1"]).args;
        assert_eq!(args1.resolved_max_outbound_peers(), Some(1));
        assert_eq!(args1.resolved_max_inbound_peers(), Some(0));

        let args2 = CommandParser::<NetworkArgs>::parse_from(["rsil", "--max-peers", "2"]).args;
        assert_eq!(args2.resolved_max_outbound_peers(), Some(1));
        assert_eq!(args2.resolved_max_inbound_peers(), Some(1));

        let args3 = CommandParser::<NetworkArgs>::parse_from(["rsil", "--max-peers", "3"]).args;
    ...
    fn resolved_peers_with_defaults() {
        let args = CommandParser::<NetworkArgs>::parse_from(["rsil"]).args;
    ...
    fn net_if_uses_resolved_addr_for_default_discovery_addr() {
        let args =
            CommandParser::<NetworkArgs>::parse_from(["rsil", "--net-if.experimental", "en0"]).args;
    ...
    fn parse_p2p_secret_key_hex() {
        ...
        let args =
            CommandParser::<NetworkArgs>::parse_from(["rsil", "--p2p-secret-key-hex", hex]).args;
    ...
    fn parse_netrestrict_single_network() {
        let args =
            CommandParser::<NetworkArgs>::parse_from(["rsil", "--netrestrict", "192.168.0.0/16"])
                .args;
    ...
    fn parse_netrestrict_ipv6() {
        let args =
            CommandParser::<NetworkArgs>::parse_from(["rsil", "--netrestrict", "2001:db8::/32"])
                .args;
    ...
    fn netrestrict_invalid_cidr() {
        let args =
            CommandParser::<NetworkArgs>::parse_from(["rsil", "--netrestrict", "invalid-cidr"])
                .args;
    ...
    fn network_config_preserves_basic_nodes_from_peers_file() {
        ...
        let peers_file = std::env::temp_dir().join(format!("rsil_peers_test_{}.json", unique));
        ...
        let builder = args.network_config::<rsil_network::SilNetworkPrimitives>(
            &Config::default(),
            SILA_MAINNET.clone(),
            secret_key,
            peers_file.clone(),
        ...
        let net_cfg = builder.build_with_noop_provider(SILA_MAINNET.clone());
