...
pub struct DefaultPruningValues {
    ...
    /// (SilaParis hardfork). If `false`, uses `full_prune_modes.bodies_history` directly.
...
impl DefaultPruningValues {
    ...
    ///
    /// When `true` (default), bodies are pruned before the SilaParis hardfork block.
...
mod tests {
    ...
    fn pruning_config_kind_classifies_presets() {
        let chain_spec = SILA_MAINNET.as_ref();
    ...
    fn parse_receiptslogfilter() {
        ...
        let args = CommandParser::<PruningArgs>::parse_from(["rsil"]).args;
