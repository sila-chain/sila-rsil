...
impl DefaultRpcServerArgs {
    ...
    /// Set the default SAVM memory limit
...
pub struct RpcServerArgs {
    ...
    /// `<DIR>/<CHAIN_ID>/jwt.hex`. For sila-mainnet this would be `~/.local/share/rsil/sila-mainnet/jwt.hex`
    /// by default.
    ...
    /// require SAVM execution. These are spawned as blocking tasks to avoid blocking the async
    /// runtime.
    ...
    /// Maximum memory the SAVM can allocate per RPC request.
    #[arg(
        long = "rpc.savm-memory-limit",
        alias = "rpc-savm-memory-limit",
    ...
    /// Maximum sil transaction fee (in sila) that can be sent via the RPC APIs (0 = no cap)
    #[arg(
        long = "rpc.txfeecap",
        alias = "rpc-txfeecap",
    ...
    #[arg(
        long = "rpc.compute-state-root-for-sil-simulate",
        env = "RSIL_RPC_COMPUTE_STATE_ROOT_FOR_ETH_SIMULATE",
    ...
    #[arg(long = "builder.disallow", value_name = "PATH", value_parser = rsil_cli_util::parsers::read_json_from_file::<AddressSet>, default_value = Resettable::from(DefaultRpcServerArgs::get_global().builder_disallow.as_ref().map(|v| format!("{:?}", v).into())))]
    ...
    /// Force upcasting SIP-4844 blob sidecars to SIP-7594 format when SilaOsaka is active.
    ///
    /// When enabled, blob transactions submitted via `eth_sendRawTransaction` with SIP-4844
    /// sidecars will be automatically converted to SIP-7594 format if the next block is SilaOsaka.
...
impl RpcServerArgs {
    ...
    /// * The `ipcpath` is appended with the instance number: `/tmp/rsil.ipc-<instance>`
    ///
    /// # Panics
    ...
    pub fn is_namespace_enabled(&self, ns: RsilRpcModule) -> bool {
    ...
    /// Enables forced blob sidecar upcasting from SIP-4844 to SIP-7594 format.
...
impl TypedValueParser for RpcModuleSelectionValueParser {
    ...
    fn possible_values(&self) -> Option<Box<dyn Iterator<Item = PossibleValue> + '_>> {
        ...
        let values = RsilRpcModule::standard_variant_names().map(PossibleValue::new);
        Some(Box::new(values))
    }
}
...
mod tests {
    ...
    fn test_rpc_server_args_parser() {
        let args =
            CommandParser::<RpcServerArgs>::parse_from(["rsil", "--http.api", "sil,admin,debug"])
                .args;

        let apis = args.http_api.unwrap();
        let expected = RpcModuleSelection::try_from_selection(["sil", "admin", "debug"]).unwrap();
    ...
    fn test_rpc_server_args_parser_none() {
        let args = CommandParser::<RpcServerArgs>::parse_from(["rsil", "--http.api", "none"]).args;
        let apis = args.http_api.unwrap();
    ...
    fn rpc_server_args_default_sanity_test() {
        ...
        let args = CommandParser::<RpcServerArgs>::parse_from(["rsil"]).args;
    ...
    fn test_rpc_disable_metrics_arg() {
        ...
        let args =
            CommandParser::<RpcServerArgs>::parse_from(["rsil", "--rpc.disable-metrics"]).args;
        assert!(args.rpc_disable_metrics);
    }

    #[test]
    fn test_rpc_tx_fee_cap_parse_integer() {
        let args = CommandParser::<RpcServerArgs>::parse_from(["rsil", "--rpc.txfeecap", "2"]).args;
        let expected = 2_000_000_000_000_000_000u128; // 2 SIL in wei
    ...
    fn test_rpc_tx_fee_cap_parse_decimal() {
        let args =
            CommandParser::<RpcServerArgs>::parse_from(["rsil", "--rpc.txfeecap", "1.5"]).args;
        let expected = 1_500_000_000_000_000_000u128; // 1.5 SIL in wei
        assert_eq!(args.rpc_tx_fee_cap, expected);
    }

    #[test]
    fn test_rpc_tx_fee_cap_parse_zero() {
        let args = CommandParser::<RpcServerArgs>::parse_from(["rsil", "--rpc.txfeecap", "0"]).args;
    ...
    fn test_rpc_tx_fee_cap_parse_none() {
        ...
        assert_eq!(args.rpc_tx_fee_cap, expected); // 1 SIL default cap
    }
    ...
    fn test_rpc_server_args() {
        let args = RpcServerArgs {
            ...
            http_port: 8545,
            http_disable_compression: false,
            http_api: Some(RpcModuleSelection::try_from_selection(["sil", "admin"]).unwrap()),
            http_corsdomain: Some("*".to_string()),
            ws: true,
            ws_addr: "127.0.0.1".parse().unwrap(),
            ws_port: 8546,
            ws_allowed_origins: Some("*".to_string()),
            ws_api: Some(RpcModuleSelection::try_from_selection(["sil", "admin"]).unwrap()),
            ipcdisable: false,
            ipcpath: "rsil.ipc".to_string(),
        ...
        let parsed_args = CommandParser::<RpcServerArgs>::parse_from([
            ...
            "--rpc.gascap",
            "50000000",
            "--rpc.savm-memory-limit",
            "256",
            "--rpc.txfeecap",
            "2.0",
            "--rpc.max-simulate-blocks",
            "256",
            "--rpc.sil-proof-window",
            "100000",
            "--rpc.proof-permits",
            "16",
            "--rpc.pending-block",
