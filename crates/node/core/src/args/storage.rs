...
mod tests {
    ...
    fn test_default_storage_args() {
        let args = CommandParser::<StorageArgs>::parse_from(["rsil"]).args;
        assert!(args.v2);
    }

    #[test]
    fn test_storage_v2_implicit_true() {
        let args = CommandParser::<StorageArgs>::parse_from(["rsil", "--storage.v2"]).args;
        assert!(args.v2);
    }

    #[test]
    fn test_storage_v2_explicit_true() {
        let args = CommandParser::<StorageArgs>::parse_from(["rsil", "--storage.v2=true"]).args;
        assert!(args.v2);
    }

    #[test]
    fn test_storage_v2_explicit_false() {
        let args = CommandParser::<StorageArgs>::parse_from(["rsil", "--storage.v2=false"]).args;
        assert!(!args.v2);
    }
}
