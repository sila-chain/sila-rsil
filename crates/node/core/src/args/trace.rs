...
use eyre::WrapErr;
use rsil_tracing::{tracing_subscriber::EnvFilter, Layers};
use rsil_tracing_otlp::OtlpProtocol;
...
///
/// Downstream binaries that embed rsil can call
...
pub struct TraceArgs {
    ...
    ///
    /// Example: --tracing-otlp.filter=info,rsil=debug,hyper_util=off
    ///
    ...
    ///
    /// Example: --logs-otlp.filter=info,rsil=debug
    ///
    ...
    /// like Jaeger or Zipkin. Useful for differentiating between multiple rsil instances.
    ///
    /// Set via `OTEL_SERVICE_NAME` environment variable. Defaults to "rsil" if not specified.
    #[arg(
...
impl TraceArgs {
    ...
    pub async fn init_otlp_tracing(
        ...
        if let Some(endpoint) = self.otlp.as_mut() {
            ...
            {
                {
                    let mut config = rsil_tracing_otlp::OtlpConfig::new(
    ...
    pub async fn init_otlp_logs(&mut self, _layers: &mut Layers) -> eyre::Result<OtlpLogsStatus> {
        if let Some(endpoint) = self.logs_otlp.as_mut() {
            ...
            {
                let mut config = rsil_tracing_otlp::OtlpLogsConfig::new(
                    self.service_name.clone(),
                    endpoint.clone(),
                    self.protocol,
                )?;
