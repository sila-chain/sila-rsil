...
mod tests {
    ...
    #[test]
    fn parse_max_value() {
        let cmd: NodeCommand = NodeCommand::try_parse_from(["rsil", "--max-value", "max"]).unwrap();
        assert_eq!(cmd.max_value, u64::MAX);

        let cmd: NodeCommand = NodeCommand::try_parse_from(["rsil", "--max-value", "42"]).unwrap();
        assert_eq!(cmd.max_value, 42);

        let result = NodeCommand::try_parse_from(["rsil", "--max-value", "invalid"]);
