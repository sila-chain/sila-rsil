...
/// 60M gas limit
const SILA_BLOCK_GAS_LIMIT_60M: u64 = 60_000_000;
...
pub trait PayloadBuilderConfig {
    ...
    /// Maximum number of blobs to include per block (SIP-7872).
    ///
...
pub trait RsilNetworkConfig {
    ...
    /// the `sil` protocol.
    ///
    /// See also [`ProtocolHandler`](rsil_network::protocol::ProtocolHandler)
...
impl<N: NetworkPrimitives> RsilNetworkConfig for rsil_network::NetworkManager<N> {
    fn add_rlpx_sub_protocol(&mut self, protocol: impl IntoRlpxSubProtocol) {
...
pub trait RsilTransactionPoolConfig {
