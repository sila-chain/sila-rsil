//! rsil data directories.

use crate::{args::DatadirArgs, utils::parse_path};
use rsil_chainspec::Chain;
use std::{
    convert::Infallible,
...
/// Returns the path to the rsil data directory.
///
/// Refer to [`dirs_next::data_dir`] for cross-platform behavior.
pub fn data_dir() -> Option<PathBuf> {
    dirs_next::data_dir().map(|root| root.join("rsil"))
}

/// Returns the path to the rsil database.
///
/// Refer to [`dirs_next::data_dir`] for cross-platform behavior.
...
/// Returns the path to the rsil configuration directory.
///
/// Refer to [`dirs_next::config_dir`] for cross-platform behavior.
pub fn config_dir() -> Option<PathBuf> {
    dirs_next::config_dir().map(|root| root.join("rsil"))
}

/// Returns the path to the rsil cache directory.
///
/// Refer to [`dirs_next::cache_dir`] for cross-platform behavior.
pub fn cache_dir() -> Option<PathBuf> {
    dirs_next::cache_dir().map(|root| root.join("rsil"))
}

/// Returns the path to the rsil logs directory.
///
/// Refer to [`dirs_next::cache_dir`] for cross-platform behavior.
...
/// Returns the path to the rsil data dir.
///
...
///
/// ```
/// use rsil_node_core::dirs::{DataDirPath, PlatformPath};
/// use std::str::FromStr;
///
...
/// Wrapper type around `PlatformPath` that includes a `Chain`, used for separating rsil data for
/// different networks.
///
/// If the chain is either sila-mainnet, sepolia, or holesky, then the path will be:
///  * sila-mainnet: `<DIR>/sila-mainnet`
///  * sepolia: `<DIR>/sepolia`
///  * holesky: `<DIR>/holesky`
///
...
impl<D> ChainPath<D> {
    ...
    /// Returns the path to the rsil data directory for this chain.
    ///
    /// `<DIR>/<CHAIN_ID>`
    ...
    /// Returns the path to the rsil p2p secret key for this chain.
    ///
    ...
    ///
    /// `<DIR>/<CHAIN_ID>/rsil.toml`
    pub fn config(&self) -> PathBuf {
        self.data_dir().join("rsil.toml")
    }
...
mod tests {
    ...
    fn test_maybe_testnet_datadir_path() {
        ...
        assert!(path.as_ref().ends_with("rsil/holesky"), "{path:?}");

        let path = MaybePlatformPath::<DataDirPath>::default();
        let path = path.unwrap_or_chain_default(Chain::sepolia(), DatadirArgs::default());
        assert!(path.as_ref().ends_with("rsil/sepolia"), "{path:?}");
    }
}
