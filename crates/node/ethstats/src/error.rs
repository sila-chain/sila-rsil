...
/// Main error type for the `SilStats` client
///
/// This enum covers all possible errors that can occur when interacting
/// with an `SilStats` server, including connection issues, authentication
/// problems, data fetching errors, and various I/O operations.
#[derive(Debug, Error)]
pub enum SilStatsError {
    ...
    /// Authentication failed with the `SilStats` server
