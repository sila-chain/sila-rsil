...
use rsil_chain_state::{CanonStateNotification, CanonStateSubscriptions};
use rsil_network_api::{NetworkInfo, Peers};
use rsil_primitives_traits::{Block, BlockBody};
use rsil_storage_api::{BlockReader, BlockReaderIdExt, NodePrimitivesProvider};
...
/// Number of historical blocks to include in a history update sent to the `SilStats` server
const HISTORY_UPDATE_RANGE: u64 = 50;
/// Duration to wait before attempting to reconnect to the `SilStats` server
...
/// Main service for interacting with an `SilStats` server
///
/// This service handles all communication with the `SilStats` server including
...
impl<Network, Provider, Pool> SilStatsService<Network, Provider, Pool>
where
    Network: NetworkInfo + Peers,
    Provider: BlockReaderIdExt + CanonStateSubscriptions,
    Pool: TransactionPool,
{
    /// Create a new `SilStats` service and establish initial connection
    ///
    /// # Arguments
    ...
    /// Establish `WebSocket` connection to the `SilStats` server
    ///
    ...
    async fn connect(&self) -> Result<(), SilStatsError> {
        ...
        let url = Url::parse(&full_url).map_err(SilStatsError::Url)?;
    ...
    /// Authenticate with the `SilStats` server
    ///
    ...
    async fn login(&self) -> Result<(), SilStatsError> {
        ...
        let network_status = self
            ...
            .network_status()
            .await
            .map_err(|e| SilStatsError::AuthError(e.to_string()))?;
        ...
        let response =
            timeout(READ_TIMEOUT, conn.read_json()).await.map_err(|_| SilStatsError::Timeout)??;
    ...
    /// Report current node statistics to the `SilStats` server
    ///
    /// Sends information about the node's current state including sync status,
    /// peer count, and uptime.
    async fn report_stats(&self) -> Result<(), SilStatsError> {
        let conn = self.conn.read().await;
        let conn = conn.as_ref().ok_or(SilStatsError::NotConnected)?;
    ...
    /// Report latency measurement to the `SilStats` server
    ///
    ...
    async fn report_latency(&self) -> Result<(), SilStatsError> {
        let start = {
    ...
    /// Report pending transaction count to the `SilStats` server
    ///
    /// Gets the current number of pending transactions from the pool and
    /// sends this information to the server.
    async fn report_pending(&self) -> Result<(), SilStatsError> {
        let conn = self.conn.read().await;
        let conn = conn.as_ref().ok_or(SilStatsError::NotConnected)?;
    ...
    /// Report block information to the `SilStats` server
    ///
    ...
    async fn report_block(
        ...
        let block_number = if let Some(head) = head {
            ...
            self.provider
                .best_block_number()
                .map_err(|e| SilStatsError::DataFetchError(e.to_string()))?
        };
    ...
    /// Report new payload information to the `SilStats` server
    ///
    ...
    /// Convert a block to `SilStats` block statistics format
    ///
    /// Extracts relevant information from a block and formats it according
    /// to the `SilStats` protocol specification.
    ///
    /// # Arguments
    ...
    ) -> Result<BlockStats, SilStatsError> {
        let body = block.body();
    ...
    /// Report historical block data to the `SilStats` server
    ///
    ...
    async fn report_history(&self, list: Option<&Vec<u64>>) -> Result<(), SilStatsError> {
        let conn = self.conn.read().await;
        let conn = conn.as_ref().ok_or(SilStatsError::NotConnected)?;
    ...
    /// Handle incoming messages from the `SilStats` server
    ///
    ...
    async fn handle_message(&self, msg: Value) -> Result<(), SilStatsError> {
    ...
    /// Main service loop that handles all `SilStats` communication
    ///
...
mod tests {
    ...
    async fn test_invalid_url_handling() {
        ...
        // Test missing secret
        let result = SilStatsService::new(
            "test-node@localhost",
            network.clone(),
            provider.clone(),
            pool.clone(),
        ...
        .await;
        assert!(
            matches!(result, Err(SilStatsError::InvalidUrl(_))),
            "Should detect invalid URL format"
        );

        // Test invalid URL format
        let result = SilStatsService::new("invalid-url", network, provider, pool).await;
        assert!(
            matches!(result, Err(SilStatsError::InvalidUrl(_))),
            "Should detect invalid URL format"
        );
