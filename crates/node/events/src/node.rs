...
use futures::Stream;
use rsil_engine_primitives::{ConsensusEngineEvent, ForkchoiceStatus, SlowBlockInfo};
use rsil_network_api::PeersInfo;
use rsil_primitives_traits::{format_gas, format_gas_throughput, BlockBody, NodePrimitives};
use rsil_prune_types::PrunerEvent;
use rsil_stages::{EntitiesCheckpoint, ExecOutput, PipelineEvent, StageCheckpoint, StageId};
use rsil_static_file_types::StaticFileProducerEvent;
use std::{
        ...
        match event {
            ...
            ConsensusEngineEvent::CanonicalBlockAdded(executed, elapsed) => {
                ...
                info!(
                    ...
                    blobs=block.blob_gas_used().unwrap_or(0) / alloy_eips::sip4844::DATA_GAS_PER_BLOB,
                    excess_blobs=block.excess_blob_gas().unwrap_or(0) / alloy_eips::sip4844::DATA_GAS_PER_BLOB,
                    ?elapsed,
        ...
        macro_rules! log_slow_block_fields {
            ($($commit_field:tt)*) => {
                warn!(
                    ...
                    state_writes.sip7702_delegations_set = stats.sip7702_delegations_set,
                    state_writes.sip7702_delegations_cleared = stats.sip7702_delegations_cleared,
        ...
        if self.current_stage.is_none() {
            match event {
                ...
                ConsensusLayerHealthEvent::HaveNotReceivedUpdatesForAWhile(period) => {
                    warn!(
                        ?period,
                        "Beacon client online, but no consensus updates received for a while. This may be because of a rsil error, or an error in the beacon client! Please investigate rsil and beacon client logs!"
                    )
                }
            }
        }
    ...
    fn update(&mut self, stage: StageId, checkpoint: StageCheckpoint) {
        ...
        if let Some(last_checkpoint_time) = &self.last_checkpoint_time {
            let Some(processed_since_last) =
                ...
                self.eta = None;
                debug!(target: "rsil::cli", %stage, ?current, ?self.last_checkpoint, "Failed to calculate the ETA: processed entities is less than the last checkpoint");
                return;
            };
            ...
            let Some(remaining) = current.total.checked_sub(current.processed) else {
                self.eta = None;
                debug!(target: "rsil::cli", %stage, ?current, "Failed to calculate the ETA: total entities is less than processed entities");
                return;
            };
