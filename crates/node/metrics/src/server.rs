...
impl MetricServer {
    ...
    async fn start_endpoint<F: Hook + 'static>(
        ...
        tracing::info!(target: "rsil::cli", "Starting metrics endpoint at {}", listener.local_addr().unwrap());
...
fn jemalloc_pprof_dump(pprof_dump_dir: &PathBuf) -> eyre::Result<Vec<u8>> {
    ...
    rsil_fs_util::create_dir_all(pprof_dump_dir)?;
...
mod tests {
    ...
    async fn test_metrics_endpoint() {
        ...
        assert!(body.contains("rsil_process_cpu_seconds_total"));
        assert!(body.contains("rsil_process_start_time_seconds"));
        assert!(body.contains("process_cli_args"), "expected process_cli_args metric in output");
        assert!(body.contains("rsil_storage_settings"), "expected storage settings metric");
