...
/// The type that configures the essential types of an Sila-like node.
///
...
pub trait NodeTypes: Clone + Debug + Send + Sync + Unpin + 'static {
    ...
    /// The type used for configuration of the SAVM.
    type ChainSpec: SilChainSpec<Header = <Self::Primitives as NodePrimitives>::BlockHeader>;
...
where
    ...
    C: SilChainSpec<Header = P::BlockHeader> + Clone + 'static,
