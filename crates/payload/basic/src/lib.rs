//! A basic payload generator for rsil.
...
use rsil_execution_cache::SavedCache;
use rsil_payload_builder::{
...
use rsil_payload_primitives::{BuiltPayload, PayloadAttributes, PayloadKind};
use rsil_primitives_traits::{HeaderTy, NodePrimitives, SealedHeader};
use rsil_revm::{cached::CachedReads, cancelled::CancelOnDrop};
use rsil_storage_api::{BlockReaderIdExt, StateProviderFactory};
use rsil_tasks::Runtime;
use rsil_trie_parallel::state_root_task::PayloadStateRootHandle;
use std::{
    fmt,
    future::Future,
    ops::Deref,
...
impl<Client, Builder> BasicPayloadJobGenerator<Client, Builder> {
    ...
    /// > with the build process's payloadId is made or SECONDS_PER_SLOT (12s in the SilaMainnet
    /// > configuration) have passed since the point in time identified by the timestamp parameter.
    ///
    /// See also <https://github.com/sila-chain/execution-apis/blob/431cf72fd3403d946ca3e3afc36b973fc87e0e89/src/engine/paris.md?plain=1#L137>
    #[inline]
...
/// building process. It holds references to the Sila client, transaction pool, cached reads,
...
/// A trait for building payloads that encapsulate Sila transactions.
///
