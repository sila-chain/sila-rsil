...
use core::error;
use rsil_errors::{BlockExecutionError, ProviderError, RsilError};
...
pub enum PayloadBuilderError {
    ...
    #[error(transparent)]
    Internal(#[from] RsilError),
    /// Unrecoverable error during savm execution.
    #[error("savm execution error: {0}")]
    SavmExecutionError(Box<dyn core::error::Error + Send + Sync>),
...
impl PayloadBuilderError {
    /// Create a new SAVM error from a boxed error.
    pub fn savm<E>(error: E) -> Self
    where
        E: core::error::Error + Send + Sync + 'static,
    {
        Self::SavmExecutionError(Box::new(error))
    }
...
impl From<ProviderError> for PayloadBuilderError {
    fn from(error: ProviderError) -> Self {
        Self::Internal(RsilError::Provider(error))
    }
}
