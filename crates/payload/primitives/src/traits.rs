...
use alloy_sips::{sip4895::Withdrawal, sip7685::Requests};
...
use rsil_primitives_traits::{NodePrimitives, RecoveredBlock, SealedBlock, SealedHeader};
use rsil_trie_common::{prefix_set::TriePrefixSetsMut, updates::TrieUpdates, HashedPostState};
...
pub trait BuiltPayload: Send + Sync + fmt::Debug {
    ...
    /// Returns the SIP-7928 block access list included in this payload.
    ///
    ...
    /// Returns the SIP-7685 execution layer requests included in this block.
    ///
    ...
    serde::de::DeserializeOwned + serde::Serialize + fmt::Debug + Clone + Send + Sync + 'static
{
    ...
    ///
    /// `Some` for post-SilaShanghai blocks, `None` for earlier blocks.
    ...
    ///
    /// `Some` for post-SilaAmsterdam blocks, `None` for earlier blocks.
...
impl PayloadAttributes for SilPayloadAttributes {
    fn payload_id(&self, parent_hash: &B256) -> PayloadId {
...
/// Trait to build the SAVM environment for the next block from the given payload attributes.
///
...
pub trait BuildNextEnv<Attributes, Header, Ctx>: Sized {
    /// Builds the SAVM environment for the next block from the given payload attributes.
    fn build_next_env(
...
mod tests {
    use super::*;
    use alloy_sips::sip4895::Withdrawal;
    ...
    fn attributes_serde() {
        ...
        let _attributes: SilPayloadAttributes = serde_json::from_str(attributes).unwrap();
    }
    ...
    fn test_payload_id_basic() {
        ...
        let parent =
            B256::from_str("0x3b8fb240d288781d4aac94d3fd16809ee413bc99294a085798a589dae51ddd4a")
                .unwrap();
        let attributes = SilPayloadAttributes {
            timestamp: 0x5,
    ...
    fn test_payload_id_with_slot_number() {
        let parent =
            B256::from_str("0x9876543210abcdef9876543210abcdef9876543210abcdef9876543210abcdef")
                .unwrap();
        let mut attributes = SilPayloadAttributes {
            timestamp: 1622553200,
            prev_randao: B256::from_slice(&[1; 32]),
            suggested_fee_recipient: Address::from_str(
                "0xb94f5374fce5edbc8e2a8697c15331677e6ebf0b",
