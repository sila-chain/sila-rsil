...
use rsil_transaction_pool::{PoolTransaction, ValidPoolTransaction};
    ...
    fn test_best_transactions_chained_iterators() {
        ...
        // Add transactions to the main pool
        {
            let prioritized_tx_a =
                MockTransaction::sip1559().with_gas_price(5).with_sender(address_a);
            // without our custom logic, B would be prioritized over A due to gas price:
            let prioritized_tx_b =
                MockTransaction::sip1559().with_gas_price(10).with_sender(address_b);
            let regular_tx =
                MockTransaction::sip1559().with_gas_price(15).with_sender(address_regular);
        ...
        {
            let prioritized_tx =
                MockTransaction::sip1559().with_gas_price(0).with_sender(address_in_priority_pool);
        ...
        let mut block = PayloadTransactionsChain::new(
            PayloadTransactionsFixed::single(
                MockTransaction::sip1559().with_sender(address_top_of_block),
            ),
            Some(100),
