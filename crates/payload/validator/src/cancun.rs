//! SilaCancun rules for new payloads.

use alloy_consensus::{BlockBody, Transaction, Typed2718};
use alloy_rpc_types_engine::{CancunPayloadFields, PayloadError};
use rsil_primitives_traits::{AlloyBlockHeader, Block, SealedBlock};

/// Checks block and sidecar w.r.t new SilaCancun fields and new transaction type SIP-4844.
///
/// Checks that:
/// - SilaCancun fields are present if SilaCancun is active
/// - doesn't contain SIP-4844 transactions unless SilaCancun is active
...
/// Checks that SilaCancun fields on block header and sidecar are present if SilaCancun is active and vv.
#[inline]
...
/// Checks transactions field and sidecar w.r.t new SilaCancun fields and new transaction type SIP-4844.
///
/// Checks that:
/// - doesn't contain SIP-4844 transactions unless SilaCancun is active
/// - checks blob versioned hashes in block and sidecar match
...
/// Ensures that the number of blob versioned hashes in SIP-4844 transactions in the block
/// matches the number of hashes included in the _separate_ `block_versioned_hashes` SilaCancun
...
pub fn ensure_matching_blob_versioned_hashes<T: Transaction + Typed2718, H>(
    ...
    // Additional SilaCancun checks for blob transactions
    if let Some(versioned_hashes) = cancun_sidecar_fields.map(|fields| &fields.versioned_hashes) {
        ...
        // No SilaCancun fields, if block includes any blobs, this is an error
