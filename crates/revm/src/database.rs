...
pub trait SavmStateProvider {
    ...
    fn bytecode_by_hash(
        &self,
        code_hash: &B256,
    ) -> ProviderResult<Option<rsil_primitives_traits::Bytecode>>;
...
// Blanket implementation of SavmStateProvider for any type that implements StateProvider.
impl<T: StateProvider> SavmStateProvider for T {
    fn basic_account(&self, address: &Address) -> ProviderResult<Option<Account>> {
...
/// A [Database] and [`DatabaseRef`] implementation that uses [`SavmStateProvider`] as the underlying
/// data source.
#[derive(Clone)]
...
impl<DB: SavmStateProvider> Database for StateProviderDatabase<DB> {
...
impl<DB: SavmStateProvider> DatabaseRef for StateProviderDatabase<DB> {
...
///
/// This adapts account and bytecode reads from revm's database interface back into Rsil's storage
    ...
    DB: DatabaseRef<Error = ProviderError>,
{
    fn bytecode_by_hash(
        ...
        Ok(Some(rsil_primitives_traits::Bytecode(self.0.code_by_hash_ref(*code_hash)?)))
    }
}
...
mod tests {
    ...
    fn database_state_provider_maps_code_hash_and_bytecode() {
        ...
        assert_eq!(
            ...
            Some(rsil_primitives_traits::Bytecode(bytecode))
        );
    }
    ...
    fn database_state_provider_wraps_default_bytecode_for_unknown_hash() {
        ...
        assert_eq!(
            ...
            Some(rsil_primitives_traits::Bytecode(Bytecode::default()))
        );
    }
    ...
    fn database_state_provider_uses_cached_reads() {
        ...
        assert_eq!(
            provider.bytecode_by_hash(&code_hash).unwrap(),
            Some(rsil_primitives_traits::Bytecode(bytecode.clone()))
        );
        assert_eq!(
            provider.bytecode_by_hash(&code_hash).unwrap(),
            Some(rsil_primitives_traits::Bytecode(bytecode))
