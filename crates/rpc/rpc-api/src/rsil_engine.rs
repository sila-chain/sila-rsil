...
/// Rsil-specific payload status that includes server-measured execution latency.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RsilPayloadStatus {
...
/// Input for `rsil_newPayload` that accepts either `ExecutionData` directly or an RLP-encoded
/// block with optional side data.
#[derive(Debug, Clone)]
pub enum RsilNewPayloadInput<ExecutionData> {
...
impl<ExecutionData> Serialize for RsilNewPayloadInput<ExecutionData>
where
...
impl<'de, ExecutionData> Deserialize<'de> for RsilNewPayloadInput<ExecutionData>
where
    ExecutionData: Deserialize<'de>,
{
        ...
        D: Deserializer<'de>,
    {
        #[derive(Deserialize)]
        #[serde(untagged)]
        enum RsilNewPayloadInputSerde<ExecutionData> {
            ExecutionData(ExecutionData),
            BlockRlp {
                block: Bytes,
                #[serde(default)]
        ...
        Ok(match RsilNewPayloadInputSerde::deserialize(deserializer)? {
            RsilNewPayloadInputSerde::ExecutionData(data) => Self::ExecutionData(data),
            RsilNewPayloadInputSerde::BlockRlp { block, bal } => Self::BlockRlp { block, bal },
            RsilNewPayloadInputSerde::LegacyBlockRlp(block) => Self::BlockRlp { block, bal: None },
        })
    }
}
...
/// Rsil-specific engine API extensions.
///
/// This trait provides a `rsil_newPayload` endpoint that accepts either `ExecutionData` directly
...
pub trait RsilEngineApi<ExecutionData> {
    /// Rsil-specific newPayload that accepts either `ExecutionData` directly or an RLP-encoded
    /// block.
    ///
    ...
    async fn rsil_new_payload(
        &self,
        payload: RsilNewPayloadInput<ExecutionData>,
        wait_for_persistence: Option<bool>,
        wait_for_caches: Option<bool>,
    ) -> RpcResult<RsilPayloadStatus>;

    /// Rsil-specific forkchoiceUpdated that sends a regular forkchoice update with no payload
    /// attributes.
    #[method(name = "forkchoiceUpdated")]
    async fn rsil_forkchoice_updated(
        &self,
...
mod tests {
    ...
    fn block_rlp_serializes_named_fields() {
        let input = RsilNewPayloadInput::<TestExecutionData>::BlockRlp {
    ...
    fn block_rlp_without_bal_serializes_legacy_bytes_roundtrip() {
        ...
        let input =
            serde_json::from_str::<RsilNewPayloadInput<TestExecutionData>>(&serialized).unwrap();

        let RsilNewPayloadInput::BlockRlp { block, bal } = input else {
    ...
    fn block_rlp_deserializes_without_bal() {
        let input =
            serde_json::from_str::<RsilNewPayloadInput<TestExecutionData>>(r#"{"block":"0x01"}"#)
                .unwrap();
    ...
    fn block_rlp_deserializes_legacy_bytes() {
        let input = serde_json::from_value::<RsilNewPayloadInput<TestExecutionData>>(json!("0x01"))
            .unwrap();
