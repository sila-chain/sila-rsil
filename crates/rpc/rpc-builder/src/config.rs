...
pub trait RsilRpcServerConfig {
    ...
    /// The configured sila RPC settings.
    fn eth_config(&self) -> SilConfig;

    /// The configured sila RPC settings.
    fn flashbots_config(&self) -> ValidationApiConfig;

    /// Returns state cache configuration.
    fn state_cache_config(&self) -> SilStateCacheConfig;
...
impl RsilRpcServerConfig for RpcServerArgs {
    fn is_ipc_enabled(&self) -> bool {
    ...
    fn auth_jwt_secret(&self, default_jwt_path: PathBuf) -> Result<JwtSecret, JwtError> {
        match self.auth_jwtsecret.as_ref() {
            Some(fpath) => {
                debug!(target: "rsil::cli", user_path=?fpath, "Reading JWT auth secret file");
...
mod tests {
    ...
    fn test_rpc_gas_cap() {
        ...
        let args =
            CommandParser::<RpcServerArgs>::parse_from(["rsil", "--rpc.gascap", "1000"]).args;
        let config = args.eth_config();
        assert_eq!(config.rpc_gas_cap, 1000);

        let args = CommandParser::<RpcServerArgs>::try_parse_from(["rsil", "--rpc.gascap", "0"]);
        assert!(args.is_err());
    }
    ...
    fn test_transport_rpc_module_config() {
        ...
        let expected = [RsilRpcModule::Sil, RsilRpcModule::Admin, RsilRpcModule::Debug];
