use crate::{cors::CorsDomainError, RsilRpcModule};
use rsil_ipc::server::IpcServerStartError;
use std::{
    collections::HashSet,
    io::{self, ErrorKind},
    net::SocketAddr,
...
pub struct ConflictingModules {
    ...
    pub overlap: HashSet<RsilRpcModule>,
    /// Modules present in http but not in ws.
    pub http_not_ws: HashSet<RsilRpcModule>,
    /// Modules present in ws but not in http.
    pub ws_not_http: HashSet<RsilRpcModule>,
}
