...
//! [`RsilRpcModule`] specific to transports (ws, http, ipc).
//!
//! The [`RpcModuleBuilder`] is the main entrypoint for configuring all rsil modules. It takes
...
use rsil_transaction_pool::{noop::NoopTransactionPool, TransactionPool};
...
pub use rsil_ipc::server::{
    Builder as IpcServerBuilder, RpcServiceBuilder as IpcRpcServiceBuilder,
};
pub use rsil_rpc_server_types::{constants, RpcModuleSelection};
...
/// Sil utils
pub mod sil;
pub use sil::SilHandlers;

// Rpc server metrics
mod metrics;
use crate::middleware::RsilRpcMiddleware;
...
use rsil_rpc::sil::sim_bundle::SilSimBundle;
...
#[derive(Debug, Clone)]
pub struct RpcModuleBuilder<N, Provider, Pool, Network, SavmConfig, Consensus> {
    ...
    network: Network,
    /// How additional tasks are spawned, for example in the sil pubsub namespace
    executor: Option<Runtime>,
    /// Defines how the SAVM should be configured before execution.
...
impl<N, Provider, Pool, Network, SavmConfig, Consensus>
    RpcModuleBuilder<N, Provider, Pool, Network, SavmConfig, Consensus>
{
    ...
    pub fn with_provider<P>(
        self,
        provider: P,
    ) -> RpcModuleBuilder<N, P, Pool, Network, SavmConfig, Consensus> {
    ...
    pub fn with_pool<P>(
        self,
        pool: P,
    ) -> RpcModuleBuilder<N, Provider, P, Network, SavmConfig, Consensus> {
    ...
    /// [`SilApi`] which requires a [`TransactionPool`] implementation.
    pub fn with_noop_pool(
        self,
    ) -> RpcModuleBuilder<N, Provider, NoopTransactionPool, Network, SavmConfig, Consensus> {
    ...
    pub fn with_network<Net>(
        self,
        network: Net,
    ) -> RpcModuleBuilder<N, Provider, Pool, Net, SavmConfig, Consensus> {
    ...
    /// This is only intended for allow easier setup of namespaces that depend on the
    /// [`SilApi`] which requires a [`NetworkInfo`] implementation.
    pub fn with_noop_network(
        self,
    ) -> RpcModuleBuilder<N, Provider, Pool, NoopNetwork, SavmConfig, Consensus> {
        let Self { provider, pool, executor, evm_config, consensus, _primitives, .. } = self;
        RpcModuleBuilder {
            provider,
            pool,
    ...
    /// Configure the savm configuration type
    ...
    pub fn with_consensus<C>(
        self,
        consensus: C,
    ) -> RpcModuleBuilder<N, Provider, Pool, Network, SavmConfig, C> {
    ...
    pub fn eth_api_builder<ChainSpec>(
        ...
        RpcConverter<Sila, SavmConfig, SilReceiptConverter<ChainSpec>>,
    >
    where
        Provider: Clone,
        Pool: Clone,
        ...
        SavmConfig: Clone,
        RpcNodeCoreAdapter<Provider, Pool, Network, SavmConfig>:
            RpcNodeCore<Provider: ChainSpecProvider<ChainSpec = ChainSpec>, Savm = SavmConfig>,
    {
        SilApiBuilder::new(
            self.provider.clone(),
            self.pool.clone(),
    ...
    /// Initializes a new [`SilApiServer`] with the configured components and default settings.
    ///
    ...
    Consensus: FullConsensus<N> + Clone + 'static,
{
    ...
    /// namespace.
    pub fn build_with_auth_server<SilApi, Payload>(
        self,
        ...
        Payload: PayloadTypes,
    {
        ...
        let mut registry = self.into_registry(config, sil, engine_events);
...
impl RpcModuleConfig {
    ...
    /// Returns a new RPC module config given the sil namespace config
    pub const fn new(sil: SilConfig) -> Self {
        Self { sil }
    }

    /// Get a reference to the sil namespace config
    pub const fn sil(&self) -> &SilConfig {
        &self.sil
    }

    /// Get a mutable reference to the sil namespace config
    pub const fn eth_mut(&mut self) -> &mut SilConfig {
        &mut self.sil
    }
}
...
pub struct RpcModuleConfigBuilder {
    sil: Option<SilConfig>,
}

// === impl RpcModuleConfigBuilder ===

impl RpcModuleConfigBuilder {
    /// Configures a custom sil namespace config
    pub fn sil(mut self, sil: SilConfig) -> Self {
        self.sil = Some(sil);
        self
    }
    ...
    pub fn build(self) -> RpcModuleConfig {
        let Self { sil } = self;
        RpcModuleConfig { sil: sil.unwrap_or_default() }
    }

    /// Get a reference to the sil namespace config, if any
    pub const fn get_eth(&self) -> Option<&SilConfig> {
        self.sil.as_ref()
    }

    /// Get a mutable reference to the sil namespace config, if any
    pub const fn eth_mut(&mut self) -> &mut Option<SilConfig> {
        &mut self.sil
    }

    /// Get the sil namespace config, creating a default if none is set
    pub fn eth_mut_or_default(&mut self) -> &mut SilConfig {
        self.sil.get_or_insert_with(SilConfig::default)
    }
}

/// A Helper type the holds instances of the configured modules.
#[derive(Debug)]
pub struct RpcRegistryInner<Provider, Pool, Network, SilApi: SilApiTypes, SavmConfig, Consensus> {
    provider: Provider,
    pool: Pool,
    network: Network,
    executor: Runtime,
    ...
    sil: SilHandlers<SilApi>,
    /// to put trace calls behind semaphore
    blocking_pool_guard: BlockingTaskGuard,
    /// Contains the [Methods] of a module
    modules: HashMap<RsilRpcModule, Methods>,
    /// sil config settings
    eth_config: SilConfig,
    /// Notification channel for engine API events
    engine_events:
        EventSender<ConsensusEngineEvent<<SilApi::RpcConvert as RpcConvert>::Primitives>>,
}

// === impl RpcRegistryInner ===

impl<N, Provider, Pool, Network, SilApi, SavmConfig, Consensus>
    RpcRegistryInner<Provider, Pool, Network, SilApi, SavmConfig, Consensus>
where
    N: NodePrimitives,
...
impl<Provider, Pool, Network, SilApi, Savm, Consensus>
    RpcRegistryInner<Provider, Pool, Network, SilApi, Savm, Consensus>
where
    SilApi: SilApiTypes,
{
    ...
    /// Returns a reference to the savm config
    pub const fn evm_config(&self) -> &Savm {
        &self.evm_config
    }
...
impl<Provider, Pool, Network, SilApi, SavmConfig, Consensus>
    RpcRegistryInner<Provider, Pool, Network, SilApi, SavmConfig, Consensus>
where
    Network: NetworkInfo + Clone + 'static,
    SilApi: SilApiTypes,
    Provider: BlockReader + ChainSpecProvider<ChainSpec: SilaHardforks>,
    SavmConfig: ConfigureEvm,
{
        ...
        Pool: TransactionPool + Clone + 'static,
    {
        ...
        self.modules.insert(RsilRpcModule::Admin, adminapi.into_rpc().into());
        self
    }
    ...
    pub fn register_web3(&mut self) -> &mut Self {
        ...
        self.modules.insert(RsilRpcModule::Web3, web3api.into_rpc().into());
        self
    }
}
...
where
    ...
    Network: NetworkInfo + Peers + Clone + 'static,
    SilApi: SilApiServer<
            RpcTxReq<SilApi::NetworkTypes>,
            RpcTransaction<SilApi::NetworkTypes>,
            RpcBlock<SilApi::NetworkTypes>,
            RpcReceipt<SilApi::NetworkTypes>,
            RpcHeader<SilApi::NetworkTypes>,
            TxTy<N>,
        > + SilApiTypes,
    SavmConfig: ConfigureEvm<Primitives = N> + 'static,
{
    /// Register Sil Namespace
    ///
    /// # Panics
    ///
        ...
        SilApi: TraceExt + SilTransactions<Primitives = N>,
    {
        ...
        self.modules.insert(RsilRpcModule::Ots, otterscan_api.into_rpc().into());
        self
    }
        ...
        SilApi: SilTransactions + TraceExt,
    {
        ...
        self.modules.insert(RsilRpcModule::Debug, debug_api.into_rpc().into());
        self
    }
        ...
        SilApi: TraceExt,
    {
        ...
        self.modules.insert(RsilRpcModule::Trace, trace_api.into_rpc().into());
        self
    }
        ...
        SilApi: SilApiSpec + 'static,
    {
        ...
        self.modules.insert(RsilRpcModule::Net, netapi.into_rpc().into());
        self
    }
    ...
    ///
    /// If called outside of the tokio runtime.
    pub fn register_rsil(&mut self) -> &mut Self {
        let rsilapi = self.rsil_api();
        self.modules.insert(RsilRpcModule::Rsil, rsilapi.into_rpc().into());
        self
    }
    ...
    SavmConfig: ConfigureEvm<Primitives = N>,
{
    ...
    pub fn rsil_api(&self) -> RsilApi<Provider, SavmConfig> {
        RsilApi::new(
            self.provider.clone(),
            self.evm_config.clone(),
            self.blocking_pool_guard.clone(),
    ...
    Consensus: FullConsensus<N> + Clone + 'static,
{
        ...
        Payload: PayloadTypes,
    {
        ...
        // Merge rsil_* endpoints
        let rsil_engine_api = RsilEngineApi::new(beacon_engine_handle);
        module
            .merge(RsilEngineApiServer::into_rpc(rsil_engine_api).remove_context())
    ...
    /// Populates a new [`RpcModule`] based on the selected [`RsilRpcModule`]s in the given
    /// [`RpcModuleSelection`]
    pub fn module_for(&mut self, config: &RpcModuleSelection) -> RpcModule<()> {
        let mut module = RpcModule::new(());
        let all_methods = self.rsil_methods(config.iter_selection());
    ...
    /// Returns the [Methods] for the given [`RsilRpcModule`]
    ///
            ...
            .map(|namespace| {
                self.modules
                    ...
                    .or_insert_with(|| match namespace.clone() {
                        RsilRpcModule::Admin => AdminApi::new(
                            self.network.clone(),
                        ...
                        .into_rpc()
                        .into(),
                        RsilRpcModule::Web3 => Web3Api::new(self.network.clone()).into_rpc().into(),
                        RsilRpcModule::Txpool => TxPoolApi::new(
                            self.sil.api.pool().clone(),
                            dyn_clone::clone(self.sil.api.converter()),
                        )
                        .into_rpc()
                        .into(),
                        RsilRpcModule::Rpc => RPCApi::new(
                            namespaces
                                .iter()
...
impl<Provider, Pool, Network, SilApi, SavmConfig, Consensus> Clone
    for RpcRegistryInner<Provider, Pool, Network, SilApi, SavmConfig, Consensus>
where
    SilApi: SilApiTypes,
    Provider: Clone,
    Pool: Clone,
...
impl RpcServerConfig {
    ...
    ///
    /// Note: this always configures an [`SilSubscriptionIdProvider`] [`IdProvider`] for
    ...
    pub fn with_http(mut self, config: ServerConfigBuilder) -> Self {
        self.http_server_config =
            Some(config.set_id_provider(SilSubscriptionIdProvider::default()));
        self
    }
    ...
    pub fn with_ws(mut self, config: ServerConfigBuilder) -> Self {
        self.ws_server_config = Some(config.set_id_provider(SilSubscriptionIdProvider::default()));
        self
    }
    ...
    pub fn with_ipc(mut self, config: IpcServerBuilder<Identity, Identity>) -> Self {
        self.ipc_server_config = Some(config.set_id_provider(SilSubscriptionIdProvider::default()));
        self
    }
}
...
impl<RpcMiddleware> RpcServerConfig<RpcMiddleware> {
    ...
    /// [`rsil_rpc_server_types::constants::DEFAULT_HTTP_RPC_PORT`]
    ...
    ///
    /// Default is [`Ipv4Addr::LOCALHOST`] and
    /// [`rsil_rpc_server_types::constants::DEFAULT_WS_RPC_PORT`]
    ...
    ///
    /// By default all transports use [`SilSubscriptionIdProvider`]
    ...
    ///
    /// Default is [`rsil_rpc_server_types::constants::DEFAULT_IPC_ENDPOINT`]
...
///
/// ```
/// use rsil_rpc_builder::{RsilRpcModule, TransportRpcModuleConfig};
/// let config =
///     TransportRpcModuleConfig::default().with_http([RsilRpcModule::Sil, RsilRpcModule::Admin]);
/// ```
...
impl TransportRpcModuleConfig {
    ...
    pub fn contains_any(&self, module: &RsilRpcModule) -> bool {
    ...
    pub fn contains_http(&self, module: &RsilRpcModule) -> bool {
...
impl TransportRpcModules {
    ...
    /// [`RsilRpcModule`] is configured for the transport.
    ///
    /// Fails if any of the methods in other is present already.
    pub fn merge_if_module_configured(
        &mut self,
        module: RsilRpcModule,
    ...
    /// Merge the given [`Methods`] in all configured transport modules if the given
    /// [`RsilRpcModule`] is configured for the transport, using a closure to lazily
    ...
    pub fn methods_by_module(&self, module: RsilRpcModule) -> Methods {
...
impl RpcServerHandle {
    ...
    /// Returns a new [`alloy_network::Sila`] http provider with its recommended fillers.
    pub fn eth_http_provider(
        &self,
    ) -> Option<impl Provider<alloy_network::Sila> + Clone + Unpin + 'static> {
        self.new_http_provider_for()
    }

    /// Returns a new [`alloy_network::Sila`] http provider with its recommended fillers and
    /// installed wallet.
    ...
    where
        W: IntoWallet<alloy_network::Sila, NetworkWallet: Clone + Unpin + 'static>,
    {
    ...
    /// Returns a new [`alloy_network::Sila`] websocket provider with its recommended fillers.
    ...
    /// Returns a new [`alloy_network::Sila`] ws provider with its recommended fillers and
    /// installed wallet.
    ...
    /// Returns a new [`alloy_network::Sila`] ipc provider with its recommended fillers.
...
mod tests {
    ...
    fn parse_eth_call_bundle_selection() {
        let selection = "sil,admin,debug".parse::<RpcModuleSelection>().unwrap();
        assert_eq!(
            selection,
            RpcModuleSelection::Selection(
                [RsilRpcModule::Sil, RsilRpcModule::Admin, RsilRpcModule::Debug,].into()
            )
        );
    }
    ...
    fn parse_rpc_unique_module_selection() {
        let selection = "sil,admin,sil,net".parse::<RpcModuleSelection>().unwrap();
        assert_eq!(
            selection,
            RpcModuleSelection::Selection(
                [RsilRpcModule::Sil, RsilRpcModule::Admin, RsilRpcModule::Net,].into()
            )
        );
    }
    ...
    fn identical_selection() {
        ...
        assert!(RpcModuleSelection::are_identical(
            Some(&RpcModuleSelection::Selection([RsilRpcModule::Sil].into())),
            Some(&RpcModuleSelection::Selection([RsilRpcModule::Sil].into())),
        ));
        assert!(RpcModuleSelection::are_identical(
            None,
    ...
    fn test_rpc_module_str() {
        macro_rules! assert_rpc_module {
            ($($s:expr => $v:expr,)*) => {
                $(
                    let val: RsilRpcModule  = $s.parse().unwrap();
                    assert_eq!(val, $v);
        ...
        assert_rpc_module!
        (
                "admin" =>  RsilRpcModule::Admin,
                "debug" =>  RsilRpcModule::Debug,
                "sil" =>  RsilRpcModule::Sil,
                "net" =>  RsilRpcModule::Net,
                "trace" =>  RsilRpcModule::Trace,
                "web3" =>  RsilRpcModule::Web3,
                "rpc" => RsilRpcModule::Rpc,
                "ots" => RsilRpcModule::Ots,
                "rsil" => RsilRpcModule::Rsil,
            );
    }
    ...
    fn test_default_selection() {
        ...
        assert_eq!(selection, [RsilRpcModule::Sil, RsilRpcModule::Net, RsilRpcModule::Web3].into())
    }
    ...
    fn test_create_rpc_module_config() {
        ...
        assert_eq!(
            config,
            RpcModuleSelection::Selection([RsilRpcModule::Sil, RsilRpcModule::Admin].into())
        );
    }
    ...
    fn test_configure_transport_config() {
        let config = TransportRpcModuleConfig::default()
            .with_http([RsilRpcModule::Sil, RsilRpcModule::Admin]);
        assert_eq!(
            config,
            TransportRpcModuleConfig {
                http: Some(RpcModuleSelection::Selection(
                    [RsilRpcModule::Sil, RsilRpcModule::Admin].into()
                )),
                ws: None,
                ipc: None,
                config: None,
    ...
    fn test_configure_transport_config_none() {
        let config = TransportRpcModuleConfig::default().with_http(Vec::<RsilRpcModule>::new());
        assert_eq!(
            config,
            TransportRpcModuleConfig {
    ...
    fn test_add_or_replace_if_module_configured() {
        // Create a config that enables RsilRpcModule::Sil for HTTP and WS, but NOT IPC
        let config = TransportRpcModuleConfig::default()
            .with_http([RsilRpcModule::Sil])
            .with_ws([RsilRpcModule::Sil]);
        ...
        // Call the function for RsilRpcModule::Sil
        let result = modules.add_or_replace_if_module_configured(RsilRpcModule::Sil, new_methods);
        ...
        // Verify IPC: no changes (Sil not configured for IPC)
    ...
    fn test_merge_if_module_configured_with_lazy_evaluation() {
        // Create a config that enables RsilRpcModule::Sil for HTTP only
        ...
        let result = modules.merge_if_module_configured_with(RsilRpcModule::Sil, || {
            closure_called = true;
        ...
        let result = modules.merge_if_module_configured_with(RsilRpcModule::Debug, || {
            closure_called = true;
            RpcModule::new(()).into()
        });
