...
use alloy_eips::{sip1898::LenientBlockNumberOrTag, BlockId, BlockNumberOrTag};
...
{
    SilFilterApiClient::<Transaction>::new_filter(client, Filter::default()).await.unwrap();
    SilFilterApiClient::<Transaction>::new_pending_transaction_filter(client, None).await.unwrap();
    SilFilterApiClient::<Transaction>::new_pending_transaction_filter(
        client,
        Some(PendingTransactionFilterKind::Full),
    )
    .await
    .unwrap();
    let id = SilFilterApiClient::<Transaction>::new_block_filter(client).await.unwrap();
    SilFilterApiClient::<Transaction>::filter_changes(client, id.clone()).await.unwrap();
    SilFilterApiClient::<Transaction>::logs(client, Filter::default()).await.unwrap();
    let id =
        SilFilterApiClient::<Transaction>::new_filter(client, Filter::default()).await.unwrap();
    SilFilterApiClient::<Transaction>::filter_logs(client, id.clone()).await.unwrap();
    SilFilterApiClient::<Transaction>::uninstall_filter(client, id).await.unwrap();
}
    ...
    C: ClientT + SubscriptionClientT + Sync,
{
    ...
    // Implemented
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::protocol_version(
        client,
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::chain_id(client)
        .await
        .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::accounts(client)
        .await
        .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::get_account(
        client,
        address,
        block_number.into(),
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::block_number(client)
        .await
        .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::get_code(
        client, address, None,
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::send_raw_transaction(
        client, tx,
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::fee_history(
        client,
        U64::from(0),
        block_number,
        None,
    ...
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::balance(
        client, address, None,
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::transaction_count(
        client, address, None,
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::storage_at(
        client,
        address,
        U256::default().into(),
        None,
    ...
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::block_by_hash(
        client, hash, false,
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::block_by_number(
        client,
        block_number,
        false,
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::block_transaction_count_by_number(
        client,
        block_number,
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::block_transaction_count_by_hash(
        client, hash,
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::block_uncles_count_by_hash(client, hash)
        .await
        .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::block_uncles_count_by_number(
        client,
        block_number,
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::uncle_by_block_hash_and_index(
        client, hash, index,
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::uncle_by_block_number_and_index(
        client,
        block_number,
        index,
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::sign(
        client,
        address,
        bytes.clone(),
    )
    .await
    .unwrap_err();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::sign_typed_data(
        client, address, typed_data,
    )
    .await
    .unwrap_err();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::transaction_by_hash(
        client, tx_hash,
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::transaction_by_block_hash_and_index(
        client, hash, index,
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::transaction_by_block_number_and_index(
        client,
        block_number,
        index,
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::create_access_list(
        client,
        call_request.clone(),
    ...
    .await
    .unwrap_err();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::estimate_gas(
        client,
        call_request.clone(),
        Some(block_number.into()),
        None,
    ...
    .await
    .unwrap_err();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::call(
        client,
        call_request.clone(),
        Some(block_number.into()),
        None,
    ...
    .await
    .unwrap_err();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::syncing(client)
        .await
        .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::send_transaction(
        client,
        transaction_request.clone(),
    )
    .await
    .unwrap_err();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::sign_transaction(
        client,
        transaction_request,
    )
    .await
    .unwrap_err();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::hashrate(client)
        .await
        .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::submit_hashrate(
        client,
        U256::default(),
        B256::default(),
    )
    .await
    .unwrap();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::gas_price(client)
        .await
        .unwrap_err();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::max_priority_fee_per_gas(client)
        .await
        .unwrap_err();
    SilApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::get_proof(
        client,
        address,
        vec![],
        None,
    ...
    SilCallBundleApiClient::call_bundle(client, Default::default()).await.unwrap_err();
}
...
async fn test_call_filter_functions_http() {
    ...
    let handle = launch_http(vec![RsilRpcModule::Sil]).await;
...
async fn test_call_admin_functions_http_and_ws() {
    ...
    let handle = launch_http_ws(vec![RsilRpcModule::Admin]).await;
    let client = handle.http_client().unwrap();
    test_basic_admin_calls(&client).await;
}
...
async fn test_call_eth_functions_ws() {
    ...
    let handle = launch_ws(vec![RsilRpcModule::Sil]).await;
    let client = handle.ws_client().await.unwrap();
    test_basic_eth_calls(&client).await;
}
...
async fn test_call_eth_functions_http_and_ws() {
    ...
    let handle = launch_http_ws(vec![RsilRpcModule::Sil]).await;
    let client = handle.http_client().unwrap();
    test_basic_eth_calls(&client).await;
}
...
async fn test_call_debug_functions_http() {
    ...
    let handle = launch_http(vec![RsilRpcModule::Debug]).await;
    let client = handle.http_client().unwrap();
...
async fn test_call_debug_functions_ws() {
    ...
    let handle = launch_ws(vec![RsilRpcModule::Debug]).await;
    let client = handle.ws_client().await.unwrap();
    test_basic_debug_calls(&client).await;
}
...
async fn test_call_debug_functions_http_and_ws() {
    ...
    let handle = launch_http_ws(vec![RsilRpcModule::Debug]).await;
    let client = handle.http_client().unwrap();
    test_basic_debug_calls(&client).await;
}
...
async fn test_call_net_functions_http() {
    ...
    let handle = launch_http(vec![RsilRpcModule::Net]).await;
    let client = handle.http_client().unwrap();
...
async fn test_call_net_functions_ws() {
    ...
    let handle = launch_ws(vec![RsilRpcModule::Net]).await;
    let client = handle.ws_client().await.unwrap();
    test_basic_net_calls(&client).await;
}
...
async fn test_call_net_functions_http_and_ws() {
    ...
    let handle = launch_http_ws(vec![RsilRpcModule::Net]).await;
    let client = handle.http_client().unwrap();
    test_basic_net_calls(&client).await;
}
...
async fn test_call_trace_functions_http() {
    ...
    let handle = launch_http(vec![RsilRpcModule::Trace]).await;
    let client = handle.http_client().unwrap();
...
async fn test_call_trace_functions_ws() {
    ...
    let handle = launch_ws(vec![RsilRpcModule::Trace]).await;
    let client = handle.ws_client().await.unwrap();
    test_basic_trace_calls(&client).await;
}
...
async fn test_call_trace_functions_http_and_ws() {
    ...
    let handle = launch_http_ws(vec![RsilRpcModule::Trace]).await;
    let client = handle.http_client().unwrap();
    test_basic_trace_calls(&client).await;
}
...
async fn test_call_web3_functions_http() {
    ...
    let handle = launch_http(vec![RsilRpcModule::Web3]).await;
    let client = handle.http_client().unwrap();
...
async fn test_call_web3_functions_ws() {
    ...
    let handle = launch_ws(vec![RsilRpcModule::Web3]).await;
    let client = handle.ws_client().await.unwrap();
    test_basic_web3_calls(&client).await;
}
...
async fn test_call_web3_functions_http_and_ws() {
    ...
    let handle = launch_http_ws(vec![RsilRpcModule::Web3]).await;
    let client = handle.http_client().unwrap();
    test_basic_web3_calls(&client).await;
}
...
async fn test_call_otterscan_functions_http() {
    ...
    let handle = launch_http(vec![RsilRpcModule::Ots]).await;
    let client = handle.http_client().unwrap();
...
async fn test_call_otterscan_functions_ws() {
    ...
    let handle = launch_ws(vec![RsilRpcModule::Ots]).await;
    let client = handle.ws_client().await.unwrap();
    test_basic_otterscan_calls(&client).await;
}
...
async fn test_call_otterscan_functions_http_and_ws() {
    ...
    let handle = launch_http_ws(vec![RsilRpcModule::Ots]).await;
    let client = handle.http_client().unwrap();
    test_basic_otterscan_calls(&client).await;
}

// <https://github.com/sila-chain/sila-rsil/issues/5830>
#[tokio::test(flavor = "multi_thread")]
