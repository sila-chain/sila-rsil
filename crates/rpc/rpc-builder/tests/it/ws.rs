...
/// Helper to launch a WS server with the Sil module.
async fn launch_ws_eth() -> rsil_rpc_builder::RpcServerHandle {
    launch_ws(vec![rsil_rpc_server_types::RsilRpcModule::Sil]).await
}
...
async fn test_eth_subscribe_pending_transactions_receives_tx() {
    ...
    use rsil_rpc_builder::RpcModuleBuilder;
    ...
    let tx = rsil_transaction_pool::test_utils::MockTransaction::sip1559();
