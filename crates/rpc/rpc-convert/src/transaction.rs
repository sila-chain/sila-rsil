...
use rsil_rpc_traits::{FromConsensusHeader, FromConsensusTx, TryIntoSimTx, TxInfoMapper};
...
pub trait RpcConvert: Send + Sync + Unpin + Debug + DynClone + 'static {
    ...
    type Savm: ConfigureEvm<Primitives = Self::Primitives>;
...
dyn_clone::clone_trait_object!(
    <Primitives, Network, Error, Savm>
    RpcConvert<Primitives = Primitives, Network = Network, Error = Error, Savm = Savm>
);
...
/// more context. For example, some configuration parameters or access handles to database, network,
/// etc.
pub trait TxEnvConverter<TxReq, Savm: ConfigureEvm>:
    Debug + Send + Sync + Unpin + Clone + 'static
{
    ...
    ) -> Result<TxEnvFor<Savm>, Self::Error>;
}

impl<TxReq, Savm> TxEnvConverter<TxReq, Savm> for ()
where
    TxReq: TryIntoTxEnv<TxEnvFor<Savm>, SpecFor<Savm>, BlockEnvFor<Savm>>,
    Savm: ConfigureEvm,
{
...
impl<F, TxReq, E, Savm> TxEnvConverter<TxReq, Savm> for F
where
    F: Fn(TxReq, &SavmEnvFor<Savm>) -> Result<TxEnvFor<Savm>, E>
        + Debug
        + Send
        + Sync
        + Unpin
...
/// Generic RPC response object converter for `Savm` and network `Network`.
///
...
/// network and SAVM associated primitives:
...
impl<Network, Savm, Receipt, Header, Map, SimTx, RpcTx, TxEnv> fmt::Debug
    for RpcConverter<Network, Savm, Receipt, Header, Map, SimTx, RpcTx, TxEnv>
{
...
impl<Network, Savm, Receipt> RpcConverter<Network, Savm, Receipt> {
...
impl<Network, Savm, Receipt, Header, Map, SimTx, RpcTx, TxEnv>
    RpcConverter<Network, Savm, Receipt, Header, Map, SimTx, RpcTx, TxEnv>
{
    /// Converts the network type
    pub fn with_network<N>(
        self,
    ) -> RpcConverter<N, Savm, Receipt, Header, Map, SimTx, RpcTx, TxEnv> {
        let Self {
            receipt_converter,
            header_converter,
            mapper,
    ...
    ) -> RpcConverter<Network, Savm, Receipt, Header, Map, SimTx, RpcTx, TxEnvNew> {
        let Self {
            receipt_converter,
            header_converter,
            mapper,
    ...
    ) -> RpcConverter<Network, Savm, Receipt, HeaderNew, Map, SimTx, RpcTx, TxEnv> {
        let Self {
            receipt_converter,
            header_converter: _,
            mapper,
    ...
    pub fn with_mapper<MapNew>(
        self,
        mapper: MapNew,
    ) -> RpcConverter<Network, Savm, Receipt, Header, MapNew, SimTx, RpcTx, TxEnv> {
        let Self {
            receipt_converter,
            header_converter,
            mapper: _,
    ...
    ) -> RpcConverter<Network, Savm, Receipt, Header, Map, SimTxNew, RpcTx, TxEnv> {
        let Self {
            receipt_converter,
            header_converter,
            mapper,
    ...
    ) -> RpcConverter<Network, Savm, Receipt, Header, Map, SimTx, RpcTxNew, TxEnv> {
        let Self {
            receipt_converter,
            header_converter,
            mapper,
...
impl<Network, Savm, Receipt, Header, Map, SimTx, RpcTx, TxEnv> Default
    for RpcConverter<Network, Savm, Receipt, Header, Map, SimTx, RpcTx, TxEnv>
where
    Receipt: Default,
    Header: Default,
...
impl<
        ...
        RpcTx: Clone,
        TxEnv: Clone,
    > Clone for RpcConverter<Network, Savm, Receipt, Header, Map, SimTx, RpcTx, TxEnv>
{
    fn clone(&self) -> Self {
        Self {
            network: Default::default(),
...
impl<N, Network, Savm, Receipt, Header, Map, SimTx, RpcTx, TxEnv> RpcConvert
    for RpcConverter<Network, Savm, Receipt, Header, Map, SimTx, RpcTx, TxEnv>
where
    N: NodePrimitives,
    ...
    TxEnv: TxEnvConverter<RpcTxReq<Network>, Savm>,
{
    type Primitives = N;
    type Savm = Savm;
