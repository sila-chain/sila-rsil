...
This crate contains end-to-end tests for Rsil's RPC implementation, including compatibility testing against the official execution-apis test suite.
...
async fn test_eth_get_logs_compat() -> Result<()> {
    ...
    let setup = Setup::<SilEngineTypes>::default()
...
1. Clone the execution-apis repository:
   ```bash
   git clone https://github.com/sila-chain/execution-apis.git
   ```
