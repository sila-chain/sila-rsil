...
use rsil_e2e_test_utils::testsuite::{actions::Action, BlockInfo, Environment};
    ...
    Engine: EngineTypes,
{
    fn execute<'a>(&'a mut self, env: &'a mut Environment<Engine>) -> BoxFuture<'a, Result<()>> {
        Box::pin(async move {
            ...
            let fcu_state = rsil_e2e_test_utils::setup_import::load_forkchoice_state(fcu_path)?;
            ...
            for (idx, client) in env.node_clients.iter().enumerate() {
                ...
                loop {
                    let response =
                        rsil_rpc_api::clients::EngineApiClient::<Engine>::fork_choice_updated_v3(
