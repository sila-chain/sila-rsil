...
/// All Engine API capabilities supported by Rsil (Sila sila-mainnet).
///
/// See <https://github.com/sila-chain/execution-apis/tree/main/src/engine> for updates.
    ...
    pub fn log_capability_mismatches(&self, cl_capabilities: &[String]) {
        ...
        if !critical_missing_in_el.is_empty() {
            warn!(
                ...
                "CL supports Engine API methods that Rsil doesn't. Consider upgrading Rsil."
            );
        }
        ...
        if !critical_missing_in_cl.is_empty() {
            warn!(
                ...
                "Rsil supports Engine API methods that CL doesn't. Consider upgrading your consensus client."
            );
        }
    }
}
...
pub struct CapabilityMismatches {
    /// Methods supported by CL but not by EL (Rsil).
    /// Operators should consider upgrading Rsil.
    pub missing_in_el: Vec<String>,
    /// Methods supported by EL (Rsil) but not by CL.
