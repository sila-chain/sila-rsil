...
use rsil_engine_primitives::{ConsensusEngineHandle, EngineApiValidator, EngineTypes};
use rsil_network_api::{CellCustody, NetworkInfo};
use rsil_payload_builder::PayloadStore;
use rsil_payload_primitives::{
    validate_payload_timestamp, EngineApiMessageVersion, MessageValidationKind,
    PayloadOrAttributes, PayloadTypes,
};
use rsil_primitives_traits::{Block, BlockBody};
use rsil_rpc_api::{EngineApiServer, IntoEngineApiRpcModule};
use rsil_storage_api::{BalProvider, BlockReader, HeaderProvider, StateProviderFactory};
...
/// use a custom [`EngineTypes`] implementation if it mirrors sila's versioned engine API
...
where
    ...
    ChainSpec: SilaHardforks + Send + Sync + 'static,
{
    ...
    /// See also <https://github.com/sila-chain/execution-apis/blob/3d627c95a4d3510a8187dd02e0250ecb4331d27e/src/engine/paris.md#engine_newpayloadv1>
    ...
    /// See also <https://github.com/sila-chain/execution-apis/blob/584905270d8ad665718058060267061ecfd79ca5/src/engine/shanghai.md#engine_newpayloadv2>
    ...
    /// See also <https://github.com/sila-chain/execution-apis/blob/fe8e13c288c592ec154ce25c534e26cb7ce0530d/src/engine/cancun.md#engine_newpayloadv3>
    ...
    /// See also <https://github.com/sila-chain/execution-apis/blob/7907424db935b93c2fe6a3c0faab943adebe8557/src/engine/prague.md#engine_newpayloadv4>
    ...
    ///
    /// Post-SilaAmsterdam payload handler.
    ///
    /// See also <https://github.com/sila-chain/execution-apis/blob/main/src/engine/amsterdam.md#engine_newpayloadv5>
    ...
    ChainSpec: SilaHardforks + Send + Sync + 'static,
{
    ...
    /// withdrawals.
    ///
    /// See also <https://github.com/sila-chain/execution-apis/blob/3d627c95a4d3510a8187dd02e0250ecb4331d27e/src/engine/paris.md#engine_forkchoiceUpdatedV1>
    ///
    /// Caution: This should not accept the `withdrawals` field
    ...
    ///
    /// See also <https://github.com/sila-chain/execution-apis/blob/3d627c95a4d3510a8187dd02e0250ecb4331d27e/src/engine/shanghai.md#engine_forkchoiceupdatedv2>
    ...
    ///
    /// See also  <https://github.com/sila-chain/execution-apis/blob/main/src/engine/cancun.md#engine_forkchoiceupdatedv3>
    ...
    ///
    /// See also  <https://github.com/sila-chain/execution-apis/blob/main/src/engine/amsterdam.md#engine_forkchoiceupdatedv4>
        ...
        EngineT::BuiltPayload: TryInto<R>,
    {
        ...
        // Enforces SilaOsaka restrictions on `getPayloadV4`.
    ...
    ///
    /// See also <https://github.com/sila-chain/execution-apis/blob/3d627c95a4d3510a8187dd02e0250ecb4331d27e/src/engine/paris.md#engine_getPayloadV1>
    ///
    ...
    /// payload build process at the time of receiving this call.
    ///
    /// See also <https://github.com/sila-chain/execution-apis/blob/3d627c95a4d3510a8187dd02e0250ecb4331d27e/src/engine/shanghai.md#engine_getpayloadv2>
    ///
    /// Note:
    /// > Provider software MAY stop the corresponding build process after serving this call.
    ...
    /// payload build process at the time of receiving this call.
    ///
    /// See also <https://github.com/sila-chain/execution-apis/blob/fe8e13c288c592ec154ce25c534e26cb7ce0530d/src/engine/cancun.md#engine_getpayloadv3>
    ///
    /// Note:
    /// > Provider software MAY stop the corresponding build process after serving this call.
    ...
    /// payload build process at the time of receiving this call.
    ///
    /// See also <https://github.com/sila-chain/execution-apis/blob/7907424db935b93c2fe6a3c0faab943adebe8557/src/engine/prague.md#engine_getpayloadv4>
    ///
    /// Note:
    /// > Provider software MAY stop the corresponding build process after serving this call.
    ...
    /// payload build process at the time of receiving this call.
    ///
    /// See also <https://github.com/sila-chain/execution-apis/blob/15399c2e2f16a5f800bf3f285640357e2c245ad9/src/engine/osaka.md#engine_getpayloadv5>
    ///
    /// Note:
    /// > Provider software MAY stop the corresponding build process after serving this call.
    ...
    ///
    /// Post-SilaAmsterdam payload handler that includes Block Access Lists (BAL).
    ///
    /// See also <https://github.com/sila-chain/execution-apis/blob/main/src/engine/amsterdam.md#engine_getpayloadv6>
    ...
    ///
    /// V2 includes the `block_access_list` field for SIP-7928 BAL support.
    ...
    ///   validated according to the SilaParis rules.
    /// * If the version is [`EngineApiMessageVersion::V2`], then the payload attributes will be
    ///   validated according to the SilaShanghai rules, as well as the validity changes from cancun:
    ///   <https://github.com/sila-chain/execution-apis/blob/584905270d8ad665718058060267061ecfd79ca5/src/engine/cancun.md#update-the-methods-of-previous-forks>
    ///
    /// * If the version above [`EngineApiMessageVersion::V3`], then the payload attributes will be
    ///   validated according to the SilaCancun rules.
    ...
    fn get_blobs_v1(
        ...
        // Only allow this method before SilaOsaka fork
        ...
        if self.inner.chain_spec.is_osaka_active_at_timestamp(current_timestamp) {
            return Err(EngineApiError::EngineObjectValidationError(
                rsil_payload_primitives::EngineObjectValidationError::UnsupportedFork,
            ));
        }
    ...
    fn get_blobs_v2(
        ...
        // Check if SilaOsaka fork is active
        let current_timestamp =
            SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap_or_default().as_secs();
        if !self.inner.chain_spec.is_osaka_active_at_timestamp(current_timestamp) {
            return Err(EngineApiError::EngineObjectValidationError(
...
// This is the concrete sila engine API implementation.
#[async_trait]
    ...
    ChainSpec: SilaHardforks + Send + Sync + 'static,
{
    ...
    /// See also <https://github.com/sila-chain/execution-apis/blob/03911ffc053b8b806123f1fc237184b0092a485a/src/engine/prague.md#engine_newpayloadv4>
    ...
    /// payload build process at the time of receiving this call.
    ///
    /// See also <https://github.com/sila-chain/execution-apis/blob/main/src/engine/prague.md#engine_getpayloadv4>
    ///
    /// Note:
    /// > Provider software MAY stop the corresponding build process after serving this call.
    ...
    /// See also <https://github.com/sila-chain/execution-apis/blob/6452a6b194d7db269bf1dbd087a267251d3cc7f8/src/engine/shanghai.md#engine_getpayloadbodiesbyhashv1>
    ...
    /// V2 includes the `block_access_list` field for SIP-7928 BAL support.
    ///
    /// See also <https://sips.sila.org/SIPS/sip-7928>
    ...
    ///
    /// See also <https://github.com/sila-chain/execution-apis/blob/6452a6b194d7db269bf1dbd087a267251d3cc7f8/src/engine/shanghai.md#engine_getpayloadbodiesbyrangev1>
    ///
    /// Returns the execution payload bodies by the range starting at `start`, containing `count`
    /// blocks.
    ///
    ...
    ///
    /// See also <https://github.com/sila-chain/execution-apis/blob/03911ffc053b8b806123f1fc237184b0092a485a/src/engine/identification.md>
    ...
    /// See also <https://github.com/sila-chain/execution-apis/blob/6452a6b194d7db269bf1dbd087a267251d3cc7f8/src/engine/common.md#capabilities>
...
mod tests {
    use super::*;
    use alloy_eips::{sip7685::Requests, NumHash};
    ...
    fn setup_engine_api() -> (
        ...
        let api = EngineApi::new(
            ...
            SilaEngineValidator::new(chain_spec.clone()),
            false,
            NoopNetwork::default(),
        );
    ...
    async fn get_payload_bodies_by_hash_v2_returns_block_access_list_from_store() {
        ...
        let api = EngineApi::new(
            ...
            client,
            EngineCapabilities::default(),
            SilaEngineValidator::new(chain_spec),
            false,
            NoopNetwork::default(),
        );
    ...
    async fn fcu_v4_updates_shared_cell_custody_when_payload_attrs_invalid() {
        ...
        assert_matches!(
            ...
            Err(EngineApiError::EngineObjectValidationError(
                rsil_payload_primitives::EngineObjectValidationError::PayloadAttributes(_)
            ))
        );
    }
    ...
    async fn fcu_v3_syncing_precedes_invalid_payload_attributes_validation() {
        ...
        let payload_attributes = PayloadAttributes {
            ...
            suggested_fee_recipient: Address::ZERO,
            withdrawals: Some(vec![]),
            // Invalid for V3/SilaCancun, but should be ignored if forkchoice is SYNCING.
            parent_beacon_block_root: None,
            slot_number: None,
            target_gas_limit: None,
        };
