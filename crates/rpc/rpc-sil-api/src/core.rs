...
/// Sil rpc interface: <https://sila.github.io/execution-apis/api-documentation>
...
>
{
    ...
    /// Returns the SIP-2718 encoded transaction if it exists.
    ///
    /// If this is an SIP-4844 transaction that is in the pool, it will include the sidecar.
    ...
    ///
    /// This method creates an [SIP2930](https://sips.sila.org/SIPS/sip-2930) type accessList based on a given Transaction.
    ///
    ...
    /// Introduced in SIP-4844, returns the current blob base fee in wei.
    #[method(name = "blobBaseFee")]
    async fn blob_base_fee(&self) -> RpcResult<U256>;

    /// Returns the Transaction fee history
    ///
    /// Introduced in SIP-1559 for getting information on the appropriate priority fee to use.
    ///
    ...
    /// Returns an Sila specific signature with: sign(keccak256("\x19Sila Signed Message:\n"
    /// + len(message) + message))).
    #[method(name = "sign")]
    ...
    /// Signs data via [SIP-712](https://github.com/sila-chain/SIPs/blob/master/SIPS/sip-712.md).
    ...
    /// Returns the SIP-7928 block access list for a given block id.
    ...
    jsonrpsee_types::error::ErrorObject<'static>: From<T::Error>,
{
    ...
    async fn call(
        ...
        Ok(SilCall::call(
            ...
            request,
            block_number,
            SavmOverrides::new(state_overrides, block_overrides),
        )
        .await?)
    }
    ...
    async fn estimate_gas(
        ...
        Ok(SilCall::estimate_gas_at(
            ...
            SavmOverrides::new(state_override, block_overrides),
        )
        .await?)
    }
    ...
    async fn block_access_list_by_block_hash(&self, block_hash: B256) -> RpcResult<Option<Value>> {
        ...
        let json = serde_json::to_value(&bal)
            .map_err(|e| SilApiError::Internal(rsil_errors::RsilError::msg(e.to_string())))?;
