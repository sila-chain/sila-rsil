...
use crate::{
    node::RpcNodeCoreExt, SilApiTypes, FromEthApiError, FullEthApiTypes, RpcBlock, RpcNodeCore,
    RpcReceipt,
};
...
use rsil_rpc_convert::{transaction::ConvertReceiptInput, RpcConvert, RpcHeader};
use rsil_storage_api::{BlockIdReader, BlockReader, ProviderHeader, ProviderReceipt, ProviderTx};
...
pub type BlockAndReceiptsResult<Sil> = Result<
    Option<(
        Arc<RecoveredBlock<<<Sil as RpcNodeCore>::Provider as BlockReader>::Block>>,
        Arc<Vec<ProviderReceipt<<Sil as RpcNodeCore>::Provider>>>,
    )>,
    <Sil as SilApiTypes>::Error,
>;

/// Block related functions for the [`SilApiServer`](crate::SilApiServer) trait in the
/// `eth_` namespace.
pub trait SilBlocks: LoadBlock<RpcConvert: RpcConvert<Primitives = Self::Primitives>> {
