...
pub trait SilCall: EstimateCall + Call + LoadPendingBlock + LoadBlock + FullEthApiTypes {
    ...
    ///
    /// See also: <https://github.com/sila-chain/go-sila/pull/27720>
    fn simulate_v1(
        &self,
        ...
        async move {
            ...
            let base_block = self
                ...
                .await?
                .ok_or_else(|| SilApiError::other(SilSimulateError::BlockNotFound { block }))?;
            ...
            self.spawn_with_state_at_block(block, move |this, db| {
                ...
                for block in block_state_calls {
                    ...
                    if !validation {
                        // If not explicitly required, we disable nonce check <https://github.com/sila-chain/sila-rsil/issues/16108>
                    ...
                    if let Some(block_overrides) = block_overrides {
                        ...
                        {
                            return Err(SilApiError::other(SilSimulateError::GasLimitReached).into())
                        }
                    ...
                    let (result, results) = if trace_transfers {
                        // prepare inspector to capture transfer inside the savm so they are recorded
                        // and included in logs
                        let inspector = TransferInspector::new(false).with_logs(true);
                        let savm = this
                            .evm_config()
                            .evm_with_env_and_inspector(&mut db, evm_env, inspector);
                        let mut builder = this.evm_config().create_block_builder(savm, &parent, ctx);
                        ...
                        let savm = this.evm_config().evm_with_env(&mut db, evm_env);
                        let mut builder = this.evm_config().create_block_builder(savm, &parent, ctx);
    ...
    ) -> impl Future<Output = Result<Vec<Vec<SilCallResponse>>, Self::Error>> + Send {
        async move {
            ...
            self.spawn_with_state_at_block(at, move |this, mut db| {
                ...
                for (bundle_index, bundle) in bundles.into_iter().enumerate() {
                    ...
                    for (tx_index, tx) in transactions.into_iter().enumerate() {
                        ...
                        // request
                        let overrides =
                            SavmOverrides::new(state_override.take(), block_overrides.clone());
                        ...
                        match Self::Error::ensure_success(res.result) {
                            Ok(output) => {
                                bundle_results
                                    .push(SilCallResponse { value: Some(output), error: None });
                            }
                            Err(err) => {
                                bundle_results.push(SilCallResponse {
                                    value: None,
    ...
    fn create_access_list_with(
        &self,
        mut evm_env: SavmEnvFor<Self::Savm>,
        at: BlockId,
        request: RpcTxReq<<Self::RpcConvert as RpcConvert>::Network>,
        state_override: Option<StateOverride>,
    ) -> impl Future<Output = Result<AccessListResult, Self::Error>> + Send
        ...
        Self: Trace,
    {
        self.spawn_blocking_io_fut(async move |this| {
            ...
            // See:
            // <https://github.com/sila-chain/go-sila/blob/8990c92aea01ca07801597b00c0d83d4e2d9b811/internal/ethapi/api.go#L1476-L1476>
            evm_env.cfg_env.disable_base_fee = true;
            ...
            // Disable SIP-7825 transaction gas limit cap so that the gas limit
            // fallback (block gas limit) is not rejected when it exceeds the
            // per-tx cap (2^24 ≈ 16.7M post-SilaOsaka).
            evm_env.cfg_env.tx_gas_limit_cap = Some(u64::MAX);
...
pub trait Call:
    LoadState<
        RpcConvert: RpcConvert<Savm = Self::Savm>,
    ...
    > + SpawnBlocking
{
    ...
    /// Returns the maximum memory the SAVM can allocate per RPC request.
    fn evm_memory_limit(&self) -> u64;

    /// Returns the max gas limit that the caller can afford given a transaction environment.
    fn caller_gas_allowance(
        &self,
        mut db: impl Database<Error: Into<SilApiError>>,
    ...
    fn transact_call_at(
        ...
        at: BlockId,
        overrides: SavmOverrides,
    ) -> impl Future<Output = Result<ResultAndState<HaltReasonFor<Self::Savm>>, Self::Error>> + Send
    where
    ...
    /// [`BlockingTaskPool`](rsil_tasks::pool::BlockingTaskPool).
    ...
    where
        ...
        F: FnOnce(
                TransactionInfo,
                ResultAndState<HaltReasonFor<Self::Savm>>,
                StateCacheDb,
            ) -> Result<R, Self::Error>
            + Send
            + 'static,
        ...
        I: IntoIterator<Item = Recovered<&'a ProviderTx<Self::Provider>>>,
    {
        ...
        for tx in transactions {
            ...
            savm.transact_commit(tx_env).map_err(Self::Error::from_evm_err)?;
            index += 1;
        }
        Ok(index)
    }
    ...
    fn create_txn_env(
        ...
        mut db: impl Database<Error: Into<SilApiError>>,
    ) -> Result<TxEnvFor<Self::Savm>, Self::Error> {
    ...
    /// ## SAVM settings
    ///
    /// This modifies certain SAVM settings to mirror geth's `SkipAccountChecks` when transacting requests, see also: <https://github.com/sila-chain/go-sila/blob/380688c636a654becc8f114438c2a5d93d2db032/core/state_transition.go#L145-L148>:
    ///
    ...
    fn prepare_call_env<DB>(
        ...
        db: &mut DB,
        overrides: SavmOverrides,
    ) -> Result<(SavmEnvFor<Self::Savm>, TxEnvFor<Self::Savm>), Self::Error>
    where
        DB: Database + DatabaseCommit + OverrideBlockHashes,
        SilApiError: From<<DB as Database>::Error>,
    {
        ...
        if let Some(requested_gas) = request.as_ref().gas_limit() {
            ...
            if global_gas_cap != 0 && global_gas_cap < requested_gas {
                warn!(target: "rpc::sil::call", ?request, ?global_gas_cap, "Capping gas limit to global gas cap");
        ...
        // gas limit): https://github.com/sila-chain/sila-rsil/issues/18577
        evm_env.cfg_env.disable_block_gas_limit = true;

        // Disabled because eth_call is sometimes used with eoa senders
        // See <https://github.com/sila-chain/sila-rsil/issues/1959>
        evm_env.cfg_env.disable_eip3607 = true;

        // The basefee should be ignored for eth_call
        // See:
        // <https://github.com/sila-chain/go-sila/blob/ee8e83fa5f6cb261dad2ed0a7bbcde4930c41e6c/internal/ethapi/api.go#L985>
        evm_env.cfg_env.disable_base_fee = true;

        // Disable SIP-7825 transaction gas limit to support larger transactions
        evm_env.cfg_env.tx_gas_limit_cap = Some(u64::MAX);

        // Disable additional fee charges, e.g. opstack operator fee charge
        // See:
        // <https://github.com/sila-chain/sila-rsil/issues/18470>
        evm_env.cfg_env.disable_fee_charge = true;

        evm_env.cfg_env.memory_limit = self.evm_memory_limit();

        // set nonce to None so that the correct nonce is chosen by the SAVM
        ...
        if let Some(state_overrides) = overrides.state {
            apply_state_overrides(state_overrides, db)
                .map_err(SilApiError::from_state_overrides_err)?;
        }

        let mut tx_env = self.create_txn_env(&evm_env, request, &mut *db)?;

        // lower the basefee to 0 to avoid breaking SAVM invariants (basefee < gasprice): <https://github.com/sila-chain/go-sila/blob/355228b011ef9a85ebc0f21e7196f892038d49f0/internal/ethapi/api.go#L700-L704>
        ...
        if !request_has_gas_limit {
            ...
            if tx_env.gas_price() > 0 {
                ...
                trace!(target: "rpc::sil::call", ?tx_env, "Applying gas limit cap with caller allowance");
