...
/// Fee related functions for the [`SilApiServer`](crate::SilApiServer) trait in the
/// `eth_` namespace.
pub trait SilFees:
    LoadFee<
    Provider: ChainSpecProvider<ChainSpec: SilChainSpec<Header = ProviderHeader<Self::Provider>>>,
>
{
    ...
    Self::Provider: BlockReaderIdExt,
{
    ...
    /// Returns the SIP-1559 fees if they are set, otherwise fetches a suggested gas price for
            ...
            let base_fee = match base_fee {
                ...
                None => {
                    ...
                    let pending_base_fee = self
                        ...
                        .ok_or(SilApiError::InvalidTransaction(
    ...
    /// Returns the SIP-4844 blob fee if it is set, otherwise fetches a blob fee.
    fn sip4844_blob_fee(
        &self,
    ...
    fn blob_base_fee(&self) -> impl Future<Output = Result<U256, Self::Error>> + Send {
        async move {
            self.provider()
                ...
                .ok_or(SilApiError::ExcessBlobGasNotSet.into())
                .map(U256::from)
        }
    }
