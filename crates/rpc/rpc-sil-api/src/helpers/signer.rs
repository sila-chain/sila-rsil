...
/// An Sila Signer used via RPC.
#[async_trait::async_trait]
pub trait SilSigner<T, TxReq = TransactionRequest>: Send + Sync + DynClone {
...
dyn_clone::clone_trait_object!(<T, TxReq> SilSigner<T, TxReq>);
...
mod tests {
    ...
    #[async_trait::async_trait]
    impl SilSigner<MockSignedTx, MockTxReq> for MockSigner {
        fn accounts(&self) -> Vec<Address> {
            Vec::new()
        }
    ...
    fn clones_trait_object_with_custom_tx_request_type() {
        let signer: Box<dyn SilSigner<MockSignedTx, MockTxReq>> = Box::new(MockSigner);
        let cloned: Box<dyn SilSigner<MockSignedTx, MockTxReq>> = dyn_clone::clone_box(&*signer);
