...
/// `Sil` API trait.
///
/// Defines core functionality of the `sil` API implementation.
#[auto_impl::auto_impl(&, Arc)]
pub trait SilApiSpec: RpcNodeCore + SilApiTypes {
    /// Returns the block node is started on.
    fn starting_block(&self) -> U256;

    /// Returns the current sila protocol version.
    fn protocol_version(&self) -> impl Future<Output = RsilResult<U64>> + Send {
        async move {
            let status = self.network().network_status().await.map_err(RsilError::other)?;
    ...
    fn chain_info(&self) -> RsilResult<ChainInfo> {
    ...
    fn capabilities(&self) -> RsilResult<SilCapabilities>
    where
        Self: SilState,
    {
        ...
        let state_proofs = SilCapabilitiesResource::window(proof_oldest, proof_window);

        Ok(SilCapabilities {
            head: SilCapabilitiesHead {
    ...
    fn sync_status(&self) -> RsilResult<SyncStatus> {
...
///
/// See also: <https://github.com/sila-chain/execution-apis/pull/755>
fn effective_resource(
    provider: &impl PruneCheckpointReader,
    segments: &[PruneSegment],
) -> RsilResult<SilCapabilitiesResource> {
    ...
    Ok(if disabled {
        SilCapabilitiesResource::disabled()
    } else if let Some(retention_blocks) = retention_blocks {
        SilCapabilitiesResource::window(oldest_block, retention_blocks)
    } else {
        SilCapabilitiesResource::available_from(oldest_block)
    })
}

/// A handle to [`SilSigner`]s with its generics set from [`TransactionsProvider`] and
/// [`rsil_rpc_convert::RpcTypes`].
pub type SignersForRpc<Provider, Rpc> = parking_lot::RwLock<
    Vec<Box<dyn SilSigner<<Provider as TransactionsProvider>::Transaction, RpcTxReq<Rpc>>>>,
>;

#[cfg(test)]
mod tests {
    use super::*;
    use rsil_prune_types::PruneCheckpoint;
    use rsil_storage_api::errors::provider::ProviderResult;
    ...
    fn full_prune_segment_without_retention_disables_resource() {
        ...
        assert_eq!(resource, SilCapabilitiesResource::disabled());
    }
    ...
    fn full_prune_segment_with_minimum_retention_uses_window() {
        ...
        assert_eq!(
            resource,
            SilCapabilitiesResource::window(11, PruneSegment::Bodies.min_blocks())
        );
    }
}
