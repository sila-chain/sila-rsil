...
pub trait SilState: LoadState + SpawnBlocking {
    ...
    fn get_proof(
        ...
        impl Future<Output = Result<SIP1186AccountProofResponse, Self::Error>> + Send,
        Self::Error,
    >
    where
        Self: SilApiSpec,
    ...
    > + RpcNodeCoreExt
{
    ...
    /// will only return canonical state. See also <https://github.com/sila-chain/sila-rsil/issues/4515>
    ...
    /// Returns the revm savm env for the given sealed header.
    fn evm_env_for_header(
        &self,
        header: &SealedHeaderFor<Self::Primitives>,
    ) -> Result<SavmEnvFor<Self::Savm>, Self::Error> {
        self.evm_config()
            .evm_env(header)
            .map_err(RsilError::other)
            .map_err(Self::Error::from_eth_err)
    ...
    fn evm_env_at(
        &self,
        at: BlockId,
    ) -> impl Future<Output = Result<(SavmEnvFor<Self::Savm>, BlockId), Self::Error>> + Send
    where
        Self: SpawnBlocking,
    {
        async move {
    ...
    /// Returns the recovered block, revm savm env, and state block id for the requested [`BlockId`].
    ///
    ...
    fn evm_env_and_recovered_block_at(
        ...
        Output = Result<
            (Arc<RecoveredBlock<BlockTy<Self::Primitives>>>, SavmEnvFor<Self::Savm>, BlockId),
            Self::Error,
        >,
    > + Send
    where
        ...
        self.spawn_blocking_io(move |this| {
                ...
                this.pool().get_highest_consecutive_transaction_by_sender(address, next_nonce)
            {
                ...
                next_nonce = highest_tx.nonce().checked_add(1).ok_or_else(|| {
                    Self::Error::from(SilApiError::InvalidTransaction(
