...
pub trait Trace: LoadState<Error: FromEvmError<Self::Savm>> + Call {
        ...
        I: InspectorFor<Self::Savm, DB>,
    {
        ...
        savm.transact(tx_env).map_err(Self::Error::from_evm_err)
    }
    ...
    where
        ...
        F: FnOnce(
            ...
            + Send
            + 'static,
        Insp: for<'a> InspectorFor<Self::Savm, &'a mut StateCacheDb> + Send + 'static,
        R: Send + 'static,
    {
        async move {
    ...
    where
        ...
        F: Fn(
                ...
                TracingCtx<
                    ...
                    SavmFor<Self::Savm, &mut StateCacheDb, TracingInspector>,
                >,
            ) -> Result<R, Self::Error>
            + Send
            + 'static,
    ...
    where
        ...
        F: Fn(
                ...
                TracingCtx<
                    '_,
                    Recovered<&ProviderTx<Self::Provider>>,
                    SavmFor<Self::Savm, &mut StateCacheDb, Insp>,
                >,
            ) -> Result<R, Self::Error>
            + Send
            + 'static,
        Setup: FnMut() -> Insp + Send + 'static,
        Insp: Clone + for<'a> InspectorFor<Self::Savm, &'a mut StateCacheDb>,
        R: Send + 'static,
    {
        async move {
            let block =
    ...
    /// 3. loops over all transactions and executes them
    /// 4. calls the callback with the transaction info, the execution result, the changed state
    ///    _after_ the transaction `SavmState` and the database that points to the state right
