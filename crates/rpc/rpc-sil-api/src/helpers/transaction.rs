...
/// Transaction related functions for the [`SilApiServer`](crate::SilApiServer) trait in
/// the `eth_` namespace.
///
/// This includes utilities for transaction tracing, transacting and inspection.
///
/// Async functions that are spawned onto the
/// [`BlockingTaskPool`](rsil_tasks::pool::BlockingTaskPool) begin with `spawn_`
///
/// ## Calls
///
...
/// Geth also disables the basefee check for tracing: <https://github.com/sila-chain/go-sila/blob/bc0b87ca196f92e5af49bd33cc190ef0ec32b197/sil/tracers/api.go#L955-L955>
...
///
/// See also <https://github.com/sila-chain/sila-rsil/issues/6240>
///
/// This implementation follows the behaviour of Geth and disables the basefee check for tracing.
pub trait SilTransactions: LoadTransaction<Provider: BlockReaderIdExt> {
        ...
        Self: LoadReceipt + 'static,
    {
        ...
        async move {
            ...
            let hash = SilTransactions::send_raw_transaction(&this, tx).await?;
    ...
    /// Returns the SIP-2718 encoded transaction by hash.
    ///
    ...
    where
        Self: SilApiSpec + LoadBlock + EstimateCall,
    {
        async move {
            ...
            let estimated_gas = self
                .estimate_gas_at(request.clone(), BlockId::pending(), SavmOverrides::default())
                .await?;
    ...
    where
        Self: SilApiSpec + LoadBlock + EstimateCall + LoadFee,
    {
        async move {
    ...
    /// Returns the SIP-2718 encoded signed transaction.
    fn sign_transaction(
        &self,
    ...
    /// Encodes and signs the typed data according SIP-712. Payload must implement Sip712 trait.
    ...
    fn find_signer(
        ...
        Box<dyn SilSigner<ProviderTx<Self::Provider>, RpcTxReq<Self::NetworkTypes>> + 'static>,
        Self::Error,
    > {
        self.signers()
            .read()
    ...
    > + Send {
        async move {
                ...
                .await?
            {
                ...
                // check for pre SIP-2 because this transaction could be pre-SIP-2.
                let transaction = tx
                    .try_into_recovered_unchecked()
                    .map_err(|_| SilApiError::InvalidTransactionSignature)?;
