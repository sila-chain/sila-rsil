//! Configuration for `sil` namespace APIs.
...
/// Additional config values for the sil namespace.
#[derive(Debug, Clone, Eq, PartialEq, Serialize, Deserialize)]
pub struct SilConfig {
    /// Settings for the caching layer
    pub cache: SilStateCacheConfig,
    ...
    ///
    /// Requests such as `eth_call`, `eth_estimateGas` and alike require savm execution, which is
    ...
    ///
    /// This is disabled by default, allowing blob transactions with SIP-4844 sidecars to be
...
impl SilConfig {
    ...
    pub const fn state_cache(mut self, cache: SilStateCacheConfig) -> Self {
        self.cache = cache;
        self
    }
    ...
    /// Configures the maximum memory the SAVM can allocate per RPC request.
    pub const fn rpc_evm_memory_limit(mut self, memory_limit: u64) -> Self {
        self.rpc_evm_memory_limit = memory_limit;
        self
    }

    /// Configures whether to force upcasting SIP-4844 blob sidecars to SIP-7594 format.
...
impl Default for SilFilterConfig {
    fn default() -> Self {
        Self {
