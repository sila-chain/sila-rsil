...
use rsil_primitives_traits::{Block, BlockBody, InMemorySize, NodePrimitives, RecoveredBlock};
use rsil_revm::{
    bytecode::Bytecode,
...
use rsil_storage_api::{BalProvider, BlockReader, TransactionVariant};
...
/// Provides async access to cached sil data
///
/// This is the frontend for the async caching service which manages cached data on a different
/// task.
#[derive(Debug)]
pub struct SilStateCache<N: NodePrimitives> {
    to_service: UnboundedSender<CacheAction<N::Block, N::Receipt>>,
}

impl<N: NodePrimitives> Clone for SilStateCache<N> {
    fn clone(&self) -> Self {
        Self { to_service: self.to_service.clone() }
    }
}

impl<N: NodePrimitives> SilStateCache<N> {
    /// Creates and returns both [`SilStateCache`] frontend and the memory bound service.
    fn create<Provider>(
        provider: Provider,
        action_task_spawner: Runtime,
        config: SilStateCacheConfig,
    ) -> (Self, SilStateCacheService<Provider, Runtime>)
    where
        Provider: BlockReader<Block = N::Block, Receipt = N::Receipt> + BalProvider,
    {
        let SilStateCacheConfig {
            max_blocks,
            max_receipts,
            max_headers,
            max_bals,
        ...
        let service = SilStateCacheService {
            provider,
            ...
            + 'static,
    {
        ...
        executor.spawn_critical_task("sil state cache", service);
        this
    }
...
/// A task that manages caches for data required by the `sil` rpc implementation.
///
/// It provides a caching layer on top of the given
/// [`StateProvider`](rsil_storage_api::StateProvider) and keeps data fetched via the provider in
...
/// [`SilStateCache`] via a channel. If the requested data is not cached then it spawns a new task
...
/// `rsil_rpc::SilApi` which is typically invoked by the RPC server, which already uses
/// permits to limit concurrent requests.
#[must_use = "Type does nothing unless spawned"]
pub(crate) struct SilStateCacheService<
    Provider,
    Tasks,
...
impl<Provider> SilStateCacheService<Provider, Runtime>
where
...
impl<Provider> Future for SilStateCacheService<Provider, Runtime>
where
    Provider: BlockReader + BalProvider + Clone + Unpin + 'static,
{
    type Output = ();
...
pub async fn cache_new_blocks_task<St, N: NodePrimitives>(
    eth_state_cache: SilStateCache<N>,
    mut events: St,
) where
...
mod tests {
    ...
    fn test_service() -> SilStateCacheService<NoopProvider, Runtime> {
        let (_cache, service) = SilStateCache::<SilPrimitives>::create(
    ...
    async fn get_bal_uses_cached_revm_bal() {
        ...
        let cache = SilStateCache::<SilPrimitives>::spawn_with(
            provider,
            SilStateCacheConfig {
                max_blocks: 0,
                max_receipts: 0,
    ...
    impl BalStore for TestBalStore {
        fn insert(&self, _num_hash: NumHash, _bal: rsil_storage_api::RawBal) -> ProviderResult<()> {
            Ok(())
        }
        ...
        fn bal_stream(&self) -> rsil_storage_api::BalNotificationStream {
            rsil_storage_api::NoopBalStore.bal_stream()
        }
    }
    ...
    impl BlockNumReader for TestBalProvider {
        fn chain_info(&self) -> ProviderResult<rsil_chainspec::ChainInfo> {
            Ok(rsil_chainspec::ChainInfo::default())
        }
