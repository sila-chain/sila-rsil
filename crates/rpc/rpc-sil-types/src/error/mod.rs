...
pub enum SilApiError {
    ...
    ///
    /// See also <https://sips.sila.org/SIPS/sip-4444>
...
impl SilApiError {
    /// Creates a new [`SilApiError::Other`] variant.
    pub fn other<E: ToRpcError>(err: E) -> Self {
        Self::Other(Box::new(err))
    }

    /// Creates a new [`SilApiError::CallManyError`] variant.
    ...
    /// Returns the [`RpcInvalidTransactionError`] if this is a [`SilApiError::InvalidTransaction`]
    ...
    /// Converts the given [`StateOverrideError`] into a new [`SilApiError`] instance.
    ...
    /// Converts the given [`CallError`] into a new [`SilApiError`] instance.
...
impl From<SilApiError> for jsonrpsee_types::error::ErrorObject<'static> {
    fn from(error: SilApiError) -> Self {
        match error {
            ...
            SilApiError::InternalJsTracerError(msg) => internal_rpc_err(msg),
            SilApiError::InvalidParams(msg) => invalid_params_rpc_err(msg),
            err @ SilApiError::ExecutionTimedOut(_) => rpc_error_with_code(
            ...
            SilApiError::PrunedHistoryUnavailable => rpc_error_with_code(4444, error.to_string()),
            SilApiError::Other(err) => err.to_rpc_error(),
            SilApiError::MuxTracerError(msg) => internal_rpc_err(msg.to_string()),
            SilApiError::BatchTxRecvError(err) => internal_rpc_err(err.to_string()),
            SilApiError::BatchTxSendError => {
                internal_rpc_err("Batch transaction sender channel closed".to_string())
            }
            SilApiError::CallManyError { bundle_index, tx_index, error } => {
            ...
            SilApiError::BlockAccessListNotAvailablePreAmsterdam => {
...
impl<E> From<CallError<E>> for SilApiError
where
    E: Into<Self>,
{
...
impl<E> From<StateOverrideError<E>> for SilApiError
where
    E: Into<Self>,
{
...
impl From<SilTxEnvError> for SilApiError {
    fn from(value: SilTxEnvError) -> Self {
        match value {
            SilTxEnvError::CallFees(CallFeesError::BlobTransactionMissingBlobHashes) => {
                Self::InvalidTransaction(
                    RpcInvalidTransactionError::BlobTransactionMissingBlobHashes,
                )
            }
            SilTxEnvError::CallFees(CallFeesError::FeeCapTooLow) => {
                Self::InvalidTransaction(RpcInvalidTransactionError::FeeCapTooLow)
            }
            SilTxEnvError::CallFees(CallFeesError::ConflictingFeeFieldsInRequest) => {
                Self::ConflictingFeeFieldsInRequest
            }
            SilTxEnvError::CallFees(CallFeesError::TipAboveFeeCap) => {
                Self::InvalidTransaction(RpcInvalidTransactionError::TipAboveFeeCap)
            }
            SilTxEnvError::CallFees(CallFeesError::TipVeryHigh) => {
                Self::InvalidTransaction(RpcInvalidTransactionError::TipVeryHigh)
            }
            SilTxEnvError::Input(err) => Self::TransactionInputError(err),
        }
    }
}
...
impl From<BalError> for SilApiError {
    fn from(err: BalError) -> Self {
        Self::SavmCustom(format!("bal error: {:?}", err))
    }
}

#[cfg(feature = "js-tracer")]
impl From<revm_inspectors::tracing::js::JsInspectorError> for SilApiError {
...
impl<Err> From<DebugInspectorError<Err>> for SilApiError
where
    Err: core::error::Error + Send + Sync + 'static,
{
    fn from(error: DebugInspectorError<Err>) -> Self {
        match error {
            ...
            DebugInspectorError::Database(err) => Self::Internal(RsilError::other(err)),
            #[cfg(feature = "js-tracer")]
...
impl From<RsilError> for SilApiError {
    fn from(error: RsilError) -> Self {
        match error {
            RsilError::Provider(err) => err.into(),
            err => Self::Internal(err),
        }
    }
}

impl From<BlockExecutionError> for SilApiError {
    fn from(error: BlockExecutionError) -> Self {
        match error {
            BlockExecutionError::Validation(validation_error) => match validation_error {
                ...
                _ => Self::Internal(RsilError::Execution(BlockExecutionError::Validation(
                    validation_error,
                ))),
            },
            BlockExecutionError::Internal(internal_error) => {
                Self::Internal(RsilError::Execution(BlockExecutionError::Internal(internal_error)))
            }
        }
    }
}

impl From<rsil_errors::ProviderError> for SilApiError {
    fn from(error: rsil_errors::ProviderError) -> Self {
...
impl From<InvalidHeader> for SilApiError {
        ...
        match err {
            EVMError::Transaction(invalid_tx) => {
                ...
                if let Some(eth_tx_err) = invalid_tx.as_invalid_tx_err() {
                    ...
                    Self::SavmCustom(invalid_tx.to_string())
                }
            }
            EVMError::Header(err) => err.into(),
            EVMError::Database(err) => err.into(),
            EVMError::Custom(err) => Self::SavmCustom(err),
            EVMError::CustomAny(err) => Self::SavmCustom(err.to_string()),
        }
    }
}

impl From<RecoveryError> for SilApiError {
    fn from(_: RecoveryError) -> Self {
        Self::InvalidTransactionSignature
    }
}

impl From<RawPoolTransactionError> for SilApiError {
...
impl From<Infallible> for SilApiError {
...
pub enum RpcInvalidTransactionError {
    ...
    /// SIP-7702 is not enabled.
    #[error("SIP-7702 authorization list not supported")]
    AuthorizationListNotSupported,
    /// SIP-7702 transaction has invalid fields set.
    #[error("SIP-7702 authorization list has invalid fields")]
    ...
    fn from(err: InvalidTransaction) -> Self {
        match err {
            ...
            InvalidTransaction::GasFloorMoreThanGasLimit { .. } => {
                // Post prague SIP-7623 tx floor calldata gas cost > tx.gas_limit
...
pub enum RpcPoolError {
    ...
    ExceedsFeeCap {
        /// max fee in wei of new tx submitted to the pool (e.g. 0.11534 SIL)
        max_tx_fee_wei: u128,
        /// configured tx fee cap in wei (e.g. 1.0 SIL)
        tx_fee_cap_wei: u128,
    },
    ...
    /// SIP-4844 related error
    #[error(transparent)]
    Sip4844(#[from] Sip4844PoolTransactionError),
    /// SIP-7702 related error
    #[error(transparent)]
    Sip7702(#[from] Sip7702PoolTransactionError),
...
impl From<InvalidPoolTransactionError> for RpcPoolError {
    fn from(err: InvalidPoolTransactionError) -> Self {
        match err {
            ...
            InvalidPoolTransactionError::Sip2681 => {
                Self::Invalid(RpcInvalidTransactionError::NonceMaxValue)
            }
            InvalidPoolTransactionError::Other(err) => Self::PoolTransactionError(err),
            InvalidPoolTransactionError::Sip4844(err) => Self::Sip4844(err),
            InvalidPoolTransactionError::Sip7702(err) => Self::Sip7702(err),
...
impl From<PoolError> for SilApiError {
    fn from(err: PoolError) -> Self {
...
mod tests {
    ...
    #[test]
    fn timed_out_error() {
        let err = SilApiError::ExecutionTimedOut(Duration::from_secs(10));
    ...
    fn header_not_found_message() {
        let err: jsonrpsee_types::error::ErrorObject<'static> =
            SilApiError::HeaderNotFound(BlockId::hash(b256!(
        ...
        let err: jsonrpsee_types::error::ErrorObject<'static> =
            SilApiError::HeaderNotFound(BlockId::hash_canonical(b256!(
                "0x1a15e3c30cf094a99826869517b16d185d45831d3a494f01030b0001a9d3ebb9"
            )))
            .into();
        assert_eq!(
        ...
        let err: jsonrpsee_types::error::ErrorObject<'static> =
            SilApiError::HeaderNotFound(BlockId::number(100000)).into();
        assert_eq!(err.message(), "block not found: 0x186a0");
        let err: jsonrpsee_types::error::ErrorObject<'static> =
            SilApiError::HeaderNotFound(BlockId::latest()).into();
        assert_eq!(err.message(), "block not found: latest");
        let err: jsonrpsee_types::error::ErrorObject<'static> =
            SilApiError::HeaderNotFound(BlockId::safe()).into();
        assert_eq!(err.message(), "block not found: safe");
        let err: jsonrpsee_types::error::ErrorObject<'static> =
            SilApiError::HeaderNotFound(BlockId::finalized()).into();
    ...
    fn receipts_not_found_message() {
        let err: jsonrpsee_types::error::ErrorObject<'static> =
            SilApiError::ReceiptsNotFound(BlockId::hash(b256!(
                "0x1a15e3c30cf094a99826869517b16d185d45831d3a494f01030b0001a9d3ebb9"
            )))
            .into();
        assert_eq!(
        ...
        let err: jsonrpsee_types::error::ErrorObject<'static> =
            SilApiError::ReceiptsNotFound(BlockId::hash_canonical(b256!(
                "0x1a15e3c30cf094a99826869517b16d185d45831d3a494f01030b0001a9d3ebb9"
            )))
            .into();
        assert_eq!(
        ...
        assert_eq!(err.message(), "block not found: latest");
        let err: jsonrpsee_types::error::ErrorObject<'static> =
            SilApiError::ReceiptsNotFound(BlockId::safe()).into();
        assert_eq!(err.message(), "block not found: safe");
        let err: jsonrpsee_types::error::ErrorObject<'static> =
            SilApiError::ReceiptsNotFound(BlockId::finalized()).into();
        assert_eq!(err.message(), "block not found: finalized");
        let err: jsonrpsee_types::error::ErrorObject<'static> =
            SilApiError::ReceiptsNotFound(BlockId::pending()).into();
        assert_eq!(err.message(), "block not found: pending");
        let err: jsonrpsee_types::error::ErrorObject<'static> =
            SilApiError::ReceiptsNotFound(BlockId::earliest()).into();
