...
use rsil_primitives_traits::{Block, BlockBody, NodePrimitives, SealedBlock};
use rsil_rpc_server_types::constants::gas_oracle::MAX_HEADER_HISTORY;
...
use super::{SilApiError, SilStateCache};
    ...
    R: TxReceipt,
{
    ...
    // SIP-8037 can make header gas used exceed the sum of per-transaction receipt gas.
...
pub struct FeeHistoryEntry<H = Header> {
    ...
    /// The base per blob gas for SIP-4844.
    /// For pre SIP-4844 equals to zero.
    ...
    /// Will be zero if no blob gas was used or pre SIP-4844.
    ...
    H: BlockHeader + Clone,
{
    ...
    /// Returns the blob fee for the next block according to the SIP-4844 spec.
    ///
    ...
    /// Calculate excess blob gas for the next block according to the SIP-4844 spec.
    ///
    /// Returns a `None` if no excess blob gas is set, no SIP-4844 support
...
mod tests {
    ...
    use rsil_sila_primitives::{Receipt, Transaction as SilTransaction, TransactionSigned};
    ...
    fn reward_percentiles_use_receipt_gas_weight_for_eip8037() {
        ...
        let transactions =
            [sip1559_transaction(low_tip, BASE_FEE), sip1559_transaction(high_tip, BASE_FEE)];
    ...
    fn sip1559_transaction(tip: u128, base_fee: u64) -> TransactionSigned {
        TransactionSigned::new_unhashed(
            SilTransaction::Sip1559(TxEip1559 {
    ...
    fn receipt(cumulative_gas_used: u64) -> Receipt {
        Receipt { tx_type: TxType::Sip1559, success: true, cumulative_gas_used, logs: Vec::new() }
    }
}
