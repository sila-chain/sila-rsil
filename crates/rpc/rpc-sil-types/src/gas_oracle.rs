//! An implementation of the sil gas price oracle, used for providing gas price estimates based on
//! previous blocks.

use super::{RpcInvalidTransactionError, SilApiError, SilResult, SilStateCache};
...
use rsil_storage_api::{BlockReaderIdExt, NodePrimitivesProvider};
    ...
    Provider: NodePrimitivesProvider,
{
    ...
    cache: SilStateCache<Provider::Primitives>,
    ...
    Provider: BlockReaderIdExt + NodePrimitivesProvider,
{
    ...
    pub async fn suggest_tip_cap(&self) -> SilResult<U256> {
        let header = self
            .provider
            ...
            let (parent_hash, block_values) =
                if let Some(vals) = inner.lowest_effective_tip_cache.get(&current_hash) {
                    ...
                    let (parent_hash, block_values) = self
                        ...
                        .await?
                        .ok_or(SilApiError::HeaderNotFound(current_hash.into()))?;
                    inner
    ...
    async fn get_block_values(
        ...
        block_hash: B256,
        limit: usize,
    ) -> SilResult<Option<(B256, Vec<U256>)>> {
    ...
    pub async fn op_suggest_tip_cap(&self, min_suggested_priority_fee: U256) -> SilResult<U256> {
        let header = self
            .provider
            .sealed_header_by_number_or_tag(BlockNumberOrTag::Latest)?
            .ok_or(SilApiError::HeaderNotFound(BlockId::latest()))?;
        ...
        let receipts = self
            ...
            .await?
            .ok_or(SilApiError::ReceiptsNotFound(BlockId::latest()))?;
    ...
    pub async fn get_block_median_tip(&self, block_hash: B256) -> SilResult<Option<U256>> {
        // check the cache (this will hit the disk if the block is not cached)
        let Some(block) = self.cache.get_recovered_block(block_hash).await? else {
            return Ok(None);
        };
