...
impl ToRpcError for SilSimulateError {
...
) -> Result<Vec<SimBlock<TxReq>>, SilApiError>
where
    H: BlockHeader,
{
    ...
    for mut block in blocks {
        ...
        if target_number <= prev_number {
            return Err(SilApiError::other(SilSimulateError::BlockNumberInvalid {
                got: target_number,
                parent: prev_number,
            }));
        }
        ...
        if target_number.saturating_sub(base_number) > max_simulate_blocks {
            return Err(SilApiError::other(SilSimulateError::TooManyBlocks));
        }
        ...
        let block_time = if let Some(t) = overrides.time {
            if t <= prev_timestamp {
                return Err(SilApiError::other(SilSimulateError::BlockTimestampInvalid {
                    got: t,
...
/// Applies precompile move overrides from state overrides to the SAVM's precompiles map.
///
    ...
    T: RpcConvert<Primitives = S::Primitives>,
{
    ...
    let block_gas_limit = builder.savm().block().gas_limit();
    let is_amsterdam = builder.savm().cfg_env().enable_amsterdam_eip8037;
    let tx_gas_limit_cap = builder.savm().cfg_env().tx_gas_limit_cap.unwrap_or(u64::MAX);
    for mut call in calls {
        ...
        let tx = resolve_transaction(
            call,
            default_gas_limit,
            builder.savm().block().basefee(),
            chain_id,
            builder.savm().cfg_env().disable_nonce_check,
...
pub fn resolve_transaction<DB: Database, Tx, T>(
    ...
    db: &mut DB,
    converter: &T,
) -> Result<Recovered<Tx>, SilApiError>
where
    DB::Error: Into<SilApiError>,
    T: RpcConvert<Primitives: NodePrimitives<SignedTx = Tx>>,
{
    ...
    let tx =
        converter.build_simulate_v1_transaction(tx).map_err(|e| SilApiError::other(e.into()))?;
...
pub fn build_simulated_block<Err, T>(
    ...
    results: Vec<ExecutionResult<HaltReasonFor<T::Savm>>>,
...
mod tests {
    use super::{
        apply_precompile_overrides, sanitize_chain, SilSimulateError, INTERNAL_ERROR_CODE,
    };
    ...
    fn nonce_max_value_error_uses_internal_error_code() {
        let err = SilSimulateError::NonceMaxValue.to_rpc_error();
    ...
    fn block_not_found_error_uses_simulate_code() {
        let err = SilSimulateError::BlockNotFound { block: 100000.into() }.to_rpc_error();
    ...
    fn precompile_self_move_requires_existing_precompile() {
        ...
        assert!(matches!(err, SilSimulateError::NotAPrecompile(addr) if addr == address));
    }
    ...
    fn precompile_self_move_errors_for_existing_precompile() {
        ...
        assert!(matches!(err, SilSimulateError::MovePrecompileToSelf(addr) if addr == address));
    }
    ...
    fn sanitize_chain_fills_gaps_with_empty_blocks() {
        ...
        // SilaMainnet hint is 12s, so timestamps should auto-increment from 100 by 12.
        let times: Vec<u64> =
