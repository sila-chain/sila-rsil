...
use super::{SilApiError, SilResult};
use alloy_consensus::TxReceipt;
use rsil_primitives_traits::{Recovered, SignedTransaction};
...
pub fn recover_raw_transaction<T: SignedTransaction>(data: &[u8]) -> SilResult<Recovered<T>> {
    ...
    let transaction =
        T::decode_2718_exact(data).map_err(|_| SilApiError::FailedToDecodeSignedTransaction)?;
