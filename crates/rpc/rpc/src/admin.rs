...
use rsil_rpc_api::AdminApiServer;
use rsil_rpc_server_types::ToRpcResult;
...
where
    ...
    ChainSpec: SilChainSpec + SilaHardforks + Send + Sync + 'static,
    Pool: TransactionPool + 'static,
{
        ...
        for peer in peers {
            infos.push(PeerInfo {
                ...
                protocols: PeerProtocolInfo {
                    sil: Some(SilPeerInfo::Info(SilInfo { version: peer.status.version as u64 })),
                    snap: None,
    ...
    async fn node_info(&self) -> RpcResult<NodeInfo> {
            ...
            ($config:expr, [$( $field:ident => $fork:ident,)*]) => {
                $(
                    ...
                    if $config.$field.is_none() {
                        $config.$field = match self.chain_spec.sila_fork_activation(SilaHardfork::$fork) {
        ...
        set_block_or_time!(config, [
            ...
            dao_fork_block => Dao,
            sip150_block => Tangerine,
            sip155_block => SpuriousDragon,
            sip158_block => SpuriousDragon,
            ...
            shanghai_time => SilaShanghai,
            cancun_time => SilaCancun,
            prague_time => SilaPrague,
        ]);
