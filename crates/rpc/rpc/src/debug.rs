...
use alloy_rpc_types_eth::{
    state::SavmOverrides, Account, AccountInfo, BlockError, Bundle, Index, StateContext,
};
...
use rsil_evm::{block::BlockExecutor, execute::Executor, ConfigureEvm, SavmEnvFor};
...
pub struct DebugApi<Sil: RpcNodeCore> {
    inner: Arc<DebugApiInner<Sil>>,
}

impl<Sil> DebugApi<Sil>
where
    Sil: RpcNodeCore,
{
    ...
    Sil: TraceExt,
{
    ...
    async fn trace_block(
        &self,
        block: Arc<RecoveredBlock<ProviderBlock<Sil::Provider>>>,
    ...
    ///
    /// Ref: <https://geth.sila.org/docs/developers/savm-tracing/built-in-tracers>
    ...
    ///  - `debug_traceCall` executes with __enabled__ basefee check, `eth_call` does not: <https://github.com/sila-chain/sila-rsil/issues/6240>
    pub async fn debug_trace_call(
        &self,
        call: RpcTxReq<Sil::NetworkTypes>,
        ...
        let overrides = SavmOverrides::new(state_overrides, block_overrides.map(Box::new));
    ...
    pub async fn debug_trace_call_many(
        &self,
        bundles: Vec<Bundle<RpcTxReq<Sil::NetworkTypes>>>,
    ...
    pub async fn debug_account_at(
        ...
        tx_index: Index,
        address: Address,
    ) -> Result<Option<Account>, Sil::Error> {
    ...
    pub async fn debug_account_info_at(
        ...
        tx_index: Index,
        address: Address,
    ) -> Result<Option<AccountInfo>, Sil::Error> {
    ...
    async fn replay_block_until<F, R>(
        ...
        tx_index: Index,
        f: F,
    ) -> Result<Option<R>, Sil::Error>
    where
        F: FnOnce(&mut StateCacheDb) -> Result<R, Sil::Error> + Send + 'static,
        R: Send + 'static,
    {
        let block = self
            .eth_api()
    ...
    fn account_info<DB>(db: &mut DB, address: Address) -> Result<AccountInfo, Sil::Error>
    where
        DB: Database,
...
#[async_trait]
impl<Sil> DebugApiServer<RpcTxReq<Sil::NetworkTypes>> for DebugApi<Sil>
where
    Sil: SilTransactions + TraceExt,
{
    ...
    ///
    /// If this is a pooled SIP-4844 transaction, the blob sidecar is included.
    ///
    ...
    async fn raw_transactions(&self, block_id: BlockId) -> RpcResult<Vec<Bytes>> {
        let block: RecoveredBlock<BlockTy<Sil::Primitives>> = self
            .provider()
        ...
        for entry in entries {
            ...
            let bad_block =
                serde_json::to_value(BadBlockSerde { block, hash, rlp, reason: entry.reason })
                    .map_err(|err| SilApiError::other(internal_rpc_err(err.to_string())))?;
    ...
    ///   See Geth impl: <https://github.com/sila-chain/go-sila/blob/737ffd1bf0cbee378d0111a5b17ae4724fb2216c/core/rawdb/schema.go#L120>
    async fn debug_db_get(&self, key: String) -> RpcResult<Option<Bytes>> {
        let key_bytes = if key.starts_with("0x") {
            decode(&key).map_err(|_| SilApiError::InvalidParams("Invalid hex key".to_string()))?
        } else {
            key.into_bytes()
        };
        ...
        if key_bytes[0] != 0x63 {
            return Err(SilApiError::InvalidParams("Key prefix must be 0x63".to_string()).into());
        }
...
impl<Sil: RpcNodeCore> std::fmt::Debug for DebugApi<Sil> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DebugApi").finish_non_exhaustive()
    }
}

impl<Sil: RpcNodeCore> Clone for DebugApi<Sil> {
    fn clone(&self) -> Self {
...
struct DebugApiInner<Sil: RpcNodeCore> {
    ...
    bad_block_store: BadBlockStore<BlockTy<Sil::Primitives>>,
}
