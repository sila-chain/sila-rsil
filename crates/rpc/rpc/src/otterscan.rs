...
/// Otterscan API.
#[derive(Debug)]
pub struct OtterscanApi<Sil> {
    sil: Sil,
}

impl<Sil> OtterscanApi<Sil> {
    /// Creates a new instance of `Otterscan`.
    pub const fn new(sil: Sil) -> Self {
        Self { sil }
    }
}
    ...
    Sil: FullEthApiTypes,
{
    ...
    fn block_details(
        &self,
        block: RpcBlock<Sil::NetworkTypes>,
        receipts: Vec<RpcReceipt<Sil::NetworkTypes>>,
    ) -> RpcResult<BlockDetails<RpcHeader<Sil::NetworkTypes>>> {
...
#[async_trait]
impl<Sil> OtterscanServer<RpcTransaction<Sil::NetworkTypes>, RpcHeader<Sil::NetworkTypes>>
    for OtterscanApi<Sil>
where
    Sil: SilApiServer<
        ...
        + 'static,
{
    ...
    ) -> RpcResult<Option<RpcHeader<Sil::NetworkTypes>>> {
        self.sil.header_by_number(block_number.into()).await
    }

    /// Handler for `ots_hasCode`
    async fn has_code(&self, address: Address, block_id: Option<BlockId>) -> RpcResult<bool> {
        SilApiServer::get_code(&self.sil, address, block_id).await.map(|code| !code.is_empty())
    }
    ...
    async fn get_block_details(
        ...
        let block = self.sil.block_by_number(block_number, true);
        let block_id = block_number.into();
        let receipts = self.sil.block_receipts(block_id);
        let (block, receipts) = futures::try_join!(block, receipts)?;
        self.block_details(
            block.ok_or(SilApiError::HeaderNotFound(block_id))?,
            receipts.ok_or(SilApiError::ReceiptsNotFound(block_id))?,
        )
    }
    ...
    async fn get_block_details_by_hash(
        ...
        let block = self.sil.block_by_hash(block_hash, true);
    ...
    async fn get_block_transactions(
        ...
        OtsBlockTransactions<RpcTransaction<Sil::NetworkTypes>, RpcHeader<Sil::NetworkTypes>>,
    > {
        let block_number = block_number.into_inner();
        ...
        let mut block = block.ok_or(SilApiError::HeaderNotFound(block_id))?;
        let mut receipts = receipts.ok_or(SilApiError::ReceiptsNotFound(block_id))?;
    ...
    async fn get_contract_creator(&self, address: Address) -> RpcResult<Option<ContractCreator>> {
        ...
        let num = binary_search::<_, _, ErrorObjectOwned>(
            1,
            self.sil.block_number()?.saturating_to(),
            |mid| {
                Box::pin(async move {
                    Ok(!SilApiServer::get_code(&self.sil, address, Some(mid.into()))
                        .await?
                        .is_empty())
                })
            },
        ...
        let traces = self
            ...
            .map(|traces| {
                traces
                    ...
                    .map(|tx_trace| {
                        ...
                        Ok(match (trace.action, trace.result, trace.error) {
                            (
                                ...
                                hash: tx_trace
                                    .transaction_hash
                                    .ok_or(SilApiError::TransactionNotFound)?,
                                creator,
                            }),
                            _ => None,
                        })
                    ...
                    .collect::<Result<Vec<_>, SilApiError>>()
            })
            .transpose()?;

        // A contract maybe created and then destroyed in multiple transactions, here we
        // return the first found transaction, this behavior is consistent with silascan's
