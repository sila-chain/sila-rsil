//! `SilApiBuilder` implementation

use crate::{sil::core::SilApiInner, SilApi};
...
use rsil_tasks::{pool::BlockingTaskPool, Runtime};
use std::{sync::Arc, time::Duration};

/// A helper to build the `SilApi` handler instance.
///
/// This builder type contains all settings to create an [`SilApiInner`] or an [`SilApi`] instance
/// directly.
#[derive(Debug)]
pub struct SilApiBuilder<N: RpcNodeCore, Rpc, NextEnv = ()> {
    components: N,
    rpc_converter: Rpc,
    gas_cap: GasCap,
...
impl<Provider, Pool, Network, SavmConfig, ChainSpec>
    SilApiBuilder<
...
{
    /// Creates a new `SilApiBuilder` instance.
    pub fn new(provider: Provider, pool: Pool, network: Network, evm_config: SavmConfig) -> Self {
        Self::new_with_components(RpcNodeCoreAdapter::new(provider, pool, network, evm_config))
    }
}

impl<N: RpcNodeCore, Rpc, NextEnv> SilApiBuilder<N, Rpc, NextEnv> {
    ...
    pub fn map_converter<F, R>(self, f: F) -> SilApiBuilder<N, R, NextEnv>
    where
        F: FnOnce(Rpc) -> R,
    {
        let Self {
...
impl<N, ChainSpec> SilApiBuilder<N, RpcConverter<Sila, N::Savm, SilReceiptConverter<ChainSpec>>>
where
    N: RpcNodeCore<Provider: ChainSpecProvider<ChainSpec = ChainSpec>>,
{
    /// Creates a new `SilApiBuilder` instance with the provided components.
    pub fn new_with_components(components: N) -> Self {
        let rpc_converter =
            RpcConverter::new(SilReceiptConverter::new(components.provider().chain_spec()));
        Self {
            components,
            rpc_converter,
            eth_cache: None,
...
impl<N, Rpc, NextEnv> SilApiBuilder<N, Rpc, NextEnv>
where
    N: RpcNodeCore,
{
    ...
    ) -> SilApiBuilder<N, RpcNew, NextEnv> {
        let Self {
            components,
            rpc_converter: _,
            gas_cap,
    ...
    pub fn with_pending_env_builder<NextEnvNew>(
        self,
        next_env: NextEnvNew,
    ) -> SilApiBuilder<N, Rpc, NextEnvNew> {
        let Self {
            components,
            rpc_converter,
            gas_cap,
    ...
    /// Builds the [`SilApiInner`] instance.
    ///
    /// If not configured, this will spawn the cache backend: [`SilStateCache::spawn_with`].
    ///
    /// # Panics
    ///
    /// This function panics if the blocking task pool cannot be built.
    /// This will panic if called outside the context of a Tokio runtime.
    pub fn build_inner(self) -> SilApiInner<N, Rpc>
    where
        Rpc: RpcConvert,
        NextEnv: PendingEnvBuilder<N::Savm>,
    {
        let Self {
            components,
            rpc_converter,
    ...
    /// This function panics if the blocking task pool cannot be built.
    /// This will panic if called outside the context of a Tokio runtime.
    pub fn build(self) -> SilApi<N, Rpc>
    where
        Rpc: RpcConvert,
        NextEnv: PendingEnvBuilder<N::Savm>,
    {
        SilApi { inner: Arc::new(self.build_inner()) }
    }
    ...
    /// Sets the maximum memory the SAVM can allocate per RPC request.
    pub const fn evm_memory_limit(mut self, memory_limit: u64) -> Self {
        self.evm_memory_limit = memory_limit;
        self
    }

    /// Sets whether to force upcasting SIP-4844 blob sidecars to SIP-7594 format.
