...
/// `Sil` bundle implementation.
pub struct SilBundle<Sil> {
    /// All nested fields bundled together.
    inner: Arc<SilBundleInner<Sil>>,
}
...
impl<Sil> SilBundle<Sil>
where
    Sil: SilTransactions + LoadPendingBlock + Call + 'static,
{
    ...
    pub async fn call_bundle(
        &self,
        bundle: SilCallBundle,
    ) -> Result<SilCallBundleResponse, Sil::Error> {
        let SilCallBundle {
            txs,
            block_number,
            coinbase,
        ...
        self.eth_api()
            .spawn_with_state_at_block(at, move |eth_api, db| {
                ...
                while let Some(tx) = transactions.next() {
                    ...
                    let tx = {
                        let mut tx = <Sil::Pool as TransactionPool>::Transaction::from_pooled(tx);
                    ...
                    let tx_res = SilCallBundleTransactionResult {
                        coinbase_diff,
...
impl<Sil> SilCallBundleApiServer for SilBundle<Sil>
where
    Sil: SilTransactions + LoadPendingBlock + Call + 'static,
{
    async fn call_bundle(&self, request: SilCallBundle) -> RpcResult<SilCallBundleResponse> {
        Self::call_bundle(self, request).await.map_err(Into::into)
    }
}

/// Container type for `SilBundle` internals
#[derive(Debug)]
...
impl<Sil> std::fmt::Debug for SilBundle<Sil> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SilBundle").finish_non_exhaustive()
    }
}

impl<Sil> Clone for SilBundle<Sil> {
    fn clone(&self) -> Self {
        Self { inner: Arc::clone(&self.inner) }
    }
}

/// [`SilBundle`] specific errors.
#[derive(Debug, thiserror::Error)]
pub enum SilBundleError {
