...
use crate::{sil::helpers::types::SilRpcConverter, SilApiBuilder};
...
pub type SilRpcConverterFor<N, NetworkT = Sila> = RpcConverter<
    NetworkT,
    <N as FullNodeComponents>::Savm,
    SilReceiptConverter<<<N as FullNodeTypes>::Provider as ChainSpecProvider>::ChainSpec>,
>;
...
#[derive(Deref)]
pub struct SilApi<N: RpcNodeCore, Rpc: RpcConvert> {
    /// All nested fields bundled together.
    #[deref]
    pub(super) inner: Arc<SilApiInner<N, Rpc>>,
}

impl<N, Rpc> Clone for SilApi<N, Rpc>
where
    N: RpcNodeCore,
    Rpc: RpcConvert,
{
...
impl
    SilApi<
        RpcNodeCoreAdapter<NoopProvider, NoopTransactionPool, NoopNetwork, SilEvmConfig>,
        SilRpcConverter<ChainSpec>,
    >
{
    /// Convenience fn to obtain a new [`SilApiBuilder`] instance with mandatory components.
    ///
    ...
    pub fn builder<Provider, Pool, Network, SavmConfig, ChainSpec>(
        provider: Provider,
        pool: Pool,
        network: Network,
    ...
    {
        SilApiBuilder::new(provider, pool, network, evm_config)
    }
}

impl<N, Rpc> SilApiTypes for SilApi<N, Rpc>
where
    N: RpcNodeCore,
    Rpc: RpcConvert<Error = SilApiError>,
{
    type Error = SilApiError;
...
impl<N, Rpc> RpcNodeCore for SilApi<N, Rpc>
where
    N: RpcNodeCore,
    Rpc: RpcConvert,
{
...
impl<N, Rpc> RpcNodeCoreExt for SilApi<N, Rpc>
where
    N: RpcNodeCore,
    Rpc: RpcConvert,
{
    #[inline]
    fn cache(&self) -> &SilStateCache<N::Primitives> {
        self.inner.cache()
    }
}

impl<N, Rpc> std::fmt::Debug for SilApi<N, Rpc>
where
    N: RpcNodeCore,
    Rpc: RpcConvert,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SilApi").finish_non_exhaustive()
    }
}

impl<N, Rpc> SpawnBlocking for SilApi<N, Rpc>
where
    N: RpcNodeCore,
    Rpc: RpcConvert<Error = SilApiError>,
{
...
/// Container type `SilApi`
#[expect(missing_debug_implementations)]
pub struct SilApiInner<N: RpcNodeCore, Rpc: RpcConvert> {
    ...
    next_env_builder: Box<dyn PendingEnvBuilder<N::Savm>>,
    ...
    /// Whether to force upcasting SIP-4844 blob sidecars to SIP-7594 format when SilaOsaka is active.
    force_blob_sidecar_upcasting: bool,
}

impl<N, Rpc> SilApiInner<N, Rpc>
where
    N: RpcNodeCore,
    Rpc: RpcConvert,
{
    ...
    pub fn new(
        ...
        proof_permits: usize,
        converter: Rpc,
        next_env: impl PendingEnvBuilder<N::Savm>,
    ...
    Rpc: RpcConvert,
{
    ...
    /// Returns a handle to the SAVM config.
    #[inline]
    pub fn evm_config(&self) -> &N::Savm {
    ...
    pub async fn add_pool_transaction(
        &self,
        origin: rsil_transaction_pool::TransactionOrigin,
        transaction: <N::Pool as TransactionPool>::Transaction,
    ) -> Result<AddedTransactionOutcome, SilApiError> {
        let (response_tx, response_rx) = tokio::sync::oneshot::channel();
        let request = rsil_transaction_pool::BatchTxRequest::new(origin, transaction, response_tx);
    ...
    /// Returns whether to force upcasting SIP-4844 blob sidecars to SIP-7594 format.
    #[inline]
...
#[cfg(test)]
mod tests {
    use crate::{sil::helpers::types::SilRpcConverter, SilApi, SilApiBuilder};
    ...
    fn build_test_eth_api<
        P: BlockReaderIdExt<
                ...
                Transaction = rsil_sila_primitives::TransactionSigned,
            > + BlockReader
            + ChainSpecProvider<ChainSpec = ChainSpec>
            + StateProviderFactory
            + CanonStateSubscriptions<Primitives = rsil_sila_primitives::SilPrimitives>
        ...
        for i in (0..block_count).rev() {
            ...
            for _ in 0..TOTAL_TRANSACTIONS {
                ...
                if let Some(base_fee_per_gas) = header.base_fee_per_gas {
                    ...
                    let transaction = TransactionSigned::new_unhashed(
                        rsil_sila_primitives::Transaction::Legacy(Default::default()),
                        Signature::test_signature(),
                    );
    ...
    async fn test_call_many_maps_provider_block_lookup_error_with_eth_api_conversion() {
        ...
        assert!(
            ...
            "best block lookup should map via SilApiError::from(ProviderError): {message}"
        );
        assert!(
