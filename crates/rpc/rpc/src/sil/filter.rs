...
use rsil_rpc_server_types::{result::rpc_error_with_code, ToRpcResult};
...
use rsil_transaction_pool::{NewSubpoolTransactionStream, PoolTransaction, TransactionPool};
use std::{
...
impl<Sil> Clone for SilFilter<Sil>
where
    Sil: SilApiTypes,
{
    fn clone(&self) -> Self {
    ...
    Sil: SilApiTypes + 'static,
{
    ...
    pub fn active_filters(&self) -> &ActiveFilters<RpcTransaction<Sil::NetworkTypes>> {
        ...
        + 'static,
{
    ...
    pub async fn filter_changes(
        &self,
        id: FilterId,
    ) -> Result<FilterChanges<RpcTransaction<Sil::NetworkTypes>>, SilFilterError> {
        ...
        let (start_block, kind) = {
            ...
            let filter = filters.get_mut(&id).ok_or(SilFilterError::FilterNotFound(id))?;
            ...
            FilterKind::Block => {
                ...
                let block_hashes =
                    self.provider().canonical_hashes_range(start_block, end_block).map_err(
                        |_| SilApiError::HeaderRangeNotFound(start_block.into(), end_block.into()),
                    )?;
    ...
    pub async fn filter_logs(&self, id: FilterId) -> Result<Vec<Log>, SilFilterError> {
        let filter = {
            let mut filters = self.inner.active_filters.inner.lock().await;
            let filter =
                filters.get_mut(&id).ok_or_else(|| SilFilterError::FilterNotFound(id.clone()))?;
...
impl<Sil> std::fmt::Debug for SilFilter<Sil>
where
    Sil: SilApiTypes,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SilFilter").finish_non_exhaustive()
    }
}

/// Container type `SilFilter`
#[derive(Debug)]
        ...
        + 'static,
{
    ...
    async fn logs_for_filter(
        ...
        match filter.block_option {
            FilterBlockOption::AtBlockHash(block_hash) => {
                ...
                // Check if the block has been pruned (SIP-4444)
                ...
                if from_block.is_some_and(|b| b.is_pending()) {
                    ...
                    if let Ok(Some(pending_block)) = self.eth_api.local_pending_block().await {
                        ...
                        if pending_block.block.number() > info.best_number {
                            ...
                            append_matching_block_logs(
                                &mut all_logs,
                                ProviderOrBlock::<Sil::Provider>::Block(pending_block.block),
                                &filter,
                                block_num_hash,
    ...
    async fn install_filter(
        &self,
        kind: FilterKind<RpcTransaction<Sil::NetworkTypes>>,
    ) -> RpcResult<FilterId> {
    ...
    async fn get_logs_in_block_range(
        ...
        trace!(target: "rpc::sil::filter", from=from_block, to=to_block, ?filter, "finding logs in range");
        ...
        rx.await.map_err(|_| SilFilterError::InternalError)?
    }
    ...
    async fn get_logs_in_block_range_inner(
            ...
            range_mode.next().await?
        {
                ...
                && all_logs.len() > max_logs_per_response
            {
                ...
                return Err(SilFilterError::QueryExceedsMaxResults {
...
impl From<SilFilterError> for jsonrpsee::types::error::ErrorObject<'static> {
    fn from(err: SilFilterError) -> Self {
        match err {
            SilFilterError::FilterNotFound(_) => rpc_error_with_code(
                jsonrpsee::types::error::INVALID_PARAMS_CODE,
                "filter not found",
            ),
            err @ SilFilterError::InternalError => {
...
impl From<ProviderError> for SilFilterError {
    fn from(err: ProviderError) -> Self {
        Self::SilAPIError(err.into())
    }
}

impl From<logs_utils::FilterBlockRangeError> for SilFilterError {
    ...
    P: ReceiptProvider + BlockReader,
{
    ...
    recovered_block: Option<Arc<rsil_primitives_traits::RecoveredBlock<ProviderBlock<P>>>>,
...
> {
    ...
    Cached(CachedMode<Sil>),
    /// Use range-based processing for older blocks
    Range(RangeBlockMode<Sil>),
}
    ...
    > RangeMode<Sil>
{
    ...
    fn new(
        filter_inner: Arc<SilFilterInner<Sil>>,
        sealed_headers: Vec<SealedHeader<<Sil::Provider as HeaderProvider>::Header>>,
        from_block: u64,
        to_block: u64,
        max_headers_range: u64,
        chain_tip: u64,
    ...
    const fn should_use_cached_mode(
        headers: &[SealedHeader<<Sil::Provider as HeaderProvider>::Header>],
        block_count: u64,
    ...
    async fn next(&mut self) -> Result<Option<ReceiptBlockResult<Sil::Provider>>, SilFilterError> {
        match self {
...
> {
    filter_inner: Arc<SilFilterInner<Sil>>,
    headers_iter: std::vec::IntoIter<SealedHeader<<Sil::Provider as HeaderProvider>::Header>>,
}
...
type ReceiptFetchFuture<P> =
    Pin<Box<dyn Future<Output = Result<Vec<ReceiptBlockResult<P>>, SilFilterError>> + Send>>;
...
> {
    filter_inner: Arc<SilFilterInner<Sil>>,
    iter: Peekable<std::vec::IntoIter<SealedHeader<<Sil::Provider as HeaderProvider>::Header>>>,
    next: VecDeque<ReceiptBlockResult<Sil::Provider>>,
    max_range: usize,
    // Stream of ongoing receipt fetching tasks
    pending_tasks: FuturesOrdered<ReceiptFetchFuture<Sil::Provider>>,
}
    ...
    > RangeBlockMode<Sil>
{
    ...
    async fn process_small_range(
        &mut self,
        range_headers: Vec<SealedHeader<<Sil::Provider as HeaderProvider>::Header>>,
    ) -> Result<Option<ReceiptBlockResult<Sil::Provider>>, SilFilterError> {
        ...
        for chunk_headers in header_chunks {
            ...
            let chunk_task = Box::pin(async move {
                ...
                match chunk_task.await {
                    ...
                    Ok(Err(e)) => Err(e),
                    Err(join_err) => {
                        trace!(target: "rpc::sil::filter", error = ?join_err, "Task join error");
                        Err(SilFilterError::InternalError)
                    }
                }
            });
...
mod tests {
    use super::*;
    use crate::{sil::SilApi, SilApiBuilder};
    ...
    > {
        SilApiBuilder::new(
            ...
            testing_pool(),
            NoopNetwork::default(),
            SilEvmConfig::new(provider.chain_spec()),
        )
        .build()
    }
    ...
    async fn test_range_block_mode_queued_results_priority() {
        ...
        let mock_receipt_1 = rsil_sila_primitives::Receipt {
            tx_type: TxType::Legacy,
            cumulative_gas_used: 100_000,
            logs: vec![],
            success: true,
        };
        let mock_receipt_2 = rsil_sila_primitives::Receipt {
            tx_type: TxType::Sip1559,
            cumulative_gas_used: 200_000,
            logs: vec![],
            success: true,
        };
        let mock_receipt_3 = rsil_sila_primitives::Receipt {
    ...
    async fn test_range_block_mode_provider_receipts() {
        ...
        let receipt_100_1 = rsil_sila_primitives::Receipt {
            tx_type: TxType::Legacy,
            cumulative_gas_used: 21_000,
            logs: vec![mock_log.clone()],
            success: true,
        };
        let receipt_100_2 = rsil_sila_primitives::Receipt {
            tx_type: TxType::Sip1559,
            cumulative_gas_used: 42_000,
            logs: vec![mock_log.clone()],
            success: true,
        };
        let receipt_101_1 = rsil_sila_primitives::Receipt {
            tx_type: TxType::Sip2930,
    ...
    async fn test_range_block_mode_iterator_exhaustion() {
        ...
        let mock_receipt = rsil_sila_primitives::Receipt {
            tx_type: TxType::Legacy,
            cumulative_gas_used: 21_000,
            logs: vec![],
            success: true,
    ...
    async fn test_log_limit_retry_range_excludes_overflow_block() {
        ...
        use rsil_sila_primitives::{TransactionSigned, TxType};
        ...
        let receipt = rsil_sila_primitives::Receipt {
            tx_type: TxType::Legacy,
            cumulative_gas_used: 21_000,
            logs: vec![mock_log],
            success: true,
        ...
        for (idx, block_number) in (100u64..=102).enumerate() {
            ...
            let block = rsil_sila_primitives::Block {
                header,
                body: rsil_sila_primitives::BlockBody {
        ...
        let SilFilterError::QueryExceedsMaxResults { max_logs, from_block, to_block } = err else {
    ...
    async fn test_non_consecutive_headers_after_bloom_filter() {
        ...
        for i in 100u64..=103 {
            ...
            let block = rsil_sila_primitives::Block {
                header,
                body: rsil_sila_primitives::BlockBody { transactions, ..Default::default() },
            };
            provider.add_block(hash, block);
        }
