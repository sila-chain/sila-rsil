...
impl DevSigner {
    ...
    /// which satisfy [`SilSigner`] trait
    pub fn random_signers<T: Decodable2718, TxReq: SignableTxRequest<T>>(
        num: u32,
    ) -> Vec<Box<dyn SilSigner<T, TxReq> + 'static>> {
        ...
        for _ in 0..num {
            ...
            signers.push(Box::new(Self { addresses, accounts }) as Box<dyn SilSigner<T, TxReq>>);
        }
        signers
    }

    /// Generates dev signers deterministically from a fixed mnemonic.
    /// Uses the Sila derivation path: `m/44'/60'/0'/0/{index}`
...
impl<T: Decodable2718, TxReq: SignableTxRequest<T>> SilSigner<T, TxReq> for DevSigner {
    ...
    async fn sign(&self, address: Address, message: &[u8]) -> Result<Signature> {
        // Hash message according to SIP 191:
        // https://sila.org/es/developers/docs/apis/json-rpc/#eth_sign
        let hash = sip191_hash_message(message);
...
mod tests {
    ...
    async fn test_sign_type_data() {
        ...
        let data: TypedData = serde_json::from_str(sip_712_example).unwrap();
        let signer = build_signer();
        let from = *signer.addresses.first().unwrap();
        let sig = SilSigner::<rsil_sila_primitives::TransactionSigned>::sign_typed_data(
            &signer, from, &data,
        )
        .unwrap();
    ...
    async fn test_signer() {
        ...
        let from = *signer.addresses.first().unwrap();
        let sig =
            SilSigner::<rsil_sila_primitives::TransactionSigned>::sign(&signer, from, message)
                .await
                .unwrap();
        let expected = Signature::new(
            U256::from_str_radix(
