...
use alloy_eips::{sip7594::BlobTransactionSidecarVariant, BlockId, Typed2718};
    ...
    Rpc: RpcConvert<Primitives = N::Primitives, Error = SilApiError>,
{
    ...
    async fn send_pool_transaction(
        ...
        // Optionally convert legacy blob sidecars to SIP-7594 format when SilaOsaka is active
        ...
        if self.inner.force_blob_sidecar_upcasting() && pool_transaction.is_eip4844() {
            let SilBlobTransactionSidecar::Present(sidecar) = pool_transaction.take_blob() else {
                return Err(SilApiError::PoolError(RpcPoolError::Sip4844(
            ...
            let sidecar = match sidecar {
                BlobTransactionSidecarVariant::Sip4844(sidecar) => {
                    let latest = self
                        .provider()
                        .latest_header()?
                        .ok_or(SilApiError::HeaderNotFound(BlockId::latest()))?;
                    // Convert to SIP-7594 if next block is SilaOsaka
                    if self
                        .provider()
                        .chain_spec()
                        .is_osaka_active_at_timestamp(latest.timestamp().saturating_add(12))
                    {
                        BlobTransactionSidecarVariant::Sip7594(
                            self.blob_sidecar_converter().convert(sidecar).await.ok_or_else(
                                || {
                                    RpcPoolError::Sip4844(
                                        Sip4844PoolTransactionError::InvalidEip4844Blob(
            ...
            pool_transaction =
                SilPoolTransaction::try_from_eip4844(pool_transaction.into_consensus(), sidecar)
                    .ok_or_else(|| {
                        RpcPoolError::Sip4844(
                            Sip4844PoolTransactionError::MissingEip4844BlobSidecar,
                        )
        ...
        if let Some(client) = self.raw_tx_forwarder() {
            tracing::debug!(target: "rpc::sil", hash = %pool_transaction.hash(), "forwarding raw transaction to forwarder");
            ...
            let hash =
                client.request("eth_sendRawTransaction", (rlp_hex,)).await.inspect_err(|err| {
                    tracing::debug!(target: "rpc::sil", %err, hash=% *pool_transaction.hash(), "failed to forward raw transaction");
                }).map_err(SilApiError::other)?;
...
impl<N, Rpc> LoadTransaction for SilApi<N, Rpc>
where
    N: RpcNodeCore,
    SilApiError: FromEvmError<N::Savm>,
    Rpc: RpcConvert<Primitives = N::Primitives, Error = SilApiError>,
...
mod tests {
    ...
    > {
        ...
        SilApi::builder(mock_provider, pool, NoopNetwork::default(), evm_config)
            .send_raw_transaction_sync_timeout(send_raw_transaction_sync_timeout)
            .build()
    }

    fn raw_transfer_tx() -> Bytes {
        // https://silascan.io/tx/0xa694b71e6c128a2ed8e2e0f6770bddbe52e3bb8f10e8472f9a79ab81497a8b5d
        Bytes::from(hex!(
    ...
    async fn send_raw_transaction() {
        ...
        // https://silascan.io/tx/0x48816c2f32c29d152b0d86ff706f39869e6c1f01dc2fe59a3c1f9ecf39384694
    ...
    async fn test_fill_transaction_eip4844_blob_fee() {
        ...
        // SIP-4844 blob transaction with versioned hashes but no blob fee
        let tx_req = TransactionRequest {
            from: Some(address),
            to: Some(Address::random().into()),
    ...
    async fn test_fill_transaction_eip4844_preserves_blob_fee() {
        ...
        // SIP-4844 blob transaction with blob fee already set
        let tx_req = TransactionRequest {
            from: Some(address),
            to: Some(Address::random().into()),
            transaction_type: Some(3), // SIP-4844
            sidecar: Some(BlobTransactionSidecarVariant::from(
                builder.build::<BlobTransactionSidecar>().unwrap(),
            )),
    ...
    async fn test_fill_transaction_non_blob_tx_no_blob_fee() {
        ...
        let tx_req = TransactionRequest {
            from: Some(address),
            to: Some(Address::random().into()),
            transaction_type: Some(2), // SIP-1559
            ..Default::default()
        };
