//! `Sil` Sim bundle implementation and helpers.
...
/// `Sil` sim bundle implementation.
pub struct SilSimBundle<Sil> {
    /// All nested fields bundled together.
    inner: Arc<SilSimBundleInner<Sil>>,
}
...
impl<Sil> SilSimBundle<Sil> {
    ...
    fn build_bundle_logs(
        ...
        flat_logs: &[Vec<Log>],
    ) -> Result<Vec<SimBundleLogs>, SilApiError> {
            ...
            match &frame.bundle.bundle_body[frame.next_idx] {
                BundleItem::Tx { .. } => {
                    let tx_logs = flat_logs.get(flat_log_idx).cloned().ok_or_else(|| {
                        SilApiError::InvalidParams(SilSimBundleError::UnmatchedBundle.to_string())
                    })?;
                ...
                BundleItem::Hash { .. } => {
                    return Err(SilApiError::InvalidParams(
                        SilSimBundleError::InvalidBundle.to_string(),
                    ));
                }
            }
        }
        ...
        if flat_log_idx != flat_logs.len() {
            return Err(SilApiError::InvalidParams(SilSimBundleError::UnmatchedBundle.to_string()));
        }
...
impl<Sil> SilSimBundle<Sil>
where
    Sil: SilTransactions + LoadBlock + Call + 'static,
{
    ...
    fn parse_and_flatten_bundle(
    ...
    ) -> Result<Vec<FlattenedBundleItem<ProviderTx<Sil::Provider>>>, SilApiError> {
        ...
        while let Some((current_bundle, mut idx, depth)) = stack.pop() {
            ...
            if depth > MAX_NESTED_BUNDLE_DEPTH {
                return Err(SilApiError::InvalidParams(SilSimBundleError::MaxDepth.to_string()));
            }
            ...
            if current_bundle.bundle_body.len() > MAX_BUNDLE_BODY_SIZE {
                return Err(SilApiError::InvalidParams(
                    SilSimBundleError::BundleTooLarge.to_string(),
                ));
            }
                ...
                if let Some(refunds) = &validity.refund {
                    ...
                    for refund in refunds {
                        if refund.body_idx as usize >= current_bundle.bundle_body.len() {
                            return Err(SilApiError::InvalidParams(
                                SilSimBundleError::InvalidValidity.to_string(),
                            ));
                        }
            ...
            while idx < body.len() {
                match &body[idx] {
                    BundleItem::Tx { tx, can_revert } => {
                        let recovered_tx = recover_raw_transaction::<PoolPooledTx<Sil::Pool>>(tx)?;
                        let tx = recovered_tx.map(
                            <Sil::Pool as TransactionPool>::Transaction::pooled_into_consensus,
                        );
    ...
    async fn sim_bundle_inner(
        ...
        logs: bool,
    ) -> Result<SimBundleResponse, Sil::Error> {
            ...
            .spawn_with_state_at_block(current_block_id, move |_, mut db| {
                ...
                for (tx_index, item) in flattened_bundle.iter().enumerate() {
                    ...
                    if !result.is_success() && !item.can_revert {
                        return Err(SilApiError::InvalidParams(
                            SilSimBundleError::BundleTransactionFailed.to_string(),
                        )
                        .into());
                    }
                ...
                for item in &flattened_bundle {
                    if let Some(refund_percent) = item.refund_percent {
                        ...
                        if payout_tx_fee > payout_value {
                            return Err(SilApiError::InvalidParams(
                                SilSimBundleError::NegativeProfit.to_string(),
                            )
                            .into());
                        }
...
impl<Sil> MevSimApiServer for SilSimBundle<Sil>
where
    Sil: SilTransactions + LoadBlock + Call + 'static,
{
    async fn sim_bundle(
        ...
        let bundle_res =
            tokio::time::timeout(timeout, Self::sim_bundle_inner(self, request, overrides, true))
                .await
                .map_err(|_| {
                    SilApiError::InvalidParams(SilSimBundleError::BundleTimeout.to_string())
                })?;
...
/// Container type for `SilSimBundle` internals
#[derive(Debug)]
...
impl<Sil> std::fmt::Debug for SilSimBundle<Sil> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SilSimBundle").finish_non_exhaustive()
    }
}

impl<Sil> Clone for SilSimBundle<Sil> {
    fn clone(&self) -> Self {
        Self { inner: Arc::clone(&self.inner) }
    }
}

/// [`SilSimBundle`] specific errors.
#[derive(Debug, thiserror::Error)]
pub enum SilSimBundleError {
...
mod tests {
    ...
    fn assert_unmatched_bundle(result: Result<Vec<SimBundleLogs>, SilApiError>) {
        assert!(matches!(
            result,
            Err(SilApiError::InvalidParams(ref message))
                if message == &SilSimBundleError::UnmatchedBundle.to_string()
        ));
    }
    ...
    fn test_build_bundle_logs_single_tx() {
        ...
        let result =
            SilSimBundle::<()>::build_bundle_logs(&bundle, &create_bundle_logs(&[1])).unwrap();
    ...
    fn test_build_bundle_logs_empty_bundle() {
        ...
        let result = SilSimBundle::<()>::build_bundle_logs(&bundle, &[]).unwrap();
    ...
    fn test_build_bundle_logs_nested_bundle() {
        ...
        let result =
            SilSimBundle::<()>::build_bundle_logs(&bundle, &create_bundle_logs(&[1, 1, 2]))
                .unwrap();
    ...
    fn test_build_bundle_logs_duplicate_transactions_same_level() {
        ...
        let result =
            SilSimBundle::<()>::build_bundle_logs(&bundle, &create_bundle_logs(&[1, 2])).unwrap();
    ...
    fn test_build_bundle_logs_deeply_nested_bundle() {
        ...
        let result =
            SilSimBundle::<()>::build_bundle_logs(&root_bundle, &create_bundle_logs(&[1, 2, 3]))
                .unwrap();
    ...
    fn test_build_bundle_logs_mismatched_flat_logs() {
        ...
        assert_unmatched_bundle(SilSimBundle::<()>::build_bundle_logs(&bundle, &[]));
        assert_unmatched_bundle(SilSimBundle::<()>::build_bundle_logs(
            &bundle,
