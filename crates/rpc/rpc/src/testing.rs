...
//!
//! ```sh
//! rsil node --http --http.api sil,testing
//! ```
//!
...
impl<Sil, Savm, Payload> TestingApi<Sil, Savm, Payload>
where
    Payload: PayloadTypes,
    Payload::ExecutionData: From<SilBuiltPayload>,
    Sil: Call<
    ...
    >,
    Savm: ConfigureEvm<NextBlockEnvCtx = NextBlockEnvAttributes, Primitives = SilPrimitives>
        + 'static,
{
    ...
    ) -> Result<SilBuiltPayload, Sil::Error> {
            ...
            .spawn_with_state_at_block(request.parent_block_hash, move |eth_api, state| {
                ...
                let recovered_txs = if use_pool_transactions {
                    ...
                    try_recover_signers(&request.transactions, |tx| {
                        TxTy::<Savm::Primitives>::decode_2718_exact(tx.as_ref())
                            .map_err(RecoveryError::from_source)
                    })
                    .or(Err(SilApiError::InvalidTransactionSignature))?
                };
                ...
                for (idx, tx) in recovered_txs.into_iter().enumerate() {
                    ...
                    // SIP-7934: Check estimated block size before adding transaction
    ...
    async fn build_block_v1(
        ...
        request: TestingBuildBlockRequestV1,
        use_pool_transactions: bool,
    ) -> Result<ExecutionPayloadEnvelopeV5, Sil::Error> {
...
#[async_trait]
impl<Sil, Savm, Payload> TestingApiServer for TestingApi<Sil, Savm, Payload>
where
    Payload: PayloadTypes,
    Payload::ExecutionData: From<SilBuiltPayload>,
    Sil: Call<
