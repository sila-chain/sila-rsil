...
pub struct TraceApi<Sil> {
    inner: Arc<TraceApiInner<Sil>>,
}
...
where
    ...
    // bound
    Sil: Trace + Call + LoadPendingBlock + LoadTransaction + 'static,
{
    /// Executes the given call and returns a number of possible traces for it.
    pub async fn trace_call(
        &self,
        trace_request: TraceCallRequest<RpcTxReq<Sil::NetworkTypes>>,
    ...
    pub async fn trace_call_many(
        &self,
        calls: Vec<(RpcTxReq<Sil::NetworkTypes>, HashSet<TraceType>)>,
        block_id: Option<BlockId>,
    ...
    ///
    /// - if SilaParis hardfork is activated, no block rewards are given
    /// - if SilaParis hardfork is not activated, calculate block rewards with block number only
    fn calculate_base_block_reward<H: BlockHeader>(
        &self,
        header: &H,
    ) -> Result<Option<u128>, Sil::Error> {
    ...
    Sil: TraceExt + 'static,
{
    ...
    pub async fn trace_filter(
        &self,
        filter: TraceFilter,
    ) -> Result<Vec<LocalizedTransactionTrace>, Sil::Error> {
        ...
        if end > latest_block {
            return Err(SilApiError::HeaderNotFound(end.into()).into());
        }
            ...
            while let Some(block_replay) = block_replays.next().await {
                ...
                let reward_traces = if include_reward_traces {
                        ...
                        self.calculate_base_block_reward(block.header())?
                    {
                        ...
                        // reaches post-SilaParis blocks, later blocks in the range have no rewards.
...
#[async_trait]
impl<Sil> TraceApiServer<RpcTxReq<Sil::NetworkTypes>> for TraceApi<Sil>
where
    Sil: TraceExt + 'static,
{
    /// Executes the given call and returns a number of possible traces for it.
    ...
    ///
    /// # Limitations
    /// This currently requires block filter fields, since rsil does not have address indices yet.
...
impl<Sil> std::fmt::Debug for TraceApi<Sil> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TraceApi").finish_non_exhaustive()
    }
}
impl<Sil> Clone for TraceApi<Sil> {
    fn clone(&self) -> Self {
