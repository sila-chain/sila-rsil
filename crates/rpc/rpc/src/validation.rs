...
use alloy_eips::sip7685::RequestsOrHash;
...
use rsil_engine_primitives::PayloadValidator;
use rsil_errors::{BlockExecutionError, ConsensusError, ProviderError};
...
use rsil_node_api::{NewPayloadError, PayloadTypes};
...
use rsil_revm::{cached::CachedReads, database::StateProviderDatabase};
use rsil_rpc_api::BlockSubmissionValidationApiServer;
    ...
    T: PayloadTypes<ExecutionData = ExecutionData>,
{
    ...
    pub async fn validate_message_against_block(
        ...
        // Ensure the submitted block access list does not exceed the block gas limit (SIP-7928)
    ...
    async fn validate_builder_submission_v5(
        ...
        // Check block size as per SIP-7934 (only applies when SilaOsaka hardfork is active)
    ...
    fn from(error: ValidationApiError) -> Self {
        match error {
            ...
            ValidationApiError::Payload(err) => match err {
                error @ NewPayloadError::Sil(_) => invalid_params_rpc_err(error.to_string()),
