...
use alloy_eips::{sip1898::ForkBlock, BlockNumHash};
...
use rsil_trie_common::LazyTrieData;
...
pub struct Chain<N: NodePrimitives = rsil_sila_primitives::SilPrimitives> {
...
#[derive(Debug)]
pub struct DisplayBlocksChain<'a, B: rsil_primitives_traits::Block>(
    pub &'a BTreeMap<BlockNumber, Arc<RecoveredBlock<B>>>,
);

impl<B: rsil_primitives_traits::Block> fmt::Display for DisplayBlocksChain<'_, B> {
...
pub struct BlockReceipts<T = rsil_sila_primitives::Receipt> {
    /// Block identifier
...
pub(super) mod serde_bincode_compat {
    ...
    /// ```rust
    /// use rsil_execution_types::{serde_bincode_compat, Chain};
    ...
    #[serde(bound = "")]
    pub struct Chain<'a, N = SilPrimitives>
    where
        N: NodePrimitives,
    {
        #[serde(skip)]
        ...
        trie_updates: BTreeMap<
            BlockNumber,
            rsil_trie_common::serde_bincode_compat::updates::TrieUpdatesSorted<'a>,
        >,
        #[serde(default)]
        hashed_state: BTreeMap<
            BlockNumber,
            rsil_trie_common::serde_bincode_compat::hashed_state::HashedPostStateSorted<'a>,
        >,
    }
...
mod tests {
    ...
    #[test]
    fn chain_append() {
        let block: RecoveredBlock<rsil_sila_primitives::Block> = Default::default();
    ...
    fn test_number_split() {
        ...
        let mut block1: RecoveredBlock<rsil_sila_primitives::Block> = Default::default();
        let block1_hash = B256::new([15; 32]);
        block1.set_block_number(1);
        block1.set_hash(block1_hash);
        block1.push_sender(Address::new([4; 20]));

        let mut block2: RecoveredBlock<rsil_sila_primitives::Block> = Default::default();
