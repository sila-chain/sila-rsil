...
pub struct ExecutionOutcome<T = rsil_sila_primitives::Receipt> {
    ...
    /// The collection of SIP-7685 requests.
...
impl<T> ExecutionOutcome<T> {
    ...
    /// bundle state, receipts, first block number, and SIP-7685 requests.
    pub const fn new(
        bundle: BundleState,
        receipts: Vec<Vec<T>>,
...
impl ExecutionOutcome {
    /// Returns the sila receipt root for all recorded receipts.
    ///
    /// Note: this function calculated Bloom filters for every receipt and created merkle trees
    /// of receipt. This is an expensive operation.
    pub fn sila_receipts_root(&self, block_number: BlockNumber) -> Option<B256> {
        self.generic_receipts_root_slow(
            block_number,
            rsil_sila_primitives::calculate_receipt_root_no_memo,
        )
    }
}
...
pub(super) mod serde_bincode_compat {
    ...
    /// ```rust
    /// use rsil_execution_types::{serde_bincode_compat, ExecutionOutcome};
    ...
    mod tests {
        ...
        fn test_chain_bincode_roundtrip() {
            ...
            struct Data<T: rsil_primitives_traits::Receipt> {
                #[serde_as(as = "serde_bincode_compat::ExecutionOutcome<'_>")]
...
mod tests {
    ...
    fn test_initialization() {
        ...
        let receipts = vec![vec![Some(rsil_sila_primitives::Receipt {
    ...
    fn test_get_logs() {
        // Create a Receipts object with a vector of receipt vectors
        let receipts = vec![vec![rsil_sila_primitives::Receipt {
            tx_type: TxType::Legacy,
            cumulative_gas_used: 46913,
    ...
    fn test_receipts_by_block() {
        ...
        assert_eq!(
            receipts_by_block,
            vec![&Some(rsil_sila_primitives::Receipt {
                tx_type: TxType::Legacy,
                cumulative_gas_used: 46913,
                logs: vec![Log::<LogData>::default()],
                success: true,
