...
/// trait builds on.
pub trait ExecutableTxIterator<Savm: ConfigureEvm>:
    ExecutableTxTuple<Tx: ExecutableTxFor<Savm, Recovered = Self::Recovered>>
{
    ...
    type Recovered: RecoveredTx<TxTy<Savm::Primitives>> + Send + Sync;
}

impl<T, Savm: ConfigureEvm> ExecutableTxIterator<Savm> for T
where
