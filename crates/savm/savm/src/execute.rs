...
pub use rsil_execution_types::{BlockExecutionOutput, ExecutionOutcome};
...
pub trait Executor<DB: Database>: Sized {
    ...
    /// Executes the SAVM with the given input and accepts a state hook closure that is invoked with
    /// the SAVM state after execution.
    ...
    /// Executes the SAVM with the given input and accepts a state closure that is invoked with
    /// the SAVM state after execution.
    ...
    /// Executes the SAVM with the given input and accepts a state closure that is always invoked
    /// with the SAVM state after execution, even after failure.
...
/// # Fields Overview
///
/// - `evm_env`: The SAVM configuration used during execution (spec ID, block env, etc.)
...
pub struct BlockAssemblerInput<'a, 'b, F: BlockExecutorFactory, H = Header> {
    /// Configuration of SAVM used when executing the block.
    ///
    ...
    N: NodePrimitives,
{
    ...
    fn finish(
        ...
        let (savm, result) = self.executor.finish()?;
        let (db, evm_env) = savm.finish();
...
/// block executor.
pub trait ExecutableTxFor<Savm: ConfigureEvm>:
    ExecutableTxParts<TxEnvFor<Savm>, TxTy<Savm::Primitives>> + RecoveredTx<TxTy<Savm::Primitives>>
{
}
...
pub struct WithTxEnv<TxEnv, T> {
    /// The transaction environment for SAVM.
    pub tx_env: TxEnv,
