//! Traits for configuring an SAVM specifics.
//!
//! # Revm features
//!
...
/// execution/building, providing a unified interface for SAVM operations.
///
/// # Architecture Overview
///
/// The SAVM abstraction consists of the following layers:
///
/// 1. **[`Savm`] (produced by [`SavmFactory`])**: The core SAVM implementation responsible for
...
///    operates on top of [`Savm`] to execute entire blocks. This involves:
...
///    - Executing system calls (e.g., SIP-4788 beacon root updates)
...
/// - `parent_beacon_block_root`: SIP-4788 beacon root
///
/// ## [`BlockAssembler`]
///
...
pub trait ConfigureEvm: Clone + Debug + Send + Sync + Unpin {
    ...
    /// Configured [`BlockExecutorFactory`], contains [`SavmFactory`] internally.
    ...
    /// Returns a [`SavmFactory::Tx`] from a transaction.
    fn tx_env(&self, transaction: impl IntoTxEnv<TxEnvFor<Self>>) -> TxEnvFor<Self> {
        transaction.into_tx_env()
    }

    /// Provides a reference to [`SavmFactory`] implementation.
    fn evm_factory(&self) -> &SavmFactoryFor<Self> {
    ...
    ///
    /// This is one of three gates required before an SAVM can execute JIT-compiled code: the binary
    /// must be built with the `jit` feature, runtime compilation must be enabled by `--jit` or the
    /// `rsil_jit` RPC method, and this local support flag must be enabled for the config that
    /// creates the SAVM.
    ...
    /// Returns a new SAVM with the given database configured with the given environment settings,
    /// including the spec id and transaction environment.
    ///
    /// This will preserve any handler modifications
    fn evm_with_env<DB: Database>(&self, db: DB, evm_env: SavmEnvFor<Self>) -> SavmFor<Self, DB> {
        self.evm_factory().create_evm(db, evm_env)
    }

    /// Returns a new SAVM with the given database configured with `cfg` and `block_env`
    ...
    ) -> Result<SavmFor<Self, DB>, Self::Error> {
    ...
    ///
    /// This will use the given external inspector as the SAVM external context.
    ///
    /// This will preserve any handler modifications
    fn evm_with_env_and_inspector<DB, I>(
        &self,
        db: DB,
        evm_env: SavmEnvFor<Self>,
        inspector: I,
    ) -> SavmFor<Self, DB, I>
    where
        DB: Database,
    ...
    /// Creates a strategy with given SAVM and execution context.
    fn create_executor<'a, DB, I>(
        &'a self,
        savm: SavmFor<Self, &'a mut State<DB>, I>,
    ...
    {
        self.block_executor_factory().create_executor(savm, ctx)
    }
    ...
    fn create_executor_with_state<'a, 'db, DB, I>(
        &'a self,
        savm: SavmFor<Self, &'db mut State<DB>, I>,
        ctx: <Self::BlockExecutorFactory as BlockExecutorFactory>::ExecutionCtx<'a>,
    ...
    fn executor_for_block<'a, DB: Database>(
        ...
        let savm = self.evm_for_block(db, block.header())?;
        let ctx = self.context_for_block(block)?;
        Ok(self.create_executor(savm, ctx))
    }
    ...
    ///
    /// ```rust,ignore
    /// // Create a builder with specific SAVM configuration
    /// let savm = evm_config.evm_with_env(&mut state_db, evm_env);
    /// let ctx = evm_config.context_for_next_block(&parent, attributes);
    /// let builder = evm_config.create_block_builder(savm, &parent, ctx);
    /// ```
    ...
    {
        BasicBlockBuilder {
            executor: self.create_executor(savm, ctx.clone()),
            ctx,
    ...
    /// 1. Creating the SAVM environment for the next block
    ...
    > {
        ...
        let savm = self.evm_with_env(db, evm_env);
        let ctx = self.context_for_next_block(parent, attributes)?;
        Ok(self.create_block_builder(savm, parent, ctx))
    }
...
/// JIT backend controls exposed by an SAVM configuration.
...
///    - Parent beacon block root for SIP-4788
///
/// 2. **Configure SAVM environment** using these attributes: ```rust,ignore let evm_env =
...
pub struct NextBlockEnvAttributes {
    ...
    /// Optional slot number for post-SilaAmsterdam payloads.
