...
use crate::{ConfigureEvm, SavmEnvFor};
use rsil_primitives_traits::{BlockTy, HeaderTy, SealedBlock, SealedHeader};

/// A no-op SAVM config that panics on any call. Used as a typesystem hack to satisfy
/// [`ConfigureEvm`] bounds.
#[derive(Debug, Clone)]
...
impl<Inner> NoopEvmConfig<Inner> {
    /// Create a new instance of the no-op SAVM config.
...
{
    fn evm_env_for_payload(&self, payload: &T) -> Result<SavmEnvFor<Self>, Self::Error> {
