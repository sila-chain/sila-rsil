    ...
    SubCmd: ExtendedCommand + Subcommand + fmt::Debug,
{
    ...
    /// [`NodeCommand`](rsil_cli_commands::node::NodeCommand).
        ...
        C: ChainSpecParser<ChainSpec = ChainSpec>,
    {
        ...
        let components = move |spec: Arc<ChainSpec>| {
            let (evm_config, _) =
                rsil_node_sila::node::build_evm_config(spec.clone(), &jit_args, None)
                    .expect("failed to start revmc JIT backend");
            (evm_config, Arc::new(SilBeaconConsensus::new(spec)))
        };

        self.run_with_components::<SilaNode>(components, async move |builder, ext| {
    ...
    ///
    /// This accepts a closure that is used to launch the node via the
    /// [`NodeCommand`](rsil_cli_commands::node::NodeCommand) and allows providing custom
    /// components.
        ...
        C: ChainSpecParser<ChainSpec = N::ChainSpec>,
    {
            ...
            None => {
                let runtime_config = match &self.cli.command {
                    Commands::Node(command) => {
                        rsil_tasks::RuntimeConfig::default().with_rayon(RayonConfig {
                    ...
                    _ => rsil_tasks::RuntimeConfig::default(),
                };
        ...
        rsil_tasks::utils::deprioritize_background_threads();
...
mod tests {
    use super::*;
    use crate::chainspec::SilaChainSpecParser;
    use clap::Parser;
    use rsil_cli_commands::node::NoArgs;

    #[test]
    fn test_cli_app_creation() {
        let args = vec!["rsil", "config"];
        let cli = Cli::<SilaChainSpecParser, NoArgs>::try_parse_from(args).unwrap();
