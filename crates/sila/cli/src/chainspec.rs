...
/// Chains supported by rsil. First value should be used as the default.
pub const SUPPORTED_CHAINS: &[&str] = &["sila-mainnet", "sepolia", "holesky", "hoodi", "dev"];
...
pub fn chain_value_parser(s: &str) -> eyre::Result<Arc<ChainSpec>, eyre::Error> {
    Ok(match s {
        "sila-mainnet" => SILA_MAINNET.clone(),
...
/// Sila chain specification parser.
#[derive(Debug, Clone, Default)]
#[non_exhaustive]
pub struct SilaChainSpecParser;

impl ChainSpecParser for SilaChainSpecParser {
...
mod tests {
    ...
    fn parse_known_chain_spec() {
        for &chain in SilaChainSpecParser::SUPPORTED_CHAINS {
            assert!(<SilaChainSpecParser as ChainSpecParser>::parse(chain).is_ok());
        }
    }
  ...
  "config": {
    ...
    "osakaTime": 0
        ...
        let spec = <SilaChainSpecParser as ChainSpecParser>::parse(s).unwrap();
