...
use crate::{app::CliApp, chainspec::SilaChainSpecParser};
use clap::{Parser, Subcommand};
use rsil_chainspec::{ChainSpec, Hardforks};
...
/// The main rsil cli interface.
///
/// This is the entrypoint to the executable.
#[derive(Debug, Parser)]
#[command(author, name = version_metadata().name_client.as_ref(), version = version_metadata().short_version.as_ref(), long_version = version_metadata().long_version.as_ref(), about = "Rsil", long_about = None)]
pub struct Cli<
    C: ChainSpecParser = SilaChainSpecParser,
    ...
    > Cli<C, Ext, Rpc, SubCmd>
{
    ...
    ///
    /// ```no_run
    /// use rsil_sila_cli::interface::Cli;
    /// use rsil_node_sila::SilaNode;
    ///
    /// Cli::parse_args()
    ///     .run(async move |builder, _| {
    ///         let handle = builder.launch_node(SilaNode::default()).await?;
    ///
    ...
    /// }
    ///
    /// Cli::<SilaChainSpecParser, MyArgs>::parse()
    ...
    pub fn init_tracing(
        ...
        info!(target: "rsil::cli", "Initialized tracing, debug log directory: {}", self.logs.log_file_directory);

        match otlp_status {
            OtlpInitStatus::Started(endpoint) => {
                info!(target: "rsil::cli", "Started OTLP {:?} tracing export to {endpoint}", self.traces.protocol);
            }
            OtlpInitStatus::NoFeature => {
                warn!(target: "rsil::cli", "Provided OTLP tracing arguments do not have effect, compile with the `otlp` feature")
            }
        ...
        match otlp_logs_status {
            OtlpLogsStatus::Started(endpoint) => {
                info!(target: "rsil::cli", "Started OTLP {:?} logs export to {endpoint}", self.traces.protocol);
            }
            OtlpLogsStatus::NoFeature => {
                warn!(target: "rsil::cli", "Provided OTLP logs arguments do not have effect, compile with the `otlp-logs` feature")
            }
...
> {
    ...
    TestVectors(rsil_cli_commands::test_vectors::Command),
    ...
    Commands<C, Ext, SubCmd>
{
    ...
    pub fn debug_namespace_enabled(&self) -> bool {
        match self {
            Self::Node(cmd) => cmd.rpc.is_namespace_enabled(RsilRpcModule::Debug),
            _ => false,
        }
    }
}
...
mod tests {
    ...
    use rsil_chainspec::{SilChainSpec, SEPOLIA};
    use rsil_node_core::args::ColorMode;

    #[test]
    fn parse_color_mode() {
        let rsil = Cli::try_parse_args_from(["rsil", "node", "--color", "always"]).unwrap();
        assert_eq!(rsil.logs.color, ColorMode::Always);
    }

    #[test]
    fn node_command_mut_accessor_returns_node_command() {
        let mut rsil = Cli::try_parse_args_from(["rsil", "node"]).unwrap();

        let node_command = rsil.as_node_command_mut().expect("expected node command");
        node_command.with_unused_ports = true;

        assert!(rsil.as_node_command_mut().unwrap().with_unused_ports);
    }

    #[test]
    fn apply_node_command_only_runs_for_node_command() {
        let mut rsil = Cli::try_parse_args_from(["rsil", "node"]).unwrap();
        rsil.apply_node_command(|node_command| node_command.with_unused_ports = true);
        assert!(rsil.as_node_command_mut().unwrap().with_unused_ports);

        let mut rsil = Cli::try_parse_args_from(["rsil", "config"]).unwrap();
        let mut applied = false;
        rsil.apply_node_command(|_| applied = true);

        assert!(rsil.as_node_command_mut().is_none());
        assert!(!applied);
    }
    ...
    fn test_parse_help_all_subcommands() {
        let rsil = Cli::<SilaChainSpecParser, NoArgs>::command();
        for sub_command in rsil.get_subcommands() {
            let err = Cli::try_parse_args_from(["rsil", sub_command.get_name(), "--help"])
                .err()
                .unwrap_or_else(|| {
    ...
    fn parse_logs_path_node() {
        let mut rsil = Cli::try_parse_args_from(["rsil", "node"]).unwrap();
        if let Some(chain_spec) = rsil.command.chain_spec() {
            rsil.logs.log_file_directory =
                rsil.logs.log_file_directory.join(chain_spec.chain.to_string());
        }
        let log_dir = rsil.logs.log_file_directory;
        let end = format!("rsil/logs/{}", SUPPORTED_CHAINS[0]);
        assert!(log_dir.as_ref().ends_with(end), "{log_dir:?}");

        let mut iter = SUPPORTED_CHAINS.iter();
        iter.next();
        for chain in iter {
            let mut rsil = Cli::try_parse_args_from(["rsil", "node", "--chain", chain]).unwrap();
            let chain =
                rsil.command.chain_spec().map(|c| c.chain.to_string()).unwrap_or(String::new());
            rsil.logs.log_file_directory = rsil.logs.log_file_directory.join(chain.clone());
            let log_dir = rsil.logs.log_file_directory;
            let end = format!("rsil/logs/{chain}");
            assert!(log_dir.as_ref().ends_with(end), "{log_dir:?}");
        }
    }
    ...
    fn parse_logs_path_init() {
        let mut rsil = Cli::try_parse_args_from(["rsil", "init"]).unwrap();
        if let Some(chain_spec) = rsil.command.chain_spec() {
            rsil.logs.log_file_directory =
                rsil.logs.log_file_directory.join(chain_spec.chain.to_string());
        }
    ...
    fn log_file_max_files_defaults() {
        use rsil_node_core::args::LogArgs;

        // Node command without explicit --log.file.max-files should get Some(5) after
        // apply_node_defaults
        let mut cli = Cli::try_parse_args_from(["rsil", "node"]).unwrap();
        ...
        let cli = Cli::try_parse_args_from(["rsil", "config"]).unwrap();
        assert!(cli.logs.log_file_max_files.is_none());
        assert_eq!(cli.logs.effective_log_file_max_files(), 0);

        // Explicitly set value should be preserved for node command
        let mut cli =
            Cli::try_parse_args_from(["rsil", "node", "--log.file.max-files", "10"]).unwrap();
        ...
        let cli =
            Cli::try_parse_args_from(["rsil", "config", "--log.file.max-files", "3"]).unwrap();
        ...
        let cli = Cli::try_parse_args_from(["rsil", "node", "--log.file.max-files", "0"]).unwrap();
    ...
    fn parse_env_filter_directives() {
        ...
        let rsil = Cli::try_parse_args_from([
            "rsil",
            "init",
            "--datadir",
        ...
        .unwrap();
        assert!(rsil.run(async move |_, _| Ok(())).is_ok());
    }
    ...
    fn test_rpc_module_validation() {
        ...
        let cli =
            Cli::try_parse_args_from(["rsil", "node", "--http.api", "sil,admin,debug"]).unwrap();
        ...
        if let Commands::Node(command) = &cli.command {
            if let Some(http_api) = &command.rpc.http_api {
                ...
                assert!(modules.contains(&RsilRpcModule::Sil));
                assert!(modules.contains(&RsilRpcModule::Admin));
                assert!(modules.contains(&RsilRpcModule::Debug));
            } else {
        ...
        let cli =
            Cli::try_parse_args_from(["rsil", "node", "--http.api", "sil,customrpc"]).unwrap();
    ...
    fn test_rpc_module_unknown_rejected() {
        ...
        let cli =
            Cli::try_parse_args_from(["rsil", "node", "--http.api", "unknownmodule"]).unwrap();
    ...
    fn parse_unwind_chain() {
        let cli = Cli::try_parse_args_from([
            "rsil", "stage", "unwind", "--chain", "sepolia", "to-block", "100",
        ])
        .unwrap();
        match cli.command {
    ...
    fn test_extensible_subcommands() {
        ...
        use rsil_cli_runner::CliRunner;
        use rsil_rpc_server_types::DefaultRpcModuleValidator;
        ...
        let cli = Cli::<
            ...
            CustomCommands,
        >::try_parse_from(["rsil", "hello", "--name", "world"])
        .unwrap();
