...
///
/// According to the [engine API specification](https://github.com/sila-chain/execution-apis/blob/main/src/engine/README.md) the execution layer should build the initial version of the payload with an empty transaction set and then keep update it in order to maximize the revenue.
///
...
/// more transactions.
#[derive(Debug, Clone)]
pub struct SilBuiltPayload<N: NodePrimitives = SilPrimitives> {
    /// The built block
...
impl<N: NodePrimitives> SilBuiltPayload<N> {
...
impl SilBuiltPayload {
    /// Try converting built payload into [`ExecutionPayloadEnvelopeV3`].
    ///
    /// Returns an error if the payload contains non SIP-4844 sidecar.
    pub fn try_into_v3(self) -> Result<ExecutionPayloadEnvelopeV3, BuiltPayloadConversionError> {
        ...
        let blobs_bundle = match sidecars {
            ...
            BlobSidecars::Sip4844(sidecars) => BlobsBundleV1::from(sidecars),
            BlobSidecars::Sip7594(_) => {
        ...
        Ok(ExecutionPayloadEnvelopeV3 {
            ...
            //
            // Spec:
            // <https://github.com/sila-chain/execution-apis/blob/fe8e13c288c592ec154ce25c534e26cb7ce0530d/src/engine/cancun.md#specification-2>
    ...
    pub fn try_into_v5(self) -> Result<ExecutionPayloadEnvelopeV5, BuiltPayloadConversionError> {
        ...
        let blobs_bundle = match sidecars {
            ...
            BlobSidecars::Sip7594(sidecars) => BlobsBundleV2::from(sidecars),
            BlobSidecars::Sip4844(_) => {
...
impl<N: NodePrimitives> BuiltPayload for SilBuiltPayload<N> {
    type Primitives = N;
...
impl From<SilBuiltPayload> for ExecutionPayloadV1 {
    fn from(value: SilBuiltPayload) -> Self {
...
impl From<SilBuiltPayload> for ExecutionPayloadEnvelopeV2 {
    fn from(value: SilBuiltPayload) -> Self {
        let SilBuiltPayload { block, fees, .. } = value;
...
impl TryFrom<SilBuiltPayload> for ExecutionPayloadEnvelopeV3 {
    type Error = BuiltPayloadConversionError;

    fn try_from(value: SilBuiltPayload) -> Result<Self, Self::Error> {
        value.try_into_v3()
    }
}

impl TryFrom<SilBuiltPayload> for ExecutionPayloadEnvelopeV4 {
    type Error = BuiltPayloadConversionError;
...
impl TryFrom<SilBuiltPayload> for ExecutionPayloadEnvelopeV5 {
    type Error = BuiltPayloadConversionError;
...
impl TryFrom<SilBuiltPayload> for ExecutionPayloadEnvelopeV6 {
    type Error = BuiltPayloadConversionError;
...
impl From<SilBuiltPayload> for ExecutionData {
    fn from(value: SilBuiltPayload) -> Self {
...
#[cfg(feature = "std")]
impl From<SilBuiltPayload> for rsil_engine_primitives::BigBlockData<ExecutionData> {
    fn from(_value: SilBuiltPayload) -> Self {
        unreachable!("payload building is not supported for big blocks");
    }
}

/// An enum representing blob transaction sidecars belonging to [`SilBuiltPayload`].
#[derive(Clone, Default, Debug)]
pub enum BlobSidecars {
    /// No sidecars (default).
    #[default]
    Empty,
    /// SIP-4844 style sidecars.
    Sip4844(Vec<BlobTransactionSidecar>),
    /// SIP-7594 style sidecars.
    Sip7594(Vec<BlobTransactionSidecarEip7594>),
}

impl BlobSidecars {
    /// Create new SIP-4844 style sidecars.
    pub const fn sip4844(sidecars: Vec<BlobTransactionSidecar>) -> Self {
        Self::Sip4844(sidecars)
    }

    /// Create new SIP-7594 style sidecars.
    pub const fn sip7594(sidecars: Vec<BlobTransactionSidecarEip7594>) -> Self {
        Self::Sip7594(sidecars)
    }

    /// Push SIP-4844 blob sidecar. Ignores the item if sidecars already contain SIP-7594 sidecars.
    pub fn push_eip4844_sidecar(&mut self, sidecar: BlobTransactionSidecar) {
        match self {
            Self::Empty => {
                *self = Self::Sip4844(Vec::from([sidecar]));
            }
            Self::Sip4844(sidecars) => {
    ...
    /// Push SIP-7594 blob sidecar. Ignores the item if sidecars already contain SIP-4844 sidecars.
    pub fn push_eip7594_sidecar(&mut self, sidecar: BlobTransactionSidecarEip7594) {
        match self {
            Self::Empty => {
                *self = Self::Sip7594(Vec::from([sidecar]));
            }
            Self::Sip7594(sidecars) => {
                sidecars.push(sidecar);
            }
...
mod tests {
    ...
    fn into_execution_data_preserves_requests() {
        ...
        let mut block = rsil_sila_primitives::Block::default();
        ...
        let payload = SilBuiltPayload::new(
    ...
    fn execution_data_has_infallible_try_from_impl() {
        fn assert_try_from<T: TryFrom<SilBuiltPayload, Error = core::convert::Infallible>>() {}
