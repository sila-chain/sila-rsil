...
//!
//! Implements the [SIP-8178] SSZ Engine API routes under `/engine/v1`.
//!
//! [SIP-8178]: https://sips.sila.org/SIPS/sip-8178
...
use alloy_eips::{sip2718::Decodable2718, sip7685::RequestsOrHash};
...
const ETH_EXECUTION_VERSION: &str = "sil-execution-version";
...
type SilEngineApi<Provider, Pool, Validator, ChainSpec> =
    EngineApi<Provider, SilEngineTypes, Pool, Validator, ChainSpec>;
type SharedEthEngineApi<Provider, Pool, Validator, ChainSpec> =
    Arc<RwLock<Option<SilEngineApi<Provider, Pool, Validator, ChainSpec>>>>;
    ...
    EngineSszProxyHandle<ChainSpec, Provider, Pool, Validator>
{
    ...
    fn with_engine_api(engine_api: SilEngineApi<Provider, Pool, Validator, ChainSpec>) -> Self {
    ...
    pub async fn set_engine_api(
        &self,
        engine_api: SilEngineApi<Provider, Pool, Validator, ChainSpec>,
    ) {
    ...
    EngineSszProxyHandle<ChainSpec, Provider, Pool, Validator>
{
    ...
    pub async fn engine_api(&self) -> Option<SilEngineApi<Provider, Pool, Validator, ChainSpec>> {
...
where
    ...
    Validator: EngineApiValidator<SilEngineTypes>,
...
impl EngineSszFork {
    const fn payloads_version(self) -> u8 {
        match self {
            ...
            Self::SilaShanghai => 2,
            Self::SilaCancun => 3,
            Self::SilaPrague | Self::SilaOsaka => 4,
            Self::SilaAmsterdam => 5,
        }
    }
    ...
    const fn forkchoice_version(self) -> u8 {
        match self {
            Self::SilaParis => 1,
            Self::SilaShanghai => 2,
            Self::SilaCancun | Self::SilaPrague | Self::SilaOsaka => 3,
            Self::SilaAmsterdam => 4,
        }
    }
}
...
impl std::str::FromStr for EngineSszFork {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        match value {
            "paris" => Ok(Self::SilaParis),
            "shanghai" => Ok(Self::SilaShanghai),
            "cancun" => Ok(Self::SilaCancun),
            "prague" => Ok(Self::SilaPrague),
            "osaka" => Ok(Self::SilaOsaka),
            "amsterdam" => Ok(Self::SilaAmsterdam),
            _ => Err(()),
        }
    }
}
...
{
    match fork {
        EngineSszFork::SilaParis => match engine_api.get_payload_v2_metered(payload_id).await {
            Ok(payload) => {
        ...
        EngineSszFork::SilaShanghai => match engine_api.get_payload_v2_metered(payload_id).await {
        ...
        EngineSszFork::SilaCancun => match engine_api.get_payload_v3_metered(payload_id).await {
            Ok(payload) => get_payload_response(BuiltPayloadCancun::from(payload)),
            Err(err) => get_payload_error_response(err),
        },
        EngineSszFork::SilaPrague => match engine_api.get_payload_v4_metered(payload_id).await {
            Ok(payload) => get_payload_response(BuiltPayloadPrague::from(payload)),
            Err(err) => get_payload_error_response(err),
        },
        EngineSszFork::SilaOsaka => match engine_api.get_payload_v5_metered(payload_id).await {
            Ok(payload) => get_payload_response(BuiltPayloadOsaka::from(payload)),
            Err(err) => get_payload_error_response(err),
        },
        EngineSszFork::SilaAmsterdam => match engine_api.get_payload_v6_metered(payload_id).await {
...
/// Decodes the SilaAmsterdam getBlobs request container with hashes and a cell index mask.
...
mod tests {
    ...
    fn decodes_forkchoice_v4_with_custody_columns() {
        ...
        let (decoded_state, decoded_attrs, custody_columns) =
            decode_forkchoice_request(EngineSszFork::SilaAmsterdam, &encoded).unwrap();
    ...
    fn decodes_forkchoice_cancun_payload_attributes() {
        ...
        let (decoded_state, decoded_attrs, custody_columns) =
            decode_forkchoice_request(EngineSszFork::SilaCancun, &encoded).unwrap();
        assert_eq!(decoded_state, forkchoice_state);
