...
async fn can_send_legacy_sidecar_post_activation() -> eyre::Result<()> {
    ...
    // fetch it from rpc
    let envelope = node.rpc.envelope_by_hash(blob_tx_hash).await?;
    // assert that sidecar was converted to sip7594 (force upcasting is enabled)
...
async fn blob_conversion_at_osaka() -> eyre::Result<()> {
    ...
    // SilaOsaka activates in 2 slots
    ...
    // build last SilaPrague payload
    node.payload.timestamp = current_timestamp + 1;
    let prague_payload = node.new_payload().await?;
    assert!(matches!(prague_payload.sidecars(), BlobSidecars::Sip4844(_)));
