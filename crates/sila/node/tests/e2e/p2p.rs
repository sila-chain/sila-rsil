...
use rsil_network::{NetworkInfo, PeersInfo};
use rsil_node_builder::{NodeBuilder, NodeHandle};
use rsil_node_core::{args::NetworkArgs, node_config::NodeConfig};
use rsil_node_sila::SilaNode;
use rsil_rpc_api::SilApiServer;
...
async fn e2e_test_send_transactions() -> eyre::Result<()> {
    ...
    let (mut nodes, _) = setup_engine::<SilaNode>(
        2,
        chain_spec.clone(),
        false,
        Default::default(),
...
async fn test_tx_propagation() -> eyre::Result<()> {
    ...
    let mut build_tx = || {
        ...
        TxEnvelope::Sip1559(tx.into_signed(signature))
    };

    // Setup 10 nodes
    let (mut nodes, _) = setup_engine_with_connection::<SilaNode>(
        10,
        chain_spec.clone(),
        false,
        Default::default(),
