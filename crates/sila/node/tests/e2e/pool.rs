...
async fn maintain_txpool_stale_eviction() -> eyre::Result<()> {
    ...
    let config = rsil_transaction_pool::maintain::MaintainPoolConfig {
    ...
    let tx = Recovered::new_unchecked(
        SilaTxEnvelope::<TxEip4844>::from(envelop.clone()),
        Default::default(),
    );
    let pooled_tx = SilPooledTransaction::new(tx.clone(), 200);
...
async fn maintain_txpool_reorg() -> eyre::Result<()> {
    ...
    runtime.spawn_critical_task(
        ...
        rsil_transaction_pool::maintain::maintain_transaction_pool_future(
            ...
            node.inner.provider.clone().canonical_state_stream(),
            runtime.clone(),
            rsil_transaction_pool::maintain::MaintainPoolConfig::default(),
        ),
    );
