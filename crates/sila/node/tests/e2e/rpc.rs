...
use alloy_provider::{network::SilaWallet, Provider, ProviderBuilder, SendableTx};
...
use rsil_chainspec::{ChainSpecBuilder, SilChainSpec, SILA_MAINNET};
use rsil_e2e_test_utils::setup_engine;
use rsil_network::{types::NatResolver, PeersInfo};
...
use rsil_rpc_api::servers::AdminApiServer;
...
fn update_block_hash_v6(request: &mut BuilderBlockValidationRequestV6) -> eyre::Result<()> {
    let block_hash = ExecutionPayload::V4(request.request.execution_payload.clone())
        .try_into_block_with_sidecar::<rsil_sila_primitives::TransactionSigned>(
...
async fn test_eth_config() -> eyre::Result<()> {
    ...
    let config = provider.client().request_noparams::<SilConfig>("eth_config").await?;
...
// <https://github.com/sila-chain/sila-rsil/issues/19765>
#[tokio::test]
async fn test_admin_external_ip() -> eyre::Result<()> {
    ...
    let chain_spec =
        Arc::new(ChainSpecBuilder::default().chain(SILA_MAINNET.chain).genesis(genesis).build());
