...
//! - Post-Dencun (SIP-6780): SELFDESTRUCT only works in same-tx creation, state persists
//!
...
use rsil_node_api::TreeConfig;
use rsil_node_sila::SilaNode;
use rsil_revm::db::BundleAccount;
use std::sync::Arc;
...
/// This tests the SIP-6780 exception where SELFDESTRUCT in same tx as creation still works.
///
/// The contract:
...
/// Tests SELFDESTRUCT behavior post-Dencun (SilaCancun+).
///
/// Post-Dencun (SIP-6780):
...
async fn test_selfdestruct_post_dencun() -> eyre::Result<()> {
    ...
    assert!(
        ...
        "Post-Dencun (SIP-6780): Account should NOT be destroyed when SELFDESTRUCT called on existing contract"
    );
...
async fn test_selfdestruct_same_tx_post_dencun() -> eyre::Result<()> {
    ...
    // Send SIL to the destroyed address in a new block to test cache behavior
    let pending = provider
        .send_transaction(transfer_tx(signer.address(), contract_address, 1, U256::from(1000)))
        .await?;
    node.advance_block().await?;
    let receipt = pending.get_receipt().await?;
    assert!(receipt.status(), "SIL transfer to destroyed address should succeed");

    // Consume the canonical notification
    let _ = node.canonical_stream.next().await;

    // Verify code is still empty and account received SIL
    let code_final = provider.get_code_at(contract_address).await?;
    ...
    assert_eq!(balance, U256::from(1000), "Post-Dencun same-tx: Account should have received SIL");

    Ok(())
}

/// Tests SELFDESTRUCT behavior pre-Dencun (SilaShanghai).
///
/// Pre-Dencun:
...
async fn test_selfdestruct_pre_dencun() -> eyre::Result<()> {
    ...
    // Send SIL to the destroyed contract address in a new block.
    // This tests cache behavior - the cache should correctly reflect the account was destroyed.
    // Pre-Dencun: the contract no longer exists, so this is just a plain SIL transfer.
    let pending = provider
        .send_transaction(transfer_tx(signer.address(), contract_address, 2, U256::from(1000)))
        .await?;
    node.advance_block().await?;
    let receipt = pending.get_receipt().await?;
    assert!(receipt.status(), "SIL transfer to destroyed contract address should succeed");
    ...
    // Verify the output state shows the account exists (received SIL) but has no code
    let account_state: Option<&BundleAccount> = execution_outcome.bundle.account(&contract_address);
    // After receiving SIL, the account should exist with balance but no code
    assert!(
        account_state.is_some(),
        "Pre-Dencun: Account should exist after receiving SIL (even though contract was destroyed)"
    );

    // Verify code is still empty (contract was destroyed, only SIL was received)
    let code_final = provider.get_code_at(contract_address).await?;
    ...
    // Verify the account now has the SIL balance we sent
    let balance = provider.get_balance(contract_address).await?;
    assert_eq!(balance, U256::from(1000), "Pre-Dencun: Account should have received SIL");

    Ok(())
}

/// Tests SELFDESTRUCT in same transaction as creation, where account previously had SIL
/// (post-Dencun).
///
/// Post-Dencun (SIP-6780):
/// - The same-tx exception applies when the CONTRACT is created in that transaction
/// - Even if the address previously had SIL (as an EOA), deploying a contract there and
...
///
/// This test verifies:
/// 1. Send SIL to the future contract address (address has balance but no code)
...
/// 5. Since account existed in DB before (had SIL), bundle marks it as Destroyed
#[tokio::test]
async fn test_selfdestruct_same_tx_preexisting_account_post_dencun() -> eyre::Result<()> {
    ...
    // Send SIL to the future contract address first (makes it a pre-existing account)
    let pending = provider
    ...
    node.advance_block().await?;
    let receipt = pending.get_receipt().await?;
    assert!(receipt.status(), "SIL transfer should succeed");
    ...
    assert_eq!(balance_before, U256::from(1000), "Account should have SIL before deployment");
    ...
    // The account should be marked as destroyed. Since it had prior state (SIL balance),
    ...
    assert!(
        ...
        "Post-Dencun same-tx with prior SIL: Account MUST be marked as destroyed"
    );
    ...
    // Send SIL to the destroyed address to verify cache behavior
    let pending = provider
        .send_transaction(transfer_tx(
            signer.address(),
            future_contract_address,
    ...
    // Verify the account received SIL and has no code (it's now just an EOA)
    let balance_final = provider.get_balance(future_contract_address).await?;
    assert_eq!(balance_final, U256::from(2000), "Account should have received SIL");

    let code_final = provider.get_code_at(future_contract_address).await?;
    assert!(code_final.is_empty(), "Code should remain empty after SIL transfer");
