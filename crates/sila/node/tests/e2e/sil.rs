...
use rsil_node_builder::{rpc::BasicEngineApiBuilder, EngineApiExt, NodeBuilder, NodeHandle};
use rsil_node_core::{
    args::RpcServerArgs,
...
use rsil_node_sila::{
    ...
    SilaAddOns, SilaEngineValidatorBuilder, SilaNode,
};
use rsil_provider::BlockNumReader;
use rsil_rpc_api::TestingBuildBlockRequestV1;
use rsil_rpc_layer::secret_to_bearer_header;
use rsil_tasks::Runtime;
use ssz::{Decode, Encode};
use std::sync::Arc;

const ENGINE_EXECUTION_VERSION_HEADER: &str = "Sil-Execution-Version";
    ...
    let node_config =
        NodeConfig::test().with_chain(chain_spec.clone()).with_unused_ports().with_rpc(
            RpcServerArgs::default()
                .with_unused_ports()
                .with_http()
                .with_http_api(rsil_rpc_server_types::RpcModuleSelection::All),
        );
...
async fn test_share_sparse_trie_with_payload_builder() -> eyre::Result<()> {
    ...
    let (mut nodes, _wallet) = setup_engine::<SilaNode>(
        1,
        Arc::new(
            ChainSpecBuilder::default()
                .chain(SILA_MAINNET.chain)
