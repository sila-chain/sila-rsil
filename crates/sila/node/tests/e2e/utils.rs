...
/// Helper function to create SilaAmsterdam payload attributes.
///
/// SilaAmsterdam payloads include block access lists, and the payload builder expects a
...
pub(crate) async fn advance_with_random_transactions<Provider>(
    node: &mut NodeHelperType<SilaNode, Provider>,
    num_blocks: usize,
    rng: &mut impl Rng,
    finalize: bool,
) -> eyre::Result<()>
where
    Provider: FullProvider<NodeTypesWithDBAdapter<SilaNode, TmpDB>>,
{
    ...
    for _ in 0..num_blocks {
        ...
        for _ in 0..tx_count {
            ...
            let should_create =
                rng.random::<bool>() && tx_type != TxType::Sip4844 && tx_type != TxType::Sip7702;
            if should_create {
            ...
            if matches!(tx_type, TxType::Legacy | TxType::Sip2930) {
                tx = tx.with_gas_price(provider.get_gas_price().await?);
            }

            if rng.random::<bool>() || tx_type == TxType::Sip2930 {
