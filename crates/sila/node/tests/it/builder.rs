...
use rsil_node_builder::{EngineNodeLauncher, FullNodeComponents, NodeBuilder, NodeConfig};
use rsil_node_sila::node::{SilaAddOns, SilaNode};
use rsil_provider::providers::BlockchainProvider;
use rsil_rpc_builder::Identity;
...
async fn test_eth_launcher() {
    ...
    let _builder =
        NodeBuilder::new(config)
            ...
            .with_types_and_provider::<SilaNode, BlockchainProvider<
                NodeTypesWithDBAdapter<SilaNode, Arc<TempDatabase<DatabaseEnv>>>,
            >>()
            .with_components(SilaNode::components())
            .with_add_ons(SilaAddOns::default())
            .apply(|builder| {
    ...
    main_rt.block_on(async {
        ...
        let _builder =
            NodeBuilder::new(config)
                ...
                .with_components(SilaNode::components())
                .with_add_ons(
                    SilaAddOns::default().with_tokio_runtime(Some(custom_rt.handle().clone())),
                )
                .apply(|builder| {
                    let _ = builder.db();
                    builder
...
fn test_node_setup() {
    ...
    let db = create_test_rw_db();
    let _builder =
        NodeBuilder::new(config).with_database(db).node(SilaNode::default()).check_launch();
}
