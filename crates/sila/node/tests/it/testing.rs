...
async fn testing_rpc_build_block_works() -> eyre::Result<()> {
    ...
    let mut rpc_args = rsil_node_core::args::RpcServerArgs::default().with_http();
    rpc_args.http_api = Some(RpcModuleSelection::from_iter([RsilRpcModule::Testing]));
    ...
    let builder = NodeBuilder::new(config)
        ...
        .on_rpc_started(move |ctx, handles| {
            ...
            let payload_attributes = SilPayloadAttributes {
...
async fn testing_rpc_commit_block_works() -> eyre::Result<()> {
    ...
    let mut rpc_args = rsil_node_core::args::RpcServerArgs::default().with_http();
    rpc_args.http_api =
        Some(RpcModuleSelection::from_iter([RsilRpcModule::Sil, RsilRpcModule::Testing]));
    let tempdir = tempdir().expect("temp datadir");
    let datadir_args = DatadirArgs {
        datadir: MaybePlatformPath::<DataDirPath>::from_str(tempdir.path().to_str().unwrap())
            .expect("valid datadir"),
        ...
        .on_rpc_started(move |ctx, handles| {
            ...
            tokio::spawn(async move {
                let res: eyre::Result<()> = async {
                    let extra_data = Bytes::from_static(b"rsil");
