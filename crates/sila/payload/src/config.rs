...
impl Default for SilaBuilderConfig {
    fn default() -> Self {
        Self::new()
    }
}

impl SilaBuilderConfig {
    ...
    /// Set the maximum number of blobs per block (SIP-7872).
...
mod tests {
    ...
    fn gas_limit_uses_payload_target_when_present() {
        ...
        let config = SilaBuilderConfig::new().with_gas_limit(parent_gas_limit + 100);
    ...
    fn gas_limit_falls_back_to_configured_target() {
        ...
        let config = SilaBuilderConfig::new().with_gas_limit(desired_gas_limit);
