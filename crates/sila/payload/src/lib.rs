//! A basic Sila payload builder implementation.
...
pub mod validator;
pub use validator::SilaExecutionPayloadValidator;

type BestTransactionsIter<Pool> = Box<
    dyn BestTransactions<Item = Arc<ValidPoolTransaction<<Pool as TransactionPool>::Transaction>>>,
>;

/// Sila payload builder
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SilaPayloadBuilder<Pool, Client, SavmConfig = SilEvmConfig> {
    /// Client providing access to node state.
    client: Client,
    /// Transaction pool.
    pool: Pool,
    /// The type responsible for creating the savm.
    evm_config: SavmConfig,
    /// Payload builder configuration.
    builder_config: SilaBuilderConfig,
}

impl<Pool, Client, SavmConfig> SilaPayloadBuilder<Pool, Client, SavmConfig> {
    /// `SilaPayloadBuilder` constructor.
    pub const fn new(
        client: Client,
        pool: Pool,
        evm_config: SavmConfig,
...
impl<Pool, Client, SavmConfig> PayloadBuilder for SilaPayloadBuilder<Pool, Client, SavmConfig>
where
    SavmConfig: ConfigureEvm<Primitives = SilPrimitives, NextBlockEnvCtx = NextBlockEnvAttributes>,
    Client: StateProviderFactory + ChainSpecProvider<ChainSpec: SilaHardforks> + Clone,
    Pool: TransactionPool<Transaction: PoolTransaction<Consensus = TransactionSigned>>,
{
    type Attributes = SilPayloadAttributes;
    type BuiltPayload = SilBuiltPayload;

    fn try_build(
        &self,
        args: BuildArguments<SilPayloadAttributes, SilBuiltPayload>,
    ) -> Result<BuildOutcome<SilBuiltPayload>, PayloadBuilderError> {
        default_sila_payload(
    ...
    ) -> Result<SilBuiltPayload, PayloadBuilderError> {
...
/// Constructs an Sila transaction payload using the best transactions from the pool.
///
/// Given build arguments including an Sila client, transaction pool,
/// and configuration, this function creates a transaction payload. Returns
/// a result indicating success with the payload or an error in case of failure.
#[inline]
pub fn default_sila_payload<SavmConfig, Client, Pool, F>(
    evm_config: SavmConfig,
    client: Client,
    pool: Pool,
    builder_config: SilaBuilderConfig,
...
where
    SavmConfig: ConfigureEvm<Primitives = SilPrimitives, NextBlockEnvCtx = NextBlockEnvAttributes>,
    Client: StateProviderFactory + ChainSpecProvider<ChainSpec: SilaHardforks>,
    Pool: TransactionPool<Transaction: PoolTransaction<Consensus = TransactionSigned>>,
    F: FnOnce(BestTransactionsAttributes) -> BestTransactionsIter<Pool>,
{
    ...
    // Apply user-configured blob limit (SIP-7872)
    // Per SIP-7872: if the minimum is zero, set it to one
    ...
    while let Some(pool_tx) = best_txs.next() {
        ...
        // the SIP-4844 can still fit in the block
            ...
            let blob_sidecar_result = 'sidecar: {
                ...
                if is_osaka {
                    if sidecar.is_eip7594() {
                        ...
                        Err(Sip4844PoolTransactionError::UnexpectedEip4844SidecarAfterOsaka)
                    }
                } else if sidecar.is_eip4844() {
                    Ok(sidecar)
                } else {
                    Err(Sip4844PoolTransactionError::UnexpectedEip7594SidecarBeforeOsaka)
                }
            };
    ...
    let payload = SilBuiltPayload::new(Arc::new(block), total_fees, requests, block_access_list)
