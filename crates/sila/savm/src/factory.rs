...
impl Default for RsilEvmFactory {
    fn default() -> Self {
        #[cfg(feature = "jit")]
        {
...
impl RsilEvmFactory {
    ...
    /// Creates a [`RsilEvmFactory`] with JIT support and compilation disabled.
    ...
    ///
    /// Enabling support only selects the JIT-capable SAVM factory path. An SAVM still requires the
    /// `jit` feature and runtime compilation enabled by `--jit` or `rsil_jit` before it can execute
    /// JIT-compiled code.
...
#[cfg(feature = "jit")]
#[derive(rsil_metrics::Metrics, Clone)]
