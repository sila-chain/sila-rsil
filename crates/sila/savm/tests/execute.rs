...
fn sip_4788_non_genesis_call() {
    ...
    let provider = SilEvmConfig::new(chain_spec);
...
#[test]
fn sip_4788_empty_account_call() {
...
fn sip_4788_genesis_call() {
    ...
    let transition_state = executor.with_state_mut(|state| {
        state.transition_state.take().expect("the savm should be initialized with bundle updates")
    });
...
/// Create a state provider with blockhashes and the SIP-2935 system contract.
...
#[test]
fn sip_2935_fork_activation_genesis() {
...
#[test]
fn sip_2935_fork_activation_within_window_bounds() {
...
// <https://github.com/sila-chain/SIPs/pull/9144>
#[test]
fn sip_2935_fork_activation_outside_window_bounds() {
...
#[test]
fn sip_2935_state_transition_inside_fork() {
...
fn block_gas_limit_error() {
    // Create a chain specification with fork conditions set for SilaPrague
    let chain_spec = Arc::new(
        ChainSpecBuilder::from(&*SILA_MAINNET)
            .shanghai_activated()
    ...
    // Insert the sender account into the state with a nonce of 1 and a balance of 1 SIL in Wei
    db.insert_account_info(
        sender_address,
        AccountInfo { nonce: 1, balance: U256::from(ETH_TO_WEI), ..Default::default() },
    );
    ...
    let evm_config = SilEvmConfig::new(chain_spec);
