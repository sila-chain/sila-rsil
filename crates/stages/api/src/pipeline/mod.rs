...
use rsil_primitives_traits::constants::BEACON_CONSENSUS_REORG_UNWIND_DEPTH;
use rsil_provider::{
...
/// # Defaults
///
/// The [`DefaultStages`](crate::sets::DefaultStages) are used to fully sync rsil.
...
impl<N: ProviderNodeTypes> Pipeline<N> {
    ...
    /// Run [static file producer](StaticFileProducer) and [pruner](rsil_prune::Pruner) to **move**
    /// all data from the database to static files for corresponding
    /// [segments](rsil_static_file_types::StaticFileSegment), according to their [stage
    /// checkpoints](StageCheckpoint):
    /// - [`StaticFileSegment::Headers`](rsil_static_file_types::StaticFileSegment::Headers) ->
    ///   [`StageId::Headers`]
    /// - [`StaticFileSegment::Receipts`](rsil_static_file_types::StaticFileSegment::Receipts) ->
    ///   [`StageId::Execution`]
    /// - [`StaticFileSegment::Transactions`](rsil_static_file_types::StaticFileSegment::Transactions)
    ///   -> [`StageId::Bodies`]
    ///
    ...
    /// lock is occupied.
    pub fn move_to_static_files(&self) -> RsilResult<()> {
...
mod tests {
    ...
    use rsil_testing_utils::generators::{self, random_block_with_parent};
