...
use rsil_era_downloader::{read_dir, read_era_dir, EraClient, EraMeta, EraStream, EraStreamConfig};
use rsil_era_utils as era;
use rsil_etl::Collector;
use rsil_primitives_traits::{FullBlockBody, FullBlockHeader, NodePrimitives};
use rsil_provider::{
...
/// [ERA1](https://github.com/sil-clients/e2store-format-specs/blob/main/formats/era1.md) files,
/// which hold pre-merge execution blocks, and
/// [ERE](https://github.com/sil-clients/e2store-format-specs/blob/main/formats/ere.md) files,
/// which hold both pre-merge and post-merge execution blocks, and consensus-layer
/// [ERA](https://github.com/sil-clients/e2store-format-specs/blob/main/formats/era.md) files, whose
    ...
    Body: FullBlockBody<OmmerHeader = Header>,
{
    fn create(self, input: ExecInput) -> Result<ThreadSafeEraStream<Header, Body>, StageError> {
        match self {
            ...
            Self::Url(url, folder) => {
                let _ = rsil_fs_util::create_dir_all(&folder);
...
mod tests {
    ...
    fn write_ere_file(path: &Path, blocks: &[SealedBlock<Block>]) {
        ...
        let file = EreFile::new(group, EreId::new("sila-mainnet", start, blocks.len() as u32));
    ...
    async fn convert_decodes_ere_files() {
        ...
        for ext in ["ere", "erae"] {
            let path = dir.path().join(format!("sila-mainnet-00000-abcd1234.{ext}"));
