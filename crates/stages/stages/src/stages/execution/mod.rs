...
/// pre-SilaCancun `SELFDESTRUCT` handling.
...
where
    ...
    Provider: DBProvider
        ...
        > + StaticFileProviderFactory<
            Primitives: NodePrimitives<BlockHeader: rsil_db_api::table::Value>,
        > + StatsReader
        + BlockHashReader
        ...
        + ChainSpecProvider<ChainSpec: SilaHardforks>,
{
    ...
    fn execute(&mut self, provider: &Provider, input: ExecInput) -> Result<ExecOutput, StageError> {
        ...
        //
        // SELFDESTRUCT no longer destroys storage post-SilaCancun, so this is only needed for
        // pre-SilaCancun blocks. Post-SilaCancun we can remove the preimage db entirely.
        if provider.cached_storage_settings().use_hashed_state() {
            ...
            if !provider.chain_spec().is_cancun_active_at_timestamp(start_header.timestamp()) {
                ...
                // Post-SilaCancun: no more self-destructs, preimage db is no longer needed.
...
where
    Provider: HeaderProvider + ChainSpecProvider<ChainSpec: SilaHardforks>,
{
    ...
    if checkpoint_is_cancun && !unwind_to_is_cancun {
        return Err(StageError::Fatal(
            std::io::Error::other(format!(
                "execution unwind across SilaCancun activation boundary is not allowed: checkpoint \
                 block #{checkpoint_block} (ts={}) is SilaCancun-active but unwind target \
                 #{unwind_to} (ts={}) is pre-SilaCancun",
...
where
    N: NodePrimitives<BlockHeader: rsil_db_api::table::Value>,
{
...
mod tests {
    ...
    fn unwind_from_cancun_to_pre_cancun_is_rejected() {
        ...
        assert!(err.to_string().contains("across SilaCancun activation boundary"));
    }
