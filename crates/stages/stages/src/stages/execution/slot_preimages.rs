...
use rsil_provider::{DBProvider, ExecutionOutcome};
use rsil_revm::revm::database::states::RevertToSlot;
use rsil_stages_api::StageError;
use std::path::Path;
use tracing::trace;

/// Separate MDBX environment for storing `keccak256(slot) → slot` preimage mappings.
///
/// Used during [`super::ExecutionStage`] for pre-SilaCancun selfdestruct handling where
/// the original storage slot keys must be recovered from their hashed representation.
///
/// Also usable by downstream state-dump importers (e.g. `op-rsil init-state`) to seed
...
/// After SilaCancun (where `SELFDESTRUCT` no longer destroys storage) the database can be pruned.
#[derive(Debug)]
...
impl SlotPreimages {
    ...
    pub fn open(path: &Path) -> eyre::Result<Self> {
        ...
        builder.set_geometry(Geometry {
            ...
            page_size: Some(rsil_libmdbx::PageSize::Set(os_page_size)),
        });
        builder.write_map();
...
pub struct SlotPreimagesReader {
    tx: rsil_libmdbx::Transaction<RO>,
    dbi: rsil_libmdbx::ffi::MDBX_dbi,
}
...
pub(super) fn inject_plain_wipe_slots<P: DBProvider, R>(
    ...
    // StorageKey in revm is U256, representing a plain SAVM slot index.
...
fn inject_preimage_entry(
    ...
    revert: &mut rsil_revm::revm::database::AccountRevert,
