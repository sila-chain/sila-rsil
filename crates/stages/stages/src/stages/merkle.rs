...
use rsil_primitives_traits::{GotExpected, SealedHeader};
use rsil_provider::{
...
use rsil_trie::{IntermediateStateRootState, StateRoot, StateRootProgress, StoredSubNode};
...
This is an error that likely requires a report to the rsil team with additional information.
Please include the following information in your report:
 * This error message
 * The state root of the block that was rejected
 * The output of `rsil db stats --checksum` from the database that was being used. This will take a long time to run!
 ...
 * The debug logs from __the same time period__. To find the default location for these logs, run:
   `rsil --help | grep -A 4 'log.file.directory'`

Once you have this information, please submit a github issue at https://github.com/sila-chain/sila-rsil/issues/new
"#;
        ...
        + StageCheckpointWriter,
{
    ...
    fn execute(&mut self, provider: &Provider, input: ExecInput) -> Result<ExecOutput, StageError> {
        ...
        let (trie_root, entities_checkpoint) = if range.is_empty() {
            ...
            let progress = rsil_trie_db::with_adapter!(provider, |A| {
    ...
    fn unwind(
        ...
        if range.is_empty() {
            ...
            let (block_root, updates) = rsil_trie_db::with_adapter!(provider, |A| {
...
mod tests {
    ...
    use rsil_db_api::cursor::{DbCursorRO, DbCursorRW, DbDupCursorRO};
    use rsil_primitives_traits::{SealedBlock, StorageEntry};
    use rsil_provider::{providers::StaticFileWriter, StaticFileProviderFactory};
    ...
    use rsil_trie::test_utils::{state_root, state_root_prehashed};
    ...
    async fn execute_chunked_merkle() {
        ...
        let actual_root = runner
            ...
            .query_with_provider(|provider| {
                Ok(rsil_trie_db::with_adapter!(provider, |A| {
    ...
    impl ExecuteStageTestRunner for MerkleTestRunner {
        type Seed = Vec<SealedBlock<rsil_sila_primitives::Block>>;
        ...
        fn seed_execution(&mut self, input: ExecInput) -> Result<Self::Seed, TestRunnerError> {
            ...
            let sealed_head = SealedBlock::<rsil_sila_primitives::Block>::from_sealed_parts(
