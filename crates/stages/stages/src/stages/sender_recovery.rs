...
use rsil_config::config::SenderRecoveryConfig;
use rsil_consensus::ConsensusError;
use rsil_db::static_file::TransactionMask;
use rsil_db_api::{
    cursor::DbCursorRW,
    table::Value,
    tables,
...
/// in [`TransactionSenders`][rsil_db_api::tables::TransactionSenders] table.
#[derive(Clone, Debug)]
        ...
        + StorageSettingsCache,
{
    ...
    /// [`BlockBodyIndices`][rsil_db_api::tables::BlockBodyIndices],
    /// collect transactions within that range, recover signer for each transaction and store
    /// entries in the [`TransactionSenders`][rsil_db_api::tables::TransactionSenders] table or
        ...
        + StaticFileProviderFactory<Primitives: NodePrimitives<SignedTx: Value + SignedTransaction>>,
{
    ...
    rsil_tasks::spawn_os_thread("sender-recovery", move || {
...
fn recover_sender<T: SignedTransaction>(
    ...
    // value is greater than `secp256k1n / 2` if past SIP-2. There are transactions
