...
use rsil_db_api::{cursor::DbCursorRO, models::BlockNumberAddress, transaction::DbTx};
...
fn assert_changesets_queryable(
    provider_factory: &rsil_provider::ProviderFactory<
        rsil_provider::test_utils::MockNodeTypesWithDB,
    >,
    ...
    if settings.storage_v2 {
        ...
        let storage_changesets: Vec<_> = provider
            .tx_ref()
            .cursor_dup_read::<rsil_db::tables::StorageChangeSets>()?
    ...
    if settings.storage_v2 {
        ...
        let account_changesets: Vec<_> = provider
            .tx_ref()
            .cursor_read::<rsil_db::tables::AccountChangeSets>()?
...
fn build_downloaders_from_file_client(
    ...
    genesis: rsil_primitives_traits::SealedHeader<Header>,
    stages_config: StageConfig,
    consensus: Arc<NoopConsensus>,
    provider_factory: rsil_provider::ProviderFactory<
        rsil_provider::test_utils::MockNodeTypesWithDB,
    >,
) -> (impl HeaderDownloader<Header = Header>, impl BodyDownloader<Block = Block>, rsil_tasks::Runtime)
{
...
///    - SIL transfer to a recipient (account state changes)
...
async fn run_pipeline_forward_and_unwind(
    ...
    // Recipient address for SIL transfers
    let recipient_address = Address::new([0x11; 20]);

    // Create a chain spec with:
    // - Signer pre-funded with 1000 SIL
    ...
    let transfer_value = U256::from(ETH_TO_WEI); // 1 SIL per block

    for block_num in 1..=num_blocks {
        // Each block has 2 transactions: SIL transfer + Counter increment
        let base_nonce = (block_num - 1) * 2;

        // Transaction 1: SIL transfer
    ...
    let resync_runtime = rsil_tasks::Runtime::test();
...
/// Tests pipeline with ALL stages enabled using both SIL transfers and contract storage changes.
///
/// This test:
/// 1. Pre-funds a signer account and deploys a Counter contract in genesis
/// 2. Each block contains two transactions:
