//! Preimage-specific pipeline tests for storage v2 selfdestruct behavior around SilaCancun.
...
use rsil_db_common::init::{init_genesis, init_genesis_with_settings};
use rsil_downloaders::{
...
use rsil_stages_api::{Pipeline, StageSet};
use rsil_static_file::StaticFileProducer;
use rsil_storage_api::{StorageChangeSetReader, StorageSettings, StorageSettingsCache};
use rsil_testing_utils::generators::{self, generate_key, sign_tx_with_key_pair};
...
type TestProviderFactory =
    rsil_provider::ProviderFactory<rsil_provider::test_utils::MockNodeTypesWithDB>;

const TEST_SELFDESTRUCT_BENEFICIARY: Address = Address::new([0x77; 20]);
const TEST_CREATE2_SALT: B256 = B256::with_last_byte(0x42);

/// Scenario coverage:
/// 1. Cross-batch pre-SilaCancun wipe writes plain slot keys into storage changesets.
/// 2. Preimage DB exists and is usable across multiple stage executions.
/// 3. Post-SilaCancun execution removes the preimage DB directory.
///
/// Verifies v2 selfdestruct handling across a pre-/post-SilaCancun boundary.
///
/// Test flow:
/// 1. Run block 1 (pre-SilaCancun) and assert the `preimage/` MDBX directory exists and contains
///    `keccak(slot) -> slot` rows for the two written storage slots.
/// 2. Run block 2 (pre-SilaCancun selfdestruct) and assert storage changesets for the destroyed account
///    contain exactly those two slots as **plain** keys with the expected prior values (`0x2a`,
///    `0x99`).
/// 3. Run block 3 (post-SilaCancun) and assert `preimage/` is removed, since this auxiliary DB is no
///    longer needed after SilaCancun semantics are active.
...
async fn test_pipeline_v2_selfdestruct_changesets_use_plain_slots() -> eyre::Result<()> {
    ...
    // - block 1/2 are pre-SilaCancun (selfdestruct still clears storage)
    // - block 3 is post-SilaCancun (no storage-destroy semantics; preimage DB should be cleaned up)
    ...
    // Phase 1 (pre-SilaCancun): preimage DB should be created and contain slot preimages.
    ...
    assert!(preimage_path.exists(), "preimage dir should exist after first pre-SilaCancun run");
...
struct SelfdestructScenario {
    chain_spec: Arc<rsil_chainspec::ChainSpec>,
...
fn setup_selfdestruct_scenario() -> eyre::Result<SelfdestructScenario> {
    ...
    let blocks = {
        ...
        let evm_config = SilEvmConfig::new(chain_spec.clone());
        ...
        for (block_num, timestamp, nonce, input, to, gas_limit, value) in [
            ...
            // - block 2 (ts=24): triggers SELFDESTRUCT (pre-SilaCancun semantics)
            // - block 3 (ts=36): post-SilaCancun no-op transfer path
...
fn setup_intra_block_and_intra_tx_selfdestruct_scenario(
    ...
    let blocks = {
        ...
        // Single pre-SilaCancun block with three txs:
...
fn init_v2_pipeline_provider_factory(
    chain_spec: Arc<rsil_chainspec::ChainSpec>,
) -> eyre::Result<(TestProviderFactory, rsil_primitives_traits::SealedHeader<Header>)> {
...
fn execute_and_commit_block(
    ...
    >;
    let (state_root, _trie_updates) = rsil_trie_db::with_adapter!(provider, |A| {
...
fn build_pipeline_without_history<H, B>(
    ...
    max_block: u64,
    tip: B256,
) -> Pipeline<rsil_provider::test_utils::MockNodeTypesWithDB>
where
    ...
    B: BodyDownloader<Block = Block> + 'static,
{
    ...
    let evm_config = SilEvmConfig::new(provider_factory.chain_spec());
...
async fn run_pipeline_range(
    ...
    local_head: rsil_primitives_traits::SealedHeader<Header>,
    ...
    P: StoragePath,
{
    ...
    assert!(
        preimage_path.exists(),
        "preimage dir should exist for pre-SilaCancun execution at {}",
