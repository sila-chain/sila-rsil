//! rsil's database abstraction layer.
//!
...
//! the same time. Strong consistency in what data is written and when is important for rsil, so it
...
//! # Tables
//!
//! rsil has two types of tables: simple KV stores (one key, one value) and dup tables (one key,
