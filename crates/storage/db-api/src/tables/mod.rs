...
//! # Overview
//!
//! This module defines the tables in rsil, as well as some table-related abstractions:
//!
...
macro_rules! tables {
    ...
    ($($(#[$attr:meta])* table $name:ident$(<$($generic:ident $(= $default:ty)?),*>)? { type Key = $key:ty; type Value = $value:ty; $(type SubKey = $subkey:ty;)? } )*) => {
        ...
        ///
        /// ```
        /// use rsil_db_api::{table::Table, Tables, tables_to_generic};
        ///
...
tables! {
    ...
    ///
    /// Code example can be found in `rsil_provider::HistoricalStateProviderRef`
