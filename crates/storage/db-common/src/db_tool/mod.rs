...
use rsil_node_types::NodeTypesWithDB;
use rsil_provider::{providers::ProviderNodeTypes, ChainSpecProvider, DBProvider, ProviderFactory};
...
impl<N: ProviderNodeTypes> DbTool<N> {
    ...
    pub fn drop<P: AsRef<Path>>(
        ...
        info!(target: "rsil::cli", "Dropping database at {:?}", db_path);
        fs::remove_dir_all(db_path)?;

        let static_files_path = static_files_path.as_ref();
        info!(target: "rsil::cli", "Dropping static files at {:?}", static_files_path);
        ...
        if exex_wal_path.as_ref().exists() {
            ...
            info!(target: "rsil::cli", "Dropping ExEx WAL at {:?}", exex_wal_path);
