//! Rsil genesis initialization utility functions.
...
use rsil_execution_errors::StateRootError;
...
where
    ...
    PF::ProviderRW: StaticFileProviderFactory<Primitives = PF::Primitives>
        ...
        + AsRef<PF::ProviderRW>,
    PF::ChainSpec: SilChainSpec<Header = <PF::Primitives as NodePrimitives>::BlockHeader>,
{
        ...
        + AsRef<Provider>,
{
    ...
    trace!(target: "rsil::cli", "Inserted state");
    ...
    Provider: DBProvider<Tx: DbTxMut> + HashingWriter,
{
    ...
    trace!(target: "rsil::cli", "Inserted account hashes");
    ...
    trace!(target: "rsil::cli", "Inserted storage hashes");
...
where
    Provider: StaticFileProviderFactory<Primitives: NodePrimitives<BlockHeader: Compact>>
        + DBProvider<Tx: DbTxMut>,
    Spec: SilChainSpec<Header = <Provider::Primitives as NodePrimitives>::BlockHeader>,
{
...
/// be set to the highest block present. One practical usecase is to import OP sila-mainnet state at
    ...
    >,
{
    ...
    info!(target: "rsil::cli", "All accounts written to database, starting state root computation (may take some time)");
...
fn parse_accounts(
    ...
    for account in stream {
        ...
        if parsed_accounts.is_multiple_of(100_000) {
            info!(target: "rsil::cli", parsed_accounts, "Parsed accounts");
        }
    }
        ...
        + NodePrimitivesProvider,
{
    ...
    for entry in collector.iter()? {
        ...
        if total_accounts.is_multiple_of(100_000) {
            info!(target: "rsil::cli", total_accounts, accounts_len, "Writing accounts...");
        }
    }
    ...
    info!(target: "rsil::cli", total_accounts, "All accounts written to database");
...
fn prepare_account_changeset_writer<N: NodePrimitives>(
    writer: &mut rsil_provider::providers::StaticFileProviderRWRefMut<'_, N>,
    block: u64,
) -> ProviderResult<()> {
...
fn reset_pre_snapshot_changeset_segment<N: NodePrimitives>(
    static_file_provider: &rsil_provider::providers::StaticFileProvider<N>,
...
{
    rsil_db_api::transaction::DbTx::commit(provider.into_tx()).map_err(ProviderError::from)
}
...
fn write_account_to_db_v2<TX, N>(
    tx: &TX,
    changeset_writers: (
        &mut rsil_provider::providers::StaticFileProviderRWRefMut<'_, N>,
        &mut rsil_provider::providers::StaticFileProviderRWRefMut<'_, N>,
    ),
    history_batch: &mut rsil_provider::providers::RocksDBBatch<'_>,
    address: &Address,
    genesis_account: &GenesisAccount,
    history_list: &IntegerList,
    seen_bytecodes: &mut B256Set,
    ...
    if let Some(storage) = &genesis_account.storage {
        ...
        for (&key, &value) in storage {
            ...
            storage_changeset_writer.append_storage_changeset_entry(
                rsil_db_api::models::StorageBeforeTx { address: *address, key, value: U256::ZERO },
            )?;
...
{
    rsil_trie_db::with_adapter!(provider, |A| {
...
where
    Provider: DBProvider<Tx: DbTxMut> + TrieWriter + StorageSettingsCache,
    A: rsil_trie_db::TrieTableAdapter,
{
    trace!(target: "rsil::cli", "Computing state root");
    ...
    >,
{
    ...
    rsil_trie_db::with_adapter!(&provider_rw, |A| {
        drop(provider_rw);
...
"#;
        ...
        let factory = create_test_provider_factory_with_chain_spec(SILA_MAINNET.clone());
        ...
        assert_eq!(
            rsil_provider::ChangeSetReader::account_block_changeset(&provider, block).unwrap(),
            vec![
        ...
        assert_eq!(
            rsil_provider::StorageChangeSetReader::storage_changeset(&provider, block).unwrap(),
            vec![(
...
"#;
        ...
        assert!(rsil_provider::ChangeSetReader::account_block_changeset(&provider, 5)
            .unwrap()
            .is_empty());
        assert!(rsil_provider::StorageChangeSetReader::storage_changeset(&provider, 5)
            .unwrap()
            .is_empty());
        ...
        assert!(rsil_provider::ChangeSetReader::account_block_changeset(
            &provider,
            account_file_start
        )
        .unwrap()
        .is_empty());
        assert!(rsil_provider::StorageChangeSetReader::storage_changeset(
            &provider,
            storage_file_start
        )
        .unwrap()
    ...
    fn init_genesis_history() {
...

        {
            ...
            let collect_rocksdb = |rocksdb: &rsil_provider::providers::RocksDBProvider| {
                (
                    rocksdb
                        .iter::<tables::AccountsHistory>()
                        .unwrap()
