...
use rsil_tracing::tracing::error;
use std::{
    collections::HashMap,
    ops::{Deref, Range},
    path::{Path, PathBuf},
...
/// See [`rsil_libmdbx::EnvironmentBuilder::set_handle_slow_readers`] for more information.
...
impl DatabaseArguments {
    ...
    pub const fn with_geometry_page_size(mut self, page_size: Option<usize>) -> Self {
        if let Some(size) = page_size {
            self.geometry.page_size = Some(rsil_libmdbx::PageSize::Set(size));
        }
...
impl DatabaseEnv {
    ...
    pub fn open(
        ...
        extern "C" fn handle_slow_readers(
            ...
            rsil_libmdbx::HandleSlowReadersReturnCode::ProceedWithoutKillingReader
        }
        ...
        //    https://github.com/sila-chain/sila-rsil/blob/2a4c78759178f66e30c8976ec5d243b53102fc9a/crates/storage/libmdbx-rs/mdbx-sys/libmdbx/mdbx.c#L11479-L11480.
        //
        ...
        // https://github.com/sila-chain/sila-rsil/blob/2a4c78759178f66e30c8976ec5d243b53102fc9a/crates/storage/libmdbx-rs/mdbx-sys/libmdbx/mdbx.c#L10018-L10024.
        //
        ...
        // https://github.com/sila-chain/sila-rsil/blob/fa2b9b685ed9787636d962f4366caf34a9186e66/crates/storage/libmdbx-rs/mdbx-sys/libmdbx/mdbx.c#L16017.
    ...
    pub fn drop_orphan_table(&self, name: &str) -> Result<bool, DatabaseError> {
        ...
        match tx.open_db(Some(name)) {
            ...
            Err(rsil_libmdbx::Error::NotFound) => Ok(false),
...
mod tests {
    ...
    use rsil_libmdbx::Error;
    use rsil_primitives_traits::{Account, StorageEntry};
    use rsil_storage_errors::db::{DatabaseWriteError, DatabaseWriteOperation};
    use std::str::FromStr;
    ...
    fn db_cursor_delete_current_non_existent() {
        ...
        assert!(matches!(
            ...
            DatabaseError::Delete(err) if err == rsil_libmdbx::Error::NoData.into()));
