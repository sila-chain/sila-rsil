...
use rsil_libmdbx::{ffi::MDBX_dbi, CommitLatency, Transaction, TransactionKind, WriteFlags, RW};
use rsil_storage_errors::db::{DatabaseWriteError, DatabaseWriteOperation};
use rsil_tracing::tracing::{debug, instrument, trace, warn};
use std::{
    backtrace::Backtrace,
    collections::HashMap,
    marker::PhantomData,
...
impl<K: TransactionKind> Tx<K> {
    ...
    pub fn id(&self) -> rsil_libmdbx::Result<u64> {
...
mod tests {
    ...
    use rsil_db_api::{database::Database, models::ClientVersion, transaction::DbTx};
    use rsil_libmdbx::MaxReadTransactionDuration;
    use rsil_storage_errors::db::DatabaseError;
    ...
    fn long_read_transaction_safety_disabled() {
        ...
        assert!(matches!(
            ...
            DatabaseError::Open(err) if err == rsil_libmdbx::Error::NotFound.into()));
    ...
    fn long_read_transaction_safety_enabled() {
        ...
        assert!(matches!(
            tx.get::<tables::Transactions>(0).unwrap_err(),
            DatabaseError::Open(err) if err == rsil_libmdbx::Error::ReadTransactionTimeout.into()));
