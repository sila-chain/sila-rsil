//! MDBX implementation for rsil's database abstraction layer.
//!
//! This crate is an implementation of `rsil-db-api` for MDBX, as well as a few other common
//! database types.
//!
//! # Overview
//!
//! An overview of the current data model of rsil can be found in the [`mod@tables`] module.
...
pub use rsil_storage_errors::db::{DatabaseError, DatabaseWriteOperation};
#[cfg(feature = "mdbx")]
...
pub mod test_utils {
    ...
    impl<DB> Drop for TempDatabase<DB> {
        fn drop(&mut self) {
            if let Some(db) = self.db.take() {
                drop(db);
                let _ = rsil_fs_util::remove_dir_all(&self.path);
            }
        }
    }
    ...
    pub fn create_test_static_files_dir() -> (TempDir, PathBuf) {
        let temp_dir = TempDir::with_prefix("rsil-test-static-").expect(ERROR_TEMPDIR);
    ...
    pub fn create_test_rocksdb_dir() -> (TempDir, PathBuf) {
        let temp_dir = TempDir::with_prefix("rsil-test-rocksdb-").expect(ERROR_TEMPDIR);
        let path = temp_dir.path().to_path_buf();
        (temp_dir, path)
    }
    ...
    pub fn tempdir_path() -> PathBuf {
        let builder = tempfile::Builder::new().prefix("rsil-test-").rand_bytes(8).tempdir();
    ...
    pub fn enable_legacy_multiopen() {
        unsafe {
            rsil_libmdbx::ffi::mdbx_setup_debug(
                rsil_libmdbx::ffi::MDBX_LOG_DONTCHANGE,
                rsil_libmdbx::ffi::MDBX_DBG_LEGACY_MULTIOPEN as rsil_libmdbx::ffi::MDBX_debug_flags,
                None,
            );
        }
    }
...
mod tests {
    ...
    use rsil_libmdbx::MaxReadTransactionDuration;
    ...
    fn db_version() {
        ...
        {
            rsil_fs_util::write(path.path().join(db_version_file_path(&path)), "invalid-version")
                .unwrap();
            let db = init_db(&path, args.clone());
            assert!(db.is_err());
            assert_matches!(
        ...
        {
            rsil_fs_util::write(path.path().join(db_version_file_path(&path)), "0").unwrap();
