...
use rsil_storage_errors::lockfile::StorageLockError;
use std::{
    path::{Path, PathBuf},
    process,
    sync::{Arc, OnceLock},
...
impl StorageLock {
    ...
    fn try_acquire_file_lock(path: &Path) -> Result<Self, StorageLockError> {
        ...
        {
            rsil_tracing::tracing::error!(
                target: "rsil::db::lockfile",
                path = ?file_path,
            ...
            if file_path.exists() {
                if let Ok(Some(process_uid)) = ProcessUID::parse(file_path) {
                    ...
                    if process_uid.pid == process::id() as usize {
                        if let Err(err) = rsil_fs_util::remove_file(file_path) {
                            rsil_tracing::tracing::error!(%err, "Failed to delete lock file");
                        }
                    } else {
...
impl StorageLockInner {
    ...
    fn new(file_path: PathBuf) -> Result<Self, StorageLockError> {
        ...
        if let Some(parent) = file_path.parent() {
            rsil_fs_util::create_dir_all(parent).map_err(StorageLockError::other)?;
        }
...
impl ProcessUID {
    ...
    fn parse(path: &Path) -> Result<Option<Self>, StorageLockError> {
        if path.exists()
            && let Ok(contents) = rsil_fs_util::read_to_string(path)
        {
    ...
    fn write(&self, path: &Path) -> Result<(), StorageLockError> {
        rsil_fs_util::write(path, format!("{}\n{}", self.pid, self.start_time))
