//! rsil's static file database table import and access

use rsil_nippy_jar::{NippyJar, NippyJarError};
...
pub fn iter_static_files(path: &Path) -> Result<SortedStaticFiles, NippyJarError> {
    if !path.exists() {
        rsil_fs_util::create_dir_all(path).map_err(|err| NippyJarError::Custom(err.to_string()))?;
    }

    let mut static_files = SortedStaticFiles::default();
    let entries = rsil_fs_util::read_dir(path)
