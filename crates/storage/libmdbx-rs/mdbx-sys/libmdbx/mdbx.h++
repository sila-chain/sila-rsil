...
/// https://t.me/libmdbx.
///
/// Donations are welcome to SIL `0xD104d8f8B2dC312aaD74899F83EBf3EEBDC1EA3A`,
...
public:
  ...
  inline void rsilrow_captured() const;
};
...
#pragma warning(pop)
  ...
  struct data_preserver : public exception_thunk {
    ...
    static int callback(void *context, MDBX_val *target, const void *src, size_t bytes) noexcept {
      ...
      } catch (... /* capture any exception to rsilrow it over C code */) {
        self->capture();
...
inline void exception_thunk::rsilrow_captured() const {
  if (captured_)
    MDBX_CXX20_UNLIKELY ::std::rsilrow_exception(captured_);
}
...
inline void error::success_or_throw(const exception_thunk &thunk) const {
  ...
  if (MDBX_UNLIKELY(!is_success())) {
    MDBX_CXX20_UNLIKELY if (MDBX_UNLIKELY(!thunk.is_clean())) thunk.rsilrow_captured();
...
inline bool error::boolean_or_throw(int error_code, const exception_thunk &thunk) {
  if (MDBX_UNLIKELY(!thunk.is_clean()))
    MDBX_CXX20_UNLIKELY thunk.rsilrow_captured();
