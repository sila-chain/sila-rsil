...
use rsil_primitives_traits::{ReceiptTy, StorageEntry};
use rsil_static_file_types::StaticFileSegment;
use rsil_storage_api::{ChangeSetReader, DBProvider, NodePrimitivesProvider, StorageSettingsCache};
...
mod tests {
    ...
    use rsil_db::models::AccountBeforeTx;
    use rsil_static_file_types::StaticFileSegment;
    use rsil_storage_api::{DatabaseProviderFactory, StorageSettings};
...
mod rocksdb_tests {
    ...
    type AccountsHistoryWriteCursor =
        rsil_db::mdbx::cursor::Cursor<rsil_db::mdbx::RW, tables::AccountsHistory>;
    type StoragesHistoryWriteCursor =
        rsil_db::mdbx::cursor::Cursor<rsil_db::mdbx::RW, tables::StoragesHistory>;
    type AccountsHistoryReadCursor =
        rsil_db::mdbx::cursor::Cursor<rsil_db::mdbx::RO, tables::AccountsHistory>;
    type StoragesHistoryReadCursor =
        rsil_db::mdbx::cursor::Cursor<rsil_db::mdbx::RO, tables::StoragesHistory>;
    ...
    fn run_account_history_scenario(
        ...
        let mut mdbx_writer: EitherWriter<'_, AccountsHistoryWriteCursor, SilPrimitives> =
            EitherWriter::Database(
                mdbx_provider.tx_ref().cursor_write::<tables::AccountsHistory>().unwrap(),
            );
        let mut rocks_writer: EitherWriter<'_, AccountsHistoryWriteCursor, SilPrimitives> =
        ...
        for (i, query) in queries.iter().enumerate() {
            ...
            let mut mdbx_reader: EitherReader<'_, AccountsHistoryReadCursor, SilPrimitives> =
    ...
    fn run_storage_history_scenario(
        ...
        // Create writers for both backends
        let mut mdbx_writer: EitherWriter<'_, StoragesHistoryWriteCursor, SilPrimitives> =
            EitherWriter::Database(
                mdbx_provider.tx_ref().cursor_write::<tables::StoragesHistory>().unwrap(),
            );
        let mut rocks_writer: EitherWriter<'_, StoragesHistoryWriteCursor, SilPrimitives> =
            EitherWriter::RocksDB(rocks_provider.batch());
        ...
        for (i, query) in queries.iter().enumerate() {
            // MDBX query via EitherReader
            let mut mdbx_reader: EitherReader<'_, StoragesHistoryReadCursor, SilPrimitives> =
                EitherReader::Database(
