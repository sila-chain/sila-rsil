...
use rsil_db_api::models::{AccountBeforeTx, BlockNumberAddress, StoredBlockBodyIndices};
use rsil_execution_types::ExecutionOutcome;
use rsil_node_types::{BlockTy, HeaderTy, ReceiptTy, TxTy};
...
impl<N: ProviderNodeTypes> BlockReaderIdExt for ConsistentProvider<N> {
    fn block_by_id(&self, id: BlockId) -> ProviderResult<Option<Self::Block>> {
        match id {
            ...
            BlockId::Hash(hash) => {
                // TODO: should we only apply this for the RPCs that are listed in SIP-1898?
...
mod tests {
    ...
    use rsil_chain_state::{ExecutedBlock, NewCanonicalChain};
    use rsil_db_api::models::AccountBeforeTx;
    use rsil_execution_types::{BlockExecutionOutput, BlockExecutionResult, ExecutionOutcome};
    use rsil_primitives_traits::{RecoveredBlock, SealedBlock};
    use rsil_sila_primitives::Block;
    use rsil_storage_api::{BlockReader, BlockSource, ChangeSetReader, StateReader};
    ...
    fn test_get_state_storage_value_plain_state() -> eyre::Result<()> {
        ...
        use rsil_db_api::{models::StorageSettings, tables, transaction::DbTxMut};
        use rsil_primitives_traits::StorageEntry;
        use rsil_storage_api::StorageSettingsCache;
        use std::collections::HashMap;

        let address = alloy_primitives::Address::with_last_byte(1);
        let account = rsil_primitives_traits::Account {
            nonce: 1,
    ...
    fn test_storage_changeset_consistent_keys_plain_state() -> eyre::Result<()> {
        ...
        use rsil_storage_api::{StorageChangeSetReader, StorageSettingsCache};
        use std::collections::HashMap;
