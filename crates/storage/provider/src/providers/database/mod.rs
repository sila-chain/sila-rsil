...
use rsil_prune_types::{PruneCheckpoint, PruneModes, PruneSegment, MINIMUM_UNWIND_SAFE_DISTANCE};
use rsil_stages_types::{PipelineTarget, StageCheckpoint, StageId};
...
pub struct ProviderFactory<N: NodeTypesWithDB> {
    ...
    /// factory writing to the database (e.g as part of a running rsil node).
...
impl<N: NodeTypesWithDB> ProviderFactory<N> {
    ...
    ///
    /// Should be used for read-only factories that are running concurrently to a rsil node writing
        ...
        N::DB: Database,
    {
        ...
        rsil_tasks::spawn_os_thread("ro-sync", move || {
                ...
                match res {
                    Ok(event) => {
                        ...
                        if let Err(err) = factory.sync_providers_if_needed() {
                            warn!(target: "rsil::provider", %err, "background ro-sync failed");
                        }
                    }
                    Err(err) => {
                        warn!(target: "rsil::provider", ?err, "MDBX directory watcher error");
                    }
                }
            }
        });
...
impl<N: ProviderNodeTypes<DB = DatabaseEnv>> ProviderFactory<N> {
    ...
    pub fn new_with_database_path<P: AsRef<Path>>(
        ...
        Self::new(
            init_db(path, args).map_err(RsilError::msg)?,
            chain_spec,
            static_file_provider,
            rocksdb_provider,
            runtime,
        )
        .map_err(RsilError::Provider)
    }
}
...
mod tests {
    ...
    use rsil_db_api::tables;
    use rsil_primitives_traits::SignerRecoverable;
    use rsil_prune_types::{PruneMode, PruneModes};
    use rsil_storage_errors::provider::ProviderError;
    use rsil_testing_utils::generators::{self, random_block, random_header, BlockParams};
