...
use rsil_chainspec::{ChainInfo, ChainSpecProvider, SilChainSpec};
use rsil_db_api::{
...
use rsil_node_types::{BlockTy, BodyTy, HeaderTy, NodeTypes, ReceiptTy, TxTy};
...
use rsil_storage_errors::provider::{ProviderResult, StaticFileWriterError};
use rsil_trie::{
    updates::{StorageTrieUpdatesSorted, TrieUpdatesSorted},
    ComputedTrieData, HashedPostStateSorted,
};
use rsil_trie_db::{ChangesetCache, DatabaseStorageTrieCursor, TrieTableAdapter};
...
impl<TX: Debug + Send, N: NodeTypes<ChainSpec: SilChainSpec + 'static>> ChainSpecProvider
...
impl<TX: DbTx + DbTxMut + 'static, N: NodeTypesForProvider> DatabaseProvider<TX, N> {
    ...
    pub fn save_blocks(
        ...
        if rocksdb_enabled {
            timings.rocksdb = rocksdb_result.ok_or_else(|| {
                ProviderError::Database(rsil_db_api::DatabaseError::Other(
    ...
    pub fn unwind_trie_state_from(&self, from: BlockNumber) -> ProviderResult<()> {
        ...
        let db_tip_block = self
            .get_stage_checkpoint(rsil_stages_types::StageId::Finish)?
            .as_ref()
...
impl<TX: DbTxMut + DbTx + 'static, N: NodeTypes> TrieWriter for DatabaseProvider<TX, N> {
    ...
    fn write_trie_updates_sorted(&self, trie_updates: &TrieUpdatesSorted) -> ProviderResult<usize> {
        ...
        rsil_trie_db::with_adapter!(self, |A| {
...
mod tests {
    ...
    use rsil_execution_types::{AccountRevertInit, BlockExecutionOutput, BlockExecutionResult};
    use rsil_primitives_traits::SealedBlock;
    use rsil_sila_primitives::Receipt;
    use rsil_storage_api::MetadataWriter;
    use rsil_testing_utils::generators::{self, random_block, BlockParams};
    use rsil_trie::{
    ...
    fn test_receipts_by_block_range_empty_range() {
        ...
        assert_eq!(result, Vec::<Vec<rsil_sila_primitives::Receipt>>::new());
    }
    ...
    fn test_write_trie_updates_sorted() {
        ...
        let expected_mask = rsil_trie::TrieMask::new(0b1111_1111_1111_1111);
        assert_eq!(
    ...
    fn test_try_into_history_rejects_unexecuted_blocks() {
        use rsil_storage_api::TryIntoHistoricalStateProvider;
    ...
    fn test_write_and_remove_state_roundtrip_legacy() {
        ...
        let hashed_state =
            execution_outcome.hash_state_slow::<rsil_trie::KeccakKeyHasher>().into_sorted();
    ...
    fn run_save_blocks_and_verify(mode: StorageMode) {
        ...
        let genesis = SealedBlock::<rsil_sila_primitives::Block>::from_sealed_parts(
            SealedHeader::new(
        ...
        for block_num in 1..=num_blocks {
            ...
            let block =
                SealedBlock::<rsil_sila_primitives::Block>::seal_parts(header, Default::default());
