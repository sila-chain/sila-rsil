...
use rsil_primitives_traits::{BlockBody as _, FastInstant as Instant};
...
impl RocksDBBuilder {
    ...
    /// Registers the default tables used by rsil for `RocksDB` storage.
    ///
    /// This registers:
    ...
    pub fn build(self) -> ProviderResult<RocksDBProvider> {
        ...
        if self.read_only {
            ...
            rsil_fs_util::create_dir_all(&secondary_path).map_err(ProviderError::other)?;
...
impl RocksDBProvider {
        ...
        self.execute_with_operation_metric(RocksDBOperation::Get, T::NAME, |this| {
            ...
            match iter.next() {
                Some(Ok((key_bytes, value_bytes))) => {
                    let key = <T::Key as rsil_db_api::table::Decode>::decode(&key_bytes)
    ...
    pub(crate) fn write_blocks_data<N: rsil_node_types::NodePrimitives>(
        &self,
    ...
    #[instrument(level = "debug", target = "providers::rocksdb", skip_all)]
    fn write_tx_hash_numbers<N: rsil_node_types::NodePrimitives>(
        &self,
        blocks: &[ExecutedBlock<N>],
        tx_nums: &[TxNumber],
        ctx: &RocksDBWriteCtx,
    ...
    #[instrument(level = "debug", target = "providers::rocksdb", skip_all)]
    fn write_account_history<N: rsil_node_types::NodePrimitives>(
        &self,
        blocks: &[ExecutedBlock<N>],
        ctx: &RocksDBWriteCtx,
    ) -> ProviderResult<()> {
    ...
    /// Derives history indices from reverts (same source as changesets) to ensure consistency.
    #[instrument(level = "debug", target = "providers::rocksdb", skip_all)]
    fn write_storage_history<N: rsil_node_types::NodePrimitives>(
        &self,
        blocks: &[ExecutedBlock<N>],
        ctx: &RocksDBWriteCtx,
    ) -> ProviderResult<()> {
...
impl<'db> RocksReadSnapshot<'db> {
    ...
    fn history_info<T>(
        ...
        lowest_available_block_number: Option<BlockNumber>,
        visible_tip: BlockNumber,
        key_matches: impl FnOnce(&[u8]) -> Result<bool, rsil_db_api::DatabaseError>,
...
/// Converts Rsil's [`LogLevel`] to `RocksDB`'s [`rocksdb::LogLevel`].
