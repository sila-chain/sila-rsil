    ...
    N: NodePrimitives,
{
    fn state_root(&self, hashed_state: HashedPostState) -> ProviderResult<B256> {
        rsil_trie_db::with_adapter!(self.provider, |A| {
        ...
        rsil_trie_db::with_adapter!(self.provider, |A| {
            ...
            let witness = TrieWitness::new(
                InMemoryTrieCursorFactory::new(
                    rsil_trie_db::DatabaseTrieCursorFactory::<_, A>::new(self.tx()),
                    nodes.as_ref(),
                ),
                HashedPostStateCursorFactory::new(
                    rsil_trie_db::DatabaseHashedCursorFactory::new(self.tx()),
                    state.as_ref(),
                ),
            )
...
rsil_storage_api::macros::delegate_provider_impls!(HistoricalStateProvider<Provider> where [Provider: DBProvider + BlockNumReader + BlockHashReader + ChangeSetReader + StorageChangeSetReader + PruneCheckpointReader + StageCheckpointReader + StorageSettingsCache + RocksDBProviderFactory + NodePrimitivesProvider]);
...
pub struct LowestAvailableBlocks {
    ...
    /// [`rsil_prune_types::PruneSegment::AccountHistory`] was pruned.
    ...
    /// [`rsil_prune_types::PruneSegment::StorageHistory`] was pruned.
    /// [`Option::None`] means all history is available.
...
pub fn compute_history_rank(
    chunk: &rsil_db_api::BlockNumberList,
    block_number: BlockNumber,
) -> (u64, Option<u64>) {
