...
use rsil_db_api::{tables, transaction::DbTx, DatabaseError};
...
#[derive(Debug, Clone)]
pub(super) enum OverlaySource<N: NodePrimitives = SilPrimitives> {
...
#[derive(Debug, Clone)]
pub struct OverlayBuilder<N: NodePrimitives = SilPrimitives> {
            ...
            + StorageSettingsCache,
    {
            ...
            self.reverts_required(provider, db_tip_block, anchor_hash)?
        {
            ...
            let mut hashed_state_reverts = {
                ...
                let start = Instant::now();
                let res = rsil_trie_db::from_reverts_auto(provider, revert_blocks)?;
...
#[derive(Debug, Clone)]
pub struct OverlayStateProviderFactory<F, N: NodePrimitives = SilPrimitives> {
...
mod tests {
    ...
    fn managed_overlay_skips_manager_for_persisted_parent() {
        ...
        let builder = OverlayBuilder::<SilPrimitives>::new(parent_hash, ChangesetCache::default())
