...
use rsil_storage_errors::provider::{ProviderError, ProviderResult, StaticFileWriterError};
use std::{
...
pub struct StaticFileWriteCtx {
    ...
    pub receipts_prune_mode: Option<rsil_prune_types::PruneMode>,
...
impl<N: NodePrimitives> StaticFileProvider<N> {
        ...
        N: NodePrimitives<Receipt: Value, BlockHeader: Value, SignedTx: Value>,
    {
            ...
            && rsil_chainspec::Chain::optimism_mainnet() == provider.chain_spec().chain_id()
        {
            ...
            if provider.block_number(OVM_HEADER_1_HASH)?.is_some() {
                info!(target: "rsil::cli",
                    "Skipping storage verification for OP sila-mainnet, expected inconsistency in OVM chain"
                );
                return Ok(None);
            }
        }

        info!(target: "rsil::cli", "Verifying storage consistency.");
        ...
        for segment in self.segments_to_check(provider) {
            ...
            debug!(target: "rsil::providers::static_file", "Checking consistency for segment");
            ...
            debug!(target: "rsil::providers::static_file", "Checking tx index segment");

            if let Some(highest_tx) = highest_tx {
                let mut last_block = highest_block.unwrap_or_default();
                debug!(target: "rsil::providers::static_file", last_block, highest_tx, "Verifying last transaction matches last block indices");
                loop {
                    let Some(indices) = provider.block_body_indices(last_block)? else {
                        debug!(target: "rsil::providers::static_file", last_block, "Block body indices not found, static files ahead of database");
            ...
            debug!(target: "rsil::providers::static_file", "Ensuring invariants for segment");

            match self.ensure_invariants_for(provider, segment, highest_tx, highest_block)? {
                Some(unwind) => {
                    debug!(target: "rsil::providers::static_file", unwind_target=unwind, "Invariants check returned unwind target");
                    update_unwind_target(unwind);
                }
                None => {
                    debug!(target: "rsil::providers::static_file", "Invariants check completed, no unwind needed")
                }
            }
        }
    ...
    {
        info!(target: "rsil::cli", "Healing static file inconsistencies.");
        ...
        match segment {
            ...
            StaticFileSegment::Receipts => {
                if EitherWriter::receipts_destination(provider).is_database() {
                    ...
                    // files.
                    debug!(target: "rsil::providers::static_file", ?segment, "Skipping receipts segment: receipts stored in database");
                    return false;
                }
                    ...
                    || NamedChain::Chiado == provider.chain_spec().chain_id()
                {
                    ...
                    debug!(target: "rsil::providers::static_file", ?segment, "Skipping receipts segment: broken historical import for gnosis/chiado");
                    return false;
                }
            ...
            StaticFileSegment::TransactionSenders => {
                if EitherWriterDestination::senders(provider).is_database() {
                    debug!(target: "rsil::providers::static_file", ?segment, "Skipping senders segment: senders stored in database");
                    return false;
                }

                if Self::is_segment_fully_pruned(provider, PruneSegment::SenderRecovery) {
                    debug!(target: "rsil::providers::static_file", ?segment, "Skipping senders segment: fully pruned");
                    return false;
                }
            ...
            StaticFileSegment::AccountChangeSets => {
                if EitherWriter::account_changesets_destination(provider).is_database() {
                    debug!(target: "rsil::providers::static_file", ?segment, "Skipping account changesets segment: changesets stored in database");
                    return false;
                }
                true
            }
    ...
    /// [`rsil_prune_types::PruneMode::Full`], indicating all data for this segment has been
    ...
    fn check_segment_consistency(&self, segment: StaticFileSegment) -> ProviderResult<()> {
        debug!(target: "rsil::providers::static_file", "Checking segment consistency");
        if let Some(latest_block) = self.get_highest_static_file_block(segment) {
            ...
            debug!(target: "rsil::providers::static_file", ?file_path, latest_block, "Loading NippyJar for consistency check");

            let jar = NippyJar::<SegmentHeader>::load(&file_path).map_err(ProviderError::other)?;
            debug!(target: "rsil::providers::static_file", "NippyJar loaded, checking consistency");

            NippyJarChecker::new(jar).check_consistency().map_err(ProviderError::other)?;
            debug!(target: "rsil::providers::static_file", "NippyJar consistency check passed");
        } else {
            debug!(target: "rsil::providers::static_file", "No static file block found, skipping consistency check");
        }
        Ok(())
    }
    ...
    fn maybe_heal_segment(
        ...
        debug!(target: "rsil::providers::static_file", ?initial_highest_block, "Initial highest block for segment");

        if self.access.is_read_only() {
            // Read-only mode: cannot modify files, so just validate consistency and error if
            // broken.
            debug!(target: "rsil::providers::static_file", "Checking segment consistency (read-only)");
            ...
            debug!(target: "rsil::providers::static_file", "Fetching latest writer which might heal any potential inconsistency");
    ...
    where
        Provider: DBProvider + BlockReader + StageCheckpointReader,
    {
        debug!(target: "rsil::providers::static_file", "Ensuring invariants");
        let mut db_cursor = provider.tx_ref().cursor_read::<T>()?;

        if let Some((db_first_entry, _)) = db_cursor.first()? {
            debug!(target: "rsil::providers::static_file", db_first_entry, "Found first database entry");
        ...
        debug!(target: "rsil::providers::static_file", ?stage_id, checkpoint_block_number, "Retrieved stage checkpoint");
        ...
        if checkpoint_block_number >= highest_static_file_block {
            debug!(target: "rsil::providers::static_file", "Invariants ensured, returning None");
            return Ok(None);
        }
        ...
        match segment {
            StaticFileSegment::Headers => {
                ...
                debug!(target: "rsil::providers::static_file", prune_count, "Pruning headers");
            ...
            | StaticFileSegment::TransactionSenders => {
                if let Some(block) = provider.block_body_indices(checkpoint_block_number)? {
                    ...
                    debug!(target: "rsil::providers::static_file", checkpoint_block_number, "No block body indices found for checkpoint block");
                }
            }
            StaticFileSegment::AccountChangeSets => {
        ...
        debug!(target: "rsil::providers::static_file", "Committing writer after pruning");
        writer.commit()?;
        debug!(target: "rsil::providers::static_file", "Writer committed successfully");
...
impl<N: NodePrimitives> ChangeSetReader for StaticFileProvider<N> {
    fn account_block_changeset(
    ...
    ) -> ProviderResult<Vec<rsil_db::models::AccountBeforeTx>> {
        ...
        if let Some(offset) = provider.read_changeset_offset(block_number)? {
            ...
            for i in offset.changeset_range() {
                if let Some(change) =
                    cursor.get_one::<rsil_db::static_file::AccountChangesetMask>(i.into())?
                {
    ...
    fn get_account_before_block(
        ...
        block_number: BlockNumber,
        address: Address,
    ) -> ProviderResult<Option<rsil_db::models::AccountBeforeTx>> {
        let provider = match self.get_segment_provider_for_block(
            StaticFileSegment::AccountChangeSets,
            block_number,
            None,
        ...
        while low < high {
            ...
            if let Some(change) =
                cursor.get_one::<rsil_db::static_file::AccountChangesetMask>(mid.into())?
            {
    ...
    ) -> ProviderResult<Vec<(BlockNumber, rsil_db::models::AccountBeforeTx)>> {
...
mod tests {
    ...
    use rsil_chain_state::SilPrimitives;
    use rsil_db::test_utils::create_test_static_files_dir;
    use rsil_static_file_types::{SegmentRangeInclusive, StaticFileSegment};
    ...
    fn test_find_fixed_range_with_block_index() -> eyre::Result<()> {
        ...
        let sf_rw: StaticFileProvider<SilPrimitives> =
