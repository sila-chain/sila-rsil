...
use rsil_nippy_jar::NippyJar;
use rsil_static_file_types::{ChangesetOffsetReader, SegmentHeader, StaticFileSegment};
use rsil_storage_errors::provider::{ProviderError, ProviderResult};
use std::{io, ops::Deref, sync::Arc};

/// Alias type for each specific `NippyJar`.
type LoadedJarRef<'a> =
    rsil_primitives_traits::dashmap::mapref::one::Ref<'a, (u64, StaticFileSegment), LoadedJar>;
...
pub struct LoadedJar {
    ...
    mmap_handle: Arc<rsil_nippy_jar::DataReader>,
...
impl LoadedJar {
    ...
    fn mmap_handle(&self) -> Arc<rsil_nippy_jar::DataReader> {
...
mod tests {
    ...
    use rsil_testing_utils::generators::{self, random_header_range};
    ...
    fn test_header_truncation() {
        ...
        fn prune_and_validate(
            writer: &mut StaticFileProviderRWRefMut<'_, SilPrimitives>,
            sf_rw: &StaticFileProvider<SilPrimitives>,
    ...
    fn test_account_changeset_static_files() {
        ...
        let sf_rw = StaticFileProvider::<SilPrimitives>::read_write(&static_dir)
