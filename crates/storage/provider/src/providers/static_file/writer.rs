...
pub struct StaticFileProviderRW<N> {
    ...
    /// stored in a [`rsil_primitives_traits::dashmap::DashMap`] inside the parent
...
impl<N: NodePrimitives> StaticFileProviderRW<N> {
    ...
    /// externally.
    ///
    /// Check [`rsil_nippy_jar::NippyJarChecker`] &
    ...
    fn heal_changeset_sidecar(&mut self) -> ProviderResult<()> {
        ...
        let actual_sidecar_blocks = if csoff_path.exists() {
            let file_len = rsil_fs_util::metadata(&csoff_path).map_err(ProviderError::other)?.len();
