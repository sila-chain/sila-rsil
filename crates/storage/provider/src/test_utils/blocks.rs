...
pub fn assert_genesis_block<DB: Database, N: NodeTypes>(
    ...
    g: SealedBlock<rsil_sila_primitives::Block>,
) {
    let n = g.number;
    let h = B256::ZERO;
    let tx = provider;
    ...
    // Reorged bytecodes are not reverted per https://github.com/sila-chain/sila-rsil/issues/1588
...
pub(crate) static TEST_BLOCK: LazyLock<SealedBlock<rsil_sila_primitives::Block>> =
    LazyLock::new(|| {
...
pub struct BlockchainTestData {
    /// Genesis
    pub genesis: SealedBlock<rsil_sila_primitives::Block>,
    /// Blocks with its execution result
    pub blocks: Vec<(RecoveredBlock<rsil_sila_primitives::Block>, ExecutionOutcome)>,
}
...
/// Genesis block
pub fn genesis() -> SealedBlock<rsil_sila_primitives::Block> {
    SealedBlock::from_sealed_parts(
        SealedHeader::new(
