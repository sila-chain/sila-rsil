...
use rsil_chain_state::{CanonStateNotifications, CanonStateSubscriptions};
use rsil_chainspec::{ChainInfo, SilChainSpec};
use rsil_db::transaction::DbTx;
use rsil_db_api::{
...
use rsil_storage_errors::provider::{ConsistentViewError, ProviderError, ProviderResult};
use rsil_trie::{
...
impl<T: NodePrimitives, ChainSpec> MockEthProvider<T, ChainSpec> {
    ...
    where
        ChainSpec: SilChainSpec<Header = <T::Block as Block>::Header>,
    ...
    for MockEthProvider<T, ChainSpec>
{
    fn storage_changeset(
        &self,
        _block_number: BlockNumber,
    ) -> ProviderResult<Vec<(rsil_db_api::models::BlockNumberAddress, StorageEntry)>> {
        Ok(Vec::default())
    }
