//! # RPC Blockchain Provider for Rsil
//!
//! This crate provides an RPC-based implementation of rsil's `StateProviderFactory` and related
...
//!
//! Originally created by [cakevm](https://github.com/cakevm/alloy-rsil-provider).
//!
//! ## Features
//!
//! - Implements `StateProviderFactory` for remote RPC state access
//! - Supports Sila and Optimism network
//! - Useful for testing without requiring a full database
//! - Can be used with rsil ExEx (Execution Extensions) for testing
...
use dashmap::DashMap;
use rsil_chainspec::{ChainInfo, ChainSpecProvider};
use rsil_db_api::{
    mock::{DatabaseMock, TxMock},
    models::StoredBlockBodyIndices,
};
use rsil_errors::{ProviderError, ProviderResult};
use rsil_node_types::{
    Block, BlockBody, BlockTy, HeaderTy, NodeTypes, PrimitivesTy, ReceiptTy, TxTy,
};
use rsil_primitives_traits::{Account, Bytecode, RecoveredBlock, SealedHeader};
use rsil_provider::{
...
pub struct RpcBlockchainProviderConfig {
    ...
    /// Whether to use Rsil-specific RPC methods for better performance
    ///
    /// If enabled, the node will use Rsil's RPC methods (`debug_codeByHash` and
    ...
    pub rsil_rpc_support: bool,
}

impl Default for RpcBlockchainProviderConfig {
    fn default() -> Self {
        Self { compute_state_root: false, rsil_rpc_support: true }
    }
}
...
impl RpcBlockchainProviderConfig {
    ...
    /// Sets whether to use Rsil-specific RPC methods for better performance
    pub const fn with_rsil_rpc_support(mut self, support: bool) -> Self {
        self.rsil_rpc_support = support;
        self
    }
}
...
where
    SilRpcConverter: RpcResponseConverter<
        N,
        Block = BlockTy<Node>,
        Transaction = TxTy<Node>,
        Receipt = ReceiptTy<Node>,
    >,
{
    /// Creates a new `RpcBlockchainProvider` with the default [`SilRpcConverter`].
    pub fn new(provider: P) -> Self
    where
        Node::ChainSpec: Default,
    {
        Self::new_with_converter(provider, SilRpcConverter)
    }
}
    ...
    Node: NodeTypes,
{
    ...
    fn create_state_provider(&self, block_id: BlockId) -> RpcBlockchainStateProvider<P, Node, N> {
        ...
        .with_rsil_rpc_support(self.config.rsil_rpc_support)
    }
...
where
    ...
    Node: NodeTypes,
{
    fn chain_info(&self) -> Result<rsil_chainspec::ChainInfo, ProviderError> {
...
impl<P: Clone, Node: NodeTypes, N> RpcBlockchainStateProvider<P, Node, N> {
    ...
    fn with_block_id(&self, block_id: BlockId) -> Self {
        Self {
            ...
            code_store: Default::default(),
            rsil_rpc_support: self.rsil_rpc_support,
        }
    }
    ...
    /// `eth_getAccountInfo`) to speed up account information retrieval. When disabled, it will
    /// use multiple standard RPC calls to get account information.
    pub const fn with_rsil_rpc_support(mut self, is_enable: bool) -> Self {
        self.rsil_rpc_support = is_enable;
        self
    }
    ...
    Node: NodeTypes,
{
    fn bytecode_by_hash(&self, code_hash: &B256) -> Result<Option<Bytecode>, ProviderError> {
        ...
        self.block_on_async(async {
            // The method `debug_codeByHash` is currently only available on a Rsil node
            let code = self
                .provider
    ...
    Node: NodeTypes,
{
    fn plain_state_storages(
    ...
    ) -> Result<Vec<(Address, Vec<rsil_primitives_traits::StorageEntry>)>, ProviderError> {
        ...
        for (address, keys) in addresses_with_keys {
            ...
            for key in keys {
                ...
                values.push(rsil_primitives_traits::StorageEntry::new(key, value));
            }
...
impl<P, Node, N> rsil_storage_api::StorageRootProvider for RpcBlockchainStateProvider<P, Node, N>
where
    P: Provider<N> + Clone + 'static,
    N: Network,
    Node: NodeTypes,
{
    fn storage_root(
        &self,
        _address: Address,
        _hashed_storage: rsil_trie::HashedStorage,
    ...
    fn storage_proof(
        ...
        _slot: B256,
        _hashed_storage: rsil_trie::HashedStorage,
    ) -> Result<rsil_trie::StorageProof, ProviderError> {
        Err(ProviderError::UnsupportedProvider)
    }
    ...
    fn storage_multiproof(
        ...
        _slots: &[B256],
        _hashed_storage: rsil_trie::HashedStorage,
    ) -> Result<rsil_trie::StorageMultiProof, ProviderError> {
        Err(ProviderError::UnsupportedProvider)
    }
}

impl<P, Node, N> rsil_storage_api::StateProofProvider for RpcBlockchainStateProvider<P, Node, N>
where
    P: Provider<N> + Clone + 'static,
    N: Network,
    Node: NodeTypes,
{
    ...
    fn multiproof(
        &self,
        _input: TrieInput,
        _targets: rsil_trie::MultiProofTargets,
    ...
    fn witness(
        ...
        _mode: rsil_trie::ExecutionWitnessMode,
    ) -> Result<Vec<alloy_primitives::Bytes>, ProviderError> {
        Err(ProviderError::UnsupportedProvider)
    }
}

impl<P, Node, N> rsil_storage_api::HashedPostStateProvider
    for RpcBlockchainStateProvider<P, Node, N>
where
    P: Provider<N> + Clone + 'static,
    N: Network,
    ...
    Node: NodeTypes,
{
    ...
    fn get_state(
        &self,
        _block: BlockNumber,
    ) -> Result<Option<rsil_execution_types::ExecutionOutcome<Self::Receipt>>, ProviderError> {
    ...
    Node: NodeTypes,
{
    ...
    fn prune_modes_ref(&self) -> &rsil_prune_types::PruneModes {
    ...
    Node: NodeTypes,
{
    ...
    fn find_block_by_hash(
        &self,
        _hash: B256,
        _source: rsil_provider::BlockSource,
    ...
    Node: NodeTypes,
{
    ...
    ) -> Result<Vec<rsil_db_api::models::AccountBeforeTx>, ProviderError> {
        Err(ProviderError::UnsupportedProvider)
    }
    ...
    fn get_account_before_block(
        ...
        _block_number: BlockNumber,
        _address: Address,
    ) -> ProviderResult<Option<rsil_db_api::models::AccountBeforeTx>> {
        Err(ProviderError::UnsupportedProvider)
    }
    ...
    ) -> ProviderResult<Vec<(BlockNumber, rsil_db_api::models::AccountBeforeTx)>> {
        Err(ProviderError::UnsupportedProvider)
    }
}
...
where
    ...
    Node: NodeTypes,
{
    fn count_entries<T: rsil_db_api::table::Table>(&self) -> Result<usize, ProviderError> {
        Ok(0)
    }
}
    ...
    Node: NodeTypes,
{
    fn block_body_indices(
        &self,
        _num: u64,
    ) -> Result<Option<rsil_db_api::models::StoredBlockBodyIndices>, ProviderError> {
        Err(ProviderError::UnsupportedProvider)
    }
    ...
    ) -> Result<Vec<rsil_db_api::models::StoredBlockBodyIndices>, ProviderError> {
        Err(ProviderError::UnsupportedProvider)
    }
}
