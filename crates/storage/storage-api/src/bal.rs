...
mod subscriptions {
    ...
    pub type BalNotificationStream = rsil_tokio_util::EventStream<BalNotification>;
}
            ...
            .map(|decoded| {
                decoded.try_map(|bal| {
                    RevmBal::try_from(Vec::from(bal))
                        .map(Arc::new)
                        .map_err(rsil_storage_errors::provider::ProviderError::other)
                })
            })
            .transpose()
    }
...
impl BalStore for NoopBalStore {
    ...
    #[cfg(feature = "std")]
    fn bal_stream(&self) -> BalNotificationStream {
        rsil_tokio_util::EventSender::new(1).new_listener()
    }
}
