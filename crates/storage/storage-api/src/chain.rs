...
/// Sila storage implementation.
#[derive(Debug, Clone, Copy)]
pub struct SilStorage<T = TransactionSigned, H = Header>(PhantomData<(T, H)>);

impl<T, H> Default for SilStorage<T, H> {
    fn default() -> Self {
...
impl<Provider, T, H> BlockBodyReader<Provider> for SilStorage<T, H>
where
    Provider: DBProvider + ChainSpecProvider<ChainSpec: SilaHardforks>,
    T: SignedTransaction,
    H: FullBlockHeader,
{
...
where
    Provider: ChainSpecProvider<ChainSpec: SilaHardforks>,
    T: SignedTransaction,
    H: FullBlockHeader,
{
    type Block = alloy_consensus::Block<T, H>;
