...
use rsil_chainspec::{ChainInfo, ChainSpecProvider, SilChainSpec, SILA_MAINNET};
#[cfg(feature = "db-api")]
...
#[derive(Debug)]
#[non_exhaustive]
pub struct NoopProvider<ChainSpec = rsil_chainspec::ChainSpec, N = SilPrimitives> {
...
impl<ChainSpec> NoopProvider<ChainSpec> {
    ...
    pub fn sil(chain_spec: Arc<ChainSpec>) -> Self {
        Self {
            chain_spec,
            bal_store: BalStoreHandle::default(),
            #[cfg(feature = "db-api")]
...
impl<ChainSpec: SilChainSpec + 'static, N: Debug + Send + Sync + 'static> ChainSpecProvider
...
impl<C: Send + Sync, N: NodePrimitives> StorageChangeSetReader for NoopProvider<C, N> {
    fn storage_changeset(
        ...
        Vec<(rsil_db_api::models::BlockNumberAddress, rsil_primitives_traits::StorageEntry)>,
    > {
        Ok(Vec::default())
    }
    ...
    fn get_storage_before_block(
        ...
        _address: Address,
        _storage_key: B256,
    ) -> ProviderResult<Option<rsil_primitives_traits::StorageEntry>> {
        Ok(None)
    }
...
impl<ChainSpec: Send + Sync, N: Send + Sync> StorageSettingsCache for NoopProvider<ChainSpec, N> {
    fn cached_storage_settings(&self) -> rsil_db_api::models::StorageSettings {
        rsil_db_api::models::StorageSettings::default()
    }

    fn set_storage_settings_cache(&self, _settings: rsil_db_api::models::StorageSettings) {}
}
