...
impl InMemoryBlobStore {
    /// Look up SIP-7594 blobs by their versioned hashes.
    ///
    ...
    /// Look up SIP-7594 blob cells by their versioned hashes.
...
mod tests {
    ...
    fn sip7594_single_blob_sidecar() -> (BlobTransactionSidecarVariant, B256, BlobAndProofV2) {
    ...
    fn mem_has_blobs_returns_ordered_availability() {
        ...
        let (sip7594_sidecar, sip7594_hash, _) = sip7594_single_blob_sidecar();
        let (sip4844_sidecar, sip4844_hash) = sip4844_single_blob_sidecar();
        store.insert(B256::random(), sip7594_sidecar).unwrap();
        store.insert(B256::random(), sip4844_sidecar).unwrap();

        let request = vec![sip7594_hash, B256::ZERO, sip4844_hash, sip7594_hash];
    ...
    fn mem_get_blobs_v3_returns_partial_results() {
        ...
        let (sidecar, versioned_hash, expected) = sip7594_single_blob_sidecar();
    ...
    fn mem_get_blobs_v4_returns_requested_cells() {
        ...
        let (sidecar, versioned_hash, _) = sip7594_single_blob_sidecar();
        store.insert(B256::random(), sidecar).unwrap();
