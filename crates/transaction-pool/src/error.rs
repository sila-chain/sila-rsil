...
/// Represents all errors that can happen when validating transactions for the pool for SIP-4844
/// transactions
#[derive(Debug, thiserror::Error)]
pub enum Sip4844PoolTransactionError {
    /// Thrown if we're unable to find the blob for a transaction that was previously extracted
    #[error("blob sidecar not found for SIP4844 transaction")]
    MissingEip4844BlobSidecar,
    /// Thrown if an SIP-4844 transaction without any blobs arrives
    #[error("blobless blob transaction")]
    NoEip4844Blobs,
    /// Thrown if an SIP-4844 transaction arrives with too many blobs
    ...
    /// SIP-4844 transactions are only accepted if they're gapless, meaning the previous nonce of
    ...
    #[error("sip-7594 sidecar disallowed")]
    Sip7594SidecarDisallowed,
}

/// Represents all errors that can happen when validating transactions for the pool for SIP-7702
/// transactions
#[derive(Debug, thiserror::Error)]
pub enum Sip7702PoolTransactionError {
    /// Thrown if the transaction has no items in its authorization list
    #[error("no items in authorization list for SIP7702 transaction")]
...
pub enum InvalidPoolTransactionError {
    ...
    Overdraft {
        /// Cost transaction is allowed to consume. See `rsil_transaction_pool::PoolTransaction`.
        cost: U256,
        /// Balance of account.
        balance: U256,
    },
    /// SIP-2681 error thrown if the nonce is higher or equal than `U64::max`
    /// `<https://sips.sila.org/SIPS/sip-2681>`
    #[error("nonce exceeds u64 limit")]
    Sip2681,
    /// SIP-4844 related errors
    #[error(transparent)]
    Sip4844(#[from] Sip4844PoolTransactionError),
    /// SIP-7702 related errors
    #[error(transparent)]
        ...
        match self {
            ...
            Self::Sip4844(sip4844_err) => {
                match sip4844_err {
                    ...
                    Sip4844PoolTransactionError::Sip4844NonceGap => {
            ...
            Self::Sip7702(sip7702_err) => match sip7702_err {
                Sip7702PoolTransactionError::MissingEip7702AuthorizationList => {
                    // as SIP-7702 specifies, 7702 transactions must have an non-empty authorization
                    // list so this is a malformed transaction and should not be
                    // sent over the network
                    true
                }
                Sip7702PoolTransactionError::OutOfOrderTxFromDelegated => false,
                Sip7702PoolTransactionError::InflightTxLimitReached => false,
                Sip7702PoolTransactionError::AuthorityReserved => false,
            },
