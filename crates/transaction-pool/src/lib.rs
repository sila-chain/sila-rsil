//! Rsil's transaction pool implementation.
//!
...
//! ### Stateless Checks
//!
//! Sila transactions undergo several stateless checks:
//!
//! - **Transaction Type**: Fork-dependent support (Legacy always, SIP-2930/1559/4844/7702 need
//!   activation)
...
//! - **Blobs** (SIP-4844): Valid count, KZG proofs
//!
//! ### Stateful Checks
//!
...
//! ### Common Errors
//!
//! - [`NonceNotConsistent`](rsil_primitives_traits::transaction::error::InvalidTransactionError::NonceNotConsistent): Nonce too low
//! - [`InsufficientFunds`](rsil_primitives_traits::transaction::error::InvalidTransactionError::InsufficientFunds): Insufficient balance
//! - [`ExceedsGasLimit`](crate::error::InvalidPoolTransactionError::ExceedsGasLimit): Gas limit too
//!   high
//! - [`SignerAccountHasBytecode`](rsil_primitives_traits::transaction::error::InvalidTransactionError::SignerAccountHasBytecode): EOA has code
...
//! - Blob errors:
//!   - [`MissingEip4844BlobSidecar`](crate::error::Sip4844PoolTransactionError::MissingEip4844BlobSidecar): Missing sidecar
//!   - [`InvalidEip4844Blob`](crate::error::Sip4844PoolTransactionError::InvalidEip4844Blob):
//!     Invalid blob proofs
//!   - [`NoEip4844Blobs`](crate::error::Sip4844PoolTransactionError::NoEip4844Blobs): SIP-4844
//!     transaction without blobs
//!   - [`TooManyEip4844Blobs`](crate::error::Sip4844PoolTransactionError::TooManyEip4844Blobs): Too
//!     many blobs
//!
//! ## Subpool Design
//!
...
//! 4. **Blob**: SIP-4844 transactions not pending due to insufficient base fee or blob fee
//!
//! ### State Transitions
//!
...
//! ### Transaction type
//!
//! The pool expects certain sila related information from the generic transaction type of the
//! pool ([`PoolTransaction`]), this includes gas price, base fee (SIP-1559 transactions), nonce
...
//! where
//!     C: ChainSpecProvider<ChainSpec: SilaHardforks> + StateProviderFactory + BlockReaderIdExt<Header = Header> + Clone + 'static,
//!     Savm: ConfigureEvm<Primitives: rsil_primitives_traits::NodePrimitives<BlockHeader = Header>> + 'static,
//! {
//!     let blob_store = InMemoryBlobStore::default();
//!     let runtime = Runtime::test();
//!     let pool = Pool::eth_pool(
//!         TransactionValidationTaskExecutor::sil(client, evm_config, blob_store.clone(), runtime),
//!         blob_store,
//!         Default::default(),
//!     );
...
//! use rsil_chainspec::{SILA_MAINNET, ChainSpecProvider, ChainSpec};
//! use rsil_storage_api::{BlockReaderIdExt, StateProviderFactory};
//! use rsil_tasks::Runtime;
...
//! use alloy_consensus::Header;
//!
//!  async fn t<C, St, Savm>(client: C, stream: St, evm_config: Savm)
//!    where C: StateProviderFactory + BlockReaderIdExt<Header = Header> + ChainSpecProvider<ChainSpec = ChainSpec> + Clone + 'static,
//!     St: Stream<Item = CanonStateNotification<SilPrimitives>> + Send + Unpin + 'static,
//!     Savm: ConfigureEvm<Primitives = SilPrimitives> + 'static,
//!     {
//!     let blob_store = InMemoryBlobStore::default();
//!     let runtime = Runtime::test();
//!     let pool = Pool::eth_pool(
//!         TransactionValidationTaskExecutor::sil(client.clone(), evm_config, blob_store.clone(), runtime.clone()),
//!         blob_store,
//!         Default::default(),
//!     );
//!
...
pub use crate::{
    ...
    traits::*,
    validate::{
        SilTransactionValidator, StatefulValidationFn, StatelessValidationFn,
...
/// Type alias for default sila transaction pool
pub type SilTransactionPool<Client, S, Savm = SilEvmConfig, T = SilPooledTransaction> = Pool<
    TransactionValidationTaskExecutor<SilTransactionValidator<Client, T, Savm>>,
...
impl<Client, S, Savm> SilTransactionPool<Client, S, Savm>
where
    Client: ChainSpecProvider<ChainSpec: SilaHardforks>
        ...
        + 'static,
    S: BlobStore,
    Savm: ConfigureEvm + 'static,
{
    /// Returns a new [`Pool`] that uses the default [`TransactionValidationTaskExecutor`] when
    /// validating [`SilPooledTransaction`]s and ords via [`CoinbaseTipOrdering`]
    ///
    /// # Example
    ///
    /// ```
    ...
    /// let blob_store = InMemoryBlobStore::default();
    /// let pool = Pool::eth_pool(
    ///     TransactionValidationTaskExecutor::sil(
    ///         client,
    ///         evm_config,
    ///         blob_store.clone(),
    ///         runtime,
    ...
    pub fn eth_pool(
        validator: TransactionValidationTaskExecutor<
            SilTransactionValidator<Client, SilPooledTransaction, Savm>,
        >,
        blob_store: S,
        config: PoolConfig,
    ) -> Self {
...
where
    V: TransactionValidator,
    <V as TransactionValidator>::Transaction: SilPoolTransaction,
    T: TransactionOrdering<Transaction = <V as TransactionValidator>::Transaction>,
    S: BlobStore + Clone,
{
