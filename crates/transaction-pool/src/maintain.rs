...
use crate::{
    ...
    traits::{CanonicalStateUpdate, SilPoolTransaction, TransactionPool, TransactionPoolExt},
...
use rsil_storage_api::{errors::provider::ProviderError, BlockReaderIdExt, StateProviderFactory};
...
where
    ...
    Client: StateProviderFactory
        ...
        + ChainSpecProvider<ChainSpec: SilChainSpec<Header = N::BlockHeader> + SilaHardforks>
        + Clone
        + 'static,
            ...
            CanonStateNotification::Commit { new } => {
                    ...
                    chain_spec.is_osaka_active_at_timestamp(tip.timestamp().saturating_add(24))
                {
                    ...
                    task_spawner.spawn_task(async move {
                        ...
                        // it's not SilaOsaka yet).
                        tokio::time::sleep(Duration::from_secs(4)).await;

                        let mut interval = tokio::time::interval(Duration::from_secs(1));
                        loop {
                            // Loop and replace blob transactions until we reach SilaOsaka transition
                            ...
                            for tx in pending.into_iter().chain(queued).filter(|tx| tx.is_eip4844())
                            {
                                ...
                                spawner.spawn_task(async move {
                                    // Convert sidecar to SIP-7594 format
                                    ...
                                    let Some(tx) = SilPoolTransaction::try_from_eip4844(
                                        tx.to_consensus(),
                                        sidecar.into(),
                                    ) else {
                                        return;
    ...
    P: TransactionPool<Transaction: PoolTransaction<Consensus: SignedTransaction>>,
{
    ...
    rsil_fs_util::remove_file(file_path)?;
    Ok(())
}
    ...
    P: TransactionPool<Transaction: PoolTransaction<Consensus: Encodable>>,
{
    ...
    match parent_dir.map(|_| rsil_fs_util::write(file_path, json_data)) {
        Ok(_) => {
...
pub async fn backup_local_transactions_task<P>(
    shutdown: rsil_tasks::shutdown::GracefulShutdown,
    pool: P,
...
mod tests {
    ...
    async fn test_save_local_txs_backup() {
        ...
        let transaction = SilPooledTransaction::from_pooled(tx.try_into_recovered().unwrap());
