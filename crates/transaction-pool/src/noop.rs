...
#[derive(Debug, Clone)]
#[non_exhaustive]
pub struct NoopTransactionPool<T = SilPooledTransaction> {
    /// Type marker
...
impl Default for NoopTransactionPool<SilPooledTransaction> {
    fn default() -> Self {
        Self { _marker: Default::default() }
    }
}

impl<T: SilPoolTransaction> TransactionPool for NoopTransactionPool<T> {
    type Transaction = T;
...
impl<T: SilPoolTransaction> TransactionValidator for MockTransactionValidator<T> {
    type Transaction = T;
    type Block = rsil_sila_primitives::Block;
...
pub struct NoopInsertError<T: SilPoolTransaction = SilPooledTransaction> {
    tx: T,
}

impl<T: SilPoolTransaction> NoopInsertError<T> {
