...
impl<T: TransactionOrdering> Iterator for BestTransactionsWithFees<T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        loop {
            ...
            // If both the base fee and blob fee (if applicable for SIP-4844) are satisfied, return
            // the transaction
...
pub struct BestTransactions<T: TransactionOrdering> {
    ...
    /// Flag to control whether to skip blob transactions (SIP4844).
...
mod tests {
    ...
    fn test_best_transactions_size_hint() {
        ...
        for nonce in 0..3 {
            let tx = MockTransaction::sip1559().rng_hash().with_nonce(nonce);
    ...
    fn test_best_transactions_with_fees_size_hint() {
        ...
        for nonce in 0..3 {
            let tx = MockTransaction::sip1559().rng_hash().with_nonce(nonce).with_max_fee(100);
            pool.add_transaction(Arc::new(f.validated(tx)), 0);
        }
    ...
    fn test_best_transactions_with_prioritized_senders_size_hint() {
        ...
        for gas_price in 0..5 {
            let tx = MockTransaction::sip1559().with_gas_price((gas_price + 1) * 10);
            pool.add_transaction(Arc::new(f.validated(tx)), 0);
        }

        let prioritized_tx = MockTransaction::sip1559().with_gas_price(5).with_gas_limit(200);
    ...
    fn test_best_with_fees_iter_mixed_fees() {
        ...
        let tx1 =
            MockTransaction::sip1559().rng_hash().with_nonce(0).with_max_fee(base_fee as u128 + 5);
        ...
        let tx3 = MockTransaction::sip4844()
            ...
            .with_blob_fee(base_fee_per_blob_gas as u128 - 5);
        let tx4 =
            MockTransaction::sip1559().rng_hash().with_nonce(3).with_max_fee(base_fee as u128 - 5);
    ...
    fn test_best_add_transaction_with_next_nonce() {
        ...
        let new_tx = MockTransaction::sip1559().rng_hash().with_nonce(5);
    ...
    fn test_best_add_transaction_with_ancestor() {
        ...
        // Create a new transaction with nonce 5 and validate it
        let base_tx1 = MockTransaction::sip1559().rng_hash().with_nonce(5);
    ...
    fn test_best_transactions_prioritized_senders() {
        ...
        // of gas limit.
        let prioritized_tx2 = MockTransaction::sip1559().with_gas_price(3);
    ...
    fn test_best_with_fees_iter_mix_of_blob_and_non_blob_transactions() {
        ...
        let tx_blob = MockTransaction::sip4844()
            .rng_hash()
            .with_nonce(1)
            .with_max_fee(base_fee as u128 + 5)
            .with_blob_fee(base_fee_per_blob_gas as u128 + 5);
    ...
    fn test_best_transactions_with_skipping_blobs() {
        ...
        let tx_blob = MockTransaction::sip4844().rng_hash().with_nonce(0).with_blob_fee(100);
        let valid_blob_tx = f.validated(tx_blob);
        pool.add_transaction(Arc::new(valid_blob_tx), 0);

        // Add a non-blob transaction
        let tx_non_blob = MockTransaction::sip1559().rng_hash().with_nonce(1).with_max_fee(200);
    ...
    fn test_best_transactions_no_updates() {
        ...
        // Add a transaction
        let tx = MockTransaction::sip1559().rng_hash().with_nonce(0).with_max_fee(100);
        let valid_tx = f.validated(tx);
        pool.add_transaction(Arc::new(valid_tx), 0);
    ...
    fn test_best_update_transaction_priority() {
        ...
        // Create a new transaction with nonce 5 and validate it
        let new_higher_fee_tx = MockTransaction::sip1559().with_nonce(0);
    ...
    fn test_blob_transaction_ordering_multiple_clients_shape() {
        ...
        let sender_a = MockTransaction::sip4844()
            .with_blob_hashes(5)
        ...
        let sender_b = MockTransaction::sip4844()
            .with_blob_hashes(1)
            .with_max_fee(base_fee as u128 + 20)
            .with_priority_fee(base_fee as u128 + 20)
            .with_blob_fee(100);
