...
//!
//!  b) Transaction does not meet the dynamic fee cap requirement introduced by SIP-1559: The
...
//!    SIP-1559 and all subsequent transactions of the sender currently invalid.
//!
...
use crate::{
    ...
    CanonicalStateUpdate, PoolConfig, SilPoolTransaction, TransactionOrdering,
...
/// Bound on number of pending transactions from `rsil_network::TransactionsManager` to buffer.
pub const PENDING_TX_LISTENER_BUFFER_SIZE: usize = 2048;
/// Bound on number of new transactions from `rsil_network::TransactionsManager` to buffer.
    ...
    S: BlobStore,
{
    ...
    /// sip4844 transaction's blob sidecar.
    ...
    ///
    /// If the transaction is an SIP-4844 transaction, the blob sidecar is fetched from the blob
                ...
                let (transaction, blob_sidecar) = match transaction {
                    ...
                    ValidTransaction::ValidWithSidecar { transaction, sidecar } => {
                        debug_assert!(
                            ...
                            "validator returned sidecar for non SIP-4844 transaction"
                        );
    ...
    fn on_added_transaction(&self, meta: AddedTransactionMeta<T::Transaction>) {
        // Handle blob sidecar storage and notifications for SIP-4844 transactions
    ...
    /// Notify all listeners about a blob sidecar for a newly inserted blob (sip4844) transaction.
...
struct AddedTransactionMeta<T: PoolTransaction> {
    ...
    /// Optional blob sidecar for SIP-4844 transactions
...
pub enum QueuedReason {
    ...
    InsufficientBaseFee,
    /// Transaction doesn't meet the blob fee requirement (SIP-4844)
    InsufficientBlobFee,
}
...
mod tests {
    ...
    fn test_discard_blobs_on_blob_tx_eviction() {
        ...
        for n in 0..blob_limit.max_txs + 10 {
            ...
            let mut tx = MockTransaction::sip4844_with_sidecar(sidecar.clone());
    ...
    fn test_auths_stored_in_identifiers() {
        ...
        let tx = MockTransaction::sip7702();
