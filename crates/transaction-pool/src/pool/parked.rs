...
mod tests {
    ...
    fn test_enforce_parked_basefee() {
        ...
        let tx = f.validated_arc(MockTransaction::sip1559().inc_price());
    ...
    fn test_enforce_parked_basefee_descendant() {
        let mut f = MockTransactionFactory::default();
        let mut pool = ParkedPool::<BasefeeOrd<_>>::default();
        let t = MockTransaction::sip1559().inc_price_by(10);
    ...
    fn test_truncate_parked_with_large_tx() {
        ...
        let a_txs = MockTransactionSet::dependent(a_sender, 0, 2, TxType::Sip1559)
            .into_iter()
            .map(|mut tx| {
    ...
    fn test_add_sender_count_multiple_senders() {
        ...
        let tx1 = f.validated_arc(MockTransaction::sip1559().inc_price());
        let tx2 = f.validated_arc(MockTransaction::sip1559().inc_price());
    ...
    fn test_pool_size() {
        ...
        let tx = f.validated_arc(MockTransaction::sip1559().set_size(1024).clone());
        pool.add_transaction(tx);
    ...
    fn test_pool_len() {
        ...
        let tx = f.validated_arc(MockTransaction::sip1559());
        pool.add_transaction(tx);
        assert_eq!(pool.len(), 1);
    }
    ...
    fn test_all_transactions() {
        ...
        let tx1 = f.validated_arc(MockTransaction::sip1559());
        let tx2 = f.validated_arc(MockTransaction::sip1559());
    ...
    fn test_satisfy_base_fee_transactions() {
        ...
        let tx1 = f.validated_arc(MockTransaction::sip1559().set_max_fee(100).clone());
        let tx2 = f.validated_arc(MockTransaction::sip1559().set_max_fee(200).clone());
        pool.add_transaction(tx1);
        pool.add_transaction(tx2.clone());
