...
impl<T: TransactionOrdering> TxPool<T> {
    ...
    ) -> Box<dyn crate::traits::BestTransactions<Item = Arc<ValidPoolTransaction<T::Transaction>>>>
    {
        ...
        {
            Ordering::Equal => {
                // for SIP-4844 transactions we also need to check if the blob fee is now lower than
    ...
    pub(crate) fn update_transaction_type_metrics(&self) {
        ...
        self.metrics.total_eip2930_transactions.set(counts.sip2930 as f64);
        self.metrics.total_eip1559_transactions.set(counts.sip1559 as f64);
        self.metrics.total_eip4844_transactions.set(counts.sip4844 as f64);
        self.metrics.total_eip7702_transactions.set(counts.sip7702 as f64);
            ...
            Err(err) => {
                ...
                match err {
                    ...
                    InsertErr::BlobTxHasNonceGap { transaction } => Err(PoolError::new(
                        *transaction.hash(),
                        PoolErrorKind::InvalidTransaction(
                            Sip4844PoolTransactionError::Sip4844NonceGap.into(),
                        ),
                    )),
    ...
    fn check_delegation_limit(
        ...
        Err(PoolError::new(
            *transaction.hash(),
            PoolErrorKind::InvalidTransaction(InvalidPoolTransactionError::Sip7702(
                Sip7702PoolTransactionError::InflightTxLimitReached,
            )),
        ))
    }

    /// This verifies that the transaction complies with code authorization
    /// restrictions brought by SIP-7702 transaction type:
...
impl<T: PoolTransaction> AllTransactions<T> {
    ...
    ///
    /// This is a noop for non SIP-7702 transactions.
    ...
    ///
    /// Note: For SIP-4844 blob transactions additional constraints are enforced:
    ///      - new blob transactions must not have any nonce gaps
    ///      - blob transactions cannot go into overdraft
    ///
    /// ## Transaction type Exclusivity
    ///
    /// The pool enforces exclusivity of sip-4844 blob vs non-blob transactions on a per sender
    /// basis:
    ...
    pub(crate) fn insert_tx(
        ...
        if transaction.is_eip4844() {
            ...
            // Non-SIP4844 transaction always satisfy the blob fee cap condition
            state.insert(TxState::ENOUGH_BLOB_FEE_CAP_BLOCK);
        }
...
impl<T: PoolTransaction> Default for AllTransactions<T> {
    fn default() -> Self {
        Self {
            ...
            block_gas_limit: SILA_BLOCK_GAS_LIMIT_30M,
...
mod tests {
    ...
    fn test_insert_blob() {
        ...
        let tx = MockTransaction::sip4844().inc_price().inc_limit();
    ...
    fn test_insert_pending() {
        ...
        let mut f = MockTransactionFactory::default();
        let mut pool = AllTransactions::default();
        let tx = MockTransaction::sip1559().inc_price().inc_limit();
        let valid_tx = f.validated(tx);
        let InsertOk { updates, replaced_tx, move_to, state, .. } =
            pool.insert_tx(valid_tx.clone(), on_chain_balance, on_chain_nonce).unwrap();
        assert!(updates.is_empty());
    ...
    fn test_simple_insert() {
        ...
        let mut f = MockTransactionFactory::default();
        let mut pool = AllTransactions::default();
        let mut tx = MockTransaction::sip1559().inc_price().inc_limit();
    ...
    fn test_on_canonical_state_change_no_double_processing() {
        ...
        let tx = MockTransaction::sip1559().with_gas_price(50).with_gas_limit(30_000);
    ...
    fn test_canonical_state_change_with_basefee_update_regression() {
        ...
        let tx1 =
            MockTransaction::sip1559().with_gas_price(60).with_gas_limit(21_000).with_nonce(0);
        let sender1 = tx1.sender();

        // Sender 2: tx will be promoted (gas price 55 > new base fee 50)
        let tx2 =
            MockTransaction::sip1559().with_gas_price(55).with_gas_limit(21_000).with_nonce(0);
        let sender2 = tx2.sender();

        // Sender 3: tx will NOT be promoted (gas price 45 < new base fee 50)
        let tx3 =
            MockTransaction::sip1559().with_gas_price(45).with_gas_limit(21_000).with_nonce(0);
    ...
    fn test_basefee_decrease_with_empty_senders() {
        ...
        let tx = MockTransaction::sip1559().with_gas_price(60).with_gas_limit(21_000);
    ...
    fn insert_conflicting_type_normal_to_blob() {
        ...
        let tx = MockTransaction::sip4844().set_sender(tx.sender()).inc_price_by(100).inc_limit();
    ...
    fn insert_conflicting_type_blob_to_normal() {
        ...
        let first = f.validated(tx.clone());
        pool.insert_tx(first, on_chain_balance, on_chain_nonce).unwrap();
        let tx = MockTransaction::sip1559().set_sender(tx.sender()).inc_price_by(100).inc_limit();
        let tx = f.validated(tx);
    ...
    fn insert_previous() {
        ...
        let mut f = MockTransactionFactory::default();
        let mut pool = AllTransactions::default();
        let tx = MockTransaction::sip1559().inc_nonce().inc_price().inc_limit();
        let first = f.validated(tx.clone());
    ...
    fn insert_with_updates() {
        ...
        let mut f = MockTransactionFactory::default();
        let mut pool = AllTransactions::default();
        let tx = MockTransaction::sip1559().inc_nonce().set_gas_price(100).inc_limit();
        let first = f.validated(tx.clone());
    ...
    fn insert_previous_blocking() {
        ...
        let tx = MockTransaction::sip1559().inc_nonce().inc_limit();
        let first = f.validated(tx.clone());
    ...
    fn rejects_spammer() {
        ...
        let mut tx = MockTransaction::sip1559();
    ...
    fn reject_tx_over_gas_limit() {
        ...
        let tx = MockTransaction::sip1559().with_gas_limit(30_000_001);
    ...
    fn test_tx_equal_gas_limit() {
        ...
        let tx = MockTransaction::sip1559().with_gas_limit(30_000_000);
    ...
    fn update_basefee_subpools() {
        ...
        let tx = MockTransaction::sip1559().inc_price_by(10);
        let validated = f.validated(tx.clone());
        let id = *validated.id();
    ...
    fn basefee_decrease_promotes_affordable_and_keeps_unaffordable() {
        ...
        let tx1 = MockTransaction::sip1559()
            .set_sender(sender_a)
            .set_nonce(0)
            .set_max_fee(500)
            .inc_limit();
        let tx2 = MockTransaction::sip1559()
            .set_sender(sender_b)
            .set_nonce(0)
            .set_max_fee(600)
            .inc_limit();
        let tx3 = MockTransaction::sip1559()
            .set_sender(sender_c)
            .set_nonce(0)
            .set_max_fee(400)
            .inc_limit();
    ...
    fn apply_fee_updates_records_promotions_after_blob_fee_drop() {
        ...
        let tx = MockTransaction::sip4844().with_blob_fee(initial_blob_fee + 100);
        let validated = f.validated(tx.clone());
        let id = *validated.id();
        pool.add_transaction(validated, U256::from(1_000_000), 0, None).unwrap();
    ...
    fn apply_fee_updates_promotes_blob_after_basefee_drop() {
        ...
        let tx = MockTransaction::sip4844()
            .with_max_fee(500)
            .with_priority_fee(1)
    ...
    fn get_highest_consecutive_transaction_by_sender() {
        ...
        for nonce in txs {
            let mut mock_tx = MockTransaction::sip1559();
    ...
    fn discard_at_capacity() {
        ...
        for _ in 0..queued_limit.max_txs {
            let tx = MockTransaction::sip1559().inc_price_by(10).inc_nonce();
            let validated = f.validated(tx.clone());
    ...
    fn discard_blobs_at_capacity() {
        ...
        for _ in 0..blob_limit.max_txs {
            let tx = MockTransaction::sip4844().inc_price_by(100).with_blob_fee(100);
            let validated = f.validated(tx.clone());
            let _id = *validated.id();
            pool.add_transaction(validated, U256::from(1_000), 0, None).unwrap();
        }
    ...
    fn account_updates_sender_balance() {
        ...
        let tx_0 = MockTransaction::sip1559().set_gas_price(100).inc_limit();
        let tx_1 = tx_0.next();
        let tx_2 = tx_1.next();
    ...
    fn test_remove_transactions() {
        ...
        let tx_0 = MockTransaction::sip1559().set_gas_price(100).inc_limit();
        let tx_1 = tx_0.next();
        let tx_2 = MockTransaction::sip1559().set_gas_price(100).inc_limit();
        let tx_3 = tx_2.next();
    ...
    fn test_pending_ordering() {
        ...
        let tx_0 = MockTransaction::sip1559().with_nonce(1).set_gas_price(100).inc_limit();
        let tx_1 = tx_0.next();
    ...
    // <https://github.com/sila-chain/sila-rsil/issues/12286>
    #[test]
    fn one_sender_one_independent_transaction() {
        ...
        let template =
            MockTransaction::sip1559().inc_price().inc_limit().with_value(U256::from(1_001));
    ...
    fn test_non_4844_blob_fee_bit_invariant() {
        ...
        let non_4844_tx = MockTransaction::sip1559().set_max_fee(200).inc_limit();
    ...
    fn test_blob_fee_enforcement_only_applies_to_eip4844() {
        ...
        // SIP-4844: blob fee enforcement applies - insufficient blob fee removes bit
    ...
    /// Test for <https://github.com/sila-chain/sila-rsil/issues/17701>
    ///
    ...
    fn best_transactions_nonce_order_on_balance_unlock() {
        ...
        let tx0 = MockTransaction::sip1559().with_sender(sender).set_gas_price(100).inc_limit();
