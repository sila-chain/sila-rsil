...
use alloy_consensus::{
    ...
    Signed, SilaTxEnvelope, TxEip1559, TxEip2930, TxEip4844, TxEip4844Variant, TxEip7702, TxLegacy,
    TxType, Typed2718,
};
use alloy_eips::{
...
impl MockTransaction {
    ...
    /// Returns a new SIP2930 transaction with random address and hash and empty values
    pub fn sip2930() -> Self {
        Self::Sip2930 {
            chain_id: 1,
            hash: B256::random(),
    ...
    /// Returns a new SIP1559 transaction with random address and hash and empty values
    pub fn sip1559() -> Self {
        Self::Sip1559 {
            chain_id: 1,
            hash: B256::random(),
    ...
    /// Returns a new SIP7702 transaction with random address and hash and empty values
    pub fn sip7702() -> Self {
        Self::Sip7702 {
            chain_id: 1,
            hash: B256::random(),
    ...
    /// Returns a new SIP4844 transaction with random address and hash and empty values
    pub fn sip4844() -> Self {
        Self::Sip4844 {
            chain_id: 1,
            hash: B256::random(),
    ...
    /// Returns a new SIP4844 transaction with a provided sidecar
    pub fn sip4844_with_sidecar(sidecar: BlobTransactionSidecarVariant) -> Self {
        let mut transaction = Self::sip4844();
        if let Self::Sip4844 { sidecar: existing_sidecar, blob_versioned_hashes, .. } =
            &mut transaction
        {
    ...
    /// Sets the max fee per blob gas for SIP-4844 transactions,
    pub const fn with_blob_fee(mut self, val: u128) -> Self {
        self.set_blob_fee(val);
        self
    }

    /// Sets the number of blob versioned hashes for SIP-4844 transactions.
    pub fn with_blob_hashes(mut self, count: usize) -> Self {
        if let Self::Sip4844 { blob_versioned_hashes, .. } = &mut self {
    ...
    pub const fn set_blob_fee(&mut self, val: u128) -> &mut Self {
        if let Self::Sip4844 { max_fee_per_blob_gas, .. } = self {
    ...
    /// Sets the access list for transactions supporting SIP-1559, SIP-4844, and SIP-2930.
    ...
    /// Sets the authorization list for SIP-7702 transactions.
    pub fn set_authorization_list(&mut self, list: Vec<SignedAuthorization>) -> &mut Self {
        if let Self::Sip7702 { authorization_list, .. } = self {
    ...
    ///
    /// If it's an SIP-4844 transaction.
    ...
    pub fn inc_blob_fee_by(&self, value: u128) -> Self {
        ...
        if let Self::Sip4844 { max_fee_per_blob_gas, .. } = &mut this {
    ...
    /// Checks if the transaction is of the SIP-1559 type.
    pub const fn is_eip1559(&self) -> bool {
        matches!(self, Self::Sip1559 { .. })
    }

    /// Checks if the transaction is of the SIP-4844 type.
    pub const fn is_eip4844(&self) -> bool {
        matches!(self, Self::Sip4844 { .. })
    }

    /// Checks if the transaction is of the SIP-2930 type.
    pub const fn is_eip2930(&self) -> bool {
        matches!(self, Self::Sip2930 { .. })
    }

    /// Checks if the transaction is of the SIP-7702 type.
    pub const fn is_eip7702(&self) -> bool {
        matches!(self, Self::Sip7702 { .. })
    }
...
impl PoolTransaction for MockTransaction {
    type TryFromConsensusError = ValueError<SilaTxEnvelope<TxEip4844>>;
...
impl alloy_consensus::Transaction for MockTransaction {
    ...
    fn gas_price(&self) -> Option<u128> {
        match self {
            Self::Legacy { gas_price, .. } | Self::Sip2930 { gas_price, .. } => Some(*gas_price),
            _ => None,
        }
    }
    ...
    fn max_fee_per_blob_gas(&self) -> Option<u128> {
        match self {
            Self::Sip4844 { max_fee_per_blob_gas, .. } => Some(*max_fee_per_blob_gas),
            _ => None,
        }
    }
    ...
    fn is_dynamic_fee(&self) -> bool {
        !matches!(self, Self::Legacy { .. } | Self::Sip2930 { .. })
    }

    fn kind(&self) -> TxKind {
        match self {
            Self::Legacy { to, .. } | Self::Sip1559 { to, .. } | Self::Sip2930 { to, .. } => *to,
            Self::Sip4844 { to, .. } | Self::Sip7702 { to, .. } => TxKind::Call(*to),
        }
    }

    fn is_create(&self) -> bool {
        match self {
            Self::Legacy { to, .. } | Self::Sip1559 { to, .. } | Self::Sip2930 { to, .. } => {
                to.is_create()
            }
            Self::Sip4844 { .. } | Self::Sip7702 { .. } => false,
        }
    }
    ...
    fn blob_versioned_hashes(&self) -> Option<&[B256]> {
        match self {
            Self::Sip4844 { blob_versioned_hashes, .. } => Some(blob_versioned_hashes),
            _ => None,
        }
    }
    ...
    fn authorization_list(&self) -> Option<&[SignedAuthorization]> {
        match self {
            Self::Sip7702 { authorization_list, .. } => Some(authorization_list),
            _ => None,
        }
    }
}

impl SilPoolTransaction for MockTransaction {
    fn take_blob(&mut self) -> SilBlobTransactionSidecar {
        match self {
            Self::Sip4844 { sidecar, .. } => SilBlobTransactionSidecar::Present(sidecar.clone()),
            _ => SilBlobTransactionSidecar::None,
        }
    }
...
impl TryFrom<Recovered<SilaTxEnvelope<TxEip4844Variant<BlobTransactionSidecarVariant>>>>
    for MockTransaction
{
    type Error = TryFromRecoveredTransactionError;

    fn try_from(
        tx: Recovered<SilaTxEnvelope<TxEip4844Variant<BlobTransactionSidecarVariant>>>,
        ...
        match transaction {
            SilaTxEnvelope::Legacy(signed_tx) => {
...
impl MockTransactionFactory {
    ...
    /// Creates a validated SIP-1559 [`MockTransaction`].
    pub fn create_eip1559(&mut self) -> MockValidTx {
        self.validated(MockTransaction::sip1559())
    }

    /// Creates a validated SIP-4844 [`MockTransaction`].
    pub fn create_eip4844(&mut self) -> MockValidTx {
        self.validated(MockTransaction::sip4844())
    }
}
...
pub struct MockTransactionRatio {
    ...
    /// Percent of transactions that are SIP-1559 transactions
    pub dynamic_fee_pct: u32,
    /// Percent of transactions that are SIP-4844 transactions
    pub blob_pct: u32,
}
...
impl MockTransactionRatio {
    ...
    /// * SIP-2930 transaction => 1
    /// * SIP-1559 transaction => 2
    /// * SIP-4844 transaction => 3
...
pub struct MockFeeRange {
    ...
    /// The range of priority fees for SIP-1559 and SIP-4844 transactions
    pub priority_fee: Uniform<u128>,
    /// The range of max fees for SIP-1559 and SIP-4844 transactions
    pub max_fee: Uniform<u128>,
    /// The range of max fees per blob gas for SIP-4844 transactions
...
impl MockFeeRange {
    ...
    /// Returns a sample of `max_priority_fee_per_gas` for SIP-1559 and SIP-4844 transactions with
    /// the given [Rng](rand::Rng).
    pub fn sample_priority_fee(&self, rng: &mut impl rand::Rng) -> u128 {
        self.priority_fee.sample(rng)
    }

    /// Returns a sample of `max_fee_per_gas` for SIP-1559 and SIP-4844 transactions with the given
    /// [Rng](rand::Rng).
    pub fn sample_max_fee(&self, rng: &mut impl rand::Rng) -> u128 {
        self.max_fee.sample(rng)
    }

    /// Returns a sample of `max_fee_per_blob_gas` for SIP-4844 transactions with the given
    /// [Rng](rand::Rng).
...
impl MockTransactionDistribution {
    ...
    pub fn tx(&self, nonce: u64, rng: &mut impl rand::Rng) -> MockTransaction {
        ...
        let tx = match transaction_sample {
            ...
            1 => MockTransaction::sip2930().with_gas_price(self.fee_ranges.sample_gas_price(rng)),
            2 => MockTransaction::sip1559()
                .with_priority_fee(self.fee_ranges.sample_priority_fee(rng))
                .with_max_fee(self.fee_ranges.sample_max_fee(rng)),
            3 => MockTransaction::sip4844()
                .with_priority_fee(self.fee_ranges.sample_priority_fee(rng))
                .with_max_fee(self.fee_ranges.sample_max_fee(rng))
    ...
    pub fn tx_set_non_conflicting_types(
        ...
        //
        // Otherwise, it will modify the transaction distribution to only generate legacy, sip2930,
        // and sip1559 transactions.
        //
...
fn test_mock_priority() {
    ...
    let lo = MockTransaction::sip1559().with_gas_limit(100_000);
