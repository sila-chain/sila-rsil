...
impl<R: RngCore> TransactionGenerator<R> {
    ...
    /// Generates and returns a pooled SIP-1559 transaction with a random signer.
    pub fn gen_eip1559_pooled(&mut self) -> SilPooledTransaction {
    ...
    /// Generates and returns a pooled SIP-4844 transaction with a random signer.
    pub fn gen_eip4844_pooled(&mut self) -> SilPooledTransaction {
...
impl TransactionBuilder {
    ...
    /// Converts the transaction builder into a transaction format using SIP-1559.
    ...
    /// Converts the transaction builder into a transaction format using SIP-4844.
...
impl Default for TransactionBuilder {
    fn default() -> Self {
        Self {
            signer: B256::random(),
            chain_id: SILA_MAINNET.chain.id(),
            nonce: 0,
            gas_limit: 0,
            max_fee_per_gas: 0,
