...
//!    - Example: SIP-4844 transactions store only blob hashes: ([`TransactionSigned::Sip4844`])
//!
//! 2. **Pooled Format** ([`PoolTransaction::Pooled`])
//!    - Extended format for network propagation
//!    - Includes additional validation data
//!    - Example: SIP-4844 transactions include full blob sidecars: ([`PooledTransactionVariant`])
//!
//! ### Type Relationships
//!
//! ```text
...
pub trait TransactionPool: Clone + Debug + Send + Sync {
    ...
    type Transaction: SilPoolTransaction;
    ...
    /// commitments/proofs) for sip-4844 transactions inserted into the pool
    ...
    /// [`GetPooledTransactions`](https://github.com/sila-chain/devp2p/blob/master/caps/sil.md#getpooledtransactions-0x09):
    ///
    ...
    /// E.g. for SIP-4844 transactions this is the consensus transaction format with the blob
    /// sidecar.
    ///
    /// Consumer: P2P
    ...
    /// <https://geth.sila.org/docs/interacting-with-geth/rpc/ns-txpool> which distinguishes
    ...
    /// See <https://github.com/sila-chain/sila-rsil/issues/12767#issuecomment-2493223579>
    ///
    /// Consumer: Block production
    ...
    ///
    /// This is primarily used for the `txpool_` namespace: <https://geth.sila.org/docs/interacting-with-geth/rpc/ns-txpool>
    ///
    /// Consumer: RPC
...
pub struct NewBlobSidecar {
    /// hash of the SIP-4844 transaction.
    pub tx_hash: TxHash,
...
pub struct CanonicalStateUpdate<'a, B: Block> {
    ...
    /// SIP-1559 Base fee of the _next_ (pending) block
    ///
...
pub trait BestTransactions: Iterator + Send {
    ...
    ///
    /// There's only limited blob space available in a block, once exhausted, SIP-4844 transactions
...
///
/// See [`SilPooledTransaction`] for a reference implementation.
///
/// ## Transaction Representations
///
...
///    - For SIP-4844: includes only blob hashes, not the actual blobs
...
///    - For SIP-4844: includes full blob sidecars (blobs, commitments, proofs)
...
///
/// - **SIP-4844 blob transactions**: Require large blob sidecars for validation that would bloat
    ...
    alloy_consensus::Transaction + InMemorySize + Debug + Send + Sync + Clone
{
    ...
    /// Converts the transaction into consensus format while preserving the SIP-2718 encoded bytes.
    ...
    ///
    /// For SIP-1559 transactions: `max_fee_per_gas * gas_limit + tx_value`.
    /// For legacy transactions: `gas_price * gas_limit + tx_value`.
    /// For SIP-4844 blob transactions: `max_fee_per_gas * gas_limit + tx_value +
...
/// Super trait for transactions that can be converted to and from Sil transactions intended for the
/// sila style pool.
///
/// This extends the [`PoolTransaction`] trait with additional methods that are specific to the
/// Sila pool.
pub trait SilPoolTransaction: PoolTransaction {
    /// Extracts the blob sidecar from the transaction.
    fn take_blob(&mut self) -> SilBlobTransactionSidecar;

    /// A specialization for the SIP-4844 transaction type.
    /// Tries to reattach the blob sidecar to the transaction.
    ///
    /// This returns an option, but callers should ensure that the transaction is an SIP-4844
...
/// The default [`PoolTransaction`] for the [Pool](crate::Pool) for Sila.
///
...
pub struct SilPooledTransaction<T = TransactionSigned> {
    ...
    pub blob_sidecar: SilBlobTransactionSidecar,
}

impl<T: SignedTransaction> SilPooledTransaction<T> {
    ...
    pub fn new(transaction: Recovered<T>, encoded_length: usize) -> Self {
        let mut blob_sidecar = SilBlobTransactionSidecar::None;
            ...
            (transaction.blob_gas_used(), transaction.max_fee_per_blob_gas())
        {
            ...
            // missing
            blob_sidecar = SilBlobTransactionSidecar::Missing;
        }
...
impl PoolTransaction for SilPooledTransaction {
    ...
    fn from_pooled(tx: Recovered<Self::Pooled>) -> Self {
        ...
        match tx {
            PooledTransactionVariant::Sip4844(tx) => {
                ...
                pooled.blob_sidecar = SilBlobTransactionSidecar::Present(blob);
                pooled
            }
            tx => {
                // no blob sidecar
...
impl<T: Typed2718> Typed2718 for SilPooledTransaction<T> {
    fn ty(&self) -> u8 {
        self.transaction.ty()
    }
}

impl<T: InMemorySize> InMemorySize for SilPooledTransaction<T> {
    fn size(&self) -> usize {
        self.transaction.size()
    }
}

impl<T: alloy_consensus::Transaction> alloy_consensus::Transaction for SilPooledTransaction<T> {
...
impl SilPoolTransaction for SilPooledTransaction {
    fn take_blob(&mut self) -> SilBlobTransactionSidecar {
        if self.is_eip4844() {
            std::mem::replace(&mut self.blob_sidecar, SilBlobTransactionSidecar::Missing)
        } else {
...
/// Represents the blob sidecar of the [`SilPooledTransaction`].
///
/// SIP-4844 blob transactions require additional data (blobs, commitments, proofs)
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum SilBlobTransactionSidecar {
    /// This transaction does not have a blob sidecar
    /// (applies to all non-SIP-4844 transaction types)
    None,
    /// This transaction has a blob sidecar (SIP-4844) but it is missing.
    ///
    ...
    Missing,
    /// The SIP-4844 transaction was received from the network with its complete sidecar.
    ///
...
impl SilBlobTransactionSidecar {
...
pub struct BlockInfo {
    ...
    /// Currently enforced blob fee: the threshold for sip-4844 blob transactions.
    ///
...
mod tests {
    ...
    fn test_eth_pooled_transaction_new_legacy() {
        ...
        let pooled_tx = SilPooledTransaction::new(transaction.clone(), 200);
        ...
        assert_eq!(pooled_tx.blob_sidecar, SilBlobTransactionSidecar::None);
        assert_eq!(pooled_tx.cost, U256::from(100) + U256::from(10 * 1000));
    }

    #[test]
    fn test_eth_pooled_transaction_new_eip2930() {
        // Create an SIP-2930 transaction with specific parameters
        let tx = TxEnvelope::Sip2930(
            TxEip2930 {
                gas_price: 10,
                gas_limit: 1000,
                value: U256::from(100),
    ...
    fn test_eth_pooled_transaction_new_eip1559() {
        // Create an SIP-1559 transaction with specific parameters
        let tx = TxEnvelope::Sip1559(
            TxEip1559 {
                max_fee_per_gas: 10,
                gas_limit: 1000,
                value: U256::from(100),
    ...
    fn test_eth_pooled_transaction_new_eip4844() {
        ...
        let transaction = Recovered::new_unchecked(tx, Default::default());
        let pooled_tx = SilPooledTransaction::new(transaction.clone(), 300);

        // Check that the pooled transaction is created correctly
        assert_eq!(pooled_tx.transaction, transaction);
        assert_eq!(pooled_tx.encoded_length, 300);
        assert_eq!(pooled_tx.blob_sidecar, SilBlobTransactionSidecar::Missing);
        let expected_cost =
