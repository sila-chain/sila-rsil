...
pub enum TransactionValidationOutcome<T: PoolTransaction> {
    ...
    Valid {
        ...
        ///
        /// If this is a _new_ SIP-4844 blob transaction, then this must contain the extracted
        /// sidecar.
        transaction: ValidTransaction<T>,
        /// Whether to propagate the transaction to the network.
        propagate: bool,
        /// The authorities of SIP-7702 transaction.
...
/// A wrapper type for a transaction that is valid and has an optional extracted SIP-4844 blob
/// transaction sidecar.
///
...
pub enum ValidTransaction<T> {
    ...
    ///
    /// Caution: The [`TransactionValidator`] must ensure that this is only returned for SIP-4844
    /// transactions.
    ValidWithSidecar {
        /// The valid SIP-4844 transaction.
        transaction: T,
...
pub trait TransactionValidator: Debug + Send + Sync {
    ...
    ///    * ...
    ///
    /// See [`InvalidTransactionError`](rsil_primitives_traits::transaction::error::InvalidTransactionError) for common
    /// errors variants.
    ///
...
///
/// For SIP-4844 blob transactions this will _not_ contain the blob sidecar which is stored
...
impl<T: PoolTransaction> ValidPoolTransaction<T> {
    ...
    /// Returns the SIP-4844 max blob fee the caller is willing to pay.
    ///
    /// For non-SIP-4844 transactions, this returns [None].
    pub fn max_fee_per_blob_gas(&self) -> Option<u128> {
        self.transaction.max_fee_per_blob_gas()
    }

    /// Returns the SIP-1559 Max base fee the caller is willing to pay.
    ///
    ...
    /// Returns the SIP-1559 Max priority fee the caller is willing to pay, or `None` for
    /// non-SIP-1559 transactions.
    ...
    ///
    /// For SIP-1559 transactions: `min(max_fee_per_gas - base_fee, max_priority_fee_per_gas)`.
    /// For legacy transactions: `gas_price - base_fee`.
    pub fn effective_tip_per_gas(&self, base_fee: u64) -> Option<u128> {
        self.transaction.effective_tip_per_gas(base_fee)
    }

    /// Returns the max priority fee per gas if the transaction is an SIP-1559 transaction, and
    ...
    /// Whether the transaction is an SIP-4844 blob transaction.
    #[inline]
    ...
    ///
    /// Returns `None` if this transaction is not SIP-7702.
    ...
    ///
    /// Returns `None` for non-sip7702 transactions.
    pub fn authorization_count(&self) -> Option<u64> {
        self.transaction.authorization_count()
    }

    /// SIP-4844 blob transactions and normal transactions are treated as mutually exclusive per
    /// account.
    ///
    /// Returns true if the transaction is an SIP-4844 blob transaction and the other is not, or
    /// vice versa.
    #[inline]
    ...
    /// This applies to both standard gas fees and, for blob-carrying transactions (SIP-4844),
    ...
    pub fn is_underpriced(&self, maybe_replacement: &Self, price_bumps: &PriceBumpConfig) -> bool {
        ...
        //
        // The bump is different for SIP-4844 and other transactions. See `PriceBumpConfig`.
