...
/// A [`TransactionValidator`] implementation that validates sila transaction.
///
/// It supports all known sila transaction types:
/// - Legacy
/// - SIP-2718
/// - SIP-1559
/// - SIP-4844
...
pub struct SilTransactionValidator<Client, T, Savm> {
    ...
    /// Fork indicator whether we are using SIP-2718 type transactions.
    sip2718: bool,
    /// Fork indicator whether we are using SIP-1559 type transactions.
    sip1559: bool,
    /// Fork indicator whether we are using SIP-4844 blob transactions.
    sip4844: bool,
    /// Fork indicator whether we are using SIP-7702 type transactions.
    sip7702: bool,
    ...
    /// SAVM configuration for fetching execution limits
    evm_config: Savm,
...
impl<Client, Tx, Savm> fmt::Debug for SilTransactionValidator<Client, Tx, Savm> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("SilTransactionValidator")
            .field("fork_tracker", &self.fork_tracker)
            .field("sip2718", &self.sip2718)
            .field("sip1559", &self.sip1559)
            .field("sip4844", &self.sip4844)
            .field("sip7702", &self.sip7702)
...
impl<Client, Tx, Savm> SilTransactionValidator<Client, Tx, Savm> {
    ...
    /// Returns the SAVM config used for transaction validation.
    pub const fn evm_config(&self) -> &Savm {
        &self.evm_config
    }

    /// Returns if there are SIP-2718 type transactions
    pub const fn sip2718(&self) -> bool {
        self.sip2718
    }

    /// Returns if there are SIP-1559 type transactions
    pub const fn sip1559(&self) -> bool {
        self.sip1559
    }

    /// Returns if there are SIP-4844 blob transactions
    pub const fn sip4844(&self) -> bool {
        self.sip4844
    }

    /// Returns if there are SIP-7702 type transactions
    pub const fn sip7702(&self) -> bool {
        self.sip7702
    }
    ...
    ///
    /// ```ignore
    /// use rsil_transaction_pool::{error::InvalidPoolTransactionError, TransactionOrigin};
    ///
...
impl<Client, Tx, Savm> SilTransactionValidator<Client, Tx, Savm>
where
    Client: ChainSpecProvider<ChainSpec: SilChainSpec + SilaHardforks> + StateProviderFactory,
    Tx: SilPoolTransaction,
    Savm: ConfigureEvm,
{
    ...
    pub fn validate_stateless(
        ...
        if transaction.is_eip4844() {
            ...
            if blob_count > max_blob_count {
                return Err(InvalidPoolTransactionError::Sip4844(
                    Sip4844PoolTransactionError::TooManyEip4844Blobs {
                        have: blob_count,
        ...
        // Transaction gas limit validation (SIP-7825 for SilaOsaka+)
    ...
    pub fn validate_sender_bytecode(
        ...
        // Unless SilaPrague is active, the signer account shouldn't have bytecode.
        //
        // If SilaPrague is active, only SIP-7702 bytecode is allowed for the sender.
        //
    ...
    /// Validates SIP-4844 blob sidecar data and returns the extracted sidecar, if any.
        ...
        if transaction.is_eip4844() {
            ...
            match transaction.take_blob() {
                ...
                SilBlobTransactionSidecar::Present(sidecar) => {
                    ...
                    // SIP-7594 sidecar version handling
                    if self.sip7594 {
                        // Standard Sila behavior
    ...
    fn on_new_head_block(&self, new_tip_block: &HeaderTy<Savm::Primitives>) {
        // update all forks
        ...
        // Get SAVM limits from evm_config.evm_env()
        let evm_env = self
            .evm_config
        ...
        // SIP-8037: When state gas is enabled, `tx.gas` can exceed the per-tx gas limit cap
    ...
    /// Returns whether SIP-7594 sidecars are allowed
    fn allow_7594_sidecars(&self) -> bool {
        let tip_timestamp = self.fork_tracker.tip_timestamp();

        // If next block is SilaOsaka, allow 7594 sidecars
        if self.chain_spec().is_osaka_active_at_timestamp(tip_timestamp.saturating_add(12)) {
            ...
            // Allow after 4 seconds into last non-SilaOsaka slot
...
impl<Client, Tx, Savm> TransactionValidator for SilTransactionValidator<Client, Tx, Savm>
where
    Client: ChainSpecProvider<ChainSpec: SilChainSpec + SilaHardforks> + StateProviderFactory,
    Tx: SilPoolTransaction,
    Savm: ConfigureEvm,
{
    type Transaction = Tx;
    type Block = BlockTy<Savm::Primitives>;
...
/// A builder for [`SilTransactionValidator`] and [`TransactionValidationTaskExecutor`]
#[derive(Debug)]
pub struct SilTransactionValidatorBuilder<Client, Savm> {
    client: Client,
    /// The chain ID transactions must use.
    chain_id: u64,
    ...
    max_blob_count: u64,
    /// Whether using SIP-2718 type transactions is allowed
    sip2718: bool,
    /// Whether using SIP-1559 type transactions is allowed
    sip1559: bool,
    /// Whether using SIP-4844 type transactions is allowed
    sip4844: bool,
    /// Whether using SIP-7702 type transactions is allowed
    sip7702: bool,
    /// The current max gas limit
    block_gas_limit: AtomicU64,
    /// The current tx fee cap limit in wei locally submitted into the pool.
    ...
    /// Bitmap of custom transaction types that are allowed.
    other_tx_types: U256,
    /// Cached max initcode size from SAVM config
    max_initcode_size: usize,
    /// Cached transaction gas limit cap from SAVM config (0 = no cap)
...
impl<Client, Savm> SilTransactionValidatorBuilder<Client, Savm> {
    /// Creates a new builder for the given client and SAVM config
    ///
    /// By default this assumes the network is on the `SilaPrague` hardfork and the following
    ...
    ///  - SIP-4844
    ///  - SIP-7702
    pub fn new(client: Client, evm_config: Savm) -> Self
    where
        Client: ChainSpecProvider<ChainSpec: SilChainSpec + SilaHardforks>
        ...
        Savm: ConfigureEvm,
    {
        ...
        Self {
            block_gas_limit: SILA_BLOCK_GAS_LIMIT_30M.into(),
            client,
            ...
            // SIP-8037: When state gas is enabled, tx.gas can exceed the per-tx cap
            ...
            // SIP-7594 sidecars are accepted by default (standard Sila behavior)
            sip7594: true,
        }
    }
    ...
    /// Disables the support for SIP-2718 transactions.
    pub const fn no_eip2718(self) -> Self {
        self.set_eip2718(false)
    }

    /// Set the support for SIP-2718 transactions.
    pub const fn set_eip2718(mut self, sip2718: bool) -> Self {
        self.sip2718 = sip2718;
        self
    }

    /// Disables the support for SIP-1559 transactions.
    pub const fn no_eip1559(self) -> Self {
        self.set_eip1559(false)
    }

    /// Set the support for SIP-1559 transactions.
    pub const fn set_eip1559(mut self, sip1559: bool) -> Self {
        self.sip1559 = sip1559;
        self
    }

    /// Disables the support for SIP-4844 transactions.
    pub const fn no_eip4844(self) -> Self {
        self.set_eip4844(false)
    }

    /// Set the support for SIP-4844 transactions.
    pub const fn set_eip4844(mut self, sip4844: bool) -> Self {
        self.sip4844 = sip4844;
        self
    }

    /// Disables the support for SIP-7702 transactions.
    pub const fn no_eip7702(self) -> Self {
        self.set_eip7702(false)
    }

    /// Set the support for SIP-7702 transactions.
    pub const fn set_eip7702(mut self, sip7702: bool) -> Self {
        self.sip7702 = sip7702;
        self
    }
    ...
    /// Builds a the [`SilTransactionValidator`] without spawning validator tasks.
    pub fn build<Tx, S>(self, blob_store: S) -> SilTransactionValidator<Client, Tx, Savm>
    where
        S: BlobStore,
    {
        let Self {
    ...
    /// Builds a [`SilTransactionValidator`] and spawns validation tasks via the
...
pub fn ensure_intrinsic_gas<T: SilPoolTransaction>(
    transaction: &T,
    fork_tracker: &ForkTracker,
) -> Result<(), InvalidPoolTransactionError> {
...
mod tests {
    ...
    fn get_transaction() -> SilPooledTransaction {
        ...
        SilPooledTransaction::from_pooled(tx.try_into_recovered().unwrap())
    }

    // <https://github.com/sila-chain/sila-rsil/issues/5178>
    #[tokio::test]
    async fn validate_transaction() {
    ...
    // <https://github.com/sila-chain/sila-rsil/issues/8550>
    #[tokio::test]
    ...
    fn reject_blob_tx_with_oversized_access_list() {
        ...
        let blob_tx_with_access_list = |storage_keys: usize| {
            ...
            SilPooledTransaction::new(tx, encoded_length)
        };

        let is_oversized = |tx: &SilPooledTransaction| {
            matches!(
