...
impl TransactionValidationTaskExecutor<()> {
    /// Convenience method to create a [`SilTransactionValidatorBuilder`]
    pub fn eth_builder<Client, Savm>(
        client: Client,
        evm_config: Savm,
    ) -> SilTransactionValidatorBuilder<Client, Savm>
    where
        Client: ChainSpecProvider<ChainSpec: SilaHardforks>
            + BlockReaderIdExt<Header = HeaderTy<Savm::Primitives>>,
        Savm: ConfigureEvm,
    {
        SilTransactionValidatorBuilder::new(client, evm_config)
    }
}
...
mod tests {
    ...
    async fn executor_forwards_same_origin_batches() {
        ...
        let transactions = vec![MockTransaction::legacy(), MockTransaction::sip1559()];
