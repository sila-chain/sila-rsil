...
async fn blobs_exclusive() {
    ...
    let sip1559_tx =
        MockTransaction::sip1559().set_sender(blob_tx.transaction.sender()).inc_price_by(10_000);

    let res =
        txpool.add_transaction(TransactionOrigin::External, sip1559_tx.clone()).await.unwrap_err();

    assert_eq!(res.hash, *sip1559_tx.get_hash());
    match res.kind {
        PoolErrorKind::ExistingConflictingTransactionType(addr, tx_type) => {
            assert_eq!(addr, sip1559_tx.sender());
            assert_eq!(tx_type, sip1559_tx.tx_type());
        }
        _ => unreachable!(),
    }
}
