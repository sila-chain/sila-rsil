...
/// Normalize an empty-trie proof for the SIP-1186 (`eth_getProof`) response.
///
/// An empty trie is internally represented by a single empty-root sentinel node (`0x80`, the
/// RLP empty string whose hash is `EMPTY_ROOT_HASH`). SIP-1186 defines the proof field as the
...
#[cfg(feature = "sip1186")]
impl AccountProof {
    /// Convert into an SIP-1186 account proof response.
    ///
    /// For non-existent accounts, this returns `KECCAK_EMPTY` for `codeHash` and
    /// `EMPTY_ROOT_HASH` for `storageHash`, matching rsil's default behavior.
    ///
    ...
    /// Convert into an SIP-1186 account proof response, with optional geth-compatible
    ...
    /// ([go-sila#28357](https://github.com/sila-chain/go-sila/pull/28357)).
    ///
    /// When `false`, returns `KECCAK_EMPTY` / `EMPTY_ROOT_HASH` (rsil default).
    ///
    /// See: <https://github.com/sila-chain/go-sila/issues/28441>
    ...
    pub fn from_eip1186_proof(proof: alloy_rpc_types_sil::EIP1186AccountProofResponse) -> Self {
            ...
            (code_hash == KECCAK_EMPTY || code_hash.is_zero())
        {
            ...
            // verification.
            //
            // Note: geth (since v1.13.4, go-sila#28357) returns `B256::ZERO` for
            // both `codeHash` and `storageHash` in exclusion proofs, while rsil
...
#[cfg(feature = "sip1186")]
impl StorageProof {
    /// Convert into an SIP-1186 storage proof
...
mod tests {
    ...
    #[cfg(feature = "sip1186")]
    #[test]
    fn sip1186_response_normalizes_empty_trie_proof() {
        let slot = B256::with_last_byte(1);
    ...
    #[cfg(feature = "sip1186")]
    #[test]
    fn sip1186_response_keeps_nonempty_proof() {
