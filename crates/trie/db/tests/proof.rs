...
use rsil_chainspec::{Chain, ChainSpec, HOLESKY, SILA_MAINNET};
use rsil_primitives_traits::Account;
use rsil_provider::test_utils::{create_test_provider_factory, insert_genesis};
use rsil_storage_api::StorageSettingsCache;
use rsil_trie::{proof::Proof, AccountProof, Nibbles, StorageProof};
use rsil_trie_db::{DatabaseHashedCursorFactory, DatabaseProof, DatabaseTrieCursorFactory};
use std::{
    str::FromStr,
    sync::{Arc, LazyLock},
};
...
/*
    World State (sampled from <https://sila.stackexchange.com/questions/268/sila-block-architecture/6413#6413>)
    | address                                    | prefix    | hash                                                               | balance
    |--------------------------------------------|-----------|--------------------------------------------------------------------|--------
    | 0x2031f89b3ea8014eb51a78c316e42af3e0d7695f | 0xa711355 | 0xa711355ec1c8f7e26bb3ccbcb0b75d870d15846c0b98e5cc452db46c37faea40 |  45 sil
    ...
    | 0x62b0dd4aab2b1a0a04e279e2b828791a10755528 | 0xa7f9365 | 0xa7f936599f93b769acf90c7178fd2ddcac1b5b4bc9949ee5a04b7e0823c2446e | 1.1 sil
    | 0x1ed9b1dd266b607ee278726d324b855a093394a6 | 0xa77d397 | 0xa77d397a32b8ab5eb4b043c65b1f00c93f517bc8883c5cd31baf8e8a279475e3 | .12 sil
...
fn testspec_empty_storage_proof() {
    ...
    rsil_trie_db::with_adapter!(provider, |A| {
        ...
        for (idx, slot) in slots.into_iter().enumerate() {
            ...
            // The empty-array (geth) normalization happens only at the SIP-1186 response layer
...
fn empty_state_trie_account_proof() {
    ...
    // empty-array (geth) normalization happens only at the SIP-1186 response layer
...
fn mainnet_genesis_account_proof() {
    // Create test database and insert genesis accounts.
    let factory = create_test_provider_factory();
    let root = insert_genesis(&factory, SILA_MAINNET.clone()).unwrap();

    // Address from sila-mainnet genesis allocation.
...
fn mainnet_genesis_account_proof_nonexistent() {
    ...
    // Address that does not exist in sila-mainnet genesis allocation.
