...
use rsil_db::{tables, test_utils::TempDatabase, DatabaseEnv};
use rsil_db_api::{
...
fn incremental_vs_full_root(inputs: &[&str], modified: &str) {
    ...
    rsil_trie_db::with_adapter!(tx, |A| {
...
fn account_and_storage_trie() {
    ...
    let account1 = Account { nonce: 0, balance: U256::from(3).mul(sila), bytecode_hash: None };
    ...
    let account2 = Account { nonce: 0, balance: sila, ..Default::default() };
    ...
    let account3 =
        Account { nonce: 0, balance: U256::from(2).mul(sila), bytecode_hash: Some(code_hash) };
    ...
    let account3_storage_root = rsil_trie_db::with_adapter!(tx, |A| {
        DbStorageRoot::<_, A>::from_tx(tx.tx_ref(), address3).root().unwrap()
    });
    hash_builder
    ...
    let account4a = Account { nonce: 0, balance: U256::from(4).mul(sila), ..Default::default() };
    ...
    let account5 = Account { nonce: 0, balance: U256::from(8).mul(sila), ..Default::default() };
    ...
    let account6 = Account { nonce: 0, balance: U256::from(1).mul(sila), ..Default::default() };
    ...
    let (root, trie_updates) = rsil_trie_db::with_adapter!(tx, |A| {
    ...
    let account4b = Account { nonce: 0, balance: U256::from(5).mul(sila), bytecode_hash: None };
...
fn account_trie_around_extension_node() {
    ...
    let (got, updates) = rsil_trie_db::with_adapter!(tx, |A| {
        DbStateRoot::<_, A>::from_tx(tx.tx_ref()).root_with_updates().unwrap()
    });
    assert_eq!(expected, got);
...
proptest! {
    ...
    fn fuzz_state_root_incremental(account_changes: [BTreeMap<B256, U256>; 5]) {
        ...
        for accounts in account_changes {
            ...
            let (state_root, trie_updates) = rsil_trie_db::with_adapter!(tx, |A| {
                DbStateRoot::<_, A>::from_tx(tx.tx_ref())
