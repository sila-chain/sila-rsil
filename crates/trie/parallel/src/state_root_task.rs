...
pub struct StateRootComputeOutcome {
    ...
    pub debug_recorders: Vec<(Option<B256>, rsil_trie_sparse::debug_recorder::TrieDebugRecorder)>,
}
...
impl StateRootHandle {
    ...
    /// Takes the authoritative update capability as an SAVM state hook.
    ///
...
pub trait StateRootSink: Send + Sync + 'static {
    ...
    fn on_state_update(&self, state: SavmState);
...
/// update stream: either the SAVM state hook made with [`Self::into_state_hook`] (finishes on
...
impl StateRootUpdateStream {
    ...
    /// Converts this capability into an SAVM state hook that finishes the stream on drop.
    ///
...
/// SAVM hook that forwards state updates into a [`StateRootSink`].
///
...
/// not by an explicit call, because the SAVM owns the hook until it is dropped and gives it no
...
impl OnStateHook for StateRootUpdateHook {
    fn on_state(&mut self, state: SavmState) {
...
/// Converts [`SavmState`] to [`HashedPostState`] by keccak256-hashing addresses and storage slots.
pub fn evm_state_to_hashed_post_state(update: SavmState) -> HashedPostState {
...
mod tests {
    ...
    fn state_root_capabilities_forward_to_sink() {
...

        {
            ...
            hook.on_state(SavmState::default());
        }
