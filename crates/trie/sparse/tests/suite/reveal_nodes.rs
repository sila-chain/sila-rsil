...
use rsil_trie_common::{BranchNodeV2, RlpNode};
...
pub(super) fn test_reveal_boundary_node_with_missing_upper_parent_branch<T: SparseTrie>(
    ...
    let leaf =
        TrieNodeV2::Leaf(rsil_trie_common::LeafNode::new(Nibbles::from_nibbles([0x3]), vec![0x01]));
    trie.reveal_nodes(&mut [rsil_trie_common::ProofTrieNodeV2 {
        path: boundary_path,
        node: leaf,
        masks: None,
    }])
