...
fn main() -> io::Result<()> {
    ...
    for (cmd, _) in &output {
        ...
        // Collect the top-level command file (e.g. rsil.mdx)
        let top_level = out_dir.join(cmd_name).with_extension("mdx");
        if top_level.exists() {
            existing_files.insert(top_level);
        }
        // Collect all .mdx files in the command's subdirectory (e.g. rsil/)
...
fn preprocess_help(s: &str) -> Cow<'_, str> {
    static REPLACEMENTS: LazyLock<Vec<(Regex, &str)>> = LazyLock::new(|| {
        let patterns: &[(&str, &str)] = &[
            ...
            (r"default: /.*/rsil", "default: <CACHE_DIR>"),
            // Remove the commit SHA and target architecture triple or fourth
            //  rustup available targets:
            //    aarch64-apple-darwin
            //    x86_64-unknown-linux-gnu
            (
                r"default: rsil/.*-[0-9A-Fa-f]{6,10}/([_\w]+)-(\w+)-(\w+)(-\w+)?",
                "default: rsil/<VERSION>-<SHA>/<ARCH>",
            ),
            // Remove the OS
            (r"default: rsil/.*/\w+", "default: rsil/<VERSION>/<OS>"),
...
struct Cmd<'a> {
    /// path to binary (e.g. ./target/debug/rsil)
    cmd: &'a Path,
