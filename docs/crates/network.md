...
The `network` crate is responsible for managing the node's connection to the Sila peer-to-peer (P2P) network, enabling communication with other nodes via the [various P2P subprotocols](https://github.com/sila-chain/devp2p).

Rsil's P2P networking consists primarily of 4 ongoing tasks:
- **Discovery**: Discovers new peers in the network
- **Transactions**: Accepts, requests, and broadcasts mempool transactions
- **SIL Requests**: Responds to incoming requests for headers and bodies
...
Let's take a look at how the main Rsil CLI (i.e., a default-configured full node) makes use of the P2P layer to explore the primary interfaces and entrypoints into the `network` crate.
...
[File: bin/rsil/src/node/mod.rs](https://github.com/sila-chain/sila-rsil/blob/1563506aea09049a85e5cc72c2894f3f7a371581/bin/rsil/src/node/mod.rs)
```rust,ignore
let network = start_network(network_config(db.clone(), chain_id, genesis_hash)).await?;

let fetch_client = Arc::new(network.fetch_client().await?);
let mut pipeline = rsil_stages::Pipeline::new()
    .push(HeaderStage {
    ...
    .push(ExecutionStage { config: ExecutorConfig::new_sila() });
        ...
        + 'static,
{
    ...
    let (handle, network, _txpool, sil) = NetworkManager::builder::<C>(self)
        .await?
...
It gets the handles for the network management, transactions, and SIL requests tasks downstream of the `NetworkManager::builder` method call, and spawns them.
...
[File: crates/net/network/src/config.rs](https://github.com/sila-chain/sila-rsil/blob/1563506aea09049a85e5cc72c2894f3f7a371581/crates/net/network/src/config.rs)
```rust,ignore
pub struct NetworkConfig<C, N: NetworkPrimitives = SilNetworkPrimitives> {
    ...
    pub discovery_v5_config: Option<rsil_discv5::Config>,
    ...
    ///
    /// See also <https://github.com/sila-chain/SIPs/blob/master/SIPS/sip-2124.md#stale-software-examples>
    ///
    /// For sync from block `0`, this should be the default chain [`ForkFilter`] beginning at the
    /// first hardfork, `Frontier` for sila-mainnet.
    ...
    /// The Sila P2P handshake, see also:
...
- `N`: The network primitives type that defines block and transaction types for the network. Defaults to `SilNetworkPrimitives` for standard Sila networks, but can be customized for other chains.
...
[File: crates/net/network/src/swarm.rs](https://github.com/sila-chain/sila-rsil/blob/1563506aea09049a85e5cc72c2894f3f7a371581/crates/net/network/src/swarm.rs)
```rust,ignore
pub(crate) struct Swarm<N: NetworkPrimitives = SilNetworkPrimitives> {
...
The SIL requests and transactions task will be explained in their own sections, following this one.
...
[File: crates/net/network/src/network.rs](https://github.com/sila-chain/sila-rsil/blob/1563506aea09049a85e5cc72c2894f3f7a371581/crates/net/network/src/network.rs)
```rust,ignore
struct NetworkInner<N: NetworkPrimitives = SilNetworkPrimitives> {
...
[File: crates/net/network/src/manager.rs](https://github.com/sila-chain/sila-rsil/blob/1563506aea09049a85e5cc72c2894f3f7a371581/crates/net/network/src/manager.rs)
```rust,ignore
pub struct NetworkManager<N: NetworkPrimitives = SilNetworkPrimitives> {
    ...
    /// Handles block imports according to the `sil` protocol.
...
In the pipeline, the `NetworkHandle` is used to instantiate the `FetchClient` - which we'll get into next - and is used in the `HeaderStage` to update the node's ["status"](https://github.com/sila-chain/devp2p/blob/master/caps/sil.md#status-0x00) (record the total difficulty, hash, and height of the last processed block).

[File: crates/stages/src/stages/headers.rs](https://github.com/sila-chain/sila-rsil/blob/1563506aea09049a85e5cc72c2894f3f7a371581/crates/stages/src/stages/headers.rs)
```rust,ignore
...
[File: crates/net/network/src/fetch/client.rs](https://github.com/sila-chain/sila-rsil/blob/1563506aea09049a85e5cc72c2894f3f7a371581/crates/net/network/src/fetch/client.rs)
```rust,ignore
pub struct FetchClient<N: NetworkPrimitives = SilNetworkPrimitives> {
...
[File: crates/net/network/src/fetch/mod.rs](https://github.com/sila-chain/sila-rsil/blob/1563506aea09049a85e5cc72c2894f3f7a371581/crates/net/network/src/fetch/mod.rs)
```rust,ignore
pub struct StateFetcher<N: NetworkPrimitives = SilNetworkPrimitives> {
...
[File: crates/net/network/src/state.rs](https://github.com/sila-chain/sila-rsil/blob/1563506aea09049a85e5cc72c2894f3f7a371581/crates/net/network/src/state.rs)
```rust,ignore
pub struct NetworkState<N: NetworkPrimitives = SilNetworkPrimitives> {
...
In the pipeline used by the main Rsil binary, the `HeaderStage` uses a `ReverseHeadersDownloader` to stream headers from the network:

[File: crates/net/downloaders/src/headers/reverse_headers.rs](https://github.com/sila-chain/sila-rsil/blob/1563506aea09049a85e5cc72c2894f3f7a371581/crates/net/downloaders/src/headers/reverse_headers.rs)
```rust,ignore
...
[File: crates/net/downloaders/src/bodies/bodies.rs](https://github.com/sila-chain/sila-rsil/blob/1563506aea09049a85e5cc72c2894f3f7a371581/crates/net/downloaders/src/bodies/bodies.rs)
```rust,ignore
...
The SIL requests task serves _incoming_ requests related to blocks in the [`sil` P2P subprotocol](https://github.com/sila-chain/devp2p/blob/master/caps/sil.md#protocol-messages) from other peers.
...
### The Operation of the SIL Requests Task
...
[File: crates/net/sil-wire/src/types/blocks.rs](https://github.com/sila-chain/sila-rsil/blob/1563506aea09049a85e5cc72c2894f3f7a371581/crates/net/sil-wire/src/types/blocks.rs)
```rust,ignore
...
The transactions task listens for, requests, and propagates transactions both from the node's peers, and those that are added locally (e.g., submitted via RPC). Note that this task focuses solely on the network communication involved with Sila transactions, we will talk more about the structure of the transaction pool itself
in the [transaction-pool](https://rsil.rs/docs/rsil_transaction_pool/index.html) chapter.

Again, like the network management and SIL requests tasks, the transactions task is implemented as an endless future that runs as a background task on a standalone `tokio::task`. It's represented by the `TransactionsManager` struct:

[File: crates/net/network/src/transactions.rs](https://github.com/sila-chain/sila-rsil/blob/1563506aea09049a85e5cc72c2894f3f7a371581/crates/net/network/src/transactions.rs)
```rust,ignore
pub struct TransactionsManager<Pool, N: NetworkPrimitives = SilNetworkPrimitives> {
...
Unlike the SIL requests task, but like the network management task's `NetworkHandle`, the transactions task can also be accessed via a shareable "handle" struct, the `TransactionsHandle`:
...
```
[File: crates/net/network-api/src/events.rs](https://github.com/sila-chain/sila-rsil/blob/c46b5fc1157d12184d1dceb4dc45e26cf74b2bc6/crates/net/network-api/src/events.rs)
...
[File: crates/net/network/src/transactions.rs](https://github.com/sila-chain/sila-rsil/blob/1563506aea09049a85e5cc72c2894f3f7a371581/crates/net/network/src/transactions.rs)
```rust,ignore
pub struct PeerMetadata<N: NetworkPrimitives = SilNetworkPrimitives> {
...
After the `PeerMetadata` is added to `TransactionsManager.peers`, the hashes of all of the transactions in the node's transaction pool are sent to the peer in a [`NewPooledTransactionHashes` message](https://github.com/sila-chain/devp2p/blob/master/caps/sil.md#newpooledtransactionhashes-0x08).
...
[File: crates/net/network/src/transactions.rs](https://github.com/sila-chain/sila-rsil/blob/1563506aea09049a85e5cc72c2894f3f7a371581/crates/net/network/src/transactions.rs)
```rust,ignore
pub enum NetworkTransactionEvent<N: NetworkPrimitives = SilNetworkPrimitives> {
...
This event is generated from the [`Transactions` protocol message](https://github.com/sila-chain/devp2p/blob/master/caps/sil.md#transactions-0x02), and is handled by the `import_transactions` method.
...
This event is generated from the [`NewPooledTransactionHashes` protocol message](https://github.com/sila-chain/devp2p/blob/master/caps/sil.md#newpooledtransactionhashes-0x08), and is handled by the `on_new_pooled_transactions` method.

Here, it begins by adding the transaction hashes included in the `NewPooledTransactionHashes` payload to the LRU cache for the `PeerMetadata` identified by `peer_id` in `TransactionsManager.peers`. Next, it filters the list of hashes to those that are not already present in the transaction pool, and for each such hash, requests its full transaction object from the peer by sending it a [`GetPooledTransactions` protocol message](https://github.com/sila-chain/devp2p/blob/master/caps/sil.md#getpooledtransactions-0x09) through the `PeerMetadata.request_tx` channel. If the request was successfully sent, a `GetPooledTxRequest` gets added to `TransactionsManager.inflight_requests` vector:
...
This event is generated from the [`GetPooledTransactions` protocol message](https://github.com/sila-chain/devp2p/blob/master/caps/sil.md#getpooledtransactions-0x09), and is handled by the `on_get_pooled_transactions` method.

Here, it collects _all_ the transactions in the node's transaction pool, recovers their signers, adds their hashes to the LRU cache of the requesting peer, and sends them to the peer in a [`PooledTransactions` protocol message](https://github.com/sila-chain/devp2p/blob/master/caps/sil.md#pooledtransactions-0x0a). This is sent through the `response` channel that's stored as a field of the `NetworkTransaction::GetPooledTransactions` variant itself.
