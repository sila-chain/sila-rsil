...
The `sil-wire` crate provides abstractions over the [`RLPx`](https://github.com/sila-chain/devp2p/blob/master/rlpx.md) and
[Sil wire](https://github.com/sila-chain/devp2p/blob/master/caps/sil.md) protocols.

This crate can be thought of as having 2 components:

1. Data structures that serialize and deserialize the Sila protocol messages into Rust-compatible types.
2. Abstractions over Tokio Streams that operate on these types.

(Note that ECIES is implemented in a separate `rsil-ecies` crate.)
Additionally, this crate focuses on stream implementations (P2P and Sil), handshakes, and multiplexing. The protocol
message types and RLP encoding/decoding live in the separate `sil-wire-types` crate and are re-exported by `sil-wire`
for convenience.
## Types
The most basic Sil-wire type is a `ProtocolMessage`. It describes all messages that rsil can send/receive.

[File: crates/net/sil-wire-types/src/message.rs](../../crates/net/sil-wire-types/src/message.rs)
```rust, ignore
...
```
Every `SilMessage` has a corresponding Rust struct that implements `alloy_rlp::Encodable` and `alloy_rlp::Decodable`
...
```
These traits describe how the `SilMessage` should be serialized/deserialized into raw bytes using the RLP format.
In rsil all [RLP](https://sila.org/en/developers/docs/data-structures-and-encoding/rlp/) encode/decode operations are handled by `alloy_rlp` and the derive macros used in `sil-wire-types`.

Note: `ProtocolMessage` implements `Encodable`, while decoding is performed via
`ProtocolMessage::decode_message(version, &mut bytes)` because decoding must respect the negotiated `SilVersion`.

### Example: The Transactions message
Let's understand how an `SilMessage` is implemented by taking a look at the `Transactions` Message. The sil specification describes a Transaction message as a list of RLP-encoded transactions:

[File: sila/devp2p/caps/sil.md](https://github.com/sila-chain/devp2p/blob/master/caps/sil.md#transactions-0x02)
```
Transactions (0x02)
[tx₁, tx₂, ...]

Specify transactions that the peer should make sure are included in its transaction queue.
The items in the list are transactions in the format described in the main Sila specification.
...

```

In rsil, this is represented as:

[File: crates/net/sil-wire-types/src/broadcast.rs](../../crates/net/sil-wire-types/src/broadcast.rs)
```rust,ignore
...
[File: crates/sila/primitives/src/transaction.rs](../../crates/sila/primitives/src/transaction.rs)
```rust, ignore
#[rsil_codec]
...
- Keeps track of the SharedCapabilities between the rsil node and its peers.
...
Decompression/Compression of bytes is done with snappy algorithm ([SIP 706](https://sips.sila.org/SIPS/sip-706))
using the external `snap` crate.

[File: crates/net/sil-wire/src/p2pstream.rs](../../crates/net/sil-wire/src/p2pstream.rs)
```rust,ignore
#[pin_project]
...
[File: crates/net/sil-wire/src/pinger.rs](../../crates/net/sil-wire/src/pinger.rs)
```rust,ignore
#[derive(Debug)]
...
[File: crates/net/sil-wire/src/pinger.rs](https://github.com/sila-chain/sila-rsil/blob/1563506aea09049a85e5cc72c2894f3f7a371581/crates/net/sil-wire/src/pinger.rs)
```rust, ignore
...
impl<S, E> Sink<SilMessage> for SilStream<S> {
    // ...
    fn start_send(self: Pin<&mut Self>, item: SilMessage) -> Result<(), Self::Error> {
...
```
## Unauthed streams
For a session to be established, peers in the Sila network must first exchange a `Hello` message in the `RLPx` layer and then a
`Status` message in the sil-wire layer.

To perform these, rsil has special `Unauthed` versions of streams described above.
...
`sil-wire` also provides `RlpxProtocolMultiplexer`/`RlpxSatelliteStream` to run the primary `sil` protocol alongside
