...
Our goal in building Rsil, apart from improving client diversity, is to create a client that delivers maximally along each of the following dimensions:
...
RPC -> SAVM -> Cache -> Codec -> DB

One of our first and foremost goals in Rsil is to minimize the latency and maximize the throughput (think: request concurrency) of this pipeline.
...
The biggest bottleneck in this pipeline is not the execution of the SAVM interpreter itself, but rather in accessing state and managing I/O. As such, we think the largest optimizations to be made are closest to the DB layer.
...
**Extension to SAVM-compatible L1s and L2s**

Another consequence of a configurable design is the ability to quickly extend the client to support other SAVM-compatible L1s and L2s, enabling innovation while retaining performance.

### How is Rsil made configurable?

**Modularity & generics**

We prioritize a modular design for Rsil with reasonable (and zero-cost!) abstractions over generic interfaces. We want it to be quick and easy for others to extend or adapt the implementation to their own needs.
...
Maintaining a client implementation is *hard*. Bringing in talent and sustaining momentum in workstreams is a known challenge. As such, we take an open-source first approach to ensure that the development of Rsil can be carried forward by the community.

We want to be as deliberate as possible in forming a feedback loop with the Sila community, and not only make it easy to contribute to Rsil, but in fact actively *encourage* doing so.
...
### How does Rsil support open-source contribution?

**Documentation**

It goes without saying that verbose and thorough documentation is a must. The docs should provide full context on the design and implementation of the client, as well as the contribution process, and should be accessible to anyone with a basic understanding of Sila.
