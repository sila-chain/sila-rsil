...
Rsil exposes a number of metrics, which can be served from an HTTP endpoint by adding the `--metrics` flag.
...
2. Add the metrics description in the crate's metrics describer module, e.g.: [network metrics describer](https://github.com/sila-chain/sila-rsil/blob/main/crates/net/network/src/metrics.rs).
...
- Use `.` to namespace metrics
  - The top-level namespace should **NOT** be `rsil`[^1]
...
- Avoid duplicate metrics
  - An example would be adding two metrics for connections: `rsil.p2p.connections` for current connections and `rsil.p2p.connections.total` for total connections. One of these metrics can be used to infer the other.
