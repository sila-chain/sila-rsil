...
This repository contains several Rust crates that implement the different building blocks of an Sila node. The high-level structure of the repository is as follows:

Generally, rsil is composed of a few components, with supporting crates. The main components can be defined as:
...
- [`storage/provider`](../../crates/storage/provider): Traits which provide a higher level api over the database to access the Sila state and historical data (transactions, blocks etc.)
...
- [`net/dns`](../../crates/net/dns): An implementation of node discovery via DNS ([SIP-1459][sip-1459])

#### Protocol

- [`net/sil-wire`](../../crates/net/sil-wire): Implements the `sil` wire protocol and the ``RLPx`` networking stack.
- [`net/ecies`](../../crates/net/ecies): Implementation of the Elliptic Curve Integrated Encryption Scheme used in the ``RLPx`` handshake.
- [`net/sil-wire-types`](../../crates/net/sil-wire-types): Common types used by the `sil` wire protocol and RLPx networking stack.
...
- [`revm`](../../crates/revm): Revm utils and implementations specific to rsil.
- [`savm`](../../crates/savm/savm): Traits for configuring an SAVM specifics.
- [`execution-types`](../../crates/savm/execution-types): Commonly used types for (SAVM) block execution.
- [`execution-errors`](../../crates/savm/execution-errors): Commonly used error types used when doing block execution.

### Sync

These crates implement the main syncing drivers of rsil.
...
- `eth_`
- `net_`
- `trace_` (OpenSila-style tracing APIs)
- `txpool_`
- `web3_`
...
- [`rpc/rpc-sil-api`](../../crates/rpc/rpc-sil-api/): Rsil RPC 'sil' namespace API (including interface and implementation), this crate is re-exported by `rpc/rpc-api`
- [`rpc/rpc-sil-types`](../../crates/rpc/rpc-sil-types/): Types `supporting the implementation` of 'sil' namespace RPC server API
- [`rpc/rpc-server-types`](../../crates/rpc/rpc-server-types/): RPC server types and constants

#### Utilities Crates

- [`rpc/rpc-convert`](../../crates/rpc/rpc-convert): This crate provides various helper functions to convert between rsil primitive types and rpc types.
- [`rpc/layer`](../../crates/rpc/rpc-layer/): Some RPC middleware layers (e.g. `AuthValidator`, `JwtAuthValidator`)
- [`rpc/rpc-testing-util`](../../crates/rpc/rpc-testing-util/): Rsil RPC testing helpers
...
- [`primitives`](../../crates/primitives): Commonly used types in Rsil.
- [`primitives-traits`](../../crates/primitives-traits/): Common abstracted types in rsil.
- [`trie`](../../crates/trie): An implementation of a Merkle Patricia Trie used for various roots (e.g. the state root) in Sila.

### Sila-Specific Crates

Sila sila-mainnet-specific implementations and primitives live in `crates/sila/`.

- **rsil-sila-engine-primitives** (`crates/sila/engine-primitives`)  
  Sila-specific types for engine API, consensus messages, and payload attributes.

- **rsil-sila-forks** (`crates/sila/hardforks`)  
  SilaMainnet-specific hardfork definitions, activation schedules, and feature flags.

- **rsil-sila-payload-builder** (`crates/sila/payload`)  
  Sila-tailored payload builder implementing sila-mainnet block production rules.
...
- [`metrics`](https://github.com/metrics-rs/metrics): Rsil use [metrics][metrics] for metrics collection, which is a lightweight metrics facade
- [`metrics/metrics-derive`](https://github.com/rkrasiuk/metrics-derive): A derive-style API for creating metrics
- [`metrics/rsil-node-metrics`](../../crates/node/metrics/): The implementation of metrics server, recorder, hooks.
...
- [`tokio-util`](../../crates/tokio-util): Tokio-related utilities used by rsil.
...
[discv4]: https://github.com/sila-chain/devp2p/blob/master/discv4.md
[jsonrpsee]: https://github.com/paritytech/jsonrpsee/
[tracing]: https://crates.io/crates/tracing
[sip-1459]: https://sips.sila.org/SIPS/sip-1459
[engine-spec]: https://github.com/sila-chain/execution-apis/tree/main/src/engine
