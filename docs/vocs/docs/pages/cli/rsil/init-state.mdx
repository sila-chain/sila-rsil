...
```bash
$ rsil init-state --help
```
```txt
Usage: rsil init-state [OPTIONS] <STATE_DUMP_FILE>
...
Storage:
      ...
      --without-savm
          ...
          When enabled, and before inserting the state, it creates a dummy chain up to the last SAVM block specified. It then appends the first provided block.
