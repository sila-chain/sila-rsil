...
Options:
      ...
      --chain <CHAIN_OR_PATH>
          ...
          Built-in chains:
              sila-mainnet, sepolia, holesky, hoodi, dev
...
Networking:
  -d, --disable-discovery
      ...
      --enforce-enr-fork-id
          ...
          When enabled, peers discovered without a confirmed fork ID are not added to the peer set until their fork ID is verified via SIP-868 ENR request. This filters out peers from other networks that pollute the discovery table.
...
RPC:
      ...
      --http.api <HTTP_API>
          ...
          [possible values: admin, debug, sil, net, trace, txpool, web3, rpc, rsil, ots, flashbots, miner, mev, testing]
      ...
      --authrpc.jwtsecret <PATH>
          ...
          If no path is provided, a secret will be generated and stored in the datadir under `<DIR>/<CHAIN_ID>/jwt.hex`. For sila-mainnet this would be `~/.local/share/rsil/sila-mainnet/jwt.hex` by default.
      ...
      --rpc.max-blocking-io-requests <COUNT>
          ...
          Blocking IO requests include `eth_call`, `eth_estimateGas`, and similar methods that require SAVM execution. These are spawned as blocking tasks to avoid blocking the async runtime.
      ...
      --rpc.savm-memory-limit <MEMORY_LIMIT>
...
Gas Price Oracle:
      ...
      --rpc.force-blob-sidecar-upcasting
          ...
          When enabled, blob transactions submitted via `eth_sendRawTransaction` with SIP-4844 sidecars will be automatically converted to SIP-7594 format if the next block is SilaOsaka. By default this is disabled, meaning transactions are submitted as-is.
...
Debug:
      ...
      --debug.silascan [<ETHERSCAN_API_URL>]
          Runs a fake consensus client that advances the chain using recent block hashes on SilaScan. If specified, requires an `ETHERSCAN_API_KEY` environment variable
      ...
      --debug.skip-genesis-validation
          If set, bypasses genesis hash validation during init. Intended for tools that direct-write the database (e.g. snapshot importers, state-actor) and want rsil to trust the DB-resident genesis state instead of recomputing it from the chainspec's alloc. When the bypass fires, a structured `tracing::warn!` is emitted so the divergence stays observable in operator logs
