# rsil stage dump storage-hashing

`StorageHashing` stage

```bash
$ rsil stage dump storage-hashing --help
```
```txt
Usage: rsil stage dump storage-hashing [OPTIONS] --output-datadir <OUTPUT_PATH> --from <FROM> --to <TO>
