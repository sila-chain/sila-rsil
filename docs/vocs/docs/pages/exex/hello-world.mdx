---
description: Example of a minimal Hello World ExEx in Rsil.
---
...
And add Rsil as a dependency in `Cargo.toml`
...
Now, let's jump to our `main.rs` and start by initializing and launching a default Rsil node

```rust
// [!include ~/snippets/sources/exex/hello-world/src/bin/1.rs]
```

You can already test that it works by running the binary and initializing the SilaHolesky node in a custom datadir
(to not interfere with any instances of Rsil you already have on your machine):

```console
$ cargo run -- init --chain holesky --datadir data

2024-06-12T16:48:06.420296Z  INFO rsil init starting
...
What we've arrived at is the [minimal ExEx example](https://github.com/sila-chain/sila-rsil-exex-examples/blob/4f3498f0cc00e038d6d8c32cd94fe82788862f49/minimal/src/main.rs) that we provide in the [rsil-exex-examples](https://github.com/sila-chain/sila-rsil-exex-examples) repository.
