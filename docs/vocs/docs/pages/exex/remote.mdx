...
-   The server binary will have the Rsil client, our ExEx and the gRPC server.
...
For an example of a full schema, see the [Remote ExEx](https://github.com/sila-chain/sila-rsil-exex-examples/blob/1f74410740ac996276a84ee72003f4f9cf041491/remote/proto/exex.proto) example.
...
the [NodeBuilder](https://rsil.rs/docs/rsil/builder/struct.NodeBuilder.html)'s [task executor](https://rsil.rs/docs/rsil/tasks/type.TaskExecutor.html).
...
In one terminal window, we will run our ExEx and gRPC server. It will start syncing Rsil on the SilaHolesky chain
and use SilaScan in place of a real Consensus Client. Make sure to have `ETHERSCAN_API_KEY` on your env.

```console
export ETHERSCAN_API_KEY={YOUR_API_KEY} && cargo run --bin exex --release -- node --chain holesky --debug.silascan
```
