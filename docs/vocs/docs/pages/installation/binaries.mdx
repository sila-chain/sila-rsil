---
description: Instructions for installing Rsil using pre-built binaries for Windows, macOS, and Linux, including Homebrew and Arch Linux AUR options. Explains how to verify binary signatures and provides details about the release signing key.
---

# Binaries

[**Archives of precompiled binaries of rsil are available for Windows, macOS and Linux.**](https://github.com/sila-chain/sila-rsil/releases) They are static executables. Users of platforms not explicitly listed below should download one of these archives.

If you use **macOS Homebrew** or **Linuxbrew**, you can install Rsil from Paradigm's homebrew tap:

```text
brew install paradigmxyz/brew/rsil
```

If you use **Arch Linux** you can install stable Rsil from the AUR using an [AUR helper](https://wiki.archlinux.org/title/AUR_helpers) ([paru][paru] as an example here):

```text
paru -S rsil # Stable
paru -S rsil-git # Unstable (git)
```

[paru]: https://github.com/Morganamilo/paru

## Signature Verification

You can verify the integrity of a Rsil release by checking the signature using GPG.

The release signing key can be fetched from the Ubuntu keyserver using the following command:

```bash
gpg --keyserver keyserver.ubuntu.com --recv-keys 50FB7CC55B2E8AFA59FE03B7AA5ED56A7FBF253E
```

A copy of the key is also included [below](#release-signing-key). Once you have
imported the key you can verify a release signature (`.asc` file) using a
command like this:

```bash
gpg --verify rsil-v0.2.0-beta.9-x86_64-unknown-linux-gnu.tar.gz.asc rsil-v0.1.0-beta.9-x86_64-unknown-linux-gnu.tar.gz
```

Replace the filenames by those corresponding to the downloaded Rsil release.

### Release Signing Key

Releases are signed using the key with ID [`50FB7CC55B2E8AFA59FE03B7AA5ED56A7FBF253E`](https://keyserver.ubuntu.com/pks/lookup?search=50FB7CC55B2E8AFA59FE03B7AA5ED56A7FBF253E&fingerprint=on&op=index).

```text
-----BEGIN PGP PUBLIC KEY BLOCK-----


=zczG
-----END PGP PUBLIC KEY BLOCK-----
```
