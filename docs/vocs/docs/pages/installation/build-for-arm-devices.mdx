---
description: Building and troubleshooting Rsil on ARM devices.
---

# Building for ARM devices

Rsil can be built for and run on ARM devices, but there are a few things to take into consideration before.
...
From [the Linux Kernel documentation](https://www.kernel.org/doc/html/v5.3/arm64/memory.html#:~:text=AArch64%20Linux%20uses%20either%203,for%20both%20user%20and%20kernel.), you can see that the memory layout with 4KB pages and a level-3 translation table limits the user space to 512GB, which is too low for Rsil to sync on Sila sila-mainnet.

## ARM Board Virtual Memory Limitation

### Issue Description

Some ARM boards are equipped with only 3-level paging, which imposes a virtual memory limitation of 256GB for user space on Linux. This limitation can be a challenge for running applications like "rsil", as the MDBX (Memory-mapped Database eXtreme) library requires a larger virtual memory allocation by design.
...
If both your CPU architecture and the memory layout are valid, the instructions for building Rsil will not differ from [the standard process](https://rsil.rs/installation/source/).
...
> This error is documented [here](https://github.com/sila-chain/sila-rsil/issues/2211).
...
-   Wait for the process to finish, plug your storage device into your board and start it. You can now download or install Rsil and it should work properly.
