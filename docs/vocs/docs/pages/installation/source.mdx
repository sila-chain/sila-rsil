---
description: How to build, update, and troubleshoot Rsil from source.
---

# Build from Source

You can build Rsil on Linux, macOS, Windows, and Windows WSL2.

:::note
Rsil does **not** work on Windows WSL1.
:::
...
These are needed to build bindings for Rsil's database.
...
With Rust and the dependencies installed, you're ready to build Rsil. First, clone the repository:
...
Then, install Rsil into your `PATH` directly via:

```bash
cargo install --locked --path bin/rsil --bin rsil
```

The binary will now be accessible as `rsil` via the command line, and exist under your default `.cargo/bin` folder.
...
This will place the rsil binary under `./target/release/rsil`, and you can copy it to your directory of preference after that.

Compilation may take around 10 minutes. Installation was successful if `rsil --help` displays the [command-line documentation](/cli/rsil).

If you run into any issues, please check the [Troubleshooting](#troubleshooting) section, or reach out to us on [Telegram](https://t.me/paradigm_rsil).

## Update Rsil

You can update Rsil to a specific version by running the commands below.

The `rsil` directory will be the location you cloned rsil to during the installation process.
...
You can customise the compiler settings used to compile Rsil via
[Cargo profiles](https://doc.rust-lang.org/cargo/reference/profiles.html).

Rsil includes several profiles which can be selected via the Cargo flag `--profile`.
...
Rsil will be installed to `CARGO_HOME` or `$HOME/.cargo`. This directory needs to be on your `PATH` before you can run `$ rsil`.
...
Rust Version (MSRV) which is listed under the `rust-version` key in Rsil's
[Cargo.toml](https://github.com/sila-chain/sila-rsil/blob/main/Cargo.toml).
...
You must increase the allocated disk size for your WSL2 instance before syncing rsil.
