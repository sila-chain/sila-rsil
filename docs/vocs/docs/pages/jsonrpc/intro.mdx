---
description: Overview of Rsil's JSON-RPC API and namespaces.
---

# JSON-RPC

You can interact with Rsil over JSON-RPC. Rsil supports all standard Sila JSON-RPC API methods.

JSON-RPC is provided on multiple transports. Rsil supports HTTP, WebSocket and IPC (both UNIX sockets and Windows named pipes). Transports must be enabled through command-line flags.
...
| -------------------- | ------------------------------------------------------------------------------------------------------ | --------- |
| [`sil`](/jsonrpc/sil)       | The `sil` API allows you to interact with Sila.                                                    | Maybe     |
...
| [`debug`](/jsonrpc/debug)   | The `debug` API provides several methods to inspect the Sila state, including Geth-style traces.   | No        |
| [`trace`](/jsonrpc/trace)   | The `trace` API provides several methods to inspect the Sila state, including Parity-style traces. | No        |
...
| [`rsil`](/jsonrpc/rsil)     | The `rsil` API provides rsil-specific methods like balance changes and chain notifications.            | No        |
...
Note that some APIs are sensitive, since they can be used to configure your node (`admin`, `miner`), access accounts stored on the node (`sil`), or perform testing operations (`testing`).

Generally, it is advisable to not expose any JSONRPC namespace publicly, unless you know what you are doing.

## Transports

Rsil supports HTTP, WebSockets and IPC.
...
To start an HTTP server, pass `--http` to `rsil node`:
...
```bash
rsil node --http --http.addr 127.0.0.1 --http.port 12345
```

To enable JSON-RPC namespaces on the HTTP server, pass each namespace separated by a comma to `--http.api`:

```bash
rsil node --http --http.api sil,net,trace
```

You can pass the `all` option, which is a convenient wrapper for all the JSON-RPC namespaces `admin,debug,sil,net,trace,txpool,web3,rpc,rsil,ots,flashbots,miner,mev,testing` on the HTTP server:

```bash
rsil node --http --http.api all
```
...
```bash
rsil node --http --http.corsdomain https://mycoolapp.rs
```

Alternatively, if you want to allow any domain, you can pass `*`:

```bash
rsil node --http --http.corsdomain "*"
```
...
Rsil creates a UNIX socket on Linux and macOS at `/tmp/rsil.ipc`. On Windows, IPC is provided using named pipes at `\\.\pipe\rsil.ipc`.

You can configure the IPC path using `--ipcpath`.

## Interacting with the RPC

One can easily interact with these APIs just like they would with any Sila client.
...
```bash
rsil node --http --http.api "admin,debug,sil,net,trace,txpool,web3,rpc,rsil,ots,flashbots,miner,mev,testing"
```
