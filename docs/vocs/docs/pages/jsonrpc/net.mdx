---
description: net_ namespace for Sila nodes.
---
...
Returns a `bool` indicating whether or not the node is listening for network connections. Currently, Rsil always returns `true`.
...
Returns the network ID (e.g. "1" for sila-mainnet) as a string corresponding to the chain ID.
