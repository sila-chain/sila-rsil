---
description: Trace API for inspecting Sila state and transactions.
---

# `trace` Namespace

The `trace` API provides several methods to inspect the Sila state, including Parity-style traces.

A similar module exists (with other debug functions) with Geth-style traces ([`debug`](https://github.com/sila-chain/sila-rsil/blob/main/docs/vocs/docs/pages/jsonrpc/debug.mdx)).
...
-   **Transaction-trace filtering APIs** for getting full externality traces on any transaction executed by rsil.
...
Transaction trace filtering APIs are similar to log filtering APIs in the `sil` namespace, except these allow you to search and filter based only upon address information.
