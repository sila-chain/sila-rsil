---
description: How to configure Rsil using rsil.toml and its options.
---

# Configuring Rsil

Rsil places a configuration file named `rsil.toml` in the data directory specified when starting the node. It is written in the [TOML] format.
...
The stages section is used to configure how individual stages in rsil behave, which has a direct impact on resource utilization and sync speed.

The defaults shipped with Rsil try to be relatively reasonable, but may not be optimal for your specific set of hardware.
...
Lower values correspond to more frequent disk writes, but also lower memory consumption. A lower value also negatively impacts sync speed, since rsil keeps a cache around for the entire duration of blocks executed in the same range.
...
The peers section is used to configure how the networking component of rsil establishes and maintains connections to peers.

In the top level of the section you can configure trusted nodes, and how often rsil will try to connect to new peers.

```toml
[peers]
# How often rsil will attempt to make outgoing connections,
# if there is room for more peers
refill_slots_interval = '5s'
# A list of ENRs for trusted peers, which are peers rsil will always try to connect to.
trusted_nodes = []
# Whether rsil will only attempt to connect to the peers specified above,
...
This section configures how many peers rsil will connect to.
...
If the peer misbehaves, various penalties are exacted to their reputation, and if it falls below a certain threshold (currently `50 * -1024`), rsil will disconnect and ban the peer temporarily (except for protocol violations which constitute a permanent ban).
...
If rsil fails to establish a connection to a peer, it will not re-attempt for some amount of time, depending on the reason the connection failed.
