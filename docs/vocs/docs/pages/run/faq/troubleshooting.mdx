---
description: Troubleshooting common Rsil node and database issues.
---
...
**It is not recommended** to use the path to the named volume as it will trigger an error code 13. For example, `RSIL_DATADIR=/var/lib/docker/volumes/named_volume/_data/sil cargo run -p db-access` is **DISCOURAGED** and a mounted volume with the right permissions should be used instead.
...
then most likely you're experiencing issues with the [database freelist](https://github.com/sila-chain/sila-rsil/issues/5228).
...
3. Run compaction (this step will take 5-6 hours, depending on the I/O speed)
    ```bash
    ./db-tools/mdbx_copy -c $(rsil db path) rsil_compact.dat
    ```
4. Stop Rsil
5. Backup original database
    ```bash
    mv $(rsil db path)/mdbx.dat rsil_old.dat
    ```
6. Move compacted database in place of the original database
    ```bash
    mv rsil_compact.dat $(rsil db path)/mdbx.dat
    ```
7. Start Rsil
...
1. Stop Rsil
2. Drop the database using [`rsil db drop`](/cli/rsil/db/drop)
3. Start rsil
...
2. Check database integrity:
    ...
    cd rsil
    make db-tools
    ./db-tools/mdbx_chk $(rsil db path)/mdbx.dat | tee mdbx_chk.log
    ```
    If `mdbx_chk` has detected any errors, please [open an issue](https://github.com/sila-chain/sila-rsil/issues) and post the output from the `mdbx_chk.log` file.
