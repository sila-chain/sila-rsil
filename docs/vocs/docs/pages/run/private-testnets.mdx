---
description: Running Rsil in a private testnet using Kurtosis.
---

# Run Rsil in a private testnet using Kurtosis

For those who need a private testnet to validate functionality or scale with Rsil.

## Using Docker locally

This guide uses [Kurtosis' sila-package](https://github.com/ethpandaops/sila-package) and assumes you have Kurtosis and Docker installed and have Docker already running on your machine.

-   Go [here](https://docs.kurtosis.com/install/) to install Kurtosis
-   Go [here](https://docs.docker.com/get-docker/) to install Docker

The [`sila-package`](https://github.com/ethpandaops/sila-package) is a [package](https://docs.kurtosis.com/advanced-concepts/packages) for a general purpose Sila testnet definition used for instantiating private testnets at any scale over Docker or Kubernetes, locally or in the cloud. This guide will go through how to spin up a local private testnet with Rsil and various CL clients locally. Specifically, you will instantiate a 2-node network over Docker with Rsil/Lighthouse and Rsil/Teku client combinations.

To see all possible configurations and flags you can use, including metrics and observability tools (e.g. Grafana, Prometheus, etc), go [here](https://github.com/ethpandaops/sila-package#configuration).

Genesis data will be generated using this [genesis-generator](https://github.com/ethpandaops/sila-genesis-generator) to be used to bootstrap the EL and CL clients for each node. The end result will be a private testnet with nodes deployed as Docker containers in an ephemeral, isolated environment on your machine called an [enclave](https://docs.kurtosis.com/advanced-concepts/enclaves/). Read more about how the `sila-package` works by going [here](https://github.com/ethpandaops/sila-package/).
...
participants:
    - el_type: rsil
      el_image: ghcr.io/sila-chain/sila-rsil
      cl_type: lighthouse
...
> [!TIP]
> If you would like to use a modified rsil node, you can build an image locally with a custom tag. The tag can then be used in the `el_image` field in the `network_params.yaml` file.
...
```bash
kurtosis run github.com/ethpandaops/sila-package --args-file ~/network_params.yaml --image-download always
```
...
========================================= Files Artifacts =========================================
UUID           Name
414a075a37aa   1-lighthouse-rsil-0-63-0
34d0b9ff906b   2-teku-rsil-64-127-0
...
e234b3b4a440   el-2-rsil-teku                                   engine-rpc: 8551/tcp -> 127.0.0.1:32808       RUNNING
...
f0a7d5343346   vc-1-rsil-lighthouse                             metrics: 8080/tcp -> http://127.0.0.1:32817   RUNNING
```

Great! You now have a private network with 2 full Sila nodes on your local machine over Docker - one that is a Rsil/Lighthouse pair and another that is Rsil/Teku. Check out the [Kurtosis docs](https://docs.kurtosis.com/cli) to learn about the various ways you can interact with and inspect your network.
...
The [`sila-package`](https://github.com/ethpandaops/sila-package) comes with many optional flags and arguments you can enable for your private network. Some include:
...
Please reach out to the [Kurtosis discord](https://discord.com/invite/6Jjp9c89z9) should you have any questions about how to use the `sila-package` for your private testnet needs. Thanks!
