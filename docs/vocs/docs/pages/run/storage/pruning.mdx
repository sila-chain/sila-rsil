---
description: Pruning and node modes in Rsil.
---

# Pruning & Node Modes

Rsil supports archive, full, minimal, and custom-pruned nodes. Archive mode is
...
Pruning is destructive. Once historical data has been pruned, Rsil cannot serve
...
[Running Rsil on Sila SilaMainnet or testnets](/run/sila) and start Rsil
...
-   bodies pruning: prune pre-Merge bodies on chains with a SilaParis activation
...
Use `rsil.toml` or individual `--prune.*` flags to configure each pruning
