...
The hardware requirements for running Rsil depend on the node configuration and can change over time as the network grows or new features are implemented.
...
Below are the requirements for running an Sila SilaMainnet node as of 2025-06-23 block number `22700000`:
...
Below are the minimum system requirements for running a Base SilaMainnet node as of 2025-06-23, block number `31900000`:
...
**On CPU cores for Base**: 5+ cores are needed because the state root task splits work into separate threads that run in parallel with each other. The state root task is generally more performant and can scale with the number of CPU cores, while regular state root always uses only one core. This is not a requirement for SilaMainnet, but for Base you may encounter block processing latencies of more than 2s, which can lead to lagging behind the head of the chain.
:::
...
As a result, the number of cores matters less, but in general higher clock speeds are better. More cores are better for parallelizable [stages](https://github.com/sila-chain/sila-rsil/blob/main/docs/crates/stages.md) (like sender recovery or bodies downloading), but these stages are not the primary bottleneck for syncing.

## Memory

It is recommended to use at least 16GB of RAM.

Most of Rsil's components tend to consume a low amount of memory, unless you are under heavy RPC load, so this should matter less than the other requirements.
...
Note that due to Rsil's staged sync, you only need an internet connection for the Headers and Bodies stages. This means that the first 1-3 hours (depending on your internet connection) will be online, downloading all necessary data, and the rest will be done offline and does not require an internet connection.
...
-   Storage: Consult the [Great and less great SSDs for Sila nodes](https://gist.github.com/yorickdowne/f3a3e79a573bf35767cd002cc977b038) gist. The Seagate Firecuda 530 and WD Black SN850(X) are popular TLC NVMe options. Ensure proper cooling via heatsinks or active fans.
