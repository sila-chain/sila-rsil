...
This guide demonstrates how to extend a Rsil node with custom functionality, including adding RPC endpoints, modifying transaction validation, and implementing custom services.

## Adding Custom RPC Endpoints

One of the most common modifications is adding custom RPC methods to expose additional functionality. This allows you to extend the standard Sila RPC API with your own methods while maintaining compatibility with existing tools and clients.

### Basic Custom RPC Module

The following example shows how to add a custom RPC namespace called `txpoolExt` that provides additional transaction pool functionality. This example is based on the `node-custom-rpc` example in the Rsil repository.
...
rsil-sila = { path = "../../crates/sila" }
tokio = { version = "1.0", features = ["full"] }
```

#### Implementation

The complete implementation can be found in the [node-custom-rpc example](https://github.com/sila-chain/sila-rsil/tree/main/examples/node-custom-rpc). Here's a summary of the key components:
...
This will start a Rsil node with your custom RPC methods available on both HTTP and WebSocket transports.
...
4. **CLI Integration**: Extend the default Rsil CLI with your own arguments to control custom functionality.
...
- Check out the [node-custom-rpc example](https://github.com/sila-chain/sila-rsil/tree/main/examples/node-custom-rpc) for the complete implementation
