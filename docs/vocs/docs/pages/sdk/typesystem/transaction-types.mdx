...
Rsil provides multiple transaction representations optimized for different stages of the transaction lifecycle. Understanding these types is crucial for working with the node's transaction handling pipeline.
...
- **Optional fields**: Supports both legacy and SIP-1559 transactions with appropriate fields
...
- **Blob support**: Handles SIP-4844 blob sidecars separately from the core transaction
...
The consensus representation is the canonical format used in blocks and for network propagation. It's the most compact representation and follows Sila's wire protocol.

Key characteristics:
- **Type safety**: Enum variants for different transaction types (Legacy, SIP-2930, SIP-1559, SIP-4844)
...
- Explore [SAVM](/sdk/node-components/savm) transaction execution