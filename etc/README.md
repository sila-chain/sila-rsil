...
The files in this directory may undergo a lot of changes while rsil is unstable, so do not expect them to necessarily be
up to date.
...
To run Rsil, Grafana or Prometheus with Docker Compose, refer to
the [docker docs](https://rsil.rs/installation/docker#using-docker-compose).

### Grafana

#### Adding a new metric to Grafana

To set up a new metric in Rsil and its Grafana dashboard (this assumes running Rsil and Grafana instances):
...
Your new metric is now integrated into the Rsil Grafana dashboard.

#### Import Grafana dashboards

If you are running Rsil and Grafana outside of Docker, and wish to import new Grafana dashboards or update a dashboard:
...
If you are running Rsil and Grafana using Docker, after having pulled the updated dashboards from `main`, restart the
