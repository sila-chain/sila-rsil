...
//! ```
//!
//! This launches a regular rsil instance and subscribes to payload attributes event stream.
//!
...
//!
//! See beacon Node API: <https://sila.github.io/beacon-APIs/>
...
use rsil_sila::{
    ...
    node::{builder::NodeHandle, SilaNode},
...
fn main() {
    Cli::<SilaChainSpecParser, BeaconSidecarConfig>::parse()
        .run(async move |builder, beacon_config| {
            // launch the node
            let NodeHandle { node, node_exit_future } =
                builder.node(SilaNode::default()).launch().await?;

            let notifications: rsil_sila::provider::CanonStateNotificationStream =
...
/// Our custom cli args extension that adds one flag to rsil default CLI.
...
impl BeaconSidecarConfig {
    ...
    /// Returns the URL to the beacon sidecars endpoint <https://sila.github.io/beacon-APIs/#/Beacon/getBlobSidecars>
    pub fn sidecar_url(&self, block_root: B256) -> String {
        format!("{}/sil/v1/beacon/blob_sidecars/{}", self.http_base_url(), block_root)
    }
}
