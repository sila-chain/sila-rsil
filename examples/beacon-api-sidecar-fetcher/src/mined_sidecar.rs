...
/// client.
///
/// First checks if the blob sidecar for a given SIP4844 is stored locally, if not attempts to
...
{
    fn process_block(&mut self, block: &RecoveredBlock<rsil_sila::Block>) {
        let txs: Vec<_> = block
            .body()
            .transactions()
        ...
        match self.pool.get_all_blobs_exact(txs.iter().map(|(tx, _)| *tx.tx_hash()).collect()) {
            Ok(blobs) => {
                ...
                for ((tx, _), sidecar) in txs.iter().zip(blobs) {
                    if let PooledTransactionVariant::Sip4844(transaction) = tx
                        .clone()
                        .try_into_pooled_eip4844(Arc::unwrap_or_clone(sidecar))
                        .expect("should not fail to convert blob tx if it is already sip4844")
                    {
...
async fn fetch_blobs_for_block(
    ...
    url: String,
    block: RecoveredBlock<rsil_sila::Block>,
    txs: Vec<(rsil_sila::TransactionSigned, usize)>,
    ...
    let sidecars: Vec<BlobTransactionEvent> = txs
        ...
        .filter_map(|(tx, blob_len)| {
            sidecar_iterator.next_sidecar(*blob_len).and_then(|sidecar| {
                if let PooledTransactionVariant::Sip4844(transaction) = tx
                    .clone()
                    .try_into_pooled_eip4844(BlobTransactionSidecarVariant::Sip4844(sidecar))
                    .expect("should not fail to convert blob tx if it is already sip4844")
                {
                    let block_metadata = BlockMetadata {
                        block_hash: block.hash(),
