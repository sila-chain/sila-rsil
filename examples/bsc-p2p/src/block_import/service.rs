...
use rsil_network_api::PeerId;
use rsil_payload_primitives::{BuiltPayload, EngineApiMessageVersion, PayloadTypes};
use rsil_primitives_traits::{AlloyBlockHeader, Block, NodePrimitives};
use rsil_provider::{BlockHashReader, BlockNumReader};
use std::{
    future::Future,
    pin::Pin,
    sync::Arc,
...
mod tests {
    ...
    struct TestFixture {
        handle: ImportHandle<SilEngineTypes>,
    }
    ...
    impl TestFixture {
        ...
        where
            F: Fn(&BlockImportEvent<NewBlock<BscBlock<SilEngineTypes>>>) -> bool,
        {
    ...
    fn create_test_block() -> NewBlockMessage<NewBlock<Block>> {
        let block: rsil_sila_primitives::Block = Block::default();
    ...
    async fn handle_engine_msg(
        mut from_engine: mpsc::UnboundedReceiver<BeaconEngineMessage<SilEngineTypes>>,
