...
use alloy_rpc_types::{
    engine::{
        ...
        ExecutionPayloadV1, PayloadAttributes as SilPayloadAttributes, PayloadId,
    },
    Withdrawal,
};
...
pub struct CustomPayloadAttributes {
    ...
    #[serde(flatten)]
    pub inner: SilPayloadAttributes,
    /// A custom field
    pub custom: u64,
}
...
impl PayloadTypes for CustomEngineTypes {
    ...
    fn block_to_payload(
        block: SealedBlock<
                <<Self::BuiltPayload as rsil_sila::node::api::BuiltPayload>::Primitives as rsil_sila::node::api::NodePrimitives>::Block,
            >,
        _bal: Option<Bytes>,
    ) -> ExecutionData {
        let (payload, sidecar) =
...
impl<Node, Pool, Savm> PayloadBuilderBuilder<Node, Pool, Savm> for CustomPayloadBuilderBuilder
where
    Node: FullNodeTypes<
        Types: NodeTypes<
    ...
    Savm: ConfigureEvm<Primitives = SilPrimitives, NextBlockEnvCtx = NextBlockEnvAttributes>
        + 'static,
{
    type PayloadBuilder = CustomPayloadBuilder<Pool, Node::Provider, Savm>;
    ...
    async fn build_payload_builder(
        ...
        let payload_builder = CustomPayloadBuilder {
            inner: rsil_sila_payload_builder::SilaPayloadBuilder::new(
                ctx.provider().clone(),
                pool,
                evm_config,
...
#[derive(Debug, Clone)]
#[non_exhaustive]
pub struct CustomPayloadBuilder<Pool, Client, Savm> {
    inner: rsil_sila_payload_builder::SilaPayloadBuilder<Pool, Client, Savm>,
}

impl<Pool, Client, Savm> PayloadBuilder for CustomPayloadBuilder<Pool, Client, Savm>
where
    ...
    Savm: ConfigureEvm<Primitives = SilPrimitives, NextBlockEnvCtx = NextBlockEnvAttributes>,
{
    ...
    fn try_build(
        ...
        // This reuses the default SilaPayloadBuilder to build the payload
