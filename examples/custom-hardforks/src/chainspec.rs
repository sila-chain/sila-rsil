...
// Define custom hardfork variants using Rsil's `hardfork!` macro.
// Each variant represents a protocol upgrade (e.g., enabling new features).
hardfork!(
    /// Custom hardforks for the example chain.
    ///
    /// These are inspired by Sila's upgrades but customized for demonstration.
...
// Custom chain spec wrapping Rsil's `ChainSpec` with our hardforks.
#[derive(Debug, Clone)]
...
// Implement `Hardforks` to integrate custom hardforks with Rsil's system.
impl Hardforks for CustomChainSpec {
    ...
    fn fork_id(&self, head: &rsil_chainspec::Head) -> rsil_chainspec::ForkId {
        self.inner.fork_id(head)
    }

    fn latest_fork_id(&self) -> rsil_chainspec::ForkId {
        self.inner.latest_fork_id()
    }

    fn fork_filter(&self, head: rsil_chainspec::Head) -> rsil_chainspec::ForkFilter {
        self.inner.fork_filter(head)
    }
}

// Implement `SilChainSpec` for compatibility with Sila-based nodes.
impl SilChainSpec for CustomChainSpec {
