...
fn main() {
    Cli::<SilaChainSpecParser, RsilCliTxpoolExt>::parse()
        .run(async move |builder, args| {
            // launch the node
            let handle = builder.node(SilaNode::default()).launch().await?;

            let node: FullNodeFor<SilaNode> = handle.node;
                        ...
                        args.is_match(&recipient)
                    {
                        ...
                        let result = eth_api
                            .spawn_with_call_at(
                                ...
                                SavmOverrides::default(),
                                move |db, evm_env, tx_env| {
                                    let mut dummy_inspector = DummyInspector::default();
                                    let mut savm = evm_config.evm_with_env_and_inspector(
                                        db,
                                        evm_env,
                                        &mut dummy_inspector,
                                    );
...
impl<CTX> Inspector<CTX, SilInterpreter> for DummyInspector
where
    CTX: ContextTr,
{
    /// This method is called at each step of the SAVM execution.
    ...
    fn step(&mut self, interp: &mut Interpreter<SilInterpreter>, _context: &mut CTX) {
