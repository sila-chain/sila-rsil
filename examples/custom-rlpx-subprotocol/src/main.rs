...
//! ```
//!
//! This launches a regular rsil node with a custom rlpx subprotocol.
...
fn main() -> eyre::Result<()> {
    rsil_sila::cli::Cli::parse_args().run(async move |builder, _args| {
        // launch the node
        ...
        let subnetwork = NetworkManager::sil(net_cfg).await?;
