...
//!
//! ```sh
//! cargo run -p example-custom-rpc-middleware node --http --dev --dev.block-time 12s --http.api=debug,sil
//! ```
//!
...
use rsil_sila::{
    ...
    node::{SilaAddOns, SilaNode},
};
use tower::Layer;

fn main() {
    Cli::<SilaChainSpecParser>::parse()
        .run(async move |builder, _| {
            let handle = builder
                .with_types::<SilaNode>()
                .with_components(SilaNode::components())
                .with_add_ons(
                    //create sila addons with our custom rpc middleware
                    SilaAddOns::default().with_rpc_middleware(ResponseMutationLayer),
                )
