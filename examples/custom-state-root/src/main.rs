//! Example: Running a rsil node with a custom state root strategy.
//!
...
impl<N, P, Savm> StateRootStrategy<N, P, Savm> for ZeroStateRootStrategy
where
    N: NodePrimitives,
    Savm: ConfigureEvm<Primitives = N>,
    DefaultStateRootStrategy: StateRootStrategy<N, P, Savm>,
{
    fn prepare(
        &self,
...
struct ZeroStateRootValidatorBuilder {
    inner: BasicEngineValidatorBuilder<SilaEngineValidatorBuilder>,
    ...
    >,
{
    type EngineValidator = BasicEngineValidator<
        N::Provider,
        N::Savm,
        <SilaEngineValidatorBuilder as rsil_sila::node::builder::rpc::PayloadValidatorBuilder<N>>::Validator,
    >;

    async fn build_tree_validator(
        self,
        ctx: &rsil_sila::node::builder::AddOnsContext<'_, N>,
        tree_config: TreeConfig,
        changeset_cache: ChangesetCache,
        state_trie_overlays: StateTrieOverlayManager<SilPrimitives>,
    ) -> eyre::Result<Self::EngineValidator> {
        let validator = self
            .inner
            .build_tree_validator(ctx, tree_config, changeset_cache, state_trie_overlays)
            .await?;
        let state_root_strategy: Arc<dyn StateRootStrategy<SilPrimitives, N::Provider, N::Savm>> =
