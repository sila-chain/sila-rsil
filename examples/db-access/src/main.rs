...
// Other parts of the code which include caching are parts of the `SilApi` abstraction.
fn main() -> eyre::Result<()> {
    // The path to data directory, e.g. "~/.local/rsil/share/sila-mainnet"
...
fn block_provider_example<T: BlockReader<Block = rsil_sila::Block>>(
    provider: T,
    number: u64,
) -> eyre::Result<()> {
...
fn receipts_provider_example<
    T: ReceiptProvider<Receipt = rsil_sila::Receipt>
...
>(
    ...
    // This matches SRC-20 Transfer events emitted by contract_addr where both indexed addresses are
