...
use rsil_sila::{
    ...
    node::{api::FullNodeComponents, builder::NodeHandleFor, SilaNode},
};
...
/// API to subscribe to storage changes for a specific Sila address.
#[rpc(server, namespace = "watcher")]
pub trait StorageWatcherApi {
    /// Subscribes to storage changes for a given Sila address and streams `StorageDiff`
    /// updates.
...
fn main() -> eyre::Result<()> {
    rsil_sila::cli::Cli::parse_args().run(async move |builder, _| {
        ...
        let handle: NodeHandleFor<SilaNode> = builder
