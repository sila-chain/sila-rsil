...
This example demonstrates how to extract the complete state of a specific contract from the rsil database.

## What it does

The example shows how to:

1. **Connect to a rsil database** - Uses the recommended builder pattern to create a read-only provider
...
- A rsil database with some data (you can sync a node or use a pre-synced database)
- Set the `RSIL_DATADIR` environment variable to point to your rsil data directory
- Set the `CONTRACT_ADDRESS` environment variable to provide target contract address

## Usage

```bash
# Set your rsil data directory
export RSIL_DATADIR="/path/to/your/rsil/datadir"
...
### Provider Pattern
The example uses rsil's provider pattern:
