//! Example of how to use the network with a proxy for sil requests handling.
//!
//! This connects two peers:
//!  - first peer installs a channel for incoming sil request
...
async fn run_peer(handle: NetworkHandle) -> eyre::Result<()> {
    ...
    let network = NetworkManager::sil(config).await?;
