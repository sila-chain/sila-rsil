...
use rsil_sila::{
    ...
    rpc::sil::primitives::TransactionRequest,
...
where
    // This enforces `SilPrimitives` types for this node, which unlocks the proper conversions when
    FC: FullNodeComponents<Types: NodeTypes<Primitives = SilPrimitives>>,
{
    ...
    let transaction: TransactionSigned =
        NetworkWallet::<Sila>::sign_request(wallet, request).await?.into();
...
/// Helper function to submit a simple SIL transfer transaction
///
...
where
    FC: FullNodeComponents<Types: NodeTypes<Primitives = SilPrimitives>>,
{
    // Create the transaction request for SIL transfer
    let request = TransactionRequest::default()
        .with_to(to)
        .with_value(value)
        .with_nonce(nonce)
