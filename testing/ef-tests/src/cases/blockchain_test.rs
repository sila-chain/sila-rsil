...
impl Case for BlockchainTestCase {
    ...
    /// Runs the test cases for the Sila Forks test suite.
    ///
    /// # Errors
...
fn run_case(case: &BlockchainTest) -> Result<(), Error> {
    ...
    let executor_provider = SilEvmConfig::sila(chain_spec.clone());
    ...
    for (block_index, block) in blocks.iter().enumerate() {
        ...
        let (computed_state_root, _) = rsil_trie_db::with_adapter!(provider, |A| {
            StateRoot::<rsil_trie_db::DatabaseTrieCursorFactory<_, A>, _>::overlay_root_with_updates(
                provider.tx_ref(),
                &sorted,
            )
        })
...
fn pre_execution_checks(
    ...
    let consensus: SilBeaconConsensus<ChainSpec> = SilBeaconConsensus::new(chain_spec);

    let sealed_header = block.sealed_header();

    <SilBeaconConsensus<ChainSpec> as Consensus<Block>>::validate_body_against_header(
        &consensus,
        block.body(),
        sealed_header,
    )?;
...
///
/// Some tests are edge cases that cannot happen on sila-mainnet, while others are skipped for
...
pub fn should_skip(path: &Path) -> bool {
    ...
    // Ignore outdated EOF tests that haven't been updated for SilaCancun yet.
